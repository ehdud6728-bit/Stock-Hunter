...
CLOSING_BET_SCANNER_VERSION = 'G_MORALES_V3_4_CORE_OUTPUT_BUCKET_PERF_20260510'
    ...
    try:
        ...
        try:
            if len(hist) >= 250 and info['amount_b'] >= MIN_AMOUNT / 1e8:
                ...
                # v3.4: 실시간 C전략이 나오는 데 백테스트에서 0건만 나오는 문제를 점검하기 위한 완화형 연결.
...
def _evaluate_backtest_hit(hit: dict, df: pd.DataFrame, signal_idx: int, hold_days: int = 5) -> dict:
    """
    백테스트 평가.
    기존 고가/종가 성과에 더해 v3.1부터 실전형 3/5 익절·손절 시뮬레이션을 추가하고, v3.4부터 튜닝 리포트/실전추천 조합을 함께 계산한다.
...
def _select_backtest_top(df: pd.DataFrame, top_per_strategy: int = 5, all_candidates: bool = False) -> pd.DataFrame:
    ...
    # v3.4: C는 엄격형을 우선하고, 완화형은 진단용 후보로 뒤에 둔다.
...
def _format_practical_combo_report(df: pd.DataFrame) -> str:
    """v3.4: 백테스트 기반 실전 추천 조합 요약. 핵심군은 S2/S1우수/A강한만 포함하고 C 완화형은 진단용으로 분리한다."""
    lines = ["[실전 추천 조합 — v3.4 핵심군 튜닝 관점]"]
    if df is None or df.empty:
        lines.append("- 데이터 없음")
        return "\n".join(lines)
    try:
...
def _format_bucket_table(df: pd.DataFrame, bucket_col: str, label: str) -> list:
    """v3.4: 조건 구간별 성과를 건수뿐 아니라 3/5규칙 성과까지 출력한다."""
    lines = []
...
def _format_tuning_report(df: pd.DataFrame) -> str:
    """v3.4: S/A/B 조건별 튜닝 리포트. 과최적화를 막기 위해 조건별 성과만 비교한다."""
    lines = ["[튜닝 리포트 — 조건별 성과]"]
    if df is None or df.empty:
        lines.append("- 데이터 없음")
        return "\n".join(lines)
...
def _build_backtest_summary(
    ...
    lines.append("- v3.4의 3/5규칙은 보수 모델입니다. 같은 날 손절가와 목표가가 모두 닿으면 손절을 먼저 본 것으로 계산합니다.")
    lines.append("- v3.4의 튜닝 리포트는 최근 백테스트 결과를 조건별 성과로 비교하는 참고용이며, 최소 4주~12주 이상 반복 검증이 필요합니다.")
    lines.append("- C 완화형은 실전 추천 후보가 아니라 C조건 점검용 진단 후보입니다.")
    lines.append("- --backtest-core-only 옵션을 쓰면 S2+S1우수+A강한 핵심군만 따로 검증합니다.")
    return "\n".join(lines)
...
def run_closing_bet_backtest(
    ...
    if core_only and selected_df is not None and not selected_df.empty:
        try:
            ...
            selected_df['core_filter'] = 'S2+S1우수+A강한'
...
if __name__ == '__main__':
    ...
    parser.add_argument('--backtest-core-only', action='store_true', help='백테스트 결과를 S2+S1우수+A강한 핵심군만 필터링')
