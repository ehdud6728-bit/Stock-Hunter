...
def attach_intent_proxy(df: pd.DataFrame) -> pd.DataFrame:
    z=df.copy()
    z['intent_proxy_version']='v49.68-machine-intent-proxy-v1'
    ...
    for m,g in z.groupby(z.get('mode',pd.Series('',index=z.index)).astype(str)):
        for status,sg in g.groupby('intent_status'):
            ...
            rows.append({'strategy':m,'intent_status':status,'n':len(sg),'share_pct':len(sg)/len(g)*100.0,
                         ...
                         'big_pct':_num(sg,'big_before_stop',0).fillna(0).gt(0).mean()*100.0,
                         'stop_first_pct':_num(sg,'stop_first_flag',0).fillna(0).gt(0).mean()*100.0,
...
def winner_loser_contrast(z:pd.DataFrame, features:list[str])->pd.DataFrame:
    ...
    for m in ('ALL',)+STRATEGIES:
        ...
        big=q[_num(q,'big_before_stop',0).fillna(0).gt(0)]; stop=q[_num(q,'stop_first_flag',0).fillna(0).gt(0)]
...
def opportunity_audit(opportunities:pd.DataFrame,raw:pd.DataFrame,selected:pd.DataFrame)->tuple[pd.DataFrame,pd.DataFrame,pd.DataFrame]:
    ...
    summary=pd.DataFrame(rows)
    # strongest missed opportunities for manual inspection
    ...
    cases=o[o.miss_stage.ne('SELECTED_TOP5')].sort_values('_op_strength',ascending=False).head(int(os.environ.get('CLOSING_BET_V4968_OPPORTUNITY_CASEBOOK_MAX','500') or 500))
    near_cols=[c for c in ['signal_date','code','name','miss_stage','planned_modes','main_hit_modes','deep_audit_performed','deep_audit_hit_modes','deep_audit_error',
                           'op_ret_next_close','op_ret_max_high_3d','op_ret_max_high_5d','op_ret_max_high_10d','opportunity_labels',
                           'entry_close_loc_pct','entry_upper_wick_pct','entry_gap_pct','entry_vol20_ratio','entry_vol50_ratio','entry_amount_b','entry_ma20_dist_pct','entry_ma60_dist_pct','entry_high20_dist_pct'] if c in cases.columns]
    return summary,o,cases[near_cols]
...
def drift_audit(z:pd.DataFrame,features:list[str])->pd.DataFrame:
    ...
    end=q['_date'].max(); recent_start=end-pd.Timedelta(days=int(os.environ.get('CLOSING_BET_V4968_DRIFT_RECENT_DAYS','28') or 28)); prior_start=recent_start-pd.Timedelta(days=int(os.environ.get('CLOSING_BET_V4968_DRIFT_PRIOR_DAYS','84') or 84))
    rows=[]
    for m in ('ALL',)+STRATEGIES:
...
def run_quality_audit(raw:pd.DataFrame,selected_ctx:pd.DataFrame,selected:pd.DataFrame,opportunities:pd.DataFrame,out:Path,feature_meta:dict[str,tuple[str,str]])->dict[str,Any]:
    ...
    opp_summary,opp_detail,opp_cases=opportunity_audit(opportunities,raw,selected)
    drift=drift_audit(enriched,features)
    enriched.to_csv(out/'v49_68_search_intent_enriched.csv',index=False,encoding='utf-8-sig')
    intent.to_csv(out/'v49_68_search_intent_summary.csv',index=False,encoding='utf-8-sig')
    contrast.to_csv(out/'v49_68_winner_loser_effect_size.csv',index=False,encoding='utf-8-sig')
    cases.to_csv(out/'v49_68_manual_casebook.csv',index=False,encoding='utf-8-sig')
    rank_summary.to_csv(out/'v49_68_ranking_summary.csv',index=False,encoding='utf-8-sig')
    rank_detail.to_csv(out/'v49_68_ranking_detail.csv',index=False,encoding='utf-8-sig')
    opp_summary.to_csv(out/'v49_68_opportunity_miss_summary.csv',index=False,encoding='utf-8-sig')
    opp_detail.to_csv(out/'v49_68_opportunity_census_global.csv',index=False,encoding='utf-8-sig')
    opp_cases.to_csv(out/'v49_68_missed_winner_casebook.csv',index=False,encoding='utf-8-sig')
    drift.to_csv(out/'v49_68_distribution_drift.csv',index=False,encoding='utf-8-sig')
    tech_fn=int((opp_detail.get('miss_stage',pd.Series(dtype=str)).astype(str)=='TECHNICAL_FALSE_NEGATIVE').sum()) if not opp_detail.empty else 0
    ...
    (out/'v49_68_search_quality_manifest.json').write_text(json.dumps(manifest,ensure_ascii=False,indent=2,default=str),encoding='utf-8')
    return {'enriched':enriched,'intent':intent,'contrast':contrast,'cases':cases,'rank_summary':rank_summary,'rank_detail':rank_detail,
            'opp_summary':opp_summary,'opp_detail':opp_detail,'opp_cases':opp_cases,'drift':drift,'manifest':manifest}
