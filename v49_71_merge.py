...
import v49_71_research as r70
import v49_71_quality as q70
import v49_71_insight as i71
...
def _deliver_telegram(parts: list[list[str]], out: Path, requested: bool) -> dict:
    ...
    path = out / 'v49_71_telegram_delivery_manifest.json'
    try:
        if requested:
            if delivery['route_validated'] != '1':
                raise RuntimeError('v49.71 Telegram requested but unified route preflight was not validated')
            if not s._telegram_route_ready():
                raise RuntimeError('v49.71 Telegram requested but scanner route is not ready after validated preflight')
            for part_no, part in enumerate(parts, start=1):
                ...
                if int(result.get('success_count', 0) or 0) < 1:
                    raise RuntimeError(f'v49.71 Telegram delivery failed for part {part_no}: {result.get("errors", [])}')
...
def _lp_validation(selected: pd.DataFrame, out: Path, start_date: str, end_date: str) -> pd.DataFrame:
    ...
    if z.empty:
        ...
        r.to_csv(out / 'v49_71_lp_walk_forward.csv', index=False, encoding='utf-8-sig')
        return r
...
def _iit_split(selected: pd.DataFrame, out: Path, start_date: str, end_date: str) -> pd.DataFrame:
    ...
    r = pd.DataFrame(rows)
    r.to_csv(out / 'v49_71_iit_flow_split.csv', index=False, encoding='utf-8-sig')
    return r


# ---- v49.71 winner/loser, market, sector and causal feature audit ----
...
def _v4967_anatomy_tables(x: pd.DataFrame, out: Path, start_date: str, end_date: str) -> tuple[pd.DataFrame,pd.DataFrame,pd.DataFrame,pd.DataFrame,pd.DataFrame]:
    ...
    z.to_csv(out/'v49_71_selected_enriched_outcomes.csv',index=False,encoding='utf-8-sig')
    outcomes.to_csv(out/'v49_71_outcome_distribution.csv',index=False,encoding='utf-8-sig')
    features.to_csv(out/'v49_71_winner_loser_feature_anatomy.csv',index=False,encoding='utf-8-sig')
    lifts.to_csv(out/'v49_71_train_oos_feature_lift.csv',index=False,encoding='utf-8-sig')
    market.to_csv(out/'v49_71_market_regime_performance.csv',index=False,encoding='utf-8-sig')
    events.to_csv(out/'v49_71_event_theme_context.csv',index=False,encoding='utf-8-sig')
    down.to_csv(out/'v49_71_down_market_cases.csv',index=False,encoding='utf-8-sig')
    return outcomes,features,lifts,market,events


def main() -> int:
    ap = argparse.ArgumentParser(description='v49.71 global merge + identity/pipeline/attribution audit')
    ap.add_argument('--input-root', default='v49_71_downloads')
    ap.add_argument('--prepare-dir', default='v49_71_prepare_output')
    ...
    if not prep_root.exists():
        prep_root = root
    preflight_path = next(iter(prep_root.rglob('v49_71_preflight.json')), None)
    universe_path = next(iter(prep_root.rglob('v49_71_universe.json')), None)
    ...
    for sid in expected_ids:
        ...
        if str(m.get('control_flow', '')) != 'V49_71_SHARD_EXPORT_RETURN_MAIN_EXIT':
    ...
    raw_path = out / 'v49_71_global_raw.csv'
    canonical_path = out / 'v49_71_global_canonical.csv'
    selected_path = out / 'v49_71_global_selected_top5.csv'
    ...
    populations['PRIMARY'].to_csv(out / 'v49_71_primary_selected_top5.csv', index=False, encoding='utf-8-sig')
    populations['ALL_MATCHED'].to_csv(out / 'v49_71_all_matched_selected_top5.csv', index=False, encoding='utf-8-sig')
    ...
    funnel.to_csv(out / 'v49_71_strategy_funnel.csv', index=False, encoding='utf-8-sig')
    strategy_status.to_csv(out / 'v49_71_strategy_stage_audit.csv', index=False, encoding='utf-8-sig')
    identity_audit.to_csv(out / 'v49_71_identity_audit.csv', index=False, encoding='utf-8-sig')
    zero_df.to_csv(out / 'v49_71_zero_mode_audit.csv', index=False, encoding='utf-8-sig')
    exc_df.to_csv(out / 'v49_71_predicate_exceptions.csv', index=False, encoding='utf-8-sig')
    ...
    if out.resolve() != generated_root.resolve():
        import shutil
        for gp in generated_root.glob('v49_71_common_strategy_*'):
            shutil.copy2(gp, out / gp.name)
    performance_csv = out / 'v49_71_common_strategy_performance.csv'
    portfolio_csv = out / 'v49_71_common_strategy_portfolio.csv'
    ...
    attribution.to_csv(out / 'v49_71_strategy_attribution_performance.csv', index=False, encoding='utf-8-sig')
    ...
    down_cases_path = out / 'v49_71_down_market_cases.csv'
    down_cases = pd.read_csv(down_cases_path, dtype={'code':str}) if down_cases_path.exists() else pd.DataFrame()

    # v49.71: TRAIN -> VALIDATION -> LOCKED_TEST research. Thresholds are discovered on
    ...
    # v49.71 independent SEARCH QUALITY AUDIT: machine intent proxy, winner/loser casebook,
    ...
    # v49.71 SEARCH GAP INSIGHT: audit-only lanes for immediate strategic misses,
    ...
    insight = i71.run_insight_audit(raw_df, quality['enriched'], quality['opp_detail'], out, V4967_ENTRY_FEATURE_META)
    ...
    lines2 = ['(2/16)', '📊 전략별 OOS · Predicate Mode | v49.71', '──────────']
    if not ptab.empty:
    ...
    lines3 = ['(3/16)', '🔬 전략 Funnel·Pipeline | v49.71', '──────────']
    ...
    lines4 = ['(4/16)', '🧭 전략 귀속·LP·I/IT | v49.71', '──────────']
    for mode in s.CLOSING_BET_V4958_PRIMARY_PRIORITY:
    ...
    lines5 = ['(5/16)', '🧬 상승·하락 경로 해부 | v49.71', '──────────']
    ...
    lines6 = ['(6/16)', '🌧️ 하락장 독립상승·개선후보 | v49.71', '──────────']
    ...
    if not feature_candidates.empty:
        lines6.append('[구형 2단계 탐색 · v49.71 최종 승격근거 아님]')
    ...
    lines6 += ['', '- 섹터동행은 전체 업종 유니버스가 아닌 신호후보 프록시이므로 자동필터 승격 금지.', '- 위 항목은 2단계 탐색 참고자료이며 v49.71 LOCKED TEST 판정과 분리합니다.']

    lines7 = ['(7/16)', '🏁 LP 청산정책 3단계 검증 | v49.71', '──────────']
    b = triple['boundaries']
    ...
    lines8 = ['(8/16)', '🧪 L·G·S·하락장 LOCKED TEST | v49.71', '──────────']
    ...
    lines9 = ['(9/16)', '🎯 검색 의도 적합성 감사 | v49.71', '──────────']
    ...
    lines10 = ['(10/16)', '🕵️ 놓친 상승종목·단계별 누락 | v49.71', '──────────']
    ...
    lines11 = ['(11/16)', '📐 랭킹·분포변화·상시감사 | v49.71', '──────────']
    ...
    lines12 = ['(12/16)', '🚨 기술적 누락 포렌식 | v49.71', '──────────']
    ...
    if tech_fn is None or tech_fn.empty:
        ...
        if len(tech_fn) > 20:
            lines12.append(f'- 나머지 {len(tech_fn)-20}건은 v49_71_technical_false_negative.csv에 전부 저장')
    ...
    lines13 = ['(13/16)', '🔎 즉시상승 전략적 미포착 | v49.71', '──────────']
    ms = insight.get('miss_summary', pd.DataFrame())
    mc = insight.get('miss_contrast', pd.DataFrame())
    if ms is not None and not ms.empty:
        lines13.append(f'- STRATEGIC_MISS_CONFIRMED × IMMEDIATE_MISS: {int(ms.n.sum())}건')
        for _, rr in ms.sort_values('n', ascending=False).head(6).iterrows():
            lines13.append(f'- 최근접 {rr.nearest_strategy_proxy}: n{int(rr.n)} · D1 {f(rr.next_close_mean_pct)} · 5일최대 {f(rr.max5_mean_pct)} · 고정3/5 {f(rr.fixed35_mean_pct)} · 거리 {float(rr.median_distance):.2f}')
        if mc is not None and not mc.empty:
            topc=mc.sort_values('abs_cliffs',ascending=False).head(6)
            lines13.append('[미포착 vs 해당전략 포착 즉시승자 · 진입시점 차이]')
        ...
        lines13.append('- 확인된 즉시상승 전략적 미포착 없음 또는 정밀감사 표본 없음')
    lines13 += ['', '- 최근접 전략은 진입시점 특징 유사도용 사후 감사 프록시이며 새 검색식이 아닙니다.', '- threshold selection NONE · auto_apply=0']

    lines14 = ['(14/16)', '🔀 LP·G 랭킹 역전 감사 | v49.71', '──────────']
    rs = insight.get('rank_summary', pd.DataFrame())
    rf = insight.get('rank_features', pd.DataFrame())
    if rs is not None and not rs.empty:
        for mode in ['LP','G']:
            q=rs[rs.strategy.eq(mode)]
            if q.empty: continue
            rr=q.iloc[0]
            lines14.append(f'- {mode}: 비교일 {int(rr.date_groups_n)} · 1위 {f(rr.rank1_net50_mean_pct)} · 2~5위 최선 {f(rr.best_rank2_5_net50_mean_pct)} · 역전 {float(rr.inversion_pct):.1f}% · 1위손실/대안수익 {int(rr.actionable_inversion_n)}건({float(rr.actionable_inversion_pct):.1f}%)')
            if rf is not None and not rf.empty:
                qq=rf[(rf.strategy.eq(mode))&(rf.research_signal.eq(1))].head(4)
                for _,fr in qq.iterrows():
                    lines14.append(f'  · 반복차이 {fr.feature_label}: 대안-1위 중앙 {float(fr.alt_minus_rank1_median):+.2f} · IQR표준 {float(fr.standardized_median_delta_iqr):+.2f} · 3구간 {fr.stable_direction}')
    else:
        lines14.append('- LP/G 다중후보 랭킹 비교 표본 없음')
    lines14 += ['', '- 실현수익은 랭킹 오류를 해부하는 사후 라벨일 뿐 재랭킹 규칙을 자동 생성하지 않습니다.', '- rank change NONE · auto_apply=0']

    lines15 = ['(15/16)', '⚖️ A BIG vs STOP_FIRST | v49.71', '──────────']
    aas = insight.get('a_summary', pd.DataFrame())
    aac = insight.get('a_contrast', pd.DataFrame())
    if aas is not None and not aas.empty:
        rr=aas.iloc[0]
        lines15.append(f'- A OOS {int(rr.oos_n)} · BIG {int(rr.big_n)}({float(rr.big_share_pct):.1f}%) · STOP_FIRST {int(rr.stop_first_n)}({float(rr.stop_first_share_pct):.1f}%)')
        lines15.append(f'- BIG net50 {f(rr.big_net50_mean_pct)} / STOP_FIRST net50 {f(rr.stop_net50_mean_pct)} · BIG MFE {f(rr.big_mfe_mean_pct)} / STOP MAE {f(rr.stop_mae_mean_pct)}')
        if aac is not None and not aac.empty:
            sig=aac[aac.research_signal.eq(1)].head(8)
            lines15.append(f'- 3개 시간구간에서 방향 반복 + |Cliff|≥0.25: {int(aac.research_signal.sum())}개 특징')
            for _,fr in sig.iterrows():
                lines15.append(f'  · {fr.feature_label}: BIG {float(fr.big_median):.2f} vs STOP {float(fr.stop_median):.2f} · Cliff {float(fr.cliffs_delta_big_vs_stop):+.2f} · {fr.stable_direction}')
    else:
        lines15.append('- A BIG/STOP 비교 표본 없음')
    lines15 += ['', '- 반복 특징은 가설 등록용이며 A 검색식·점수·랭킹에 자동 반영하지 않습니다.']

    lines16 = ['(16/16)', '🧭 H 의도 재감사·가설 레지스트리 | v49.71', '──────────']
    hs=insight.get('h_summary', pd.DataFrame()); hm=insight.get('h_matrix', pd.DataFrame()); reg=insight.get('registry', pd.DataFrame())
    if hs is not None and not hs.empty:
        for _,rr in hs.sort_values('oos_n',ascending=False).iterrows():
            lines16.append(f'- H/{getattr(rr, "h_intent_profile", "UNKNOWN")} {rr.h_intent_v2_status}: n{int(rr.n)} · OOS n{int(rr.oos_n)} · OOS50 {f(rr.oos_net50_mean_pct)} · BIG {float(rr.big_pct):.1f}% · STOP {float(rr.stop_first_pct):.1f}%')
        if hm is not None and not hm.empty:
            ...
            lines16.append(f'- 기존 H MISMATCH → H 전용 의미프록시 CORE/RELAXED 재분류: {rec}건')
    else:
        lines16.append('- H 전용 의도 재감사 표본 없음')
    if reg is not None and not reg.empty:
        lines16.append('[가설 레지스트리 · 자동적용 없음]')
        for _,rr in reg.iterrows():
            lines16.append(f'- {rr.lane}: {rr.status} · evidence {int(rr.evidence_n)} · {rr.next_action}')
    lines16 += ['', '- H v2는 실제 H 권위식의 HIGH_DRYUP_STRICT와 CP6_PRIOR_HIGH_RECLAIM 두 프로필을 각각 감사용 의미프록시로 재표현합니다.', '- search/rank/exit/LIVE change NONE · auto_apply=0 · PAPER 유지']

    report = '\n'.join(lines1 + [''] + lines2 + [''] + lines3 + [''] + lines4 + [''] + lines5 + [''] + lines6 + [''] + lines7 + [''] + lines8 + [''] + lines9 + [''] + lines10 + [''] + lines11 + [''] + lines12 + [''] + lines13 + [''] + lines14 + [''] + lines15 + [''] + lines16)
    (out / 'v49_71_global_summary.txt').write_text(report, encoding='utf-8')
    files = [
        raw_path, canonical_path, selected_path,
        out / 'v49_71_primary_selected_top5.csv', out / 'v49_71_all_matched_selected_top5.csv',
        out / 'v49_71_strategy_funnel.csv', out / 'v49_71_strategy_stage_audit.csv', out / 'v49_71_identity_audit.csv',
        out / 'v49_71_zero_mode_audit.csv', out / 'v49_71_predicate_exceptions.csv',
        out / 'v49_71_strategy_attribution_performance.csv', out / 'v49_71_lp_walk_forward.csv', out / 'v49_71_iit_flow_split.csv',
        out / 'v49_71_selected_enriched_outcomes.csv', out / 'v49_71_outcome_distribution.csv',
        out / 'v49_71_winner_loser_feature_anatomy.csv', out / 'v49_71_train_oos_feature_lift.csv',
        out / 'v49_71_market_regime_performance.csv', out / 'v49_71_event_theme_context.csv',
        out / 'v49_71_down_market_cases.csv',
        out / 'v49_71_lgs_candidate_library.csv', out / 'v49_71_lgs_redundancy.csv', out / 'v49_71_lgs_locked_test.csv',
        out / 'v49_71_down_market_candidate_library.csv', out / 'v49_71_down_market_redundancy.csv', out / 'v49_71_down_market_locked_test.csv',
        out / 'v49_71_lp_exit_policy_audit.csv', out / 'v49_71_lp_exit_selected.csv', out / 'v49_71_lp_locked_test_stop_cases.csv',
        out / 'v49_71_triple_split_manifest.json',
        out / 'v49_71_search_intent_enriched.csv', out / 'v49_71_search_intent_summary.csv',
        out / 'v49_71_winner_loser_effect_size.csv', out / 'v49_71_manual_casebook.csv',
        out / 'v49_71_ranking_summary.csv', out / 'v49_71_ranking_detail.csv',
        out / 'v49_71_opportunity_miss_summary.csv', out / 'v49_71_opportunity_census_global.csv',
        out / 'v49_71_missed_winner_casebook.csv', out / 'v49_71_technical_false_negative.csv',
        out / 'v49_71_technical_false_negative.json', out / 'v49_71_opportunity_path_summary.csv',
        out / 'v49_71_distribution_drift.csv', out / 'v49_71_search_quality_manifest.json',
        out / 'v49_71_immediate_strategic_miss_summary.csv', out / 'v49_71_immediate_strategic_miss_feature_contrast.csv',
        out / 'v49_71_immediate_strategic_miss_casebook.csv', out / 'v49_71_immediate_strategic_miss_detail.csv',
        out / 'v49_71_rank_inversion_summary.csv', out / 'v49_71_rank_inversion_feature_stability.csv', out / 'v49_71_rank_inversion_cases.csv',
        out / 'v49_71_a_big_vs_stop_summary.csv', out / 'v49_71_a_big_vs_stop_feature_stability.csv', out / 'v49_71_a_big_vs_stop_casebook.csv',
        out / 'v49_71_h_intent_v2_summary.csv', out / 'v49_71_h_intent_v1_v2_matrix.csv', out / 'v49_71_h_intent_v2_condition_pass.csv',
        out / 'v49_71_h_intent_v2_detail.csv', out / 'v49_71_hypothesis_registry.csv', out / 'v49_71_insight_manifest.json',
    ]
    manifest = {
    ...
    (out / 'v49_71_global_manifest.json').write_text(json.dumps(manifest, ensure_ascii=False, indent=2, default=str), encoding='utf-8')
    print(report)
    _deliver_telegram([lines1, lines2, lines3, lines4, lines5, lines6, lines7, lines8, lines9, lines10, lines11, lines12, lines13, lines14, lines15, lines16], out, bool(args.send_telegram))
