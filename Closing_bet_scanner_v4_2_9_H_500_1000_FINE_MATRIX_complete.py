...
# =============================================================
# closing_bet_scanner.py — 종가배팅 타점 스캐너 (G Morales v4.2.9 S/G/H 통합 운영 FINAL)
# =============================================================
# 전략 구성
# A  : 돌파형 종가배팅
...
CLOSING_BET_SCANNER_VERSION = 'G_MORALES_V4_2_9_H_500_1000_FINE_MATRIX_20260512'
...
def _send_results(hits: list, mins_left: int):
    ...
    def _build_practical_block():
        ...
        block = ['[🎯 실전 운영 후보 — v4.2.9 S/G/H 통합 FINAL]']
        summary = (
...
def _format_h_v427_core_report(df: pd.DataFrame) -> str:
    ...
    lines = ["[H v4.2.9 핵심 운용분류 — TRIANGLE / AMOUNT×VOL]"]
    try:
        if df is None or df.empty or 'mode' not in df.columns:
            lines.append("- 데이터 없음")
            return "\n".join(lines)
...
def _format_h_breakout_volume_report(df: pd.DataFrame) -> str:
    ...
    try:
        ...
        lines.append("- v4.2.9 운용분류: 🟢H-VOL SAFE=2~3배 / 🟣H-VOL SWING=3~5배 / ⚠️H-OVERHEAT=5~8배 / 🔥H-AGGRESSIVE=8배 이상")
...
def _format_h_reason_report(df: pd.DataFrame) -> str:
    ...
    try:
        ...
        lines.append("- 해석: H-STRICT는 장대양봉 신고가 돌파봉을 먼저 찾고, 이후 거래량이 마른 짧은 타점봉을 검증합니다. v4.2.9에서는 돌파Vol60 2~3배만 최종 H-SAFE로 분류합니다.")
    except Exception as e:
...
def _build_backtest_summary(
    ...
    if not h_df.empty:
        ...
        lines.append(_format_h_500_1000_fine_matrix_report(h_df))
        lines.append("")
        ...
        lines.append(_format_h_ma5_block(h_df[_bt_mask_h_core_500_700_vol23(h_df)], 'H-MA5 CORE 500~700억×2~3배'))
        lines.append(_format_h_ma5_block(h_df[_bt_mask_h_core_700_1000_vol23(h_df)], 'H-MA5 CORE 700~1000억×2~3배'))
        lines.append(_format_h_ma5_block(h_df[_bt_mask_h_watch_500_1000_vol30_40(h_df)], 'H-MA5 WATCH 500~1000억×3~4배'))
        lines.append(_format_h_ma5_block(h_df[_bt_mask_h_watch_500_1000_vol40_50(h_df)], 'H-MA5 WATCH 500~1000억×4~5배'))
    ...
    lines.append("- v4.2.9부터 실시간 스캔에서도 H-TRIANGLE SAFE와 거래대금×Vol60 핵심셀(500~1000억×2~3배, 500~1000억 세부구간, 300~500억×3~5배)을 S/G/H 통합 운영 후보로 표시하고, 돌파봉 Vol60 배율 운용분류에 더해, 거래대금 구간을 <30억/30~50억/50~100억/100~200억/200~300억/300~500억/500~1000억/1000~2000억/2000~3000억/3000~5000억/5000억~1조/1조+로 세분화해 성과를 분해합니다. H-VOL SAFE는 2~3배이며 v4.2.9에서는 500~1000억 내부를 500~600/600~700/700~850/850~1000억 및 Vol60 1.5~5배로 추가 세분화합니다. H-VOL SAFE는 2~3배, H-VOL SWING은 3~5배, H-OVERHEAT는 5~8배, H-AGGRESSIVE는 8배 이상입니다. 수급 데이터는 이번 버전에서 필수 조건이 아니라 후속 가점 후보입니다.")
