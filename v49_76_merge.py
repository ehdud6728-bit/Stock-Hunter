...
import v49_76_research as r70
import v49_76_quality as q70
import v49_76_insight as i72
import v49_76_input as ia72
import v49_76_marcap as mc74
import v49_76_pit_marcap as pit75
import v49_76_practical as practical76
...
def _deliver_telegram(parts: list[list[str]], out: Path, requested: bool) -> dict:
    ...
    path = out / 'v49_76_telegram_delivery_manifest.json'
    try:
        if requested:
            if delivery['route_validated'] != '1':
                raise RuntimeError('v49.76 Telegram requested but unified route preflight was not validated')
            if not s._telegram_route_ready():
                raise RuntimeError('v49.76 Telegram requested but scanner route is not ready after validated preflight')
            for part_no, part in enumerate(parts, start=1):
                ...
                if int(result.get('success_count', 0) or 0) < 1:
                    raise RuntimeError(f'v49.76 Telegram delivery failed for part {part_no}: {result.get("errors", [])}')
...
def _lp_validation(selected: pd.DataFrame, out: Path, start_date: str, end_date: str) -> pd.DataFrame:
    ...
    if z.empty:
        ...
        r.to_csv(out / 'v49_76_lp_walk_forward.csv', index=False, encoding='utf-8-sig')
        return r
...
def _iit_split(selected: pd.DataFrame, out: Path, start_date: str, end_date: str) -> pd.DataFrame:
    ...
    r = pd.DataFrame(rows)
    r.to_csv(out / 'v49_76_iit_flow_split.csv', index=False, encoding='utf-8-sig')
    return r


# ---- v49.76 winner/loser, market, sector and causal feature audit ----
...
def _v4967_anatomy_tables(x: pd.DataFrame, out: Path, start_date: str, end_date: str) -> tuple[pd.DataFrame,pd.DataFrame,pd.DataFrame,pd.DataFrame,pd.DataFrame]:
    ...
    z.to_csv(out/'v49_76_selected_enriched_outcomes.csv',index=False,encoding='utf-8-sig')
    outcomes.to_csv(out/'v49_76_outcome_distribution.csv',index=False,encoding='utf-8-sig')
    features.to_csv(out/'v49_76_winner_loser_feature_anatomy.csv',index=False,encoding='utf-8-sig')
    lifts.to_csv(out/'v49_76_train_oos_feature_lift.csv',index=False,encoding='utf-8-sig')
    market.to_csv(out/'v49_76_market_regime_performance.csv',index=False,encoding='utf-8-sig')
    events.to_csv(out/'v49_76_event_theme_context.csv',index=False,encoding='utf-8-sig')
    down.to_csv(out/'v49_76_down_market_cases.csv',index=False,encoding='utf-8-sig')
    return outcomes,features,lifts,market,events


def main() -> int:
    ap = argparse.ArgumentParser(description='v49.76 global merge + input reproducibility/completeness authority audit')
    ap.add_argument('--input-root', default='v49_76_downloads')
    ap.add_argument('--prepare-dir', default='v49_76_prepare_output')
    ...
    ap.add_argument('--baseline-input-authority', default='', help='optional prior same-period v49.76 input authority manifest')
    ap.add_argument('--baseline-prepared-snapshot', default='', help='optional prior same-period v49.76 prepared authority snapshot CSV')
    ap.add_argument('--legacy-v4974-prepared-snapshot', default='', help='optional prior v49.74 same-request-date snapshot; value parity only, as-of label may differ')
    ap.add_argument('--pit-cache-dir', default='.v49_76_pit_marcap_cache', help='persistent date-level point-in-time MARCAP cache')
    args = ap.parse_args()
    root = Path(args.input_root)
    prep_root = Path(args.prepare_dir)
    if not prep_root.exists():
        prep_root = root
    preflight_path = next(iter(prep_root.rglob('v49_76_preflight.json')), None)
    universe_path = next(iter(prep_root.rglob('v49_76_universe.json')), None)
    prepared_snapshot_path = next(iter(prep_root.rglob('v49_76_prepared_authority_snapshot.csv')), None)
    prepared_snapshot_manifest_path = next(iter(prep_root.rglob('v49_76_prepared_authority_snapshot.json')), None)
    if preflight_path is None or universe_path is None or prepared_snapshot_path is None or prepared_snapshot_manifest_path is None:
    ...
    _snap_manifest=json.loads(prepared_snapshot_manifest_path.read_text(encoding='utf-8'))
    if file_sha(prepared_snapshot_path) != str(_snap_manifest.get('csv_sha256','')):
        raise RuntimeError('prepared authority snapshot CSV sha mismatch')
    _expected_marcap_sha=str((preflight.get('input_authority') or {}).get('marcap_map_sha256',''))
    if not _expected_marcap_sha or _expected_marcap_sha != str(_snap_manifest.get('marcap_map_sha256','')):
        raise RuntimeError(f'prepare MARCAP authority mismatch: preflight {_expected_marcap_sha} != snapshot {_snap_manifest.get("marcap_map_sha256","")}')
    ...
    history_files = sorted(root.rglob('shard_*_history_authority.csv'))
    counts = {'manifest': len(manifests), 'raw': len(raws), 'complete': len(completes), 'funnel': len(funnels), 'zero': len(zero_files), 'exceptions': len(exc_files), 'opportunity': len(opportunity_files), 'history': len(history_files)}
    ...
    raw_by, funnel_by, zero_by, exc_by, opportunity_by, history_by = map(idmap, (raws, funnels, zero_files, exc_files, opportunity_files, history_files))
    for sid in expected_ids:
        ...
        hp=history_by[sid]
        if file_sha(hp) != str(m.get('history_file_sha256','')) or file_sha(hp) != str(c.get('history_file_sha256','')):
            raise RuntimeError(f'history authority sha mismatch shard {sid}')
        _prep_auth=str((m.get('prepared_authority') or {}).get('prepared_authority_sha256',''))
        _expected_auth=str((preflight.get('input_authority') or {}).get('prepared_authority_sha256',''))
        if not _prep_auth or _prep_auth != _expected_auth:
            raise RuntimeError(f'prepared input authority mismatch shard {sid}: {_prep_auth} != {_expected_auth}')
        _pm=str(m.get('prepared_marcap_sha256','')); _rm=str(m.get('runtime_marcap_sha256',''))
        if not _pm or _pm != _expected_marcap_sha or _rm != _expected_marcap_sha or str(m.get('marcap_snapshot_consensus',''))!='VALID':
            raise RuntimeError(f'MARCAP_SNAPSHOT_CONSENSUS_FAIL shard {sid}: prepared={_pm} runtime={_rm} expected={_expected_marcap_sha}')
        if str(m.get('marcap_snapshot_source','')) != str(_snap_manifest.get('source','')) or str(m.get('marcap_snapshot_asof_date','')) != str(_snap_manifest.get('asof_date','')):
            raise RuntimeError(f'MARCAP_SNAPSHOT_METADATA_CONSENSUS_FAIL shard {sid}')
        if str(m.get('marcap_snapshot_calendar_source','')) != str(_snap_manifest.get('calendar_source','')) or str(m.get('marcap_snapshot_calendar_probe','')) != str(_snap_manifest.get('calendar_probe','')):
            raise RuntimeError(f'MARCAP_TRADING_CALENDAR_CONSENSUS_FAIL shard {sid}')
        if str(m.get('marcap_snapshot_requested_is_trading_day')) != str(_snap_manifest.get('requested_is_trading_day')):
            raise RuntimeError(f'MARCAP_TRADING_DATE_FLAG_CONSENSUS_FAIL shard {sid}')
        if str(m.get('engine_status', '')).upper() != 'VALID':
            raise RuntimeError(f'shard engine invalid {sid}')
        if str(m.get('control_flow', '')) != 'V49_75_SHARD_EXPORT_RETURN_MAIN_EXIT':
    ...
    history_df = _read_csvs(history_files, dtype={'code': str})
    if raw_input.empty:
    ...
    raw_path = out / 'v49_76_global_raw.csv'
    canonical_path = out / 'v49_76_global_canonical.csv'
    selected_path = out / 'v49_76_global_selected_top5.csv'
    ...
    populations['PRIMARY'].to_csv(out / 'v49_76_primary_selected_top5.csv', index=False, encoding='utf-8-sig')
    populations['ALL_MATCHED'].to_csv(out / 'v49_76_all_matched_selected_top5.csv', index=False, encoding='utf-8-sig')
    ...
    # v49.76 INPUT AUTHORITY: reproducibility and data completeness are independent axes.
    ...
    input_authority={
        'version':'v49.76','authority_schema_version':'V49_75_INPUT_AUTHORITY_SCHEMA_4_TRADING_DAY_MARCAP_LOCK',
        ...
        **_prepared_auth,**_hist_agg,**_pop_auth,
        ...
        'marcap_snapshot_consensus':'VALID','marcap_snapshot_source':str(_snap_manifest.get('source','')),
        'marcap_snapshot_asof_date':str(_snap_manifest.get('asof_date','')),'marcap_snapshot_unit':str(_snap_manifest.get('unit','')),
        'marcap_snapshot_requested_is_trading_day':_snap_manifest.get('requested_is_trading_day'),
        'marcap_snapshot_calendar_source':str(_snap_manifest.get('calendar_source','')),'marcap_snapshot_calendar_probe':str(_snap_manifest.get('calendar_probe','')),
        'marcap_snapshot_calendar_lookback_days':_snap_manifest.get('calendar_lookback_days'),
        'marcap_snapshot_authority_quality':str(_snap_manifest.get('authority_quality','')),
        'marcap_snapshot_csv_sha256':file_sha(prepared_snapshot_path),'marcap_snapshot_manifest_sha256':file_sha(prepared_snapshot_manifest_path),
    }
    ...
    # v49.76 same-period MARCAP parity forensic. This is diagnostic detail for any
    # prepared-authority drift and never changes strategy thresholds.
    _marcap_parity,_marcap_detail=mc74.compare_marcap_snapshots(prepared_snapshot_path,args.baseline_prepared_snapshot,mcap_or_min=float(s.MCAP_OR_MIN))
    input_authority['marcap_parity']=dict(_marcap_parity)
    _marcap_detail.to_csv(out / 'v49_76_marcap_parity_detail.csv',index=False,encoding='utf-8-sig')
    (out / 'v49_76_marcap_parity_summary.json').write_text(json.dumps(_marcap_parity,ensure_ascii=False,indent=2,default=str),encoding='utf-8')
    _legacy_marcap_parity,_legacy_marcap_detail=mc74.compare_marcap_snapshots(prepared_snapshot_path,args.legacy_v4974_prepared_snapshot,mcap_or_min=float(s.MCAP_OR_MIN),require_same_asof=False)
    input_authority['legacy_v4974_marcap_parity']=dict(_legacy_marcap_parity)
    _legacy_marcap_detail.to_csv(out / 'v49_76_legacy_v4974_marcap_parity_detail.csv',index=False,encoding='utf-8-sig')
    (out / 'v49_76_legacy_v4974_marcap_parity_summary.json').write_text(json.dumps(_legacy_marcap_parity,ensure_ascii=False,indent=2,default=str),encoding='utf-8')
    pd.read_csv(prepared_snapshot_path,dtype={'code':str}).to_csv(out / 'v49_76_prepared_authority_snapshot.csv',index=False,encoding='utf-8-sig')
    (out / 'v49_76_prepared_authority_snapshot.json').write_text(prepared_snapshot_manifest_path.read_text(encoding='utf-8'),encoding='utf-8')

    history_df.to_csv(out / 'v49_76_history_authority_global.csv',index=False,encoding='utf-8-sig')
    ...
    _issue_rows.to_csv(out / 'v49_76_history_completeness_issues.csv',index=False,encoding='utf-8-sig')
    (out / 'v49_76_input_authority_manifest.json').write_text(json.dumps(input_authority,ensure_ascii=False,indent=2,default=str),encoding='utf-8')
    ...
    funnel.to_csv(out / 'v49_76_strategy_funnel.csv', index=False, encoding='utf-8-sig')
    strategy_status.to_csv(out / 'v49_76_strategy_stage_audit.csv', index=False, encoding='utf-8-sig')
    identity_audit.to_csv(out / 'v49_76_identity_audit.csv', index=False, encoding='utf-8-sig')
    zero_df.to_csv(out / 'v49_76_zero_mode_audit.csv', index=False, encoding='utf-8-sig')
    exc_df.to_csv(out / 'v49_76_predicate_exceptions.csv', index=False, encoding='utf-8-sig')
    ...
    if out.resolve() != generated_root.resolve():
        import shutil
        for gp in generated_root.glob('v49_76_common_strategy_*'):
            shutil.copy2(gp, out / gp.name)
    performance_csv = out / 'v49_76_common_strategy_performance.csv'
    portfolio_csv = out / 'v49_76_common_strategy_portfolio.csv'
    ...
    attribution.to_csv(out / 'v49_76_strategy_attribution_performance.csv', index=False, encoding='utf-8-sig')
    ...
    down_cases_path = out / 'v49_76_down_market_cases.csv'
    down_cases = pd.read_csv(down_cases_path, dtype={'code':str}) if down_cases_path.exists() else pd.DataFrame()

    # v49.76: TRAIN -> VALIDATION -> LOCKED_TEST research. Thresholds are discovered on
    ...
    # v49.76 independent SEARCH QUALITY AUDIT: machine intent proxy, winner/loser casebook,
    ...
    # v49.76 SEARCH GAP INSIGHT: audit-only lanes for immediate strategic misses,
    # LP/G rank inversions, A BIG-vs-STOP anatomy, and H intent proxy re-audit.
    # This layer registers hypotheses only; it cannot change search/rank/exit/LIVE behavior.
    _standard_oos_start=_split_date(args.start_date,args.end_date)
    insight = i72.run_insight_audit(raw_df, quality['enriched'], quality['opp_detail'], out, V4967_ENTRY_FEATURE_META,
                                    _standard_oos_start, float(getattr(s,'MIN_PRICE',5000)), float(getattr(s,'MIN_AMOUNT',3_000_000_000))/1e8)
    insight_manifest = insight['manifest']
    insight_status = str(insight_manifest.get('status','NO-DATA'))

    # v49.76 HISTORICAL POINT-IN-TIME MARCAP LOOK-AHEAD AUDIT.
    # Production/search results remain on the v49.74 trading-day END-DATE MARCAP authority.
    # This lane only asks how many currently emitted signals would have failed the 2,000억
    # market-cap OR gate on their own signal date. Historical index membership is deliberately
    # held constant and reverse additions require a separate full predicate shadow replay.
    try:
        pit_audit=pit75.run_audit(
            s, canonical, selected, prepared_snapshot_path, out, args.pit_cache_dir,
            _standard_oos_start, top_per_strategy=max(1,args.top_per_strategy),
            mcap_or_min=float(getattr(s,'MCAP_OR_MIN',200_000_000_000)),
        )
        pit_manifest=dict(pit_audit.get('manifest') or {})
    except Exception as _pit_exc:
        pit_manifest={
            'version':'v49.76','lane':'HISTORICAL_POINT_IN_TIME_MARCAP_LOOKAHEAD_AUDIT',
            'status':'INVALID_AUDIT_ERROR','error':f'{type(_pit_exc).__name__}: {_pit_exc}',
            'policy':'AUDIT_ONLY_NO_AUTO_APPLY','full_reverse_replay_performed':False,
        }
        (out/'v49_76_pit_marcap_audit_manifest.json').write_text(json.dumps(pit_manifest,ensure_ascii=False,indent=2,default=str),encoding='utf-8')
        pit_audit={'manifest':pit_manifest,'strategy_impact':pd.DataFrame(),'future_cases':pd.DataFrame(),'future_selected':pd.DataFrame(),'code_summary':pd.DataFrame(),'date_coverage':pd.DataFrame(),'shadow_selected':pd.DataFrame()}

    # v49.76 PRACTICAL PAPER EXECUTION RESEARCH INTEGRATION.
    # A MA60 uses one TRAIN-derived fixed threshold, H HIGH_DRYUP is prospective-label only,
    # and PRE-IGNITION remains a retrospective casebook. None of these lanes mutates search/rank/exit/LIVE.
    try:
        practical_audit=practical76.run_practical_audit(
            quality['enriched'], ptab, out, out,
            min_validation_n=int(os.environ.get('CLOSING_BET_V4976_A_MA60_MIN_VALIDATION_N','15') or 15),
            min_test_n=int(os.environ.get('CLOSING_BET_V4976_A_MA60_MIN_TEST_N','15') or 15),
            min_lift_pctp=float(os.environ.get('CLOSING_BET_V4976_A_MA60_MIN_LIFT_PCTP','0.30') or 0.30),
        )
        practical_manifest=dict(practical_audit.get('manifest') or {})
    except Exception as _prac_exc:
        practical_manifest={'version':'v49.76','status':'INVALID_AUDIT_ERROR','error':f'{type(_prac_exc).__name__}: {_prac_exc}','search_change':'NONE','rank_change':'NONE','exit_change':'NONE','auto_apply':0,'paper_only':True}
        (out/'v49_76_practical_manifest.json').write_text(json.dumps(practical_manifest,ensure_ascii=False,indent=2,default=str),encoding='utf-8')
        practical_audit={'manifest':practical_manifest,'a':{'manifest':{},'summary':pd.DataFrame()},'h':{'manifest':{},'current':pd.DataFrame()},'preignition':{'manifest':{},'casebook':pd.DataFrame()},'readiness':pd.DataFrame()}
    ...
    lines1 = [
        ...
        f'- MARCAP AUTHORITY: CONSENSUS VALID ✅ · {input_authority.get("marcap_snapshot_source","-")} · requested {universe.get("requested_end_date","-")} → trading-asof {input_authority.get("marcap_snapshot_asof_date","-")} · sha {str(input_authority.get("marcap_map_sha256",""))[:16]}',
        ...
        f'- SEARCH GAP INSIGHT: {insight_status} · threshold selection NONE · auto_apply 0', '',
        '[전체 성과]',
    ...
    lines2 = ['(2/20)', '📊 전략별 OOS · Predicate Mode | v49.76', '──────────']
    if not ptab.empty:
    ...
    lines3 = ['(3/20)', '🔬 전략 Funnel·Pipeline | v49.76', '──────────']
    ...
    lines4 = ['(4/20)', '🧭 전략 귀속·LP·I/IT | v49.76', '──────────']
    for mode in s.CLOSING_BET_V4958_PRIMARY_PRIORITY:
    ...
    lines5 = ['(5/20)', '🧬 상승·하락 경로 해부 | v49.76', '──────────']
    ...
    lines6 = ['(6/20)', '🌧️ 하락장 독립상승·개선후보 | v49.76', '──────────']
    ...
    if not feature_candidates.empty:
        lines6.append('[구형 2단계 탐색 · v49.76 최종 승격근거 아님]')
    ...
    lines7 = ['(7/20)', '🏁 LP 청산정책 3단계 검증 | v49.76', '──────────']
    b = triple['boundaries']
    ...
    lines8 = ['(8/20)', '🧪 L·G·S·하락장 LOCKED TEST | v49.76', '──────────']
    ...
    lines9 = ['(9/20)', '🎯 검색 의도 적합성 감사 | v49.76', '──────────']
    ...
    lines10 = ['(10/20)', '🕵️ 놓친 상승종목·단계별 누락 | v49.76', '──────────']
    ...
    lines11 = ['(11/20)', '📐 랭킹·분포변화·상시감사 | v49.76', '──────────']
    ...
    lines12 = ['(12/20)', '🚨 기술적 누락 포렌식 | v49.76', '──────────']
    ...
    lines13 = ['(13/20)', '🔎 즉시상승 미포착 · 정책제외 vs 진짜 패턴누락 | v49.76', '──────────']
    ...
    if ps is not None and not ps.empty:
        ...
        for _,rr in ps.sort_values('n',ascending=False).iterrows():
            lines13.append(f'- {rr.miss_policy_class}: n{int(rr.n)} ({float(rr.share_pct):.1f}%) · 거래대금 중앙 {float(rr.median_amount_b):.1f}억 · D1 {f(rr.next_close_mean_pct)} · 5일최대 {f(rr.max5_mean_pct)} · 고정3/5 {f(rr.fixed35_mean_pct)}')
        true=ms[ms.miss_policy_class.eq('TRUE_PATTERN_MISS_WITHIN_CURRENT_POLICY')] if ms is not None and not ms.empty else pd.DataFrame()
        if not true.empty:
            lines13.append(f'[진짜 Pattern Miss · 현재 가격/{float(getattr(s,"MIN_AMOUNT",3_000_000_000))/1e8:.0f}억 유동성 정책 충족]')
            for _,rr in true.sort_values('n',ascending=False).head(6).iterrows():
                lines13.append(f'- 최근접 {rr.nearest_strategy_proxy}: n{int(rr.n)} · D1 {f(rr.next_close_mean_pct)} · 5일최대 {f(rr.max5_mean_pct)} · 거리 {float(rr.median_distance):.2f}')
        if mc is not None and not mc.empty:
            lines13.append('[TRUE Pattern Miss vs 포착 즉시승자 · 진입시점 차이]')
            for _,rr in mc.sort_values('abs_cliffs',ascending=False).head(5).iterrows():
                lines13.append(f'- {rr.nearest_strategy_proxy}/{rr.feature_label}: miss {float(rr.miss_median):.2f} vs capture {float(rr.captured_median):.2f} · Cliff {float(rr.cliffs_delta_miss_vs_capture):+.2f}')
    else:
    ...
    lines14 = ['(14/20)', '🔀 LP·G 랭킹 역전 · STANDARD_OOS 전용 | v49.76', '──────────']
    rs=insight.get('rank_summary',pd.DataFrame()); rf=insight.get('rank_features',pd.DataFrame())
    if rs is not None and not rs.empty:
        for mode in ['LP','G']:
            q=rs[rs.strategy.eq(mode)]
            if q.empty: continue
            rr=q.iloc[0]
            lines14.append(f'- {mode}: STANDARD_OOS 비교일 {int(rr.date_groups_n)} · 1위 {f(rr.rank1_net50_mean_pct)} · 2~5위 최선 {f(rr.best_rank2_5_net50_mean_pct)} · 역전 {float(rr.inversion_pct):.1f}% · 1위손실/대안수익 {int(rr.actionable_inversion_n)}건({float(rr.actionable_inversion_pct):.1f}%)')
            qq=rf[(rf.strategy.eq(mode))&(rf.research_signal.eq(1))].head(4) if rf is not None and not rf.empty else pd.DataFrame()
            for _,fr in qq.iterrows():
                lines14.append(f'  · 반복차이 {fr.feature_label}: 대안-1위 {float(fr.alt_minus_rank1_median):+.2f} · IQR {float(fr.standardized_median_delta_iqr):+.2f} · {fr.stable_direction}')
    ...
    lines15 = ['(15/20)', '⚖️ A BIG_CAPTURABLE vs STOP_FIRST · 상호배타 | v49.76', '──────────']
    ...
    if aas is not None and not aas.empty:
        for scope in ['STANDARD_OOS','LOCKED_TEST','VALIDATION']:
            q=aas[aas.sample_authority.eq(scope)]
            if q.empty: continue
            rr=q.iloc[0]
            lines15.append(f'- {scope}: n{int(rr.n)} = BIG {int(rr.big_n)} + STOP {int(rr.stop_first_n)} + 기타 {int(rr.other_n)} · overlap {int(rr.overlap_n)} · 회계 {"PASS" if int(rr.accounting_ok)==1 else "FAIL"}')
            lines15.append(f'  · BIG net50 {f(rr.big_net50_mean_pct)} / STOP {f(rr.stop_net50_mean_pct)} · BIG MFE {f(rr.big_mfe_mean_pct)} / STOP MAE {f(rr.stop_mae_mean_pct)}')
        if aac is not None and not aac.empty:
            sig=aac[aac.research_signal.eq(1)].head(8); lines15.append(f'- STANDARD_OOS 3구간 반복 + |Cliff|≥0.25: {int(aac.research_signal.sum())}개 특징')
            for _,fr in sig.iterrows(): lines15.append(f'  · {fr.feature_label}: BIG {float(fr.big_median):.2f} vs STOP {float(fr.stop_median):.2f} · Cliff {float(fr.cliffs_delta_big_vs_stop):+.2f} · {fr.stable_direction}')
    ...
    lines16 = ['(16/20)', '🧭 H 이중프로필 · 표본권위 분리 | v49.76', '──────────']
    ...
    if hs is not None and not hs.empty:
        for scope in ['STANDARD_OOS','LOCKED_TEST']:
            q=hs[hs.sample_authority.eq(scope)].sort_values('n',ascending=False)
            if q.empty: continue
            lines16.append(f'[{scope}]')
            for _,rr in q.head(8).iterrows():
                lines16.append(f'- H/{rr.h_intent_profile} {rr.h_intent_v2_status}: n{int(rr.n)} · net50 {f(rr.net50_mean_pct)} · BIG {float(rr.big_pct):.1f}% · STOP {float(rr.stop_first_pct):.1f}%')
        if hm is not None and not hm.empty:
            rec=int(hm[(hm.old_intent_status=='INTENT_MISMATCH')&hm.h_intent_v2_status.astype(str).str.contains('_(?:CORE|RELAXED)_INTENT$',regex=True)]['n'].sum())
            lines16.append(f'- 기존 H MISMATCH → 권위식 의미프록시 CORE/RELAXED 재분류: {rec}건')
    ...
    lines17 = ['(17/20)', '🔒 입력·MARCAP 권위·표본권위 | v49.76', '──────────']
    ...
    lines17.append(f'- KRX TRADING-DATE AUTHORITY: {input_authority.get("marcap_snapshot_calendar_source","-")} · probe {input_authority.get("marcap_snapshot_calendar_probe","-")} · calendar lookback {input_authority.get("marcap_snapshot_calendar_lookback_days","-")}d · quality {input_authority.get("marcap_snapshot_authority_quality","-")}')
    _mp=input_authority.get('marcap_parity',{}) or {}
    lines17.append(f'- MARCAP BASELINE PARITY: {_mp.get("status","NO_BASELINE")} · changed {int(_mp.get("changed_codes_n",0) or 0)} · mcap PASS→FAIL {int(_mp.get("mcap_pass_to_fail_n",0) or 0)} / FAIL→PASS {int(_mp.get("mcap_fail_to_pass_n",0) or 0)} · universe PASS→FAIL {int(_mp.get("universe_pass_to_fail_n",0) or 0)} / FAIL→PASS {int(_mp.get("universe_fail_to_pass_n",0) or 0)}')
    _lmp=input_authority.get('legacy_v4974_marcap_parity',{}) or {}
    if _lmp.get('status') not in ('NO_BASELINE','CURRENT_SNAPSHOT_MISSING','BASELINE_READ_ERROR','NOT_COMPARABLE_PERIOD'):
        lines17.append(f'- LEGACY v49.74 MARCAP VALUE PARITY: {_lmp.get("status")} · label {_lmp.get("baseline_asof","-")}→{_lmp.get("current_asof","-")} · changed {int(_lmp.get("changed_codes_n",0) or 0)} · universe cross {int(_lmp.get("universe_pass_to_fail_n",0) or 0)+int(_lmp.get("universe_fail_to_pass_n",0) or 0)}')
    if _mp.get('status')=='CHANGED':
        lines17.append(f'  ⛔ zero→positive {int(_mp.get("zero_to_positive_n",0) or 0)} · positive→zero {int(_mp.get("positive_to_zero_n",0) or 0)} · ≥10x {int(_mp.get("ratio_ge_10x_n",0) or 0)} · ≤0.1x {int(_mp.get("ratio_le_0_1x_n",0) or 0)} · ≥2x {int(_mp.get("ratio_ge_2x_n",0) or 0)} · ≤0.5x {int(_mp.get("ratio_le_0_5x_n",0) or 0)}')
        _tops=list(_mp.get('top_changes',[]) or [])
        if _tops:
            lines17.append('[MARCAP 차이 TOP · 최대 5건]')
            for it in _tops[:5]:
                _bv=float(it.get('baseline_marcap_krw',0) or 0)/1e8; _cv=float(it.get('current_marcap_krw',0) or 0)/1e8
                _rv=it.get('ratio'); _rt='-' if _rv is None else f'{float(_rv):.3g}x'
                _cross=[]
                if it.get('mcap_pass_to_fail'): _cross.append('MCAP PASS→FAIL')
                if it.get('mcap_fail_to_pass'): _cross.append('MCAP FAIL→PASS')
                if it.get('universe_pass_to_fail'): _cross.append('UNIV PASS→FAIL')
                if it.get('universe_fail_to_pass'): _cross.append('UNIV FAIL→PASS')
                lines17.append(f'  · {it.get("code","-")} {it.get("name","")} · { _bv:.1f}억→{_cv:.1f}억 · {_rt} · {"/".join(_cross) or "VALUE_CHANGE"}')
        lines17.append('  · 전체 종목별 차이는 v49_76_marcap_parity_detail.csv에 저장')
    ...
    if sa is not None and not sa.empty:
        ...
        lines17.append(f'- SAMPLE AUTHORITY A: STANDARD_OOS {vals.get("STANDARD_OOS",0)} · VALIDATION {vals.get("VALIDATION",0)} · LOCKED_TEST {vals.get("LOCKED_TEST",0)} · RESEARCH_HOLDOUT {vals.get("RESEARCH_HOLDOUT",0)}')
    reg=insight.get('registry',pd.DataFrame())
    ...
    lines18=['(18/20)','🕰️ Signal-Date MARCAP · Look-Ahead 감사 | v49.76','──────────']
    _pm=pit_manifest or {}
    ...
    lines18.append(f'- 확정 영향범위 A/B1/B2/S/SLOCK/C/G · 현재 raw 중 FUTURE_MCAP_INCLUDED {_pm.get("future_mcap_included_raw_n",0)}건 ({float(_pm.get("future_mcap_included_raw_pct",0) or 0):.2f}%) · 현재 TOP5 {_pm.get("future_mcap_included_selected_n",0)}건 / {_pm.get("future_mcap_included_selected_dates_n",0)}일')
    _pi=pit_audit.get('strategy_impact',pd.DataFrame())
    if _pi is not None and not _pi.empty:
        lines18.append('[현재 TOP5 vs PIT one-sided 제거 shadow · STANDARD_OOS]')
        for _mode in ['LP','A','B1','B2','S','SLOCK','C','G','H','L','I','IT']:
            _q=_pi[_pi.strategy.eq(_mode)]
            if _q.empty: continue
            _r=_q.iloc[0]
            if str(_r.get('gate_scope',''))!='DEFINITIVE_MARCAP_OR_INDEX':
                continue
            lines18.append(f'- {_mode}: futureRaw {int(_r.future_mcap_included_raw_n)} · futureTOP5 {int(_r.future_mcap_included_selected_n)} · OOS50 {f(_r.current_oos_net50_mean_pct)} → shadow {f(_r.shadow_oos_net50_mean_pct)} · Δ {f(_r.shadow_minus_current_oos_net50_pctp)}')
    _pcs=pit_audit.get('code_summary',pd.DataFrame())
    if _pcs is not None and not _pcs.empty:
        lines18.append('[미래 시총 때문에 현재 raw에 포함된 종목 · 상위]')
        for _,_r in _pcs.head(5).iterrows():
            _sig=float(_r.get('median_signal_marcap_krw',0) or 0)/1e8; _end=float(_r.get('end_marcap_krw',0) or 0)/1e8
            _ratio=_r.get('end_to_signal_marcap_ratio',np.nan); _rt='-' if pd.isna(_ratio) else f'{float(_ratio):.2f}x'
            lines18.append(f'- {str(_r.get("code",""))} {_r.get("name","")} · {int(_r.get("future_included_raw_n",0))}건/{int(_r.get("signal_dates_n",0))}일 · signal중앙 {_sig:.0f}억 → END {_end:.0f}억 ({_rt})')
    if _pm.get('error'):
    ...
    lines19=['(19/20)','🧪 A-MA60 고정가설 · H-Dryup · PRE-IGNITION | v49.76','──────────']
    ...
    if isinstance(_as,pd.DataFrame) and not _as.empty:
        for _,_r in _as.iterrows():
            lines19.append(f'- {_r.stage}: base n{int(_r.base_n)} {f(_r.base_net50)} → MA60통과 n{int(_r.filtered_n)} {f(_r.filtered_net50)} · lift {f(_r.lift_pctp)} · Top3제거 {f(_r.top3_removed_net50)} · BIG {f(_r.big_pct)} / STOP {f(_r.stop_pct)}')
    _hm=dict((practical_audit.get('h') or {}).get('manifest') or {})
    lines19.append(f'- H HIGH_DRYUP prospective: {_hm.get("status","NO-DATA")} · latest {_hm.get("latest_signal_date","-")} · 현재관찰 {_hm.get("current_candidates",0)}개 · 권위 H 검색식 변경 없음')
    _pm2=dict((practical_audit.get('preignition') or {}).get('manifest') or {})
    ...
    lines20=['(20/20)','⚡ 실전 PAPER 운용 준비도 | v49.76','──────────']
    ...
    if isinstance(_ready,pd.DataFrame) and not _ready.empty:
        for _mode in ['LP','SLOCK','G','A','H','IT','I','L','S','B1','B2','C']:
            _q=_ready[_ready.strategy.eq(_mode)]
            if _q.empty: continue
            _r=_q.iloc[0]; lines20.append(f'- {_mode}: {_r.practical_tier} · OOS n{int(_r.oos_n)} · net50 {f(_r.oos_net50_mean_pct)}')
    lines20 += ['', '[15:03 실행 계약]', '- 14:40 = 예비후보/관찰만 · 실제추천 원장 확정 금지', '- 15:03 최종킥 = 재검증된 ENTER 상위 최대 2종목만 PAPER 실행보드/추천원장 후보', '- 추천가 이하 지정가만 · +3% 우선 · 구조지지/하드 -3 가드 · 미체결 추격 금지', '- 실제추천 통계 = Telegram 성공 + Google Sheet DELIVERED 증거만 · Replay/Blocked/14:40 Preview 제외', '- 실제주문 0건 · LIVE 자동전환 없음']

    report = '\n'.join(lines1 + [''] + lines2 + [''] + lines3 + [''] + lines4 + [''] + lines5 + [''] + lines6 + [''] + lines7 + [''] + lines8 + [''] + lines9 + [''] + lines10 + [''] + lines11 + [''] + lines12 + [''] + lines13 + [''] + lines14 + [''] + lines15 + [''] + lines16 + [''] + lines17 + [''] + lines18 + [''] + lines19 + [''] + lines20)
    (out / 'v49_76_global_summary.txt').write_text(report, encoding='utf-8')
    files = [
        raw_path, canonical_path, selected_path,
        out / 'v49_76_primary_selected_top5.csv', out / 'v49_76_all_matched_selected_top5.csv',
        out / 'v49_76_strategy_funnel.csv', out / 'v49_76_strategy_stage_audit.csv', out / 'v49_76_identity_audit.csv',
        out / 'v49_76_zero_mode_audit.csv', out / 'v49_76_predicate_exceptions.csv',
        out / 'v49_76_strategy_attribution_performance.csv', out / 'v49_76_lp_walk_forward.csv', out / 'v49_76_iit_flow_split.csv',
        out / 'v49_76_selected_enriched_outcomes.csv', out / 'v49_76_outcome_distribution.csv',
        out / 'v49_76_winner_loser_feature_anatomy.csv', out / 'v49_76_train_oos_feature_lift.csv',
        out / 'v49_76_market_regime_performance.csv', out / 'v49_76_event_theme_context.csv',
        out / 'v49_76_down_market_cases.csv',
        out / 'v49_76_lgs_candidate_library.csv', out / 'v49_76_lgs_redundancy.csv', out / 'v49_76_lgs_locked_test.csv',
        out / 'v49_76_down_market_candidate_library.csv', out / 'v49_76_down_market_redundancy.csv', out / 'v49_76_down_market_locked_test.csv',
        out / 'v49_76_lp_exit_policy_audit.csv', out / 'v49_76_lp_exit_selected.csv', out / 'v49_76_lp_locked_test_stop_cases.csv',
        out / 'v49_76_triple_split_manifest.json',
        out / 'v49_76_search_intent_enriched.csv', out / 'v49_76_search_intent_summary.csv',
        out / 'v49_76_winner_loser_effect_size.csv', out / 'v49_76_manual_casebook.csv',
        out / 'v49_76_ranking_summary.csv', out / 'v49_76_ranking_detail.csv',
        out / 'v49_76_opportunity_miss_summary.csv', out / 'v49_76_opportunity_census_global.csv',
        out / 'v49_76_missed_winner_casebook.csv', out / 'v49_76_technical_false_negative.csv',
        out / 'v49_76_technical_false_negative.json', out / 'v49_76_opportunity_path_summary.csv',
        out / 'v49_76_distribution_drift.csv', out / 'v49_76_search_quality_manifest.json',
        out / 'v49_76_history_authority_global.csv', out / 'v49_76_history_completeness_issues.csv', out / 'v49_76_input_authority_manifest.json', out / 'v49_76_sample_authority_reconciliation.csv',
        out / 'v49_76_prepared_authority_snapshot.csv', out / 'v49_76_prepared_authority_snapshot.json', out / 'v49_76_marcap_parity_summary.json', out / 'v49_76_marcap_parity_detail.csv',
        out / 'v49_76_legacy_v4974_marcap_parity_summary.json', out / 'v49_76_legacy_v4974_marcap_parity_detail.csv',
        out / 'v49_76_immediate_miss_policy_summary.csv', out / 'v49_76_immediate_strategic_miss_summary.csv', out / 'v49_76_immediate_strategic_miss_feature_contrast.csv',
        out / 'v49_76_immediate_strategic_miss_casebook.csv', out / 'v49_76_immediate_strategic_miss_detail.csv',
        out / 'v49_76_rank_inversion_summary.csv', out / 'v49_76_rank_inversion_feature_stability.csv', out / 'v49_76_rank_inversion_cases.csv',
        out / 'v49_76_a_big_vs_stop_summary.csv', out / 'v49_76_a_big_vs_stop_feature_stability.csv', out / 'v49_76_a_big_vs_stop_casebook.csv',
        out / 'v49_76_h_intent_v2_summary.csv', out / 'v49_76_h_intent_v1_v2_matrix.csv', out / 'v49_76_h_intent_v2_condition_pass.csv',
        out / 'v49_76_h_intent_v2_detail.csv', out / 'v49_76_hypothesis_registry.csv', out / 'v49_76_insight_manifest.json',
        out / 'v49_76_pit_marcap_audit_manifest.json', out / 'v49_76_pit_marcap_date_coverage.csv', out / 'v49_76_pit_marcap_matrix.csv.gz',
        out / 'v49_76_pit_marcap_current_raw_enriched.csv.gz', out / 'v49_76_pit_marcap_current_selected_enriched.csv',
        out / 'v49_76_pit_marcap_strategy_impact.csv', out / 'v49_76_pit_marcap_future_included_cases.csv',
        out / 'v49_76_pit_marcap_future_included_selected_cases.csv', out / 'v49_76_pit_marcap_future_included_code_summary.csv',
        out / 'v49_76_pit_marcap_one_sided_shadow_top5.csv',
        out / 'v49_76_a_ma60_fixed_hypothesis.csv', out / 'v49_76_a_ma60_fixed_hypothesis_cases.csv', out / 'v49_76_a_ma60_fixed_hypothesis_manifest.json',
        out / 'v49_76_h_high_dryup_prospective_candidates.csv', out / 'v49_76_h_high_dryup_prospective_manifest.json',
        out / 'v49_76_preignition_casebook.csv', out / 'v49_76_preignition_manifest.json',
        out / 'v49_76_practical_readiness.csv', out / 'v49_76_practical_manifest.json',
    ]
    manifest = {
        ...
        'search_gap_insight_status': insight_status,
        'search_gap_insight_manifest': insight_manifest,
        'search_gap_insight_policy': 'HYPOTHESIS_REGISTRATION_ONLY_NO_AUTO_APPLY',
        'pit_marcap_audit_status': pit_manifest.get('status'),
        'pit_marcap_audit_manifest': pit_manifest,
        'pit_marcap_audit_policy': 'ONE_SIDED_REMOVE_ONLY_AUDIT_NO_AUTO_APPLY',
        'practical_audit_status': practical_manifest.get('status'),
        'practical_audit_manifest': practical_manifest,
        'practical_execution_contract': '14:40_PREVIEW_ONLY__15:03_TOP2_PAPER_LEDGER_ONLY',
    ...
    (out / 'v49_76_global_manifest.json').write_text(json.dumps(manifest, ensure_ascii=False, indent=2, default=str), encoding='utf-8')
    print(report)
    _deliver_telegram([lines1, lines2, lines3, lines4, lines5, lines6, lines7, lines8, lines9, lines10, lines11, lines12, lines13, lines14, lines15, lines16, lines17, lines18], out, bool(args.send_telegram))
