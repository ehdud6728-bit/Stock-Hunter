...
CLOSING_BET_SCANNER_VERSION = 'G_MORALES_V4_2_1_HIGH_BREAKOUT_DRYUP_STRICT_20260512'
        ...
        try:
            if len(hist) >= 150 and info['amount_b'] >= MIN_AMOUNT / 1e8:
                ...
                for d in range(1, max_days + 1):
                    ...
                    # v4.2.1 핵심: 신고가를 '장대양봉'으로 종가 돌파한 기준봉만 인정한다.
...
def _bt_mask_h_safe(df: pd.DataFrame):
    """v4.2.1: H 신고가 거자름 STRICT SAFE 후보.
...
def _bt_mask_h_neutral(df: pd.DataFrame):
    """v4.2.1: H 후보 중 SAFE도 RISK도 아닌 중립 후보."""
    if df is None or df.empty or 'mode' not in df.columns:
        return pd.Series(False, index=df.index if df is not None else [])
...
def _format_h_reason_report(df: pd.DataFrame) -> str:
    ...
    lines = ["[H 신고가 거자름 STRICT SAFE/RISK 탈락 사유 분석 — v4.2.1]"]
    try:
        if df is None or df.empty or 'mode' not in df.columns:
            lines.append("- 데이터 없음")
            return "\n".join(lines)
...
def _build_backtest_summary(
    ...
    lines.append("- v4.2.1부터 H 신고가거자름은 '신고가 장대양봉 돌파봉'과 '거래량 마른 짧은 타점봉'을 분리하여 STRICT 조건으로 검증합니다. 수급 데이터는 이번 버전에서 필수 조건이 아니라 후속 가점 후보입니다.")
