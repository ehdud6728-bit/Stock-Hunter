...
# =============================================================
# V1068: 예비 삼각수렴·BB응축 선별 강화
# - 예비 TOP3는 "아직 돌파 전 + BB/MA 에너지 응축 + 수급 확인" 전용으로 조정
# - 후행형/이미 발사형/BB40 돌파형은 예비 TOP3에서 제외 또는 강한 감점
# - 외인·기관 동반매도는 점수 상한을 낮춰 TOP3 과대 노출 방지
# =============================================================

def _v1068_parse_money_value(text: str, label: str) -> float:
    """문자열에서 개인/외인/기관 금액(억 단위)을 최대한 안전하게 추출."""
    try:
        s = _txt(text, '')
        if not s:
            return 0.0
        # 예: 외인 +26.1억 / 외인 -1,234.5억 / 외인 +0.0
        m = re.search(rf'{re.escape(label)}\s*([+\-]?\s*[0-9,]+(?:\.[0-9]+)?)\s*억?', s)
        if not m:
            return 0.0
        return _safe_float(m.group(1).replace(' ', ''), 0.0)
    except Exception:
        return 0.0


def _v1068_row_dict(row: Any) -> Dict[str, Any]:
    try:
        if hasattr(row, 'to_dict'):
            return row.to_dict()
...
def _v1068_pick_text(row: Any, *keys: str) -> str:
    d = _v1068_row_dict(row)
    for k in keys:
        try:
            v = d.get(k)
        except Exception:
...
def _v1068_supply_detail(row: Any) -> Dict[str, Any]:
    label = _txt(_row_get(row, '수급', '수급요약', '수급판정', default=''), '')
    d1 = _v1068_pick_text(row, '수급당일', '최근1일수급', '수급1일', '1일수급')
    d3 = _v1068_pick_text(row, '수급3일누적', '최근3일수급', '수급3일', '3일수급')
    d5 = _v1068_pick_text(row, '수급5일누적', '최근5일수급', '수급5일', '5일수급')
    blob = ' | '.join([label, d1, d3, d5])
    p1, f1, i1 = (_v1068_parse_money_value(d1, '개인'), _v1068_parse_money_value(d1, '외인'), _v1068_parse_money_value(d1, '기관'))
    p3, f3, i3 = (_v1068_parse_money_value(d3, '개인'), _v1068_parse_money_value(d3, '외인'), _v1068_parse_money_value(d3, '기관'))
    p5, f5, i5 = (_v1068_parse_money_value(d5, '개인'), _v1068_parse_money_value(d5, '외인'), _v1068_parse_money_value(d5, '기관'))
    smart1 = f1 + i1
    smart3 = f3 + i3
    smart5 = f5 + i5
    return {
        'label': label, 'd1': d1, 'd3': d3, 'd5': d5, 'blob': blob,
        'p1': p1, 'f1': f1, 'i1': i1, 'smart1': smart1,
        'p3': p3, 'f3': f3, 'i3': i3, 'smart3': smart3,
        'p5': p5, 'f5': f5, 'i5': i5, 'smart5': smart5,
    }


def _supply_score_and_label(row: Any) -> Tuple[int, str]:
    """V1068: 수급 점수 재정의. 예비/삼각 블록 모두 외인·기관 실제 방향을 더 강하게 반영."""
    info = _v1068_supply_detail(row)
    label = info.get('label') or '수급 확인 필요'
    blob = info.get('blob', '')
    score = 0

    # 명시 태그 우선
    if any(k in blob for k in ['쌍끌강', '외인·기관 동반매수 강함', '동반 강매집']):
        score += 15
    elif any(k in blob for k in ['쌍끌', '동반매수', '외인·기관 동반']):
        score += 12
    elif any(k in blob for k in ['외인강', '외인우세', '외인 중심']):
        score += 8
    elif any(k in blob for k in ['기관강', '기관우세', '기관 중심']):
        score += 8

    # 실제 1일/3일 금액 방향 보정
    f1, i1, p1, smart1 = info['f1'], info['i1'], info['p1'], info['smart1']
    f3, i3, p3, smart3 = info['f3'], info['i3'], info['p3'], info['smart3']
    ...
    # 개인이 받고 외인·기관이 동시에 파는 경우는 예비 후보에서 특히 위험
    if p1 > 0 and f1 < 0 and i1 < 0:
        score -= 18
    if p3 > 0 and f3 < 0 and i3 < 0:
        score -= 25
    if smart1 < 0 and smart3 < 0:
        score -= 18
    elif smart1 < 0 or smart3 < 0:
        score -= 8

    if any(k in blob for k in ['개인수급/외인기관매도', '외인기관매도', '동반 매도', '수급부담']):
        score -= 18
    if any(k in blob for k in ['혼조', '단기수급부담']):
        score -= 8

    score = max(-35, min(25, int(score)))

    # 라벨이 너무 장문이면 기존 태그를 우선 보존하되 비어 있으면 방향 라벨 생성
    if label == '수급 확인 필요':
        if f1 > 0 and i1 > 0 or f3 > 0 and i3 > 0:
            label = '🤝쌍끌'
        elif f1 > 0 or f3 > 0:
            label = '🔵외인우세'
        elif i1 > 0 or i3 > 0:
            label = '🔴기관우세'
        elif p1 > 0 and smart1 < 0 or p3 > 0 and smart3 < 0:
            label = '⚠️개인수급/외인기관매도'
    return score, label


def score_triangle_pre_squeeze_from_row(row: Any) -> Dict[str, Any]:
    """V1068: 예비 삼각수렴·BB응축 후보 점수화 강화 버전."""
    name = _txt(_row_get(row, '종목명', 'Name', 'name', default='종목명확인'), '종목명확인')
    code = _txt(_row_get(row, 'code', 'Code', '종목코드', default=''), '')
    current = _safe_int(_row_get(row, '현재가', 'Close', 'Price', default=0), 0)
    blob = ' '.join(_txt(_row_get(row, k, default=''), '') for k in [
        'N구분', 'N조합', '검색패턴', '구조', '수박정제태그', '수박상태', '유형', '신호', '저항구름상태'
    ])
    ...
    # BB 응축은 예비 TOP3의 핵심. BB40이 넓으면 상한을 낮춘다.
    if 0 < bb20 <= 8:
        score += 10
    elif 0 < bb20 <= 12:
        score += 7
    ...
    # 예비 TOP3 제외/상한 조건
    strong_breakout = _contains_any(blob, [
        'BB40저항돌파', 'BB40 재지지', 'BB40재지지', 'BB40 상단 저항 돌파',
        '거래량폭발초동돌파', '진짜매집발사', '수박재폭발', '독사발사', '발사형', '2차파동'
    ])
    late_blob = _contains_any(blob, ['후행형', '후행수박', '추격금지'])
    supply_bad = any(k in str(supply_label) for k in ['개인수급/외인기관매도', '수급부담', '동반 매도']) or supply_score <= -18

    # 삼각/구조수렴 + BB응축이 없으면 예비 블록 후보가 아니다.
    has_triangle_energy = bool(tri_flag or structure_squeeze)
    has_band_energy = bool((0 < bb40 <= 22) or (0 < bb20 <= 18))
    has_ma_energy = bool((0 < ma_ultra <= 5.5) and (0 < ma_short <= 7.5))
    if not (has_triangle_energy and (has_band_energy or has_ma_energy)):
        score = min(score, 39)
    ...
    if supply_bad:
        score = min(score, 62)
    if rsi >= 70 or disp >= 115:
        score = min(score, 55)

    score = max(0, min(100, int(score)))
    if score >= 82:
        readiness = '응축 우수'
        comment = '삼각수렴과 BB응축이 같이 보이는 예비 구간입니다. 아직 돌파 전 성격이므로 기준선 위 재지지와 거래량 증가를 확인합니다.'
    elif score >= 68:
        readiness = '응축 관찰'
        comment = '수렴 에너지는 보이지만 확정 돌파 전입니다. 파란점선 위 종가와 거래량 재유입이 붙을 때 관심도가 올라갑니다.'
    else:
        readiness = '예비 관찰'
        comment = '수렴 단서는 있으나 BB폭·수급·과열 조건을 더 확인해야 합니다. 5일선 이탈 시 관찰 우선순위를 낮춥니다.'
    ...
    return {
        ...
        'ma_struct': ma_struct, 'vol_ratio': vol_ratio, 'rsi': rsi, 'disp': disp,
        'supply_label': supply_label, 'supply_score': supply_score,
        'strong_breakout': bool(strong_breakout), 'late_blob': bool(late_blob), 'supply_bad': bool(supply_bad),
    }


def build_triangle_pre_squeeze_top3_df(df: pd.DataFrame, limit: int = 3) -> pd.DataFrame:
    """V1068: 예비 삼각수렴·BB응축 TOP3. 돌파 전/응축/수급 기준을 엄격 적용."""
    if df is None or not hasattr(df, 'empty') or df.empty:
        return pd.DataFrame()
    rows: List[Dict[str, Any]] = []
    for _, row in df.iterrows():
        try:
            scored = score_triangle_pre_squeeze_from_row(row)
            if scored.get('score', 0) < 65:
                continue
            if scored.get('late_blob') or scored.get('strong_breakout'):
                continue
            if scored.get('bb40', 0) and scored.get('bb40', 0) > 22:
                continue
            if scored.get('rsi', 0) >= 70 or scored.get('disp', 100) >= 115:
                continue
            item = row.to_dict()
            item['_triangle_pre_score'] = scored['score']
            item['_triangle_pre_readiness'] = scored['readiness']
...
# V1092_TOP15_GENERATION_HARD_GATE: triangle analyzer unchanged from V1091.
