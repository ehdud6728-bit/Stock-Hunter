...
# =============================================================
# closing_bet_scanner.py — 종가배팅 타점 스캐너 (G Morales v4.1.8 G 모랄레스갭 RISK 사유 분석)
# =============================================================
# 전략 구성
# A  : 돌파형 종가배팅
...
CLOSING_BET_SCANNER_VERSION = 'G_MORALES_V4_1_8_G_REASON_REPORT_20260511'
...
def _send_results(hits: list, mins_left: int):
    ...
    header = (
        ...
        f"ENV엄격형(B1) {min(len(hits_b1), 5)}개 | BB확장형(B2) {min(len(hits_b2), 5)}개 | 역매공파(C) {min(len(hits_c), 5)}개 | C-눌림재상승 {c_pullback_output_n}개\n"
    ...
    def _build_practical_block():
        ...
        c_pullback_pool = sorted(hits_c_pullback_reentry, key=_priority_practical)
        ...
        block = ['[🎯 실전 최우선 후보 — v4.1.6 SAFE + C-눌림재상승 검증]']
        summary = f"요약: 🟢SAFE {len(safe_pool)}개 | 🟡NEUTRAL {len(neutral_pool)}개 | ⚠️S-RISK {len(risk_pool)}개 | A보조 출력 {a_output_n}개/전체 {len(a_pool)}개 | 🏆C-눌림재상승 {c_pullback_output_n}개"
        if a_risk_pool:
        ...
        if c_pullback_pool:
            block.append('')
            block.append('[🏆 C-SWING 눌림재상승형 — 검증 후보]')
            block.append('종가배팅 후보가 아닙니다. 1파 이후 눌림·재지지·재상승 확인이 잡힌 후보만 별도 추적합니다.')
    ...
    sections.append(_build_block("🏆 C-SWING 눌림재상승형 TOP3", hits_c_pullback_reentry[:PRACTICAL_C_PULLBACK_TOP_N], "C"))
...
def _bt_g_reason_flags(df: pd.DataFrame) -> pd.DataFrame:
    """v4.1.8: G 모랄레스갭이 SAFE/RISK로 분류된 사유를 조건별 boolean으로 반환한다.
...
def _format_g_reason_report(df: pd.DataFrame, max_reasons: int = 12) -> str:
    """v4.1.8: G 후보가 G-SAFE/NEUTRAL/RISK로 나뉘지 않은 원인을 사유별로 보여준다."""
    lines = ["[G 모랄레스갭 RISK/SAFE 탈락 사유 분석 — v4.1.8]"]
    try:
        if df is None or df.empty or 'mode' not in df.columns:
            lines.append("- 데이터 없음")
            return "\n".join(lines)
        ...
        _append_counts('RISK 판정 사유 TOP', risk_cols)
...
def _format_practical_combo_report(df: pd.DataFrame) -> str:
    ...
    try:
        ...
        if len(g) > 0:
            ...
            lines.append(_format_backtest_trade_rule_block(g_risk, '⚠️ G-RISK 모랄레스갭'))
        ...
        lines.append("- G-RISK 기준: 갭과대, Vol50 과열, 종가위치 약함, 윗꼬리 과다, 과열/유동성 문제 중 하나라도 걸린 후보입니다.")
        lines.append("- 해석: 실전 최우선은 S1 우수응축형 중 SAFE 후보입니다. G는 v4.1.8에서 RISK/SAFE 탈락 사유를 먼저 확인한 뒤 기준을 재설계합니다.")
    except Exception as e:
...
def _format_tuning_report(df: pd.DataFrame) -> str:
    ...
    try:
        ...
        if len(g_df) > 0:
            ...
            lines.append(_format_backtest_trade_rule_block(df[_bt_mask_g_risk(df)], '⚠️ G-RISK'))
            lines.append(_format_g_reason_report(df))
...
def _build_backtest_summary(
    ...
    try:
        ...
        if len(g_all_df) > 0:
            ...
            lines.append(_format_backtest_trade_rule_block(df[_bt_mask_g_risk(df)], '⚠️ G-RISK 모랄레스갭'))
    except Exception:
        pass
    lines.append("")
    ...
    try:
        ...
        try:
            ...
            if core_only or safe_only or neutral_only or risk_only or c_only or c_swing_only or c_strict_only or c_pullback_only or g_only:
                if c_pullback_only:
    ...
    lines.append("- v4.1.8부터 G 모랄레스갭은 G-SAFE/G-NEUTRAL/G-RISK 분류와 함께 RISK/SAFE 탈락 사유를 출력합니다. C-SWING은 실전 후보가 아니라 진단용으로 유지합니다.")
...
def run_closing_bet_backtest(
    ...
    if raw_df.empty:
        report = _build_backtest_summary(
            ...
            all_candidates, weekly_breakdown=weekly_breakdown, diag=diag, core_only=core_only, safe_only=safe_only, neutral_only=neutral_only, risk_only=risk_only, c_only=c_only, c_swing_only=c_swing_only, c_strict_only=c_strict_only, c_pullback_only=c_pullback_only, g_only=g_only,
        )
        try:
    ...
    if (safe_only or neutral_only or risk_only or core_only or c_only or c_swing_only or c_strict_only or c_pullback_only or g_only) and selected_df is not None and not selected_df.empty:
        try:
            if g_only:
    ...
    report = _build_backtest_summary(
        ...
        all_candidates, weekly_breakdown=weekly_breakdown, diag=diag if debug else None, core_only=core_only, safe_only=safe_only, neutral_only=neutral_only, risk_only=risk_only, c_only=c_only, c_swing_only=c_swing_only, c_strict_only=c_strict_only, c_pullback_only=c_pullback_only, g_only=g_only,
    )
    try:
        BACKTEST_SUMMARY_TXT.write_text(report, encoding='utf-8')
        if debug:
