...
CLOSING_BET_SCANNER_VERSION = 'G_MORALES_V4_4_9_24_H_CORE_PROMOTION_AUDIT_20260528'
...
def _send_results(hits: list, mins_left: int):
    ...
    header = (
        ...
        f"전체 후보 {len(hits)}개 | v4.4.9.24 H-CORE AUDIT: 14:40 후보발굴 + 15:03 실행/포기 압축 최종킥\n"
    ...
    def _build_practical_block():
        ...
        def _build_final_kick_block() -> str:
            ...
            # v4.4.9.24: 최종킥 우선순위는 실전 운용순서 그대로 둔다.
        ...
        block = ['[🎯 실전 운영 후보 — v4.4.9.24 H-CORE PROMOTION AUDIT]']
        summary = (
        ...
        if safe_pool:
            ...
            block.append('v4.4.9.24 기준: S-SAFE는 ST30 통과형을 S-RECLAIM MEGA(5000억+)·3000~5000·PRIME(1000억+)·WATCH(300~1000)·LIQUIDITY RISK(<300)로 나누고, ST30 미통과형은 S-MOMENTUM/S-LIQUIDITY로 분리합니다. ST30은 하드필터가 아닙니다.')
...
def _v44924_h_core_promotion_audit_lines(df: pd.DataFrame, compact: bool = True) -> list[str]:
    ...
    try:
        ...
        if h_core.empty:
            lines.append('[🧪 H-CORE 승격검증 — v4.4.9.24]')
            lines.append('- H 전체 후보는 있으나 H-CORE/삼각/거래대금×Vol60 핵심셀은 없습니다. 실시간에서는 H를 계속 숨김/관찰로 유지합니다.')
            return lines
        ...
        lines.append('[🧪 H-CORE 승격검증 — v4.4.9.24]')
        lines.append('- 목적: v4.4.9.23에서 H-CORE/H-TRIANGLE 핵심셀이 승격 기준을 통과했는지, FINAL KICK에 올릴 수 있는지 별도 검증합니다.')
        ...
        lines.append('- 다음 단계: H-CORE가 2~3회 더 반복 통과하면 LP/L/S 다음 순위의 보조 실행 후보로 승격하고, 실패 샘플은 고변동·과열 Vol60·돌파권 이탈 조건으로 제외식을 만듭니다.')
    except Exception as e:
...
def _v449_fast_live_policy_lines() -> list[str]:
    return [
        ...
        '- H-CORE는 v4.4.9.24에서 승격검증 중입니다. 통과 전까지는 H-CORE WATCH/지정가/다음날확인 후보로만 보고, 넓은 A/C/B는 숨김 또는 관찰 요약만 유지.',
        '- 승격 조건을 반복 통과한 패턴만 이후 SAFE/CORE 섹션으로 올립니다. 현재 H-CORE는 별도 감사 중, A-RETEST/C-RECLAIM/B-CONFIRM은 검증 단계입니다.',
    ]
...
def _v439_build_compact_backtest_report(original_report: str, selected_csv: str = '', raw_csv: str = '', start_date: str = '', end_date: str = '', hold_days: int = 0, top_per_strategy: int = 5, all_candidates: bool = False) -> str:
    ...
    try:
        ...
        sel_text = '전체후보' if all_candidates else f'날짜별 전략별 TOP{top_per_strategy}'
        lines = []
        lines.append("🧪 v4.4.9.24 H-CORE PROMOTION AUDIT")
        lines.append(f"버전: {CLOSING_BET_SCANNER_VERSION}")
        lines.append(f"기간: {start_date} ~ {end_date} | 보유평가: 다음 {hold_days}거래일")
        ...
        lines.append("- 신규 검색식 LP/SLOCK/IT는 위 성과검증 섹션과 실시간 후보 출력 섹션에서 함께 확인합니다. v4.4.9.24는 ST30-RECLAIM을 하드필터가 아닌 가점/주의 태그로 유지하고, 실시간 S-SAFE 후보를 S-RECLAIM MEGA·S-RECLAIM 3000~5000·S-RECLAIM PRIME/WATCH/RISK·S-MOMENTUM PRIME/WATCH·S-LIQUIDITY RISK로 분리 표시합니다. FAST 실전 출력 후보만 우선 표시하며, A-CONFIRM PRIME/CALM·시장/섹터압력·실전 추적로그를 유지합니다. H-CORE는 승격감사 후 H-CORE WATCH 후보로 검토하고, 넓은 A/C/B/SLOCK은 기본 숨김/연구로 유지합니다.")
        compact = "\n".join([str(x) for x in lines if x is not None])
        try:
            out_path = LOG_DIR / 'closing_bet_backtest_summary_v4_4_9_24_h_core_promotion_audit.txt'
