...
CLOSING_BET_SCANNER_VERSION = 'G_MORALES_V4_4_9_22_S_RECLAIM_LIQUIDITY_DRILLDOWN_20260528'
...
# v4.4.9.22: 2차 스캔 전용 FINAL KICK COMPACT MODE
...
# v4.4.9.22: 최종킥은 실행/포기만 빠르게 보는 압축 모드가 기본값이다.
...
def _send_results(hits: list, mins_left: int):
    ...
    header = (
        ...
        f"전체 후보 {len(hits)}개 | v4.4.9.22 S-RECLAIM LIQUIDITY: 14:40 후보발굴 + 15:03 실행/포기 압축 최종킥\n"
    ...
    # v4.4.9.22: S-SAFE live labels. ST30은 하드필터가 아니라 가점/강등 태그이고,
    ...
    def _s_safe_live_class(h: dict) -> dict:
        ...
        if st30_pass:
            # v4.4.9.22: ST30 통과군도 거래대금에 따라 강도/비중을 나눈다.
    ...
    def _build_practical_block():
        ...
        # v4.4.9.22: S-SAFE live label pools
        ...
        # v4.4.9.22: 2차 스캔 전용 — 동시호가 직전 실행/포기 압축 최종킥
            ...
            if mode == 'S':
                if _is_s_core_safe_hit(h):
                    ...
                    if skey == 'S_RECLAIM_PRIME' and close_loc >= 70 and volr <= 1.50:
                        if amount_b >= 5000:
                            return '✅ 종가진입 가능', 'S-RECLAIM MEGA: ST30 통과 + 5000억+ 고유동성. 그래도 +3 자동익절 우선', reasons
                        if amount_b >= 3000:
                            return '✅ 소액 종가 가능', 'S-RECLAIM 3000~5000: ST30 통과 준대형 수급. 5000억+보다 비중은 낮춤', reasons
                        if amount_b >= 1000:
                            return '🟡 지정가만 가능', 'S-RECLAIM PRIME: ST30 통과이나 1000~3000억 구간. 종가 추격보다 지정가/다음날 확인 우선', reasons
                        return '❌ 오늘 포기', 'S-RECLAIM이라도 거래대금 1000억 미만은 유동성 확인 전 신규진입 보류', reasons
        ...
        def _build_final_kick_block() -> str:
            ...
            # v4.4.9.22: 최종킥 우선순위는 실전 운용순서 그대로 둔다.
        ...
        block = ['[🎯 실전 운영 후보 — v4.4.9.22 S-RECLAIM LIQUIDITY DRILLDOWN]']
        summary = (
        ...
        if safe_pool:
            ...
            block.append('v4.4.9.22 기준: S-SAFE는 S-RECLAIM PRIME(ST30통과), S-MOMENTUM PRIME(5000억+), S-MOMENTUM 3000~5000, S-MOMENTUM WATCH(1000억+), S-LIQUIDITY RISK(1000억 미만)로 분리합니다. ST30은 하드필터가 아닙니다.')
...
def _v44919_s_safe_liquidity_rule_repeat_lines(df: pd.DataFrame, compact: bool = True) -> list[str]:
    ...
    try:
        ...
        def _quality_trade_line(source_df: pd.DataFrame, label: str) -> str:
            """v4.4.9.22: 거래대금 버킷별 단기 성과뿐 아니라 실제 최대상승/종가수익/흔들림을 함께 본다."""
        ...
        lines.append('[🔁 S-SAFE 거래대금 규칙 반복검증 — v4.4.9.22]')
...
def _v439_build_compact_backtest_report(original_report: str, selected_csv: str = '', raw_csv: str = '', start_date: str = '', end_date: str = '', hold_days: int = 0, top_per_strategy: int = 5, all_candidates: bool = False) -> str:
    ...
    try:
        ...
        sel_text = '전체후보' if all_candidates else f'날짜별 전략별 TOP{top_per_strategy}'
        lines = []
        lines.append("🧪 v4.4.9.22 S-RECLAIM LIQUIDITY DRILLDOWN")
        lines.append(f"버전: {CLOSING_BET_SCANNER_VERSION}")
        lines.append(f"기간: {start_date} ~ {end_date} | 보유평가: 다음 {hold_days}거래일")
        ...
        lines.append("- 신규 검색식 LP/SLOCK/IT는 위 성과검증 섹션과 실시간 후보 출력 섹션에서 함께 확인합니다. v4.4.9.22는 ST30-RECLAIM을 하드필터가 아닌 가점/주의 태그로 유지하고, 실시간 S-SAFE 후보를 S-RECLAIM PRIME·S-MOMENTUM PRIME·S-MOMENTUM 3000~5000·S-MOMENTUM WATCH·S-LIQUIDITY RISK로 분리 표시합니다. FAST 실전 출력 후보만 우선 표시하며, A-CONFIRM PRIME/CALM·시장/섹터압력·실전 추적로그를 유지합니다. 넓은 A/C/B/H/SLOCK은 기본 숨김/연구로 유지합니다.")
        compact = "\n".join([str(x) for x in lines if x is not None])
        try:
            out_path = LOG_DIR / 'closing_bet_backtest_summary_v4_4_9_22_s_reclaim_liquidity_drilldown.txt'
