...
CLOSING_BET_SCANNER_VERSION = 'G_MORALES_V4_4_9_2_CLOSE_LOC_FALLBACK_TUNING_RETEST_20260520'
...
def _send_results(hits: list, mins_left: int):
    ...
    def _build_practical_block():
        ...
        block = ['[🎯 실전 운영 후보 — v4.4.9.2 CLOSE LOC FALLBACK + TUNING RETEST]']
        summary = (
...
def _v439_i_samples_section(df: pd.DataFrame, top_n: int = 5) -> list[str]:
    ...
    try:
        ...
        if i.empty:
            return lines
        lines.append("[📋 I-MAIN 발생 샘플 — 종목별 중복제거 v4.4.8.1]")
...
def _v445_pattern_path_profile_lines(df: pd.DataFrame) -> list[str]:
    ...
    try:
        ...
        lines.append('[🧭 패턴별 타점 이후 흐름 프로파일 — v4.4.9.2 보정판]')
        ...
        lines.append('- 해석: v4.4.9.2는 1~3일 첫흔들림, +3전흔들림, 전체최대흔들림을 분리합니다. 전체최대흔들림은 목표 도달 후 흔들림까지 포함될 수 있으므로 실전 판단은 +3전흔들림과 지지선 종가유지를 우선 봅니다.')
    except Exception as e:
...
def _v442_new_pattern_performance_lines(df: pd.DataFrame) -> list[str]:
    ...
    try:
        ...
        lines.append('- 해석: v4.4.9.2부터 LP는 단기 최우선 운용 후보로 보되, D1/D23/D45 타점과 다음날 대응 시나리오를 함께 확인합니다. LP-SAFE는 실행 후보, LP-WATCH는 다음날 양봉·전일고가 회복·거래대금 유지 시 승격, LP-RISK는 제외입니다. SLOCK은 실시간 기본 숨김, IT는 ACCEL 중심으로만 봅니다.')
    except Exception as e:
...
# =============================================================
# v4.4.9.2 CLOSE LOC FALLBACK + TUNING RETEST — A/C/H/B 승격검증
# =============================================================
...
def _v449_price_series_any(df: pd.DataFrame, candidates: list[str]) -> pd.Series:
    """v4.4.9.2: OHLC 유사 컬럼을 여러 이름에서 찾아 숫자 Series로 반환."""
    try:
        if df is None or df.empty:
            return pd.Series(dtype=float)
        for c in candidates:
...
# =============================================================
# v4.4.9.2 CLOSE LOC FALLBACK + TUNING RETEST — A/C/B/H 0건 원인 진단
# =============================================================
...
def _v4492_close_loc_fallback_diagnostic_lines(df: pd.DataFrame) -> list[str]:
    """v4.4.9.2: close_loc 기존값과 fallback 적용 후 커버리지 비교."""
    lines = []
    try:
        if df is None or df.empty:
            return lines
        ...
        amount = _v449_base_liquidity_series(df)
        lines.append('[🧩 종가위치 fallback 보정 — v4.4.9.2]')
...
def _v439_build_compact_backtest_report(original_report: str, selected_csv: str = '', raw_csv: str = '', start_date: str = '', end_date: str = '', hold_days: int = 0, top_per_strategy: int = 5, all_candidates: bool = False) -> str:
    ...
    try:
        ...
        sel_text = '전체후보' if all_candidates else f'날짜별 전략별 TOP{top_per_strategy}'
        lines = []
        lines.append("🧪 v4.4.9.2 CLOSE LOC FALLBACK + TUNING RETEST")
        lines.append(f"버전: {CLOSING_BET_SCANNER_VERSION}")
        lines.append(f"기간: {start_date} ~ {end_date} | 보유평가: 다음 {hold_days}거래일")
        ...
        lines.append("- 신규 검색식 LP/SLOCK/IT는 위 성과검증 섹션과 실시간 후보 출력 섹션에서 함께 확인합니다. v4.4.9.2는 패턴별 멘탈 요약/후보별 멘탈기준을 유지하고, A/C/B/H 튜닝 마스크가 0건이 되는 원인을 단계별로 진단합니다. LP-D23 과최적화 검증과 LP 없는 날 대체 시나리오를 같이 봅니다. SLOCK은 실시간 기본 숨김, IT는 ACCEL 중심으로 봅니다.")
        compact = "\n".join([str(x) for x in lines if x is not None])
        try:
            out_path = LOG_DIR / 'closing_bet_backtest_summary_v4_4_9_2_close_loc_fallback_tuning_retest.txt'
        ...
        return (str(original_report or '') + f"\n\n[v4.4.8.1 압축 요약 생성 실패] {type(e).__name__}: {e}")
