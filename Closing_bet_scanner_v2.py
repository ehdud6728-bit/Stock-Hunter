...
# =============================================================
# closing_bet_scanner.py — 종가배팅 타점 스캐너 (G Morales v4.2.8 S/G/H 통합 운영 FINAL)
# =============================================================
# 전략 구성
# A  : 돌파형 종가배팅
...
CLOSING_BET_SCANNER_VERSION = 'G_MORALES_V4_2_8_1_OPERATION_H_RUNTIME_FIX_20260512'
...
# v4.2.8: H 신고가거자름은 실전 통합 출력에 포함하되, 신규검증 후보로 별도 표시한다.
...
def _check_high_dryup_bet(code: str, name: str) -> dict | None:
    ...
    try:
        ...
        # v4.2.8 실시간 운영에서는 일반 H를 노출하지 않는다.
...
def _check_closing_bet(code: str, name: str) -> dict | None:
    ...
    v4.2.8: H 신고가거자름 TRIANGLE/CORE 후보는 S/G 다음 신규검증 후보로 실시간 포함한다.
    """
    with DIAG_LOCK:
    ...
    candidates = [x for x in [g, s, h_sig, a, b1, b2, c] if x]
    if not candidates:
        return None
    ...
    def _priority(h):
        ...
        mode_rank = {'G': 0, 'S': 1, 'H': 2, 'A': 3, 'B1': 4, 'B2': 5, 'C': 6}.get(mode, 9)
...
def _send_results(hits: list, mins_left: int):
    ...
    header = (
        ...
        f"⏰ {time_text}\n"
        f"전체 후보 {len(hits)}개 | v4.1.9 운영블록 우선 출력\n"
        f"전략 후보: G {len(hits_g)}개 | S {len(hits_s)}개(S2 {len(hits_s2)}/S1 {len(hits_s1)}) | A {len(hits_a)}개 | "
    ...
    def _build_practical_block():
        ...
        # v4.2.8: H 신고가거자름은 기존 S/G와 분리해 신규검증 후보로 표시한다.
        ...
        block = ['[🎯 실전 운영 후보 — v4.2.8 S/G/H 통합 FINAL]']
        summary = (
            f"요약: 🟢S-SAFE {len(safe_pool)}개 | 🟢G-SAFE {len(g_safe_pool)}개 | "
            f"🟡S관찰 {len(neutral_pool)}개 | 🟡G관찰 {len(g_neutral_pool)}개 | "
        ...
        block.append('운용: S-CORE SAFE 1순위, G-SAFE 2순위입니다. H-TRIANGLE/H-CORE는 신규검증 실전 후보로 함께 표시하되, H-OVERHEAT·H-RISK·일반 H는 기본 제외합니다.')
...
def run_closing_bet_scan(force: bool = False) -> list:
    ...
    # v4.2.8.1: 실시간 H 신고가거자름 후보가 최종 합산에서 누락되지 않도록 H도 포함
    # 기존 버그 방지: C전략과 신규 G/S/H전략이 최종 합산에서 빠지지 않도록 모두 포함
    hits = hits_g + hits_s + hits_h + hits_a + hits_b1 + hits_b2 + hits_c
...
def _evaluate_h_pre_breakout_context(hist: pd.DataFrame, breakout_pos: int) -> dict:
    """v4.2.8: H 신고가거자름 돌파 직전 구조/변동성 진단.
        ...
        try:
            if len(hist) >= 150 and info['amount_b'] >= MIN_AMOUNT / 1e8:
                ...
                for d in range(1, max_days + 1):
                    ...
                    # v4.2.8 핵심: 신고가를 '장대양봉'으로 종가 돌파한 기준봉만 인정한다.
                    close_new_high = bool(bclose >= base_high * 1.002)
                    strong_vol = bool(bvol60_ratio >= 1.5)
                    long_bull_body = bool(
                        bclose > bopen
...
def _bt_mask_h_all(df: pd.DataFrame):
    """v4.2.8: H 신고가 거자름 STRICT 전체 후보."""
    if df is None or df.empty or 'mode' not in df.columns:
        return pd.Series(False, index=df.index if df is not None else [])
    return df['mode'].astype(str).eq('H')


def _bt_mask_h_struct_safe(df: pd.DataFrame):
    """v4.2.8: H 신고가 거자름 STRUCT-SAFE 후보.
...
def _bt_mask_h_vol_safe(df: pd.DataFrame):
    """v4.2.8: H-VOL SAFE. 돌파Vol60 2~3배의 정상 강도 신고가 거자름."""
    if df is None or df.empty or 'mode' not in df.columns:
        return pd.Series(False, index=df.index if df is not None else [])
    bvol = pd.to_numeric(df.get('breakout_vol60_ratio', 0), errors='coerce').fillna(0)
...
def _bt_mask_h_vol_swing(df: pd.DataFrame):
    """v4.2.8: H-VOL SWING. 돌파Vol60 3~5배, 3/5보다 MA5 기준 스윙 검증 후보."""
    if df is None or df.empty or 'mode' not in df.columns:
        return pd.Series(False, index=df.index if df is not None else [])
    bvol = pd.to_numeric(df.get('breakout_vol60_ratio', 0), errors='coerce').fillna(0)
...
def _bt_mask_h_vol_overheat(df: pd.DataFrame):
    """v4.2.8: H-VOL OVERHEAT. 돌파Vol60 5~8배, 과열/실패 가능성 후보."""
    if df is None or df.empty or 'mode' not in df.columns:
        return pd.Series(False, index=df.index if df is not None else [])
    bvol = pd.to_numeric(df.get('breakout_vol60_ratio', 0), errors='coerce').fillna(0)
...
def _bt_mask_h_vol_aggressive(df: pd.DataFrame):
    """v4.2.8: H-VOL AGGRESSIVE. 돌파Vol60 8배 이상, 빠른 +3/+5 익절 전용 후보."""
    if df is None or df.empty or 'mode' not in df.columns:
        return pd.Series(False, index=df.index if df is not None else [])
    bvol = pd.to_numeric(df.get('breakout_vol60_ratio', 0), errors='coerce').fillna(0)
...
def _bt_mask_h_safe(df: pd.DataFrame):
    """v4.2.8: 최종 H-SAFE는 돌파Vol60 2~3배 구간으로 제한한다."""
    return _bt_mask_h_vol_safe(df)


def _bt_mask_h_risk(df: pd.DataFrame):
    """v4.2.8: H 신고가 거자름 STRICT 위험 후보.
...
def _bt_mask_h_neutral(df: pd.DataFrame):
    """v4.2.8: H 후보 중 최종 SAFE/SWING/OVERHEAT/AGGRESSIVE/RISK 어디에도 속하지 않는 잔여 관찰 후보."""
    if df is None or df.empty or 'mode' not in df.columns:
        return pd.Series(False, index=df.index if df is not None else [])
    return (
        _bt_mask_h_all(df)
...
def _bt_mask_h_lowvol(df: pd.DataFrame):
    """v4.2.8: H 후보 중 직전 구조 기준 고변동이 아닌 후보."""
    if df is None or df.empty or 'mode' not in df.columns:
        return pd.Series(False, index=df.index if df is not None else [])
...
def _bt_mask_h_prior_structure(df: pd.DataFrame):
    """v4.2.8: H 후보 중 돌파 직전 삼각수렴/횡보/역매공파/눌림 구조가 1개 이상 있는 후보."""
    if df is None or df.empty or 'mode' not in df.columns:
        return pd.Series(False, index=df.index if df is not None else [])
...
def _bt_mask_h_pattern_safe(df: pd.DataFrame):
    """v4.2.8: H-STRUCT + 저변동 + 직전 구조 존재 후보."""
    if df is None or df.empty or 'mode' not in df.columns:
        return pd.Series(False, index=df.index if df is not None else [])
...
def _bt_mask_h_vol_safe_pattern(df: pd.DataFrame):
    """v4.2.8: H-VOL SAFE 2~3배 + 저변동 + 직전 구조 존재 후보."""
    if df is None or df.empty or 'mode' not in df.columns:
        return pd.Series(False, index=df.index if df is not None else [])
...
def _bt_h_breakout_amount(df: pd.DataFrame) -> pd.Series:
    """v4.2.8: 신고가 장대양봉 돌파봉 거래대금(억원) 우선 사용."""
    if df is None:
...
def _bt_mask_h_triangle(df: pd.DataFrame):
    """v4.2.8: 돌파 직전 삼각수렴이 있었던 H 전체 후보."""
    if df is None or df.empty or 'mode' not in df.columns:
        return pd.Series(False, index=df.index if df is not None else [])
...
def _bt_mask_h_triangle_struct(df: pd.DataFrame):
    """v4.2.8: H-STRUCT 조건까지 통과한 직전 삼각수렴 후보."""
    if df is None or df.empty or 'mode' not in df.columns:
        return pd.Series(False, index=df.index if df is not None else [])
...
def _bt_mask_h_core_500_1000_vol23(df: pd.DataFrame):
    """v4.2.8: 돌파봉 거래대금 500~1000억 × Vol60 2~3배 핵심 H 후보."""
    if df is None or df.empty or 'mode' not in df.columns:
        return pd.Series(False, index=df.index if df is not None else [])
...
def _bt_mask_h_core_300_500_vol35(df: pd.DataFrame):
    """v4.2.8: 돌파봉 거래대금 300~500억 × Vol60 3~5배 핵심 H 후보."""
    if df is None or df.empty or 'mode' not in df.columns:
        return pd.Series(False, index=df.index if df is not None else [])
    amt = _bt_h_breakout_amount(df)
    bvol = _bt_h_breakout_vol60(df)
...
def _bt_mask_h_core_1000_2000_vol23(df: pd.DataFrame):
    """v4.2.8: 돌파봉 거래대금 1000~2000억 × Vol60 2~3배 보조 핵심 후보."""
    if df is None or df.empty or 'mode' not in df.columns:
        return pd.Series(False, index=df.index if df is not None else [])
    amt = _bt_h_breakout_amount(df)
    bvol = _bt_h_breakout_vol60(df)
...
def _format_h_v427_core_report(df: pd.DataFrame) -> str:
    """v4.2.8: H-TRIANGLE / 거래대금×Vol 핵심 운용분류 요약."""
    lines = ["[H v4.2.8 핵심 운용분류 — TRIANGLE / AMOUNT×VOL]"]
    try:
        if df is None or df.empty or 'mode' not in df.columns:
            lines.append("- 데이터 없음")
            return "\n".join(lines)
        ...
        lines.append("- 해석: v4.2.8은 일반 H를 실전 후보로 보지 않고, 직전 삼각수렴 또는 돌파봉 거래대금×Vol60 핵심셀만 따로 검증합니다.")
    except Exception as e:
        lines.append(f"- H v4.2.8 핵심분류 리포트 오류: {e}")
    return "\n".join(lines)

def _format_h_pre_structure_report(df: pd.DataFrame) -> str:
    """v4.2.8: H 돌파 직전 구조/고변동 필터 성과."""
    lines = ["[H 직전 구조·고변동 필터 성과 — v4.2.8]"]
    try:
        if df is None or df.empty or 'mode' not in df.columns:
            lines.append("- 데이터 없음")
            return "\n".join(lines)
...
def _format_h_amount_vol_matrix_report(df: pd.DataFrame) -> str:
    """v4.2.8: 돌파봉 거래대금 × 돌파Vol60 교차 성과.

    v4.2.5의 amount_b는 타점봉 거래대금에 가까웠다. v4.2.8에서는 breakout_amount_b를 사용해
    신고가 장대양봉이 실제로 어느 정도 자금으로 터졌는지와 Vol60 배율의 조합을 본다.
    """
    lines = ["[H 돌파봉 거래대금 × Vol60 매트릭스 — v4.2.8]"]
    try:
        if df is None or df.empty or 'mode' not in df.columns:
            lines.append("- 데이터 없음")
            return "\n".join(lines)
...
def _format_h_breakout_volume_report(df: pd.DataFrame) -> str:
    ...
    try:
        ...
        lines.append("- v4.2.8 운용분류: 🟢H-VOL SAFE=2~3배 / 🟣H-VOL SWING=3~5배 / ⚠️H-OVERHEAT=5~8배 / 🔥H-AGGRESSIVE=8배 이상")
...
def _format_h_amount_report(df: pd.DataFrame) -> str:
    """v4.2.8: H 신고가거자름의 거래대금 구간별 성과를 분해한다.
    ...
    """
    lines = ["[H 거래대금 세분화 구간별 성과 — v4.2.8]"]
    try:
        if df is None or df.empty or 'mode' not in df.columns:
            lines.append("- 데이터 없음")
            return "\n".join(lines)
        ...
        def _amount_bucket(frame: pd.DataFrame) -> pd.Series:
            ...
            # v4.2.8: 거래대금 구간 세분화. amount_b 단위는 억원.
...
def _format_h_reason_report(df: pd.DataFrame) -> str:
    ...
    lines = ["[H 신고가 거자름 STRICT VOL 분류/탈락 사유 분석 — v4.2.8]"]
    try:
        if df is None or df.empty or 'mode' not in df.columns:
            lines.append("- 데이터 없음")
            return "\n".join(lines)
        ...
        lines.append("- 해석: H-STRICT는 장대양봉 신고가 돌파봉을 먼저 찾고, 이후 거래량이 마른 짧은 타점봉을 검증합니다. v4.2.8에서는 돌파Vol60 2~3배만 최종 H-SAFE로 분류합니다.")
    except Exception as e:
...
def _build_backtest_summary(
    ...
    try:
        ...
        try:
            ...
            if core_only or safe_only or neutral_only or risk_only or c_only or c_swing_only or c_strict_only or c_pullback_only or g_only or h_only:
                if h_only:
                    _scope = 'H 신고가거자름 선택군'
    ...
    if not h_df.empty:
        ...
        lines.append(_format_backtest_trade_rule_block(h_df, 'H 신고가거자름 STRICT 전체'))
        lines.append(_format_backtest_trade_rule_block(h_df[_bt_mask_h_struct_safe(h_df)], 'H-STRUCT 신고가거자름 STRICT'))
        lines.append(_format_backtest_trade_rule_block(h_df[_bt_mask_h_triangle(h_df)], '🧊 H-TRIANGLE SAFE 직전삼각'))
        lines.append(_format_backtest_trade_rule_block(h_df[_bt_mask_h_triangle_struct(h_df)], '🧊 H-TRIANGLE STRUCT 직전삼각+구조통과'))
        lines.append(_format_backtest_trade_rule_block(h_df[_bt_mask_h_core_500_1000_vol23(h_df)], '🟢 H-CORE 500~1000억×2~3배'))
        lines.append(_format_backtest_trade_rule_block(h_df[_bt_mask_h_core_300_500_vol35(h_df)], '🟣 H-CORE 300~500억×3~5배'))
        lines.append(_format_backtest_trade_rule_block(h_df[_bt_mask_h_v427_core_union(h_df)], '✅ H-CORE UNION 삼각/핵심셀'))
        lines.append(_format_backtest_trade_rule_block(h_df[_bt_mask_h_safe(h_df)], '🟢 H-VOL SAFE 2~3배'))
        lines.append(_format_backtest_trade_rule_block(h_df[_bt_mask_h_vol_safe_pattern(h_df)], '🧊 H-PATTERN SAFE 2~3배+저변동+직전구조'))
        lines.append(_format_backtest_trade_rule_block(h_df[_bt_mask_h_vol_swing(h_df)], '🟣 H-VOL SWING 3~5배'))
        lines.append(_format_backtest_trade_rule_block(h_df[_bt_mask_h_vol_overheat(h_df)], '⚠️ H-OVERHEAT 5~8배'))
        lines.append(_format_backtest_trade_rule_block(h_df[_bt_mask_h_vol_aggressive(h_df)], '🔥 H-AGGRESSIVE 8배+'))
        lines.append(_format_backtest_trade_rule_block(h_df[_bt_mask_h_neutral(h_df)], '🟡 H-NEUTRAL 잔여관찰'))
        lines.append(_format_backtest_trade_rule_block(h_df[_bt_mask_h_risk(h_df)], '⚠️ H-RISK 구조불량'))
        lines.append(_format_h_v427_core_report(h_df))
        lines.append("")
        lines.append(_format_h_reason_report(h_df))
        lines.append("")
        lines.append(_format_h_breakout_volume_report(h_df))
        lines.append("")
        lines.append(_format_h_amount_report(h_df))
        lines.append("")
        lines.append(_format_h_amount_vol_matrix_report(h_df))
        lines.append("")
        lines.append(_format_h_pre_structure_report(h_df))
        lines.append("")
        lines.append("[H-MA5 전용 — 5일선 종가이탈 손절·+5/+10 검증]")
        lines.append(_format_h_ma5_block(h_df, 'H-MA5 전체'))
        lines.append(_format_h_ma5_block(h_df[_bt_mask_h_struct_safe(h_df)], 'H-MA5 STRUCT'))
        lines.append(_format_h_ma5_block(h_df[_bt_mask_h_triangle(h_df)], 'H-MA5 TRIANGLE SAFE'))
        lines.append(_format_h_ma5_block(h_df[_bt_mask_h_core_500_1000_vol23(h_df)], 'H-MA5 CORE 500~1000억×2~3배'))
        lines.append(_format_h_ma5_block(h_df[_bt_mask_h_core_300_500_vol35(h_df)], 'H-MA5 CORE 300~500억×3~5배'))
        lines.append(_format_h_ma5_block(h_df[_bt_mask_h_v427_core_union(h_df)], 'H-MA5 CORE UNION'))
        lines.append(_format_h_ma5_block(h_df[_bt_mask_h_safe(h_df)], 'H-MA5 VOL-SAFE 2~3배'))
        lines.append(_format_h_ma5_block(h_df[_bt_mask_h_vol_safe_pattern(h_df)], 'H-MA5 PATTERN SAFE 2~3배+저변동+직전구조'))
        lines.append(_format_h_ma5_block(h_df[_bt_mask_h_vol_swing(h_df)], 'H-MA5 SWING 3~5배'))
        lines.append(_format_h_ma5_block(h_df[_bt_mask_h_vol_overheat(h_df)], 'H-MA5 OVERHEAT 5~8배'))
        lines.append(_format_h_ma5_block(h_df[_bt_mask_h_vol_aggressive(h_df)], 'H-MA5 AGGRESSIVE 8배+'))
        lines.append(_format_h_ma5_block(h_df[_bt_mask_h_neutral(h_df)], 'H-MA5 NEUTRAL'))
        lines.append(_format_h_ma5_block(h_df[_bt_mask_h_risk(h_df)], 'H-MA5 RISK'))
    ...
    lines.append("- v4.2.8.1부터 실시간 스캔에서도 H-TRIANGLE SAFE와 거래대금×Vol60 핵심셀(500~1000억×2~3배, 300~500억×3~5배)을 S/G/H 통합 운영 후보로 표시하고, 돌파봉 Vol60 배율 운용분류에 더해, 거래대금 구간을 <30억/30~50억/50~100억/100~200억/200~300억/300~500억/500~1000억/1000~2000억/2000~3000억/3000~5000억/5000억~1조/1조+로 세분화해 성과를 분해합니다. H-VOL SAFE는 2~3배, H-VOL SWING은 3~5배, H-OVERHEAT는 5~8배, H-AGGRESSIVE는 8배 이상입니다. 수급 데이터는 이번 버전에서 필수 조건이 아니라 후속 가점 후보입니다.")
...
def run_closing_bet_backtest(
    ...
    if raw_df.empty:
        report = _build_backtest_summary(
            ...
            all_candidates, weekly_breakdown=weekly_breakdown, diag=diag, core_only=core_only, safe_only=safe_only, neutral_only=neutral_only, risk_only=risk_only, c_only=c_only, c_swing_only=c_swing_only, c_strict_only=c_strict_only, c_pullback_only=c_pullback_only, g_only=g_only, h_only=h_only,
        )
        try:
    ...
    if (safe_only or neutral_only or risk_only or core_only or c_only or c_swing_only or c_strict_only or c_pullback_only or g_only or h_only) and selected_df is not None and not selected_df.empty:
        try:
            if h_only:
    ...
    report = _build_backtest_summary(
        ...
        all_candidates, weekly_breakdown=weekly_breakdown, diag=diag if debug else None, core_only=core_only, safe_only=safe_only, neutral_only=neutral_only, risk_only=risk_only, c_only=c_only, c_swing_only=c_swing_only, c_strict_only=c_strict_only, c_pullback_only=c_pullback_only, g_only=g_only, h_only=h_only,
    )
    try:
        BACKTEST_SUMMARY_TXT.write_text(report, encoding='utf-8')
        if debug:
