...
CLOSING_BET_SCANNER_VERSION = 'G_MORALES_V4_2_0_HIGH_BREAKOUT_DRYUP_BACKTEST_20260511'
...
def _check_backtest_strategies_on_df(code: str, name: str, hist: pd.DataFrame) -> list:
    ...
    try:
        ...
        # H — 신고가 거자름 / 신고가 돌파 후 거래량 마른 눌림형
        # v4.2.0 실험형: 52주(또는 120일) 신고가를 강한 거래량으로 돌파한 뒤,
        # 1~10거래일 안에 거래량이 마른 짧은 음봉/짧은 양봉이 나오면 단기 스윙 후보로 기록한다.
        try:
            if len(hist) >= 150 and info['amount_b'] >= MIN_AMOUNT / 1e8:
                ...
                short_candle = bool(body_pct <= 4.5 and range_pct <= 9.0)
                short_red_or_small_bull = bool((close < open_p) or (close >= open_p and body_pct <= 3.0))
                volume_dry = bool(
                    (prev5_vol > 0 and vol <= prev5_vol) or
                    (vma20_now > 0 and vol <= vma20_now * 0.85)
                )
                ma_support = bool(
                    (ma5_now > 0 and close >= ma5_now * 0.99) or
                    (ma10_now > 0 and close >= ma10_now * 0.985)
                )

                breakout_candidates = []
                # 최근 1~10거래일 전의 신고가 돌파봉 탐색. 현재봉은 거자름 후보이므로 제외한다.
                ...
                for d in range(1, max_days + 1):
                    ...
                    is_new_high = bool((bclose >= base_high * 1.002) or (bhigh >= base_high * 1.005))
                    strong_vol = bool(bvol60_ratio >= 1.5)
                    strong_close = bool(bclose >= base_high * 0.998)
                    if is_new_high and strong_vol and strong_close:
                        breakout_candidates.append({
                            'pos': pos, 'days': d, 'date': bdate, 'close': bclose, 'high': bhigh,
                            'base_high': base_high, 'vol60_ratio': bvol60_ratio,
                ...
                if breakout_candidates:
                    ...
                    hold_breakout_zone = bool(close >= br['base_high'] * 0.985 and close >= br['close'] * 0.92)
                    not_over_pullback = bool(0.0 <= pullback_pct <= 12.0)
                    not_cliff = bool(close >= max(ma20_now, br['base_high'] * 0.92) if ma20_now > 0 else close >= br['base_high'] * 0.92)

                    cond = {
                        '①신고가돌파이력': True,
                        '②돌파거래량1.5배↑': br['vol60_ratio'] >= 1.5,
                        '③1~10일내거자름': 1 <= br['days'] <= 10,
                        '④거래량마름': volume_dry,
                        '⑤짧은음봉/짧은양봉': short_candle and short_red_or_small_bull,
                        '⑥5/10일선지지': ma_support,
                        '⑦돌파권유지': hold_breakout_zone,
                        '⑧눌림12%이내': not_over_pullback,
                        '⑨급락아님': not_cliff,
                    }
                    passed = [k for k, v in cond.items() if bool(v)]
                    score = 0
                    score += 18 if cond['①신고가돌파이력'] else 0
                    score += 14 if cond['②돌파거래량1.5배↑'] else 0
                    score += 10 if cond['③1~10일내거자름'] else 0
                    score += 18 if cond['④거래량마름'] else 0
                    score += 14 if cond['⑤짧은음봉/짧은양봉'] else 0
                    score += 12 if cond['⑥5/10일선지지'] else 0
                    score += 8 if cond['⑦돌파권유지'] else 0
                    score += 4 if cond['⑧눌림12%이내'] else 0
                    score += 2 if cond['⑨급락아님'] else 0
                    score = min(int(score), 100)

                    if score >= 70:
                        grade = '완전체' if score >= 90 else ('✅A급' if score >= 80 else 'B급')
                        h = _bt_common_payload(code, name, 'H', '신고가거자름', grade, score, row, hist, idx_label, marcap, passed)
                        h_stop = ma5_now * 0.985 if ma5_now > 0 else low
                        h.update({
                            'band_type': 'HIGH_DRYUP',
                            'band_reason': '신고가 돌파 후 거래량 마른 짧은 눌림 캔들',
                            ...
                            'high_dryup_volume_dry': int(bool(volume_dry)),
            ...
            log_debug(f"H 신고가거자름 백테스트 오류 [{code}/{name}]: {e}")
...
def _bt_mask_h_safe(df: pd.DataFrame):
    """v4.2.0: H 신고가 거자름 SAFE 후보.
    돌파 후 1~7거래일 이내, 거래량 마름, 짧은 캔들, 5/10일선 지지, 돌파권 유지가 핵심이다.
    """
    if df is None or df.empty or 'mode' not in df.columns:
        return pd.Series(False, index=df.index if df is not None else [])
    ...
    support = pd.to_numeric(df.get('high_dryup_ma_support', 0), errors='coerce').fillna(0).astype(int)
...
def _bt_mask_h_risk(df: pd.DataFrame):
    """v4.2.0: H 신고가 거자름 위험 후보. 거래량이 안 마르거나, 5/10일선 지지가 없거나, 눌림이 깊은 경우."""
    if df is None or df.empty or 'mode' not in df.columns:
        return pd.Series(False, index=df.index if df is not None else [])
    mode_h = _bt_mask_h_all(df)
    days = pd.to_numeric(df.get('days_since_high_breakout', 999), errors='coerce').fillna(999)
    ...
    amount = pd.to_numeric(df.get('amount_b', 0), errors='coerce').fillna(0)
    return mode_h & ((days > 10) | (dry == 0) | (short == 0) | (support == 0) | (zone == 0) | (pullback > 12.0) | (amount < 50.0))


def _bt_mask_h_neutral(df: pd.DataFrame):
    """v4.2.0: H 후보 중 SAFE도 RISK도 아닌 중립 후보."""
    if df is None or df.empty or 'mode' not in df.columns:
        return pd.Series(False, index=df.index if df is not None else [])
    return _bt_mask_h_all(df) & (~_bt_mask_h_safe(df)) & (~_bt_mask_h_risk(df))
...
def _format_h_reason_report(df: pd.DataFrame) -> str:
    """H 신고가 거자름 SAFE/RISK 탈락 원인 요약."""
    lines = ["[H 신고가 거자름 SAFE/RISK 탈락 사유 분석 — v4.2.0]"]
    try:
        if df is None or df.empty or 'mode' not in df.columns:
            lines.append("- 데이터 없음")
            return "\n".join(lines)
        ...
        lines.append(f"- H 전체 {len(h)}건 | SAFE {len(safe)}건 | NEUTRAL {len(neutral)}건 | RISK {len(risk)}건")
        ...
        flags = {
            ...
            '5/10일선지지아님': _num('high_dryup_ma_support', 0).astype(int) != 1,
        ...
        lines.append("- 해석: H는 돌파 당일 추격이 아니라 신고가 돌파 후 거래량이 마른 짧은 눌림 캔들을 검증하는 실험 전략입니다.")
    except Exception as e:
...
def _build_backtest_summary(
    ...
    if c_pullback_only:
        ...
        sel_text += ' → H 신고가거자름만 필터(신고가 돌파 후 거래량 마른 눌림)'
    elif safe_only:
    ...
    try:
        ...
        if len(h_all_df) > 0:
            lines.append(_format_backtest_trade_rule_block(h_all_df, 'H 신고가거자름 전체'))
            lines.append(_format_backtest_trade_rule_block(df[_bt_mask_h_safe(df)], '🟢 H-SAFE 신고가거자름'))
            lines.append(_format_backtest_trade_rule_block(df[_bt_mask_h_neutral(df)], '🟡 H-NEUTRAL 신고가거자름'))
            lines.append(_format_backtest_trade_rule_block(df[_bt_mask_h_risk(df)], '⚠️ H-RISK 신고가거자름'))
    except Exception:
        pass
    lines.append("")
    ...
    if not h_df.empty:
        lines.append("")
        lines.append("[H전략 세부 — 신고가 거자름 분리]")
        lines.append(_format_backtest_trade_rule_block(h_df, 'H 신고가거자름 전체'))
        lines.append(_format_backtest_trade_rule_block(h_df[_bt_mask_h_safe(h_df)], '🟢 H-SAFE 신고가거자름'))
        lines.append(_format_backtest_trade_rule_block(h_df[_bt_mask_h_neutral(h_df)], '🟡 H-NEUTRAL 신고가거자름'))
        lines.append(_format_backtest_trade_rule_block(h_df[_bt_mask_h_risk(h_df)], '⚠️ H-RISK 신고가거자름'))
        ...
        lines.append(_format_h_ma5_block(h_df[_bt_mask_h_safe(h_df)], 'H-MA5 SAFE'))
    ...
    lines.append("- --backtest-h-only 옵션을 쓰면 H 신고가거자름만 필터링하여 H-SAFE/NEUTRAL/RISK 성과와 5일선 종가이탈 손절 기준을 확인합니다.")
    lines.append("- v4.2.0부터 H 신고가거자름은 신고가 돌파 후 거래량 마른 눌림형을 별도 검증합니다. 수급 데이터는 이번 버전에서 필수 조건이 아니라 후속 가점 후보입니다.")
    ...
    if (safe_only or neutral_only or risk_only or core_only or c_only or c_swing_only or c_strict_only or c_pullback_only or g_only or h_only) and selected_df is not None and not selected_df.empty:
        try:
            if h_only:
                ...
                log_info(f"H 신고가거자름 전용 백테스트 필터 적용: {len(selected_df)}건")
            elif g_only:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--backtest-h-only', action='store_true', help='H 신고가거자름만 필터링하여 H-SAFE/NEUTRAL/RISK 성과를 확인')
