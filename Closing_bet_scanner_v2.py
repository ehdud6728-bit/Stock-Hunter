...
CLOSING_BET_SCANNER_VERSION = 'G_MORALES_V4_4_9_29_ONE_PICK_SHORT_MID_SPLIT_20260601'
...
def _send_results(hits: list, mins_left: int):
    ...
    header = (
        ...
        f"전체 후보 {len(hits)}개 | v4.4.9.29 ONE-PICK SHORT/MID SPLIT: 14:40 후보발굴 + 15:03 실행/포기 압축 최종킥\n"
    ...
    def _build_practical_block():
        ...
        def _build_final_kick_block() -> str:
            ...
            op = '금지' if not picked_short else ('적극' if short_decision_counts.get('✅ 종가진입 가능', 0) >= 2 else ('보통' if short_decision_counts.get('✅ 종가진입 가능', 0) >= 1 else '지정가만'))
            ...
            short_top_line = _final_kick_top_line(picked_short, '없음')
            ...
            lines = [
                '🚨 [동시호가 최종킥 — v4.4.9.29 COMPACT]',
                f'시간: {TODAY_STR} {_now_kst().strftime("%H:%M")} | FINAL_KICK_ONLY | 카드최대 {max_cards}개',
                f'오늘 단기 운용: {op}',
                f'오늘 단기 종가배팅: {short_top_line}',
                f'중기 분할관찰 1순위: {mid_top_line}',
                core_line,
                s_label_line,
        ...
        block = ['[🎯 실전 운영 후보 — v4.4.9.29 ONE-PICK SHORT/MID SPLIT]']
        summary = (
...
# =============================================================
# v4.4.9.29 ONE-PICK SHORT/MID SPLIT — 신고가거자름 핵심셀 승격검증
# =============================================================
...
def _v439_build_compact_backtest_report(original_report: str, selected_csv: str = '', raw_csv: str = '', start_date: str = '', end_date: str = '', hold_days: int = 0, top_per_strategy: int = 5, all_candidates: bool = False) -> str:
    ...
    try:
        ...
        sel_text = '전체후보' if all_candidates else f'날짜별 전략별 TOP{top_per_strategy}'
        lines = []
        lines.append("🧪 v4.4.9.29 ONE-PICK SHORT/MID SPLIT")
        lines.append(f"버전: {CLOSING_BET_SCANNER_VERSION}")
        lines.append(f"기간: {start_date} ~ {end_date} | 보유평가: 다음 {hold_days}거래일")
