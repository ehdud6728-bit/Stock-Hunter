...
CLOSING_BET_SCANNER_VERSION = 'G_MORALES_V4_4_9_6_A_CONFIRM_LIVE_ENTRY_COMMENT_20260520'
...
def _check_breakout_bet(code: str, name: str) -> dict | None:
    ...
    try:
        ...
        # v4.4.9.6: A-RETEST / A-CONFIRM 실시간 진입가격 코멘트용 일봉 컨텍스트.
        ...
        if len(df) >= 2:
            ...
            info.update({'a_prev_core2': 0, 'a_confirm_live': 0, 'a_prev_high': 0, 'a_prev_close': 0, 'a_prev_ma5': 0})
...
def _send_results(hits: list, mins_left: int):
    ...
    header = (
        ...
        f"전체 후보 {len(hits)}개 | v4.4.9.6 기존 검색식 유지 + A-CONFIRM 진입가격 코멘트 + 전용방 강제전송\n"
    ...
    def _pattern_path_text(h: dict, mode: str = '') -> str:
        ...
        if mode == 'A':
            if _is_a_confirm_live(h):
                return "전일 A-RETEST CORE가 다음날 살아난 확인형입니다. 전일고가 회복 후 +3/+5 빠른 익절형으로 보고, 전일종가 이탈은 위험신호입니다."
            if _is_a_retest_core2_live(h):
    ...
    def _candidate_mental_guide(h: dict, mode: str = '') -> str:
        ...
        if mode == 'A':
            if _is_a_confirm_live(h):
                return "A-CONFIRM 확인형입니다. 전일고가 회복가가 가장 좋은 실전 타점으로 검증됐고, +3 1차익절·전일종가 이탈 무효를 우선합니다."
            if _is_a_retest_core2_live(h):
    ...
    def _brief_practical_line(h: dict, idx: int) -> str:
        ...
        if mode == 'S' and _is_s1_good(h):
            ...
            if _is_a_confirm_live(h):
                tag = 'A-SAFE 다음날확인형'
                reason = '전일 A-RETEST CORE 후 오늘 전일고가 회복/양봉·거래대금 유지가 붙은 확인형 후보'
            elif _is_a_retest_core2_live(h):
                tag = 'A-RETEST CORE②'
    ...
    def _build_practical_block():
        ...
        block = ['[🎯 실전 운영 후보 — v4.4.9.6 A-CONFIRM LIVE ENTRY COMMENT]']
        summary = (
            ...
            f"🔁A-CONFIRM {len(a_confirm_pool)}개 | 🟣A-CORE {len(a_core_pool)}개 | A보조 출력 {a_output_n}개/전체 {len(a_pool)}개"
        )
        if a_risk_pool:
        ...
        if a_confirm_pool:
            ...
            block.append('v4.4.9.6 기준: 전일 A-RETEST CORE였고 오늘 전일고가 회복/양봉/거래대금 유지가 확인된 후보입니다. 백테스트상 전일고가 회복가 진입이 가장 우수했습니다. 시초 +2% 이상 급등 추격은 피하고, 전일고가 회복가 또는 회복 후 눌림 재지지를 우선합니다.')
        ...
        if a_core_pool:
            ...
            block.append('조건: 5000억+·종가80+·거래량≤1.8·RR0.8~1.5. LP/L/S 후보가 부족한 날만 보조로 참고하고, 핵심 진입은 다음날 전일고가 회복 확인입니다. RR 1.8 초과 A는 제외합니다.')
...
def _v439_i_samples_section(df: pd.DataFrame, top_n: int = 5) -> list[str]:
    ...
    try:
        ...
        if i.empty:
            return lines
        lines.append("[📋 I-MAIN 발생 샘플 — 종목별 중복제거 v4.4.9.6]")
...
def _v445_pattern_path_profile_lines(df: pd.DataFrame) -> list[str]:
    ...
    try:
        ...
        lines.append('[🧭 패턴별 타점 이후 흐름 프로파일 — v4.4.9.6 보정판]')
        ...
        lines.append('- 해석: v4.4.9.6은 1~3일 첫흔들림, +3전흔들림, 전체최대흔들림을 분리합니다. 전체최대흔들림은 목표 도달 후 흔들림까지 포함될 수 있으므로 실전 판단은 +3전흔들림과 지지선 종가유지를 우선 봅니다.')
    except Exception as e:
...
def _v442_new_pattern_performance_lines(df: pd.DataFrame) -> list[str]:
    ...
    try:
        ...
        lines.append('- 해석: v4.4.9.6부터 LP는 단기 최우선 운용 후보로 보되, D1/D23/D45 타점과 다음날 대응 시나리오를 함께 확인합니다. LP-SAFE는 실행 후보, LP-WATCH는 다음날 양봉·전일고가 회복·거래대금 유지 시 승격, LP-RISK는 제외입니다. SLOCK은 실시간 기본 숨김, IT는 ACCEL 중심으로만 봅니다.')
    except Exception as e:
...
# =============================================================
# v4.4.9.6 A-CONFIRM LIVE ENTRY COMMENT — A/C/H/B 승격검증
# =============================================================
...
def _v449_price_series_any(df: pd.DataFrame, candidates: list[str]) -> pd.Series:
    """v4.4.9.6: OHLC 유사 컬럼을 여러 이름에서 찾아 숫자 Series로 반환."""
    try:
        if df is None or df.empty:
            return pd.Series(dtype=float)
        for c in candidates:
...
# =============================================================
# v4.4.9.6 A-CONFIRM LIVE ENTRY COMMENT — 손절률 25% 이하 후보 찾기
# =============================================================
...
# =============================================================
# v4.4.9.6 A-CONFIRM LIVE ENTRY COMMENT — A/C/B/H 0건 원인 진단
# =============================================================
...
def _v4492_close_loc_fallback_diagnostic_lines(df: pd.DataFrame) -> list[str]:
    ...
    try:
        ...
        amount = _v449_base_liquidity_series(df)
        lines.append('[🧩 종가위치 fallback 보정 — v4.4.9.6]')
...
# =============================================================
# v4.4.9.6 A-CONFIRM LIVE ENTRY COMMENT — 재료/시황 동반 성과검증
# =============================================================
...
# =============================================================
# v4.4.9.6 A-CONFIRM LIVE ENTRY COMMENT — 다음날 진입가별 성과검증
# =============================================================
...
def _v4495_a_confirm_entry_price_test_lines(df: pd.DataFrame, hold_days: int = 60) -> list[str]:
    ...
    try:
        ...
        lines.append('[🎯 A-SAFE 다음날확인 진입가별 백테스트 — v4.4.9.6]')
...
def _v439_build_compact_backtest_report(original_report: str, selected_csv: str = '', raw_csv: str = '', start_date: str = '', end_date: str = '', hold_days: int = 0, top_per_strategy: int = 5, all_candidates: bool = False) -> str:
    ...
    try:
        ...
        sel_text = '전체후보' if all_candidates else f'날짜별 전략별 TOP{top_per_strategy}'
        lines = []
        lines.append("🧪 v4.4.9.6 A-CONFIRM LIVE ENTRY COMMENT")
        lines.append(f"버전: {CLOSING_BET_SCANNER_VERSION}")
        lines.append(f"기간: {start_date} ~ {end_date} | 보유평가: 다음 {hold_days}거래일")
        ...
        lines.append("- 신규 검색식 LP/SLOCK/IT는 위 성과검증 섹션과 실시간 후보 출력 섹션에서 함께 확인합니다. v4.4.9.6은 패턴별 멘탈 요약/후보별 멘탈기준을 유지하고, A-RETEST CORE②와 A-SAFE 다음날확인형의 전일종가·다음날시가·다음날종가·전일고가회복가 진입 성과를 비교합니다. LP-D23 과최적화 검증과 LP 없는 날 대체 시나리오를 같이 봅니다. SLOCK은 실시간 기본 숨김, IT는 ACCEL 중심으로 봅니다.")
        ...
        return (str(original_report or '') + f"\n\n[v4.4.9.6 압축 요약 생성 실패] {type(e).__name__}: {e}")
