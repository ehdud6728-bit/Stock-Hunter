...
CLOSING_BET_SCANNER_VERSION = 'G_MORALES_V4_4_9_19_S_SAFE_LIQUIDITY_RULE_REPEAT_20260527'
    ...
    def _build_practical_block():
        ...
        try:
            if CLOSING_BET_LIVE_OPERATION_GUARD and CLOSING_BET_LIVE_TRACKING_LOG:
                ...
                _add_live_pool(safe_pool + neutral_pool, 'S-SAFE/NEUTRAL', 5, PRACTICAL_SAFE_TOP_N)
        ...
        def _build_final_kick_block() -> str:
            ...
            _push(safe_pool + neutral_pool, 'S-SAFE/NEUTRAL', PRACTICAL_SAFE_TOP_N)
...
def _v44919_s_safe_liquidity_rule_repeat_lines(df: pd.DataFrame, compact: bool = True) -> list[str]:
    ...
    try:
        ...
        lines.append('[🔁 S-SAFE 거래대금 규칙 반복검증 — v4.4.9.19]')
        ...
        lines.append('- 운용안: S-SAFE+ST30 통과=S-RECLAIM PRIME, S-SAFE+ST30 미통과+고유동성=S-MOMENTUM WATCH, S-SAFE+ST30 미통과+저유동성=S-LIQUIDITY RISK로 분리합니다.')
    except Exception as e:
...
def _v439_build_compact_backtest_report(original_report: str, selected_csv: str = '', raw_csv: str = '', start_date: str = '', end_date: str = '', hold_days: int = 0, top_per_strategy: int = 5, all_candidates: bool = False) -> str:
    ...
    try:
        ...
        sel_text = '전체후보' if all_candidates else f'날짜별 전략별 TOP{top_per_strategy}'
        lines = []
        lines.append("🧪 v4.4.9.18 S-SAFE MISS SUCCESS/FAIL AUDIT")
        lines.append(f"버전: {CLOSING_BET_SCANNER_VERSION}")
        lines.append(f"기간: {start_date} ~ {end_date} | 보유평가: 다음 {hold_days}거래일")
        ...
        lines.append("- 신규 검색식 LP/SLOCK/IT는 위 성과검증 섹션과 실시간 후보 출력 섹션에서 함께 확인합니다. v4.4.9.19는 ST30-RECLAIM을 하드필터가 아닌 패턴별 교차검증/성공누락 감사 태그로 유지하고, S-SAFE 미통과 성공/실패군을 거래대금 버킷과 S-MOMENTUM/S-LIQUIDITY 후보규칙으로 반복검증하며, FAST 실전 출력 후보만 우선 표시하며, A-CONFIRM PRIME/CALM·시장/섹터압력·실전 추적로그를 추가합니다. 넓은 A/C/B/H/SLOCK은 기본 숨김/연구로 유지합니다.")
        compact = "\n".join([str(x) for x in lines if x is not None])
        try:
            out_path = LOG_DIR / 'closing_bet_backtest_summary_v4_4_9_19_s_safe_liquidity_rule_repeat.txt'
