...
CLOSING_BET_SCANNER_VERSION = 'G_MORALES_V4_4_9_20_S_SAFE_LIVE_LABEL_APPLY_20260527'
...
# v4.4.9.14: 2차 스캔 전용 FINAL KICK MODE
...
def _send_results(hits: list, mins_left: int):
    ...
    header = (
        ...
        f"전체 후보 {len(hits)}개 | v4.4.9.14 FINAL KICK MODE: 14:40 후보발굴 + 15:03 실행/포기 최종킥\n"
    ...
    # v4.4.9.20: S-SAFE live labels. ST30은 하드필터가 아니라 가점/강등 태그이고,
    ...
    def _build_practical_block():
        ...
        # v4.4.9.20: S-SAFE live label pools
        ...
        # v4.4.9.14: 2차 스캔 전용 — 동시호가 직전 실행/포기 최종킥
        ...
        def _build_final_kick_block() -> str:
            ...
            lines = [
                '🚨 [동시호가 최종킥 — v4.4.9.14]',
                f'시간: {TODAY_STR} {_now_kst().strftime("%H:%M")} | 모드: FINAL_KICK_ONLY',
                f'오늘 운용: {op}',
                f'오늘 한 종목만 한다면: {top_name}',
                '',
                f"✅ 종가진입 가능: {decision_counts.get('✅ 종가진입 가능', 0)}개",
                f"🟡 지정가만 가능: {decision_counts.get('🟡 지정가만 가능', 0)}개",
                f"❌ 포기 전환/제외: {decision_counts.get('❌ 오늘 포기', 0) + len(risk_pool) + len(a_risk_pool) + len(lp_risk_pool)}개",
                '',
            ]
            if not picked:
                lines += [
        ...
        block = ['[🎯 실전 운영 후보 — v4.4.9.14 FINAL KICK MODE]']
        summary = (
        ...
        if safe_pool:
            ...
            block.append('v4.4.9.20 기준: S-SAFE는 S-RECLAIM PRIME(ST30통과), S-MOMENTUM PRIME(5000억+), S-MOMENTUM 3000~5000, S-MOMENTUM WATCH(1000억+), S-LIQUIDITY RISK(1000억 미만)로 분리합니다. ST30은 하드필터가 아닙니다.')
...
def _v44919_s_safe_liquidity_rule_repeat_lines(df: pd.DataFrame, compact: bool = True) -> list[str]:
    ...
    try:
        ...
        lines.append('[🔁 S-SAFE 거래대금 규칙 반복검증 — v4.4.9.20]')
...
def _v439_build_compact_backtest_report(original_report: str, selected_csv: str = '', raw_csv: str = '', start_date: str = '', end_date: str = '', hold_days: int = 0, top_per_strategy: int = 5, all_candidates: bool = False) -> str:
    ...
    try:
        ...
        sel_text = '전체후보' if all_candidates else f'날짜별 전략별 TOP{top_per_strategy}'
        lines = []
        lines.append("🧪 v4.4.9.20 S-SAFE LIVE LABEL APPLY")
        lines.append(f"버전: {CLOSING_BET_SCANNER_VERSION}")
        lines.append(f"기간: {start_date} ~ {end_date} | 보유평가: 다음 {hold_days}거래일")
        ...
        lines.append("- 신규 검색식 LP/SLOCK/IT는 위 성과검증 섹션과 실시간 후보 출력 섹션에서 함께 확인합니다. v4.4.9.20은 ST30-RECLAIM을 하드필터가 아닌 가점/주의 태그로 유지하고, 실시간 S-SAFE 후보를 S-RECLAIM PRIME·S-MOMENTUM PRIME·S-MOMENTUM 3000~5000·S-MOMENTUM WATCH·S-LIQUIDITY RISK로 분리 표시합니다. FAST 실전 출력 후보만 우선 표시하며, A-CONFIRM PRIME/CALM·시장/섹터압력·실전 추적로그를 유지합니다. 넓은 A/C/B/H/SLOCK은 기본 숨김/연구로 유지합니다.")
        compact = "\n".join([str(x) for x in lines if x is not None])
        try:
            out_path = LOG_DIR / 'closing_bet_backtest_summary_v4_4_9_20_s_safe_live_label_apply.txt'
