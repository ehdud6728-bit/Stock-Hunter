...
CLOSING_BET_SCANNER_VERSION = 'G_MORALES_V4_4_0_OPERATION_LIVE_DECISION_20260518'
...
# ── 종가배팅 전용 텔레그램 설정
# v4.4.0: 기본방 오발송 방지. CLOSING_BET/CLOSE_BET 전용 CHAT_ID가 있으면 반드시 그 방으로 보낸다.
...
def _check_closing_bet(code: str, name: str) -> dict | None:
    ...
    candidates = [x for x in [s, l_sig, g, h_sig, i_sig, a, b1, b2, c] if x]
    if not candidates:
        return None
    ...
    def _priority(h):
        ...
        mode_rank = {'S': 0, 'L': 1, 'G': 2, 'H': 3, 'I': 4, 'A': 5, 'B1': 6, 'B2': 7, 'C': 8}.get(mode, 9)
...
def _send_results(hits: list, mins_left: int):
    ...
    hits_i = [x for x in hits if _pick_strategy(x) == "I" or _safe_int(x.get('i_core', 0), 0) == 1]
    ...
    header = (
        ...
        f"전체 후보 {len(hits)}개 | v4.4.0 L/S/I 우선순위 실시간 운용판정 + 전용방 강제전송\n"
        f"전략 후보: S {len(hits_s)}개(S2 {len(hits_s2)}/S1 {len(hits_s1)}) | L {len(hits_l)}개 | G {len(hits_g)}개 | H {len([x for x in hits if _pick_strategy(x) == 'H' or str(x.get('band_type', '') or '') == 'HIGH_DRYUP_STRICT'])}개 | I {len(hits_i)}개 | A {len(hits_a)}개 | "
    ...
    def _build_practical_block():
        ...
        block = ['[🎯 실전 운영 후보 — v4.4.0 LIVE DECISION]']
        summary = (
            f"요약: 👑L-SAFE {len(l_safe_pool)}개 | 🌊L-TAIL {len(l_tail_pool)}개 | 💰L-MEGA {len(l_mega_pool)}개 | 🟢S-SAFE {len(safe_pool)}개 | 🟡S관찰 {len(neutral_pool)}개 | 🟢G-SAFE {len(g_safe_pool)}개 | "
        ...
        block.append('[🚦 v4.4.0 실시간 운용판정]\n단기 실행 우선순위: L 5000억+ 리더갭 > S2 실행형/S-NEUTRAL > S-SAFE 참고 > G-SAFE. A/C 5000억+는 즉시매수 아님, 다음날 전일고가 회복+거래대금 유지 시만 승격합니다.\n중기 관찰 우선순위: I-MAIN ACCEL > I-MAIN CORE > I-WATCH/ADD/CONFIRM. H/B는 검증·관찰만, H-RISK/과열 H는 제외합니다.\n매수계획: 단기 후보는 1차/2차 진입·손절/익절, I-MAIN 후보는 분할매집·박스/150/200MA 무효조건을 함께 표시합니다.')

        if l_core_pool:
            block.append('')
            block.append('[🥇 단기 1순위 L-CORE/MEGA 리더갭 — 5000억+ 우선]')
            block.append('v4.4.0 검증 반영: 단기 종가배팅 1순위는 L 5000억+ 리더갭입니다. 5000억~1조, 갭 6~12%, 종가위치 70%+는 가점이지만 표본이 작으면 과신 금지입니다.')
            used_codes = set()
            shown = 0
        ...
        if safe_pool:
            ...
            block.append('v4.4.0 기준: S는 S2 실행형/S-NEUTRAL을 우선하고, SAFE는 종가위치·RR·거래대금 조건이 맞을 때만 소액 확인합니다.')
        ...
        if PRACTICAL_SHOW_I_MAIN and (i_accel_pool or i_core_pool or i_watch_pool or i_add_pool or i_confirm_pool):
            ...
            block.append('v4.4.0 기준: I-MAIN은 단기 종가배팅이 아니라 20/40/60거래일 중기 누적관찰입니다. ACCEL은 고수익 시세분출형, CORE는 안정형 누적관찰로 봅니다.')
            used_codes = set()
            shown_total = 0
            groups = [
        ...
        if neutral_pool:
            ...
            block.append('v4.4.0 기준: S2 실행형/S-NEUTRAL은 단기 2순위입니다. 단, 고점권 전략이므로 KOSPI200·5조+·거래대금 3000억+·종가위치70%+를 우대합니다.')
...
def run_closing_bet_scan(force: bool = False) -> list:
    ...
    hits_i = [h for h in hits if h.get('mode') == 'I' or _safe_int(h.get('i_core', 0), 0) == 1]
    ...
    hits = hits_s + hits_l + hits_g + hits_h + hits_i + hits_a + hits_b1 + hits_b2 + hits_c
    ...
    log_info(
        ...
        f"신고가거자름(H): {len(hits_h)}개 | I-MAIN(I): {len(hits_i)}개 | ENV엄격형(B1): {len(hits_b1)}개 | BB확장형(B2): {len(hits_b2)}개 | 역매공파(C): {len(hits_c)}개"
    )
...
def _v439_i_samples_section(df: pd.DataFrame, top_n: int = 5) -> list[str]:
    ...
    try:
        ...
        if i.empty:
            return lines
        lines.append("[📋 I-MAIN 발생 샘플 — 종목별 중복제거 v4.4.0]")
...
def _v439_build_compact_backtest_report(original_report: str, selected_csv: str = '', raw_csv: str = '', start_date: str = '', end_date: str = '', hold_days: int = 0, top_per_strategy: int = 5, all_candidates: bool = False) -> str:
    ...
    try:
        ...
        sel_text = '전체후보' if all_candidates else f'날짜별 전략별 TOP{top_per_strategy}'
        lines = []
        lines.append("🧪 v4.4.0 OPERATION LIVE DECISION SUMMARY")
        lines.append(f"버전: {CLOSING_BET_SCANNER_VERSION}")
        lines.append(f"기간: {start_date} ~ {end_date} | 보유평가: 다음 {hold_days}거래일")
        ...
        return (str(original_report or '') + f"\n\n[v4.4.0 압축 요약 생성 실패] {type(e).__name__}: {e}")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--compact-summary', action='store_true', help='v4.4.0 압축/실시간 운용 요약을 텔레그램 전송/로그에 사용')
