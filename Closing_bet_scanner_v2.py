...
CLOSING_BET_SCANNER_VERSION = 'G_MORALES_V4_4_9_9_A_CONFIRM_VC_PERFORMANCE_CHECK_20260520'
...
def _check_breakout_bet(code: str, name: str) -> dict | None:
    ...
    try:
        ...
        if len(df) >= 2:
            ...
            # 하드필터가 아니라 VC-SAFE/VC-WATCH 태그로 먼저 검증한다.
            ...
            if vc_safe:
                vc_reason = 'VC-SAFE: 거래량이 식거나 과열되지 않은 상태에서 전일종가/5MA 지지 유지'
            elif vc_watch:
                vc_reason = 'VC-WATCH: 가격지지는 유지되나 거래량 수축 강도는 추가 확인 필요'
            else:
                vc_reason = 'VC-확인필요: 거래량 수축 또는 가격지지 근거가 약함'
...
def _send_results(hits: list, mins_left: int):
    ...
    header = (
        ...
        f"전체 후보 {len(hits)}개 | v4.4.9.9 기존 검색식 유지 + A-CONFIRM VC 태그별 성과검증 + 진입가격 코멘트 + 전용방 강제전송\n"
    ...
    def _pattern_path_text(h: dict, mode: str = '') -> str:
        ...
        if mode == 'A':
            if _is_a_confirm_live(h):
                return "전일 A-RETEST CORE가 다음날 살아난 확인형입니다. 전일고가 회복 후 +3/+5 빠른 익절형으로 보되, VC-SAFE면 거래량이 식으며 지지된 정상 흔들림으로 봅니다. 전일종가 이탈은 위험신호입니다."
            if _is_a_retest_core2_live(h):
    ...
    def _candidate_mental_guide(h: dict, mode: str = '') -> str:
        ...
        if mode == 'A':
            if _is_a_confirm_live(h):
                return "A-CONFIRM 확인형입니다. 전일고가 회복가가 가장 좋은 실전 타점으로 검증됐고, VC-SAFE/VC-WATCH로 거래량 수축 여부를 함께 봅니다. +3 1차익절·전일종가 이탈 무효를 우선합니다."
            if _is_a_retest_core2_live(h):
    ...
    def _brief_practical_line(h: dict, idx: int) -> str:
        ...
        if mode == 'A' and _is_a_confirm_live(h):
            vc_line = f"   🧊 VC체크: {_a_vc_reason_text(h)}\n"
        return (
            f"{idx}) {safe_label} | {tag} | {name}({code}) | {int(close):,}원 | 점수 {score:.1f} | "
    ...
    def _build_practical_block():
        ...
        block = ['[🎯 실전 운영 후보 — v4.4.9.9 A-CONFIRM VC PERFORMANCE CHECK]']
        summary = (
            f"요약: 🥇LP-SAFE {len(lp_safe_pool)}개 | 🔁LP-WATCH {len(lp_watch_pool)}개 | ⚠️LP-RISK {len(lp_risk_pool)}개 | 👑L-SAFE {len(l_safe_pool)}개 | 🔒SLOCK검증/숨김 {len(slock_pool)}개 | ⚡IT-ACCEL {len(it_accel_live_pool)}개 | 🌊L-TAIL {len(l_tail_pool)}개 | 💰L-MEGA {len(l_mega_pool)}개 | 🟢S-SAFE {len(safe_pool)}개 | 🟡S관찰 {len(neutral_pool)}개 | 🟢G-SAFE {len(g_safe_pool)}개 | "
            ...
            f"A전체 {len(a_all_pool)}개 | 🔁A-CONFIRM {len(a_confirm_pool)}개(VC-SAFE {a_confirm_vc_safe_n}/WATCH {a_confirm_vc_watch_n}) | 🟣A-CORE {len(a_core_pool)}개 | A보조관찰 {a_output_n}개"
        )
        if a_risk_pool:
        ...
        if lp_d23_n > 0:
            ...
            op_line = f"🧭 오늘 운용강도: 보통 — LP/L 핵심은 없지만 A-CONFIRM {len(a_confirm_pool)}개(VC-SAFE {a_confirm_vc_safe_n}개). 전일고가 회복가·눌림 재지지 기준 소액 확인만."
        ...
        block.append('[🚦 v4.4.9.9 실시간 운용판정]\n기존 검색식은 훼손하지 않고 LP 운용만 세분화했습니다. 단기 실행 우선순위: LP-SAFE > L 5000억+ 리더갭 > S2/S-NEUTRAL > A-CONFIRM/A-RETEST CORE > A/C 5000억+ 관찰. LP는 D1/D23/D45 타점을 구분하고 다음날 대응 시나리오를 함께 표시합니다.\n중기 관찰 우선순위: I-MAIN ACCEL > IT-ACCEL 촉발형 > I-MAIN CORE. IT-CORE는 보조관찰, SLOCK은 실시간 기본 숨김/성과검증만 유지합니다. H/B는 검증·관찰만, H-RISK/과열 H는 제외합니다.\n파일저장/CSV 확인 없이 이 텔레그램 블록만으로 실시간 후보와 운용판정을 확인할 수 있게 구성했습니다.')
        block.append('')
        ...
        if safe_pool:
            ...
            block.append('v4.4.9 기준: S는 S2 실행형/S-NEUTRAL을 우선하고, SAFE는 종가위치·RR·거래대금 조건이 맞을 때만 소액 확인합니다.')
            ...
            block.append('')
            block.append('[🥈 단기 2순위 S2/S-CORE — 실행·관찰 후보]')
            block.append('해당 종목 없음 — 오늘은 S-CORE SAFE 조건을 만족한 후보가 없습니다. S2/S-NEUTRAL은 아래 관찰 후보에서 확인합니다.')
        ...
        if a_confirm_pool:
            ...
            block.append('v4.4.9.9 기준: 전일 A-RETEST CORE였고 오늘 전일고가 회복/양봉/거래대금 유지가 확인된 후보입니다. v4.4.9.9부터 거래량 수축·가격지지를 VC-SAFE/VC-WATCH로 함께 표시합니다. 시초 +2% 이상 급등 추격은 피하고, 전일고가 회복가 또는 회복 후 눌림 재지지를 우선합니다.')
        ...
        if a_core_pool:
            ...
            block.append('조건: 5000억+·종가80+·거래량≤1.8·RR0.8~1.5. LP/L/S 후보가 부족한 날만 보조로 참고하고, 핵심 진입은 다음날 전일고가 회복 확인입니다. 다음날 거래량 수축·가격지지가 붙으면 A-CONFIRM VC-SAFE로 봅니다. RR 1.8 초과 A는 제외합니다.')
...
def _build_backtest_detail_df(df: pd.DataFrame) -> pd.DataFrame:
    ...
    try:
        ...
        out = out.rename(columns={
            ...
            'close': '진입종가', 'amount_b': '거래대금_억', 'i_anchor_days': '기준봉후_거래일',
...
def _build_backtest_summary(
    ...
    lines.append("[전략별]")
    mode_order = ['I', 'G', 'L', 'H', 'S', 'A', 'B1', 'B2', 'C']
...
def _v439_i_samples_section(df: pd.DataFrame, top_n: int = 5) -> list[str]:
    ...
    try:
        ...
        if i.empty:
            return lines
        lines.append("[📋 I-MAIN 발생 샘플 — 종목별 중복제거 v4.4.9.9]")
...
def _v445_pattern_path_profile_lines(df: pd.DataFrame) -> list[str]:
    ...
    try:
        ...
        lines.append('[🧭 패턴별 타점 이후 흐름 프로파일 — v4.4.9.9 보정판]')
        ...
        lines.append('- 해석: v4.4.9.9는 1~3일 첫흔들림, +3전흔들림, 전체최대흔들림을 분리합니다. 전체최대흔들림은 목표 도달 후 흔들림까지 포함될 수 있으므로 실전 판단은 +3전흔들림과 지지선 종가유지를 우선 봅니다.')
    except Exception as e:
...
def _v442_new_pattern_performance_lines(df: pd.DataFrame) -> list[str]:
    ...
    try:
        ...
        lines.append('- 해석: v4.4.9.9부터 LP는 단기 최우선 운용 후보로 보되, D1/D23/D45 타점과 다음날 대응 시나리오를 함께 확인합니다. LP-SAFE는 실행 후보, LP-WATCH는 다음날 양봉·전일고가 회복·거래대금 유지 시 승격, LP-RISK는 제외입니다. SLOCK은 실시간 기본 숨김, IT는 ACCEL 중심으로만 봅니다.')
    except Exception as e:
...
# =============================================================
# v4.4.9.9 A-CONFIRM VC PERFORMANCE CHECK — A/C/H/B 승격검증
# =============================================================
...
# =============================================================
# v4.4.9.9 A-CONFIRM VC PERFORMANCE CHECK — 손절률 25% 이하 후보 찾기
# =============================================================
...
# =============================================================
# v4.4.9.9 A-CONFIRM VC PERFORMANCE CHECK — A/C/B/H 0건 원인 진단
# =============================================================
...
def _v4492_close_loc_fallback_diagnostic_lines(df: pd.DataFrame) -> list[str]:
    ...
    try:
        ...
        amount = _v449_base_liquidity_series(df)
        lines.append('[🧩 종가위치 fallback 보정 — v4.4.9.9]')
...
# =============================================================
# v4.4.9.9 A-CONFIRM VC PERFORMANCE CHECK — 재료/시황 동반 성과검증
# =============================================================
...
# =============================================================
# v4.4.9.9 A-CONFIRM VC PERFORMANCE CHECK — 다음날 진입가별 성과검증
# =============================================================
...
def _v4495_a_confirm_entry_price_test_lines(df: pd.DataFrame, hold_days: int = 60) -> list[str]:
    ...
    try:
        ...
        lines.append('[🎯 A-SAFE 다음날확인 진입가별 백테스트 — v4.4.9.9]')
...
# =============================================================
# v4.4.9.9 A-CONFIRM VC PERFORMANCE CHECK — VC 태그별 성과검증
# =============================================================
def _v4499_vc_performance_lines(df: pd.DataFrame) -> list[str]:
    """A-CONFIRM 거래량수축(VC) 태그가 실제 성과를 개선하는지 분리 검증한다.

    v4.4.9.8에서는 실시간 카드에 VC-SAFE/VC-WATCH 태그를 붙였고,
    백테스트 요약에는 태그별 성과가 분리되지 않았다.
    이 함수는 A-CONFIRM 전체와 VC-SAFE/VC-WATCH/확인필요 성과를 나눠 보여준다.
    """
    lines: list[str] = []
    try:
        ...
        # 우선 저장된 실시간 VC 컬럼을 사용한다. 없거나 전부 0이면 가격/거래량 프록시 컬럼으로 재구성한다.
        vc_safe = _v439_num_series(df, 'a_vc_safe', 0).astype(int).eq(1)
        vc_watch_raw = _v439_num_series(df, 'a_vc_watch', 0).astype(int).eq(1)
        vc_watch = vc_watch_raw & (~vc_safe)

        if int((vc_safe & target).sum() + (vc_watch & target).sum()) == 0:
            support = _v439_num_series(df, 'a_support_hold', 0).astype(int).eq(1)
            amount_hold = _v439_num_series(df, 'a_amount_hold', 0).astype(int).eq(1)
            no_bear = _v439_num_series(df, 'a_no_bear_expand', 0).astype(int).eq(1)
            vcp = _v439_num_series(df, 'a_vol_contract_prev', 0).astype(int).eq(1)
            vc3 = _v439_num_series(df, 'a_vol_contract_3d', 0).astype(int).eq(1)
            today_vr = _v439_num_series(df, 'a_today_vol_ratio', np.nan)
            # 컬럼이 없으면 broad A rows에서는 NaN/False가 되어 데이터없음으로 남긴다.
            vc_safe = target & support & amount_hold & no_bear & (vcp | vc3 | today_vr.between(0.01, 1.20))
            vc_watch = target & support & no_bear & (~vc_safe) & (vcp | vc3 | today_vr.between(0.01, 1.80))

        target_df = df[target].copy()
        safe_df = df[target & vc_safe].copy()
        watch_df = df[target & vc_watch].copy()
        need_df = df[target & (~vc_safe) & (~vc_watch)].copy()
        core2_df = df[core2].copy()
        core2_safe_df = df[core2 & vc_safe].copy()
        confirm_df = df[a_confirm].copy()
        confirm_safe_df = df[a_confirm & vc_safe].copy()
        ...
        lines.append('[🧊 A-CONFIRM VC 태그별 성과검증 — v4.4.9.9]')
        lines.append(_line(target_df, 'A-CONFIRM/CORE 전체'))
        lines.append(_line(safe_df, 'A-CONFIRM/CORE VC-SAFE'))
        lines.append(_line(watch_df, 'A-CONFIRM/CORE VC-WATCH', min_n=5))
        lines.append(_line(need_df, 'A-CONFIRM/CORE VC-확인필요', min_n=5))
        lines.append(_line(core2_df, 'A-RETEST CORE② 전체'))
        lines.append(_line(core2_safe_df, 'A-RETEST CORE② VC-SAFE', min_n=5))
        lines.append(_line(confirm_df, 'A-SAFE 다음날확인 전체'))
        lines.append(_line(confirm_safe_df, 'A-SAFE 다음날확인 VC-SAFE', min_n=5))
        lines.append(f'- VC 판정 기준: 저장된 a_vc_safe/a_vc_watch 컬럼 우선, 없으면 a_support_hold·a_amount_hold·a_no_bear_expand·거래량수축 프록시 사용. 다음날확인 기준: {next_note}')
        lines.append('- 해석: VC-SAFE가 전체 대비 승률 상승 또는 손절 하락을 보일 때만 하드필터 승격을 검토합니다. 표본이 부족하면 실시간에서는 해석 보조 태그로만 사용합니다.')
    except Exception as e:
        lines.append(f'[A-CONFIRM VC 성과검증 오류] {type(e).__name__}: {e}')
    return lines
...
def _v439_build_compact_backtest_report(original_report: str, selected_csv: str = '', raw_csv: str = '', start_date: str = '', end_date: str = '', hold_days: int = 0, top_per_strategy: int = 5, all_candidates: bool = False) -> str:
    ...
    try:
        ...
        sel_text = '전체후보' if all_candidates else f'날짜별 전략별 TOP{top_per_strategy}'
        lines = []
        lines.append("🧪 v4.4.9.9 A-CONFIRM VC PERFORMANCE CHECK")
        lines.append(f"버전: {CLOSING_BET_SCANNER_VERSION}")
        lines.append(f"기간: {start_date} ~ {end_date} | 보유평가: 다음 {hold_days}거래일")
        ...
        lines.append("- 신규 검색식 LP/SLOCK/IT는 위 성과검증 섹션과 실시간 후보 출력 섹션에서 함께 확인합니다. v4.4.9.9는 패턴별 멘탈 요약/후보별 멘탈기준을 유지하고, A-CONFIRM에 거래량 수축/가격지지 VC-SAFE·VC-WATCH 태그를 추가해 A 확인형의 품질을 더 명확히 표시합니다. LP-D23 과최적화 검증과 LP 없는 날 대체 시나리오를 같이 봅니다. SLOCK은 실시간 기본 숨김, IT는 ACCEL 중심으로 봅니다.")
        compact = "\n".join([str(x) for x in lines if x is not None])
        try:
            out_path = LOG_DIR / 'closing_bet_backtest_summary_v4_4_9_9_a_confirm_vc_performance_check.txt'
        ...
        return (str(original_report or '') + f"\n\n[v4.4.9.9 압축 요약 생성 실패] {type(e).__name__}: {e}")
