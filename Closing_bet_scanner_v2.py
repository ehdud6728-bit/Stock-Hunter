...
CLOSING_BET_SCANNER_VERSION = 'G_MORALES_V4_4_9_25_H_FAST_H_CORE_TIMING_AUDIT_20260528'
...
def _send_results(hits: list, mins_left: int):
    ...
    header = (
        ...
        f"전체 후보 {len(hits)}개 | v4.4.9.25 H-FAST/H-CORE TIMING AUDIT: 14:40 후보발굴 + 15:03 실행/포기 압축 최종킥\n"
    ...
    def _build_practical_block():
        ...
        try:
            if CLOSING_BET_LIVE_OPERATION_GUARD and CLOSING_BET_LIVE_TRACKING_LOG:
                ...
                _add_live_pool(s_reclaim_exec_pool + s_momentum_prime_pool + s_momentum_3000_pool + s_momentum_watch_pool + neutral_pool, 'S-RECLAIM/MOMENTUM/NEUTRAL', 5, PRACTICAL_SAFE_TOP_N)
        ...
        def _final_kick_decision(h: dict, pattern_label: str) -> tuple[str, str, list]:
            ...
            if mode == 'H':
                if _is_h_fast_hit(h):
                    return '🟡 지정가만 가능', 'H-FAST 8배+는 보유형이 아니라 +3 빠른익절 전용. 시초/종가 추격 금지', reasons
        ...
        def _final_kick_execution_style(h: dict, decision: str) -> str:
            ...
            if mode == 'S' and _is_s_core_safe_hit(h):
                ...
                if skey == 'S_MOMENTUM_PRIME':
                    return '⚡ +3 단타/다음날 확인: ST30 미통과지만 5000억+ 추세지속형'
                if skey in ('S_MOMENTUM_3000_5000', 'S_MOMENTUM_WATCH', 'S_MOMENTUM_CALM'):
            ...
            if mode == 'H':
                if _is_h_fast_hit(h):
                    return '🔥 +3 초단타 전용: H-FAST 8배+, 보유/추격 금지'
        ...
        def _build_final_kick_block() -> str:
            ...
            # v4.4.9.25: 최종킥 우선순위는 실전 운용순서 그대로 둔다.
            # LP → L → S-RECLAIM/S-MOMENTUM/S-NEUTRAL → A-CONFIRM → G → A-CORE → H-FAST(+3전용) → IT/I.
            ...
            _push(s_reclaim_exec_pool + s_momentum_prime_pool + s_momentum_3000_pool + s_momentum_watch_pool + neutral_pool, 'S-RECLAIM/MOMENTUM/NEUTRAL', PRACTICAL_SAFE_TOP_N)
            ...
            _push(h_fast_pool, 'H-FAST +3전용', PRACTICAL_H_FAST_TOP_N)
            ...
            s_label_line = (
                ...
                f"🔥MOM5000 {len(s_momentum_prime_pool)} | 🟡MOM3000~5000 {len(s_momentum_3000_pool)} | "
                f"🟡MOM-WATCH {len(s_momentum_watch_pool)} | ⚠️RISK {len(s_liquidity_risk_pool)}"
            )
            core_line = (
                f"핵심후보: LP {len(lp_safe_pool)} | L {len(l_safe_pool) + len(l_mega_pool) + len(l_tail_pool)} | "
                f"S실전 {len(s_reclaim_exec_pool) + len(s_momentum_prime_pool) + len(s_momentum_3000_pool) + len(s_momentum_watch_pool)} | "
                f"A확인 {len(a_confirm_pool)} | H-FAST {len(h_fast_pool)} | IT/I {len(it_accel_live_pool) + len(i_accel_pool)}"
            )
            lines = [
                '🚨 [동시호가 최종킥 — v4.4.9.25 COMPACT]',
        ...
        block = ['[🎯 실전 운영 후보 — v4.4.9.25 H-FAST/H-CORE TIMING AUDIT]']
        summary = (
            f"요약: 🥇LP-SAFE {len(lp_safe_pool)}개 | 🔁LP-WATCH {len(lp_watch_pool)}개 | ⚠️LP-RISK {len(lp_risk_pool)}개 | 👑L-SAFE {len(l_safe_pool)}개 | 🔒SLOCK검증/숨김 {len(slock_pool)}개 | ⚡IT-ACCEL {len(it_accel_live_pool)}개 | 🌊L-TAIL {len(l_tail_pool)}개 | 💰L-MEGA {len(l_mega_pool)}개 | 💎S-RE-MEGA {len(s_reclaim_mega_pool)}개 | 🟢S-RE3000 {len(s_reclaim_3000_pool)}개 | 🟢S-RE-PRIME {len(s_reclaim_prime_pool)}개 | 🟡S-RE-WATCH {len(s_reclaim_watch_pool)}개 | 🔥S-MOMENTUM {len(s_momentum_prime_pool)}개 | 🟡S-3000~5000 {len(s_momentum_3000_pool)}개 | 🟡S-WATCH {len(s_momentum_watch_pool)}개 | ⚠️S-유동성주의 {len(s_liquidity_risk_pool)}개 | 🟡S관찰 {len(neutral_pool)}개 | 🟢G-SAFE {len(g_safe_pool)}개 | "
        ...
        block.append('[🚦 v4.4.9.25 실시간 운용판정]\nFAST 실전 출력은 LP-D23/D45, L 5000억+, A-CONFIRM PRIME/CALM, S-RECLAIM/S-MOMENTUM/S-NEUTRAL, A-RETEST CORE②, IT/I-MAIN ACCEL만 우선합니다. S-SAFE는 ST30 통과형도 거래대금별 S-RECLAIM MEGA/3000~5000/PRIME/WATCH/RISK로 나누고, ST30 미통과형은 S-MOMENTUM/RISK로 분리합니다. 넓은 A/C/B/H/SLOCK은 기본 숨김 또는 연구/관찰입니다.\n직장인 모드: 장중 5분봉 확인보다 지정가·추격금지·+3/+5 자동익절·무효가를 먼저 봅니다. 미체결은 포기하는 방식으로 운용합니다.\nA-RETEST CORE②는 당일 보조 후보, A-CONFIRM은 다음날 전일고가 회복·양봉·거래대금 유지 확인형입니다. PRIME은 힘이 남은 우선 확인형, CALM은 안정형 보조 후보입니다.\n시장/섹터 압력이 부담이면 같은 후보라도 소액·확인 후 진입으로 낮추고, 지지선 종가이탈+거래량 증가 음봉은 정상 흔들림이 아니라 위험신호로 봅니다.')
        block.append('')
        ...
        if safe_pool:
            ...
            block.append('v4.4.9.25 기준: S-SAFE는 ST30 통과형을 S-RECLAIM MEGA(5000억+)·3000~5000·PRIME(1000억+)·WATCH(300~1000)·LIQUIDITY RISK(<300)로 나누고, ST30 미통과형은 S-MOMENTUM/S-LIQUIDITY로 분리합니다. ST30은 하드필터가 아닙니다.')
...
# =============================================================
# v4.4.9.25 H-FAST/H-CORE TIMING AUDIT — 신고가거자름 핵심셀 승격검증
# =============================================================
def _v44924_h_core_promotion_audit_lines(df: pd.DataFrame, compact: bool = True) -> list[str]:
    ...
    try:
        ...
        if h_core.empty and int(h_fast_m.sum()) <= 0:
            lines.append('[🧪 H-CORE/H-FAST 타이밍 감사 — v4.4.9.25]')
        ...
        # v4.4.9.25: H-FAST 8배+는 H-CORE와 성격이 다르므로 별도 타이밍 감사로 분리한다.
        ...
        if _v439_bool_env('CLOSING_BET_H_FAST_TIMING_AUDIT', '1'):
            ...
            lines.append('')
            lines.append('[🔥 H-FAST 8배+ 타이밍 감사 — v4.4.9.25]')
            ...
            if not h_fast.empty:
                ...
                detail_n_fast = int(float(os.environ.get('CLOSING_BET_H_FAST_AUDIT_DETAIL_N', os.environ.get('CLOSING_BET_H_CORE_AUDIT_DETAIL_N', '5' if compact else '20'))))
                if detail_n_fast > 0:
        ...
        detail_n = int(float(os.environ.get('CLOSING_BET_H_CORE_AUDIT_DETAIL_N', '6' if compact else '20')))
...
def _v449_fast_live_policy_lines() -> list[str]:
    return [
        ...
        '- H-CORE는 v4.4.9.25에서 타이밍 검증 중입니다. 통과 전까지는 H-CORE WATCH/지정가/다음날확인 후보로만 보고, H-FAST 8배+는 +3 초단타 전용으로만 별도 추적합니다.',
...
def _v439_build_compact_backtest_report(original_report: str, selected_csv: str = '', raw_csv: str = '', start_date: str = '', end_date: str = '', hold_days: int = 0, top_per_strategy: int = 5, all_candidates: bool = False) -> str:
    ...
    try:
        ...
        sel_text = '전체후보' if all_candidates else f'날짜별 전략별 TOP{top_per_strategy}'
        lines = []
        lines.append("🧪 v4.4.9.25 H-FAST/H-CORE TIMING AUDIT")
        lines.append(f"버전: {CLOSING_BET_SCANNER_VERSION}")
        lines.append(f"기간: {start_date} ~ {end_date} | 보유평가: 다음 {hold_days}거래일")
        ...
        lines.append("- 신규 검색식 LP/SLOCK/IT는 위 성과검증 섹션과 실시간 후보 출력 섹션에서 함께 확인합니다. v4.4.9.25는 ST30-RECLAIM을 하드필터가 아닌 가점/주의 태그로 유지하고, 실시간 S-SAFE 후보를 S-RECLAIM MEGA·S-RECLAIM 3000~5000·S-RECLAIM PRIME/WATCH/RISK·S-MOMENTUM PRIME/WATCH·S-LIQUIDITY RISK로 분리 표시합니다. FAST 실전 출력 후보만 우선 표시하며, A-CONFIRM PRIME/CALM·시장/섹터압력·실전 추적로그를 유지합니다. H-CORE는 WATCH로 유지하고 H-FAST는 +3 초단기 전용 후보로 분리검토하며, 넓은 A/C/B/SLOCK은 기본 숨김/연구로 유지합니다.")
        compact = "\n".join([str(x) for x in lines if x is not None])
        try:
            out_path = LOG_DIR / 'closing_bet_backtest_summary_v4_4_9_25_h_fast_h_core_timing_audit.txt'
