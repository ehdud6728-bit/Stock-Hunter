...
CLOSING_BET_SCANNER_VERSION = 'G_MORALES_V4_4_9_28_SCHEDULE_EMPTY_ENV_SAFE_FIX_20260529'
...
def _send_results(hits: list, mins_left: int):
    ...
    header = (
        ...
        f"전체 후보 {len(hits)}개 | v4.4.9.28 SCHEDULE EMPTY ENV SAFE FIX: 14:40 후보발굴 + 15:03 실행/포기 압축 최종킥\n"
    ...
    def _build_practical_block():
        ...
        def _build_final_kick_block() -> str:
            ...
            # v4.4.9.28: 최종킥 우선순위는 실전 운용순서 그대로 둔다.
            ...
            def _final_kick_pick_sort_key(row):
                # v4.4.9.28: "오늘 한 종목"은 후보 수집순서/거래대금순서가 아니라 실제 실행판정 1등을 고른다.
        ...
        block = ['[🎯 실전 운영 후보 — v4.4.9.28 SCHEDULE EMPTY ENV SAFE FIX]']
        summary = (
        ...
        if safe_pool:
            ...
            block.append('v4.4.9.28 기준: S-SAFE는 ST30 통과형을 S-RECLAIM MEGA(5000억+)·3000~5000·PRIME(1000억+)·WATCH(300~1000)·LIQUIDITY RISK(<300)로 나누고, ST30 미통과형은 S-MOMENTUM/S-LIQUIDITY로 분리합니다. ST30은 하드필터가 아닙니다.')
...
# =============================================================
# v4.4.9.28 SCHEDULE EMPTY ENV SAFE FIX — 신고가거자름 핵심셀 승격검증
# =============================================================
def _v44924_h_core_promotion_audit_lines(df: pd.DataFrame, compact: bool = True) -> list[str]:
    ...
    try:
        ...
        if h_core.empty and int(h_fast_m.sum()) <= 0:
            lines.append('[🧪 H-CORE/H-FAST 타이밍 감사 — v4.4.9.28]')
...
def _v449_fast_live_policy_lines() -> list[str]:
    return [
        ...
        '- H-CORE는 v4.4.9.28에서는 H-CORE를 WATCH로 유지하고, H-FAST는 PRIME 조건을 만족한 경우에만 +3 초단타 전용 후보로 표시합니다.',
...
def _v439_build_compact_backtest_report(original_report: str, selected_csv: str = '', raw_csv: str = '', start_date: str = '', end_date: str = '', hold_days: int = 0, top_per_strategy: int = 5, all_candidates: bool = False) -> str:
    ...
    try:
        ...
        sel_text = '전체후보' if all_candidates else f'날짜별 전략별 TOP{top_per_strategy}'
        lines = []
        lines.append("🧪 v4.4.9.28 SCHEDULE EMPTY ENV SAFE FIX")
        lines.append(f"버전: {CLOSING_BET_SCANNER_VERSION}")
        lines.append(f"기간: {start_date} ~ {end_date} | 보유평가: 다음 {hold_days}거래일")
        ...
        lines.append("- 신규 검색식 LP/SLOCK/IT는 위 성과검증 섹션과 실시간 후보 출력 섹션에서 함께 확인합니다. v4.4.9.28은 ST30-RECLAIM을 하드필터가 아닌 가점/주의 태그로 유지하고, 실시간 S-SAFE 후보를 S-RECLAIM MEGA·S-RECLAIM 3000~5000·S-RECLAIM PRIME/WATCH/RISK·S-MOMENTUM PRIME/WATCH·S-LIQUIDITY RISK로 분리 표시합니다. FAST 실전 출력 후보만 우선 표시하며, A-CONFIRM PRIME/CALM·시장/섹터압력·실전 추적로그를 유지합니다. H-CORE는 WATCH로 유지하고 H-FAST는 PRIME만 +3 초단기 전용 후보로 표시하며, 넓은 A/C/B/SLOCK은 기본 숨김/연구로 유지합니다.")
        compact = "\n".join([str(x) for x in lines if x is not None])
        try:
            out_path = LOG_DIR / 'closing_bet_backtest_summary_v4_4_9_28_one_pick_decision_align_fix.txt'
