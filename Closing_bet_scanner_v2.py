...
def _evaluate_high_reaccum_signal(df: pd.DataFrame) -> dict:
    ...
    그날 종가가 고점권에서 잠기는지까지 확인하므로 종가배팅용 필터로 사용한다.
    """
    try:
        ...
        vma50 = _safe_float(row.get('VMA50', df['Volume'].tail(50).mean()), 0.0)
        ...
        # 필수 게이트: 강한 상승 이력 + 고점권 위치 + 추세 생존 + 종가 고점 마감 + 클라이맥스 제외
        if not strong_runup:
            fail = 'runup'
        ...
        if high_close:
            score += 20; passed.append(f'⑧종가고점마감{close_loc_pct:.0f}%')
        ...
        if stop_candidates:
            ...
            stoploss = close * 0.90
        # 너무 가까운 손절은 흔들림에 잘릴 수 있어 최소 3%는 여유를 둔다.
    ...
    try:
        ...
        return {
            **info,
            ...
            'volatility_type': band_rec['volatility_type'],
            'universe_tag': band_rec['universe_tag'],
            'band_comment': f"고점권 재응축 2차 슈팅 후보 | {sig.get('high_close_rule','')}",
            'band_recommend_reason': band_rec['reason'],
            'kki_score': kki_rec['kki_score'],
            'absorb_score': kki_rec['absorb_score'],
            'kki_pattern': kki_rec['kki_pattern'],
...
def _send_results(hits: list, mins_left: int):
    ...
    header = (
        ...
        f"모랄레스갭(G) {min(len(hits_g), 5)}개 | 고점재응축(S) {min(len(hits_s), 5)}개 | 돌파형(A) {min(len(hits_a), 5)}개 | "
...
def run_closing_bet_scan(force: bool = False) -> list:
    ...
    log_info(f"\n종가배팅 후보: {len(hits)}개")
    log_info(
        f"모랄레스갭(G): {len(hits_g)}개 | 고점재응축(S): {len(hits_s)}개 | 돌파형(A): {len(hits_a)}개 | "
