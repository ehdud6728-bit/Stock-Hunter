...
CLOSING_BET_SCANNER_VERSION = 'G_MORALES_V4_4_5_STOP_PATH_PROFILE_20260519'
...
# ── 종가배팅 전용 텔레그램 설정
# v4.4.5: 기본방 오발송 방지 유지 + 출력 문구/파일저장 안내 정리.
...
# v4.4.5: SLOCK은 백테스트 검증에는 남기되 실시간 후보에서는 기본 숨김 처리한다.
...
def _classify_lp_candidate(h: dict) -> dict:
    """v4.4.5: LP 리더갭 눌림재지지 후보를 SAFE/WATCH/RISK로 분류한다.
...
def _lp_timing_bucket(h: dict) -> tuple[str, str]:
    """v4.4.5: LP 타점 시차를 D1 / D23 / D45로 구분한다."""
    try:
...
def _lp_next_day_scenario(h: dict) -> str:
    """v4.4.5: LP-SAFE/LP-WATCH 다음날 대응 시나리오를 텔레그램에 직접 출력한다."""
...
def _send_results(hits: list, mins_left: int):
    ...
    header = (
        ...
        f"전체 후보 {len(hits)}개 | v4.4.1 기존 검색식 유지 + 신규검증 LP/SLOCK/IT 추가 + 전용방 강제전송\n"
    ...
    def _gap_support_levels(h: dict, mode: str = '') -> dict:
        """v4.4.5: L/LP/G 갭형 후보의 실전 지지선을 따로 표시한다.
    ...
    def _pattern_entry_tip(h: dict, mode: str = '') -> str:
        ...
        if mode == 'LP':
            ...
            return "추천타점: 갭후 1~5일 눌림 중 갭하단/전일종가/5·10일선 재지지 확인."
        if mode == 'L':
            return "추천타점: 당일 추격보다 다음날 갭하단/전일종가 지지 확인 후, 강하면 5분봉 첫 눌림."
        if mode == 'S':
            if str(h.get('s_type','')) == 'S2':
                return "추천타점: 고점권 재응축 후 거래량 재점화. +3% 추격보다 눌림·전고점 지지 확인."
            return "추천타점: 거래량 마른 응축이 유지될 때 20~30% 소액, 2~5일 횡보 감안."
        if mode == 'G':
            return "추천타점: 갭하단/전일고가 지지 확인 후만. 시초 급등 추격 금지."
        if mode == 'IT':
            return "추천타점: 단기 진입이 아니라 I-MAIN 1차 분할매집. 5MA/20MA 재지지 때 추가."
        if mode == 'I':
            return "추천타점: 20/40/60일 중기 관찰. 장기선 위 재지지·5MA 회복 때 분할."
        if mode == 'H':
            return "추천타점: 돌파 후 거래량 마른 눌림봉. 5일선 재지지 확인 후만."
        if mode == 'A':
            return "추천타점: 즉시매수보다 다음날 전일고가 회복+거래대금 유지 확인 후 승격."
        if mode == 'C':
            return "추천타점: 장기선 돌파 후 눌림재지지. 당일 추격보다 재지지 확인."
        return "추천타점: 추격보다 핵심 지지선 재확인 후 소액/분할."

    def _pattern_path_text(h: dict, mode: str = '') -> str:
        """v4.4.5: 패턴별 타점 이후 예상흐름·멘탈기준."""
        mode = str(mode or _pick_strategy(h) or '').upper()
        if mode == 'LP':
            b, _ = _lp_timing_bucket(h)
            if b == 'LP-D1':
                return "예상흐름: 빠른 눌림후상승형. 정상흔들림 -1~-3%, 갭하단/전일종가 유지가 멘탈 기준."
            if b == 'LP-D23':
                return "예상흐름: 핵심 눌림후상승형. 2~3일 식힌 뒤 +3/+5 반등 시도가 가장 자연스러운 흐름."
            if b == 'LP-D45':
                return "예상흐름: 충분한 식힘 후 재상승형. 늦을 수 있지만 안 깨지면 안정성이 높음."
            return "예상흐름: 리더갭 후 1~5일 식힘→재지지→+3/+5 반등 시도. 위험은 갭하단 종가이탈."
        if mode == 'L':
            return "예상흐름: 강한 갭상승 후 변동성 확대형. 다음날 갭하단 테스트가 정상일 수 있음."
        if mode == 'S':
            return "예상흐름: 응축 후 짧은 분출 또는 2~5일 횡보. 거래량 증가 음봉이면 실패위험."
        if mode == 'G':
            return "예상흐름: 갭지지 후 단기 반등형. 갭하단 이탈 시 바로 약해지는 패턴."
        if mode == 'IT':
            return "예상흐름: 중기 촉발형. 단기 흔들림보다 20/40/60일 구조 유지가 핵심."
        if mode == 'I':
            return "예상흐름: 중기 시세분출형. -3~-5% 흔들림도 구조가 살아 있으면 정상 범위일 수 있음."
        if mode == 'A':
            return "예상흐름: 돌파 확인 전 흔들림이 잦음. 전일고가 재회복 전까지 관찰 우선."
        if mode == 'C':
            return "예상흐름: 장기선 돌파 후 횡보/눌림이 길 수 있음. 재지지 확인이 핵심."
        return "예상흐름: 패턴별 지지선 유지 여부를 먼저 확인. 거래량 증가 장대음봉은 위험."
    ...
    def _build_practical_block():
        ...
        # v4.4.5 신규검증 검색식 — 기존 L/S/I 검색식은 그대로 두고 별도 섹션에만 표시한다.
        ...
        block = ['[🎯 실전 운영 후보 — v4.4.5 STOP + PATH PROFILE]']
        summary = (
        ...
        block.append('[🚦 v4.4.5 실시간 운용판정]\n기존 검색식은 훼손하지 않고 LP 운용만 세분화했습니다. 단기 실행 우선순위: LP-SAFE > L 5000억+ 리더갭 > S2/S-NEUTRAL > A/C 5000억+ 관찰. LP는 D1/D23/D45 타점을 구분하고 다음날 대응 시나리오를 함께 표시합니다.\n중기 관찰 우선순위: I-MAIN ACCEL > IT-ACCEL 촉발형 > I-MAIN CORE. IT-CORE는 보조관찰, SLOCK은 실시간 기본 숨김/성과검증만 유지합니다. H/B는 검증·관찰만, H-RISK/과열 H는 제외합니다.\n파일저장/CSV 확인 없이 이 텔레그램 블록만으로 실시간 후보와 운용판정을 확인할 수 있게 구성했습니다.')

        if PRACTICAL_SHOW_NEW_PATTERNS and lp_safe_pool:
            block.append('')
            block.append('[🥇 LP-SAFE — 리더갭 눌림재지지 단기 최우선]')
            block.append('v4.4.5 운용 반영: LP는 1~5일 내 5000억+ 리더갭 이후 갭하단/전일종가/5·10일선 재지지 후보입니다. D1/D23/D45 타점을 구분하고, 다음날 시초가별 대응 시나리오를 함께 봅니다.')
            shown = 0
            used_codes = set()
        ...
        if l_core_pool:
            ...
            block.append('v4.4.5 기준: L 당일 후보는 단기 2순위입니다. 5000억+·종가70%+는 우대하지만, 가능하면 LP 눌림재지지 타점으로 재진입하는 쪽을 더 우선합니다.')
            used_codes = set()
            shown = 0
        ...
        if safe_pool:
            ...
            block.append('v4.4.1 기준: S는 S2 실행형/S-NEUTRAL을 우선하고, SAFE는 종가위치·RR·거래대금 조건이 맞을 때만 소액 확인합니다.')
        ...
        if PRACTICAL_SHOW_NEW_PATTERNS and it_accel_live_pool:
            ...
            block.append('v4.4.5 기준: IT는 ACCEL만 실시간 촉발 후보로 표시합니다. 단기 종가배팅이 아니라 20/40/60일 중기 1차 분할매집 타이밍입니다. IT-CORE는 보조관찰입니다.')
            used_codes = set()
            shown = 0
        ...
        if PRACTICAL_SHOW_I_MAIN and (i_accel_pool or i_core_pool or i_watch_pool or i_add_pool or i_confirm_pool):
            ...
            block.append('v4.4.1 기준: I-MAIN은 단기 종가배팅이 아니라 20/40/60거래일 중기 누적관찰입니다. ACCEL은 고수익 시세분출형, CORE는 안정형 누적관찰로 봅니다.')
            used_codes = set()
            shown_total = 0
            groups = [
        ...
        if neutral_pool:
            ...
            block.append('v4.4.1 기준: S2 실행형/S-NEUTRAL은 단기 2순위입니다. 단, 고점권 전략이므로 KOSPI200·5조+·거래대금 3000억+·종가위치70%+를 우대합니다.')
...
def _bt_check_leader_gap_pullback_on_hist(code: str, name: str, hist: pd.DataFrame, row: pd.Series, info: dict, idx_label: str, marcap: float) -> dict | None:
    """v4.4.5 백테스트용 LP — L-PULLBACK 리더갭 눌림재지지형.
...
def _check_backtest_strategies_on_df(code: str, name: str, hist: pd.DataFrame) -> list:
    ...
    try:
        ...
        # v4.4.5: 기존 I 후보는 그대로 두고, IT(I-TRIGGER) 신규검색식 후보를 별도 mode로 추가해 텔레그램에서 성과검증한다.
        try:
        ...
        # LP — v4.4.5 신규검색식 백테스트: 기존 L 리더갭 이후 1~5일 눌림재지지
        try:
...
def _evaluate_backtest_hit(hit: dict, df: pd.DataFrame, signal_idx: int, hold_days: int = 5) -> dict:
    ...
    # v4.4.5: 타점 이후 실제 흐름 프로파일(직행/눌림/횡보/개미털기/실패)을 계산한다.
    ...
    if stop_before_3 == 1 or stop_rule == 1:
        path_type = '실패형'
    elif path_first_plus3_day and path_first_plus3_day <= 2 and (pd.isna(path_min_low_ret) or path_min_low_ret > -1.5):
        path_type = '직행형'
    elif path_first_plus3_day and (not pd.isna(path_min_low_ret)) and path_min_low_ret <= -3.0:
        path_type = '개미털기후상승형'
    elif path_first_plus3_day and (not pd.isna(path_min_low_ret)) and path_min_low_ret <= -1.0:
        path_type = '눌림후상승형'
...
def _v439_i_samples_section(df: pd.DataFrame, top_n: int = 5) -> list[str]:
    ...
    try:
        ...
        if i.empty:
            return lines
        lines.append("[📋 I-MAIN 발생 샘플 — 종목별 중복제거 v4.4.5]")
...
def _v445_path_profile_line(sub: pd.DataFrame, label: str, entry_tip: str, normal: str, danger: str) -> str:
    ...
    try:
        ...
        top_path = '확인필요'
    dd = _v439_mean(sub, 'path_min_low_ret') if 'path_min_low_ret' in sub.columns else _v439_mean(sub, 'ret_min_low_hd')
    d3 = _v445_mean_positive_day(sub, 'path_first_plus3_day')
    d5 = _v445_mean_positive_day(sub, 'path_first_plus5_day')
    shake = 0.0
    try:
        shake = ((path.eq('개미털기후상승형')) | (pd.to_numeric(sub.get('path_min_low_ret'), errors='coerce').fillna(0).le(-3) & pd.to_numeric(sub.get('hit3_before_stop'), errors='coerce').fillna(0).eq(1))).mean() * 100.0
    except Exception:
        pass
    stop = _v439_rate(sub, 'stop_before_3') if 'stop_before_3' in sub.columns else _v439_rate(sub, 'rule35_stop')
    ...
    return (
        f"- {label}: 대표흐름 {top_path} | 평균첫눌림 {dd:.2f}% | +3평균 {d3_txt} | +5평균 {d5_txt} | "
        f"개미털기 {shake:.1f}% | 손절선행 {stop:.1f}%\n"
        f"  ↳ 추천타점: {entry_tip}\n"
        f"  ↳ 정상흔들림: {normal} / 위험신호: {danger}"
    )


def _v445_pattern_path_profile_lines(df: pd.DataFrame) -> list[str]:
    """v4.4.5: 패턴별 타점 이후 흐름을 텔레그램 백테스트 요약에 압축 표시."""
    lines = []
    try:
        if df is None or df.empty or 'mode' not in df.columns:
            return lines
        mode = _v439_str_series(df, 'mode')
        lines.append('[🧭 패턴별 타점 이후 흐름 프로파일 — 멘탈관리용]')
        ...
        if not lp.empty:
            ...
            lines.append(_v445_path_profile_line(lp_safe, 'LP-SAFE 전체', '갭후 2~5일 식힘 후 갭하단/전일종가 재지지', '-1~-3% 흔들림은 정상 가능', '갭하단 종가이탈+거래량 증가 음봉'))
            if not d1.empty:
                lines.append(_v445_path_profile_line(d1, 'LP-D1 공격타점', '갭후 1일 빠른 재지지, 소액만', '-1~-3% 장중 흔들림 가능', '갭하단 이탈 시 빠르게 제외'))
            if not d23.empty:
                lines.append(_v445_path_profile_line(d23, 'LP-D23 핵심타점', '갭후 2~3일 식힘 후 재지지', '식힌 뒤 양봉전환/전일고가 회복', '전일종가·5/10일선 동시 이탈'))
            if not d45.empty:
                lines.append(_v445_path_profile_line(d45, 'LP-D45 안정타점', '갭후 4~5일 충분히 식힌 뒤 재상승', '조용한 횡보·거래량 감소는 정상', '식혔는데도 갭하단 이탈하면 실패'))
        ...
        lines.append('- 해석: 흐름 프로파일은 “빠지면 무조건 실패”가 아니라, 패턴별 정상 흔들림과 진짜 위험신호를 나눠 보기 위한 멘탈 기준입니다.')
    except Exception as e:
...
def _v442_new_pattern_performance_lines(df: pd.DataFrame) -> list[str]:
    """v4.4.5: 신규 검색식 LP/SLOCK/IT 성과를 텔레그램 요약 안에서 바로 확인한다.
    ...
    try:
        ...
        lines.append('- 해석: v4.4.5부터 LP는 단기 최우선 운용 후보로 보되, D1/D23/D45 타점과 다음날 대응 시나리오를 함께 확인합니다. LP-SAFE는 실행 후보, LP-WATCH는 다음날 양봉·전일고가 회복·거래대금 유지 시 승격, LP-RISK는 제외입니다. SLOCK은 실시간 기본 숨김, IT는 ACCEL 중심으로만 봅니다.')
    except Exception as e:
...
def _v439_build_compact_backtest_report(original_report: str, selected_csv: str = '', raw_csv: str = '', start_date: str = '', end_date: str = '', hold_days: int = 0, top_per_strategy: int = 5, all_candidates: bool = False) -> str:
    ...
    try:
        ...
        sel_text = '전체후보' if all_candidates else f'날짜별 전략별 TOP{top_per_strategy}'
        lines = []
        lines.append("🧪 v4.4.5 STOP + PATH PROFILE SUMMARY")
        lines.append(f"버전: {CLOSING_BET_SCANNER_VERSION}")
        lines.append(f"기간: {start_date} ~ {end_date} | 보유평가: 다음 {hold_days}거래일")
        ...
        lines.append("단기 실행 우선순위: LP-SAFE > L 5000억+ > S2/S-NEUTRAL > A/C 5000억+ 관찰. LP는 D1/D23/D45 타점과 패턴별 흐름 프로파일을 함께 확인합니다. A/C는 즉시매수 아님, 다음날 전일고가 회복+거래대금 유지 시만 승격. 중기 관찰 우선순위: I-ACCEL > IT-ACCEL > I-CORE. H/B/SLOCK은 검증·관찰만.")
        lines.append("")
        ...
        lines.append("- A/C 관찰: 5000억+만 참고. 즉시매수 아님. 다음날 전일고가 회복 + 거래대금 유지 시만 승격.")
        ...
        lines.append("- 신규 검색식 LP/SLOCK/IT는 위 성과검증 섹션과 실시간 후보 출력 섹션에서 함께 확인합니다. v4.4.5는 LP-D1/D23/D45와 다음날 대응 시나리오를 추가했습니다. SLOCK은 실시간 기본 숨김, IT는 ACCEL 중심으로 봅니다.")
        compact = "\n".join([str(x) for x in lines if x is not None])
        try:
            out_path = LOG_DIR / 'closing_bet_backtest_summary_v4_4_5_stop_path_profile.txt'
        ...
        return (str(original_report or '') + f"\n\n[v4.4.5 압축 요약 생성 실패] {type(e).__name__}: {e}")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--compact-summary', action='store_true', help='v4.4.5 압축/실시간 운용 요약을 텔레그램 전송/로그에 사용')
