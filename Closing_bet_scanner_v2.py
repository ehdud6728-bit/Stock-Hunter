...
CLOSING_BET_SCANNER_VERSION = 'G_MORALES_V4_4_9_21_FINAL_KICK_COMPACT_S_LABEL_SUMMARY_20260528'
...
I_CORE_MIN_SCORE = int(float(os.environ.get('I_CORE_MIN_SCORE', '78')))
I_CORE_MIN_AMOUNT_B = float(os.environ.get('I_CORE_MIN_AMOUNT_B', '50'))
...
I_CORE_MAIN_MIN_MATERIAL = int(float(os.environ.get('I_CORE_MAIN_MIN_MATERIAL', '3')))
I_CORE_MAIN_LONG_MIN = float(os.environ.get('I_CORE_MAIN_LONG_MIN', '-3'))
I_CORE_MAIN_LONG_MAX = float(os.environ.get('I_CORE_MAIN_LONG_MAX', '18'))
...
CLOSING_BET_STOCK_FEATURE_MIN_N = int(float(os.environ.get('CLOSING_BET_STOCK_FEATURE_MIN_N', '5')))
...
CLOSING_BET_COMPACT_I_MAIN_SAMPLE_TOP_N = int(float(os.environ.get('CLOSING_BET_COMPACT_I_MAIN_SAMPLE_TOP_N', os.environ.get('CLOSING_BET_BACKTEST_DETAIL_TOP_N', '5'))))
...
PRACTICAL_I_MAIN_CORE_TOP_N = int(os.environ.get('CLOSING_BET_I_MAIN_CORE_TOP_N', '5'))
PRACTICAL_I_MAIN_ACCEL_TOP_N = int(os.environ.get('CLOSING_BET_I_MAIN_ACCEL_TOP_N', '5'))
PRACTICAL_I_MAIN_WATCH_TOP_N = int(os.environ.get('CLOSING_BET_I_MAIN_WATCH_TOP_N', '5'))
PRACTICAL_I_MAIN_ADD_TOP_N = int(os.environ.get('CLOSING_BET_I_MAIN_ADD_TOP_N', '3'))
PRACTICAL_I_MAIN_CONFIRM_TOP_N = int(os.environ.get('CLOSING_BET_I_MAIN_CONFIRM_TOP_N', '3'))
...
PRACTICAL_L_PULLBACK_TOP_N = int(os.environ.get('CLOSING_BET_L_PULLBACK_TOP_N', '3'))
PRACTICAL_S2_LOCK_TOP_N = int(os.environ.get('CLOSING_BET_S2_LOCK_TOP_N', '3'))
...
PRACTICAL_I_TRIGGER_TOP_N = int(os.environ.get('CLOSING_BET_I_TRIGGER_TOP_N', '3'))
...
CLOSING_BET_TELEGRAM_MAX_LEN = int(float(os.environ.get('CLOSING_BET_TELEGRAM_MAX_LEN', '3400')))

# v4.4.9.21: 2차 스캔 전용 FINAL KICK COMPACT MODE
...
CLOSING_BET_MAX_FINAL_KICK_CARDS = int(float(os.environ.get('CLOSING_BET_MAX_FINAL_KICK_CARDS', '3')))
# v4.4.9.21: 최종킥은 실행/포기만 빠르게 보는 압축 모드가 기본값이다.
...
ST30_OVERSOLD_LEVEL = float(os.environ.get('CLOSING_BET_ST30_OVERSOLD_LEVEL', '30'))
ST30_OVERSOLD_LOOKBACK = int(float(os.environ.get('CLOSING_BET_ST30_OVERSOLD_LOOKBACK', '7')))
...
CLOSING_BET_LIVE_PROMOTION_MIN_N = int(float(os.environ.get('CLOSING_BET_LIVE_PROMOTION_MIN_N', '10')))
BACKTEST_DETAIL_TOP_N = int(os.environ.get('CLOSING_BET_BACKTEST_DETAIL_TOP_N', '8'))
BACKTEST_DETAIL_MAX_ROWS = int(os.environ.get('CLOSING_BET_BACKTEST_DETAIL_MAX_ROWS', '20000'))
...
def _send_results(hits: list, mins_left: int):
    ...
    header = (
        ...
        f"전체 후보 {len(hits)}개 | v4.4.9.21 FINAL KICK COMPACT: 14:40 후보발굴 + 15:03 실행/포기 압축 최종킥\n"
    ...
    # v4.4.9.21: S-SAFE live labels. ST30은 하드필터가 아니라 가점/강등 태그이고,
    ...
    def _s_safe_live_class(h: dict) -> dict:
        ...
        if st30_pass:
            return {
                ...
                'reason': 'ST30 조정후 재상승 확인. S-SAFE 중 손절선행 0%로 검증된 안정형 가점 후보',
        ...
        if amount_b < 300:
            return {
                ...
                'reason': 'ST30 미통과 + 거래대금 300억 미만. 반복검증에서 손절선행 100%로 나온 저유동성 위험구간',
        ...
        if amount_b < 1000:
            return {
                ...
                'reason': 'ST30 미통과 + 거래대금 1000억 미만. 종가위치/RR이 좋아도 실패군이 많았던 구간',
                'guide': '저유동성 S-SAFE입니다. 바로 사기보다 다음날 양봉·전일고가 회복·거래대금 재증가 확인 전까지 관망 우선입니다.',
            }
        if amount_b >= 5000 and quality:
            return {
                'key': 'S_MOMENTUM_PRIME',
                'label': '🔥S-MOMENTUM PRIME',
                'tag': 'S-MOMENTUM PRIME 고유동성 추세지속',
                'reason': 'ST30 미통과지만 거래대금 5000억+ 고유동성. 미통과 성공군에서 강한 추세지속 사례가 확인된 구간',
                'guide': 'ST30 조정형은 아니지만 고유동성 추세지속형입니다. LP/L/A-CONFIRM이 없을 때 보조 후보로만 보고, 추격보다 지정가·다음날 지지 확인을 우선합니다.',
            }
        if 3000 <= amount_b < 5000 and quality:
            return {
                'key': 'S_MOMENTUM_3000_5000',
                'label': '🟡S-MOMENTUM 3000~5000',
                'tag': 'S-MOMENTUM WATCH 3000~5000억',
                'reason': 'ST30 미통과 + 거래대금 3000~5000억 구간. 아직 표본이 부족해 5000억+와 분리 추적하는 준대형 수급 후보',
                'guide': '3000~5000억 준대형 수급형입니다. 성과판정은 보류지만 빼면 안 되는 구간이므로 소액 지정가 또는 다음날 확인형으로 추적합니다.',
            }
        if amount_b >= 1000 and quality:
            return {
                'key': 'S_MOMENTUM_WATCH',
                'label': '🟡S-MOMENTUM WATCH',
                'tag': 'S-MOMENTUM WATCH 추세지속',
                'reason': 'ST30 미통과지만 거래대금 1000억+·종가75%+·Vol≤1.8·RR정상. 제외 금지, 추세지속형 관찰 후보',
                'guide': 'ST30 미통과지만 1000억+ 유동성은 살아 있습니다. 당일 몰빵보다 소액 지정가/다음날 전일고가·5MA·거래대금 유지 확인이 맞습니다.',
            }
        if amount_b >= 1000 and calm:
            return {
                'key': 'S_MOMENTUM_CALM',
                'label': '🟡S-MOMENTUM CALM',
                'tag': 'S-MOMENTUM CALM 저거래냉각',
                'reason': 'ST30 미통과지만 대금1000억+·Vol≤1.2·RR정상. 조용한 추세지속 가능성',
                'guide': '거래량이 과열되지 않은 CALM형입니다. 다음날 가격지지와 거래대금 유지 확인으로 승격합니다.',
            }
        return {
            'key': 'S_SAFE_CHECK',
            'label': '🟡S-SAFE 확인',
    ...
    def _build_practical_block():
        ...
        # v4.4.9.21: S-SAFE live label pools
        ...
        s_momentum_watch_pool = [h for h in safe_pool if _s_safe_live_key(h) in ('S_MOMENTUM_WATCH', 'S_MOMENTUM_CALM')]
        s_liquidity_risk_pool = [h for h in safe_pool if _s_safe_live_key(h) in ('S_LIQUIDITY_RISK', 'S_LIQUIDITY_EXCLUDE')]
            ...
            if CLOSING_BET_LIVE_OPERATION_GUARD and CLOSING_BET_LIVE_TRACKING_LOG:
                ...
                def _add_live_pool(pool, pattern_label: str, priority: int, limit: int = 5):
                    ...
                    for hh in pool:
                        ...
                        if local >= limit:
                            break
                _add_live_pool([h for h in lp_safe_pool if _lp_timing_bucket(h)[0] in ('LP-D23', 'LP-D45')], 'LP-D23/D45', 1, PRACTICAL_L_PULLBACK_TOP_N)
                _add_live_pool(lp_safe_pool, 'LP-SAFE', 2, PRACTICAL_L_PULLBACK_TOP_N)
                ...
                _add_live_pool(s_reclaim_prime_pool + s_momentum_prime_pool + s_momentum_3000_pool + s_momentum_watch_pool + neutral_pool, 'S-RECLAIM/MOMENTUM/NEUTRAL', 5, PRACTICAL_SAFE_TOP_N)
                ...
                _add_live_pool(it_accel_live_pool + i_accel_pool, 'IT/I-MAIN ACCEL', 8, PRACTICAL_I_MAIN_ACCEL_TOP_N)
        ...
        # v4.4.9.21: 2차 스캔 전용 — 동시호가 직전 실행/포기 압축 최종킥
            ...
            if mode == 'S':
                if _is_s_core_safe_hit(h):
                    ...
                    if skey == 'S_RECLAIM_PRIME' and close_loc >= 70 and volr <= 1.50:
                        return '✅ 종가진입 가능', 'S-RECLAIM PRIME: ST30 통과 안정형. 단, 소액 지정가와 +3 자동익절 우선', reasons
                    if skey in ('S_MOMENTUM_PRIME', 'S_MOMENTUM_3000_5000', 'S_MOMENTUM_WATCH', 'S_MOMENTUM_CALM'):
                        return '🟡 지정가만 가능', f'{sdesc} — ST30 미통과형은 추격보다 지정가/다음날 확인 우선', reasons
        ...
        def _build_final_kick_block() -> str:
            ...
            # v4.4.9.21: 최종킥 우선순위는 실전 운용순서 그대로 둔다.
            # LP → L → S-RECLAIM/S-MOMENTUM/S-NEUTRAL → A-CONFIRM → G → A-CORE → IT/I.
            _push([h for h in lp_safe_pool if _lp_timing_bucket(h)[0] in ('LP-D23', 'LP-D45')], 'LP-D23/D45', PRACTICAL_L_PULLBACK_TOP_N)
            _push(l_safe_pool + l_mega_pool + l_tail_pool, 'L 5000억+', PRACTICAL_L_CORE_TOP_N)
            _push(s_reclaim_prime_pool + s_momentum_prime_pool + s_momentum_3000_pool + s_momentum_watch_pool + neutral_pool, 'S-RECLAIM/MOMENTUM/NEUTRAL', PRACTICAL_SAFE_TOP_N)
            ...
            picked = []
            for hh, label in candidates:
                decision, why, reasons = _final_kick_decision(hh, label)
                decision_counts[decision] = decision_counts.get(decision, 0) + 1
                if decision != '❌ 오늘 포기' and len(picked) < max_cards:
                    picked.append((hh, label, decision, why, reasons))

            op = '금지' if not picked else ('적극' if decision_counts.get('✅ 종가진입 가능', 0) >= 2 else ('보통' if decision_counts.get('✅ 종가진입 가능', 0) >= 1 else '지정가만'))
            top_name = '없음'
            if picked:
                top_code = str(picked[0][0].get('code', '') or '')
                top_name = _clean_stock_name(top_code, str(picked[0][0].get('name', '') or top_code))

            top_line = '없음'
            if picked:
                top_h, top_label, top_decision, _, _ = picked[0]
                top_code = str(top_h.get('code', '') or '')
                top_line = f'{top_name} — {top_decision} | {top_label}'
            s_label_line = (
                f"S라벨: 🟢RECLAIM {len(s_reclaim_prime_pool)} | 🔥MOM5000 {len(s_momentum_prime_pool)} | "
                f"🟡MOM3000~5000 {len(s_momentum_3000_pool)} | 🟡WATCH {len(s_momentum_watch_pool)} | ⚠️RISK {len(s_liquidity_risk_pool)}"
            )
            core_line = (
                f"핵심후보: LP {len(lp_safe_pool)} | L {len(l_safe_pool) + len(l_mega_pool) + len(l_tail_pool)} | "
                f"S실전 {len(s_reclaim_prime_pool) + len(s_momentum_prime_pool) + len(s_momentum_3000_pool) + len(s_momentum_watch_pool)} | "
                f"A확인 {len(a_confirm_pool)} | IT/I {len(it_accel_live_pool) + len(i_accel_pool)}"
            )
            lines = [
                '🚨 [동시호가 최종킥 — v4.4.9.21 COMPACT]',
                f'시간: {TODAY_STR} {_now_kst().strftime("%H:%M")} | FINAL_KICK_ONLY | 카드최대 {max_cards}개',
                f'오늘 운용: {op}',
                f'오늘 한 종목만 한다면: {top_line}',
                core_line,
                s_label_line,
                f"판정: ✅ {decision_counts.get('✅ 종가진입 가능', 0)} | 🟡 {decision_counts.get('🟡 지정가만 가능', 0)} | ❌ {decision_counts.get('❌ 오늘 포기', 0) + len(risk_pool) + len(a_risk_pool) + len(lp_risk_pool)}",
                '',
            ]
            if not picked:
                lines += [
            ...
            lines += [
                '운용규칙: 15시 최종킥은 새 분석이 아니라 실행/포기 판정입니다. 지정가 미체결은 포기하고, 체결 시 +3/+5 자동익절을 먼저 걸어둡니다.',
                '[전체완료]',
            ]
            return '\n'.join(lines).rstrip()
        ...
        block = ['[🎯 실전 운영 후보 — v4.4.9.21 FINAL KICK COMPACT S-LABEL SUMMARY]']
        summary = (
            f"요약: 🥇LP-SAFE {len(lp_safe_pool)}개 | 🔁LP-WATCH {len(lp_watch_pool)}개 | ⚠️LP-RISK {len(lp_risk_pool)}개 | 👑L-SAFE {len(l_safe_pool)}개 | 🔒SLOCK검증/숨김 {len(slock_pool)}개 | ⚡IT-ACCEL {len(it_accel_live_pool)}개 | 🌊L-TAIL {len(l_tail_pool)}개 | 💰L-MEGA {len(l_mega_pool)}개 | 🟢S-RECLAIM {len(s_reclaim_prime_pool)}개 | 🔥S-MOMENTUM {len(s_momentum_prime_pool)}개 | 🟡S-3000~5000 {len(s_momentum_3000_pool)}개 | 🟡S-WATCH {len(s_momentum_watch_pool)}개 | ⚠️S-유동성주의 {len(s_liquidity_risk_pool)}개 | 🟡S관찰 {len(neutral_pool)}개 | 🟢G-SAFE {len(g_safe_pool)}개 | "
        ...
        if PRACTICAL_SHOW_NEW_PATTERNS and lp_safe_pool:
            ...
            block.append('v4.4.8.1 운용 반영: LP는 1~5일 내 5000억+ 리더갭 이후 갭하단/전일종가/5·10일선 재지지 후보입니다. D1/D23/D45 타점을 구분하고, 다음날 시초가별 대응 시나리오를 함께 봅니다.')
            shown = 0
            used_codes = set()
        ...
        if safe_pool:
            ...
            block.append('v4.4.9.21 기준: S-SAFE는 S-RECLAIM PRIME(ST30통과), S-MOMENTUM PRIME(5000억+), S-MOMENTUM 3000~5000, S-MOMENTUM WATCH(1000억+), S-LIQUIDITY RISK(1000억 미만)로 분리합니다. ST30은 하드필터가 아닙니다.')
        ...
        if PRACTICAL_SHOW_NEW_PATTERNS and it_accel_live_pool:
            ...
            block.append('v4.4.9 기준: IT는 ACCEL만 실시간 촉발 후보로 표시합니다. 단기 종가배팅이 아니라 20/40/60일 중기 1차 분할매집 타이밍입니다. IT-CORE는 보조관찰입니다.')
            used_codes = set()
            shown = 0
        ...
        if PRACTICAL_SHOW_I_MAIN and (i_accel_pool or i_core_pool or i_watch_pool or i_add_pool or i_confirm_pool):
            ...
            block.append('v4.4.9 기준: I-MAIN은 단기 종가배팅이 아니라 20/40/60거래일 중기 누적관찰입니다. ACCEL은 고수익 시세분출형, CORE는 안정형 누적관찰로 봅니다.')
            used_codes = set()
            shown_total = 0
            groups = [
...
def _v44916_pattern_cross_audit_lines(df: pd.DataFrame, compact: bool = True) -> list[str]:
    ...
    try:
        ...
        max_scopes = int(float(os.environ.get('CLOSING_BET_ST30_PATTERN_AUDIT_MAX_SCOPES', '10' if compact else '20')))
        min_n = int(float(os.environ.get('CLOSING_BET_ST30_PATTERN_AUDIT_MIN_N', '3')))
        shown = 0
        for label, mask, stat_type in scopes:
            sub = df[mask].copy()
            if len(sub) < min_n:
...
def _v44917_s_safe_st30_drilldown_lines(df: pd.DataFrame, compact: bool = True) -> list[str]:
    ...
    try:
        ...
        # 개별 샘플
        show_pass_n = int(float(os.environ.get('CLOSING_BET_SSAFE_ST30_PASS_DETAIL_N', '8' if compact else '20')))
        show_miss_n = int(float(os.environ.get('CLOSING_BET_SSAFE_ST30_MISSED_DETAIL_N', '8' if compact else '20')))
...
def _v44918_s_safe_miss_success_fail_audit_lines(df: pd.DataFrame, compact: bool = True) -> list[str]:
    ...
    try:
        ...
        show_n = int(float(os.environ.get('CLOSING_BET_SSAFE_MISS_AUDIT_DETAIL_N', '8' if compact else '20')))
        ...
        if not miss_bad.empty:
            ...
            for _, r in tmp.sort_values('_sort', ascending=True).head(show_n).iterrows():
                lines.append('    ' + _fmt_row(r))
        lines.append('- 운용안: S-SAFE+ST30 통과는 S-RECLAIM PRIME, S-SAFE+ST30 미통과라도 강한 대금/종가위치/거래량/RR이면 S-MOMENTUM WATCH, 약한 종가위치·과열거래량·RR과대는 S-FAIL RISK로 분리합니다.')
    except Exception as e:
...
def _v44919_s_safe_liquidity_rule_repeat_lines(df: pd.DataFrame, compact: bool = True) -> list[str]:
    ...
    try:
        ...
        lines.append('[🔁 S-SAFE 거래대금 규칙 반복검증 — v4.4.9.21]')
        lines.append('- 목적: v4.4.9.18에서 S-SAFE ST30 미통과 성공/실패를 가른 핵심 단서가 거래대금/대표성이었는지 반복 확인합니다.')
        lines.append('- 원칙: ST30은 하드필터가 아니며, S-SAFE는 ST30 통과형·고유동성 추세지속형·저유동성 위험형으로 분리합니다.')
        lines.append(f'- 표본: S-SAFE 전체 {len(sub)}건 | ST30 통과 {int(st30_pass.reindex(sub.index).fillna(False).sum())}건 | ST30 미통과 {len(miss)}건 | ST30-WAIT {len(wait)}건 | ST30-NO {len(no)}건')
        ...
        if not miss.empty:
            ...
            lines.append('')
            lines.append('후보규칙 반복검증 — S-MOMENTUM WATCH 후보')
        ...
        lines.append('- 운용안: S-SAFE+ST30 통과=S-RECLAIM PRIME, 미통과+5000억+=S-MOMENTUM PRIME, 미통과+3000~5000억=S-MOMENTUM 3000~5000, 미통과+1000억+=S-MOMENTUM WATCH, 미통과+1000억 미만=S-LIQUIDITY RISK로 분리합니다.')
    except Exception as e:
...
def _v439_i_samples_section(df: pd.DataFrame, top_n: int = 5) -> list[str]:
    ...
    try:
        ...
        if i.empty:
            return lines
        lines.append("[📋 I-MAIN 발생 샘플 — 종목별 중복제거 v4.4.9.13]")
...
def _v447_mental_summary_lines() -> list[str]:
    ...
    return [
        "[🧠 패턴별 멘탈 요약 — 매번 확인]",
        "- LP-D23: 최고 타점. 갭후 2~3일 식힘 뒤 재지지, 흔들림 작고 +3/+5가 빠른 편. 전일종가·5/10일선 동시 이탈 전에는 과민반응 금지.",
...
def _v445_pattern_path_profile_lines(df: pd.DataFrame) -> list[str]:
    ...
    try:
        ...
        lines.append('[🧭 패턴별 타점 이후 흐름 프로파일 — v4.4.9.13 보정판]')
        ...
        lines.append('- 해석: v4.4.9.13은 1~3일 첫흔들림, +3전흔들림, 전체최대흔들림을 분리합니다. 전체최대흔들림은 목표 도달 후 흔들림까지 포함될 수 있으므로 실전 판단은 +3전흔들림과 지지선 종가유지를 우선 봅니다.')
    except Exception as e:
...
def _v4492_close_loc_fallback_diagnostic_lines(df: pd.DataFrame) -> list[str]:
    ...
    try:
        ...
        amount = _v449_base_liquidity_series(df)
        lines.append('[🧩 종가위치 fallback 보정 — v4.4.9.13]')
...
def _v4495_a_confirm_entry_price_test_lines(df: pd.DataFrame, hold_days: int = 60) -> list[str]:
    ...
    try:
        ...
        max_n = int(os.environ.get('CLOSING_BET_A_CONFIRM_ENTRY_MAX', '80') or 80)
        if len(cand) > max_n:
        ...
        lines.append('[🎯 A-SAFE 다음날확인 진입가별 백테스트 — v4.4.9.13]')
...
def _v4499_vc_performance_lines(df: pd.DataFrame) -> list[str]:
    ...
    try:
        ...
        lines.append('[🧊 A-CONFIRM VC 라벨 재보정 — v4.4.9.13]')
...
def _v449_fast_live_policy_lines() -> list[str]:
    return [
        ...
        '- A/C/H/B 튜닝 패턴: 승격 전에는 백테스트·연구 모드에서만 넓게 검증하고, 실시간에서는 숨김 또는 관찰 요약만 유지.',
        '- 승격 조건을 통과한 패턴만 이후 SAFE/CORE 섹션으로 올립니다. 현재는 A-RETEST/C-RECLAIM/H-CORE/B-CONFIRM 모두 검증 단계입니다.',
    ]
...
def _v439_build_compact_backtest_report(original_report: str, selected_csv: str = '', raw_csv: str = '', start_date: str = '', end_date: str = '', hold_days: int = 0, top_per_strategy: int = 5, all_candidates: bool = False) -> str:
    ...
    try:
        ...
        sel_text = '전체후보' if all_candidates else f'날짜별 전략별 TOP{top_per_strategy}'
        lines = []
        lines.append("🧪 v4.4.9.21 FINAL KICK COMPACT S-LABEL SUMMARY")
        lines.append(f"버전: {CLOSING_BET_SCANNER_VERSION}")
        lines.append(f"기간: {start_date} ~ {end_date} | 보유평가: 다음 {hold_days}거래일")
        ...
        if len(l_combo) > 0 and len(l_combo) < int(os.environ.get('CLOSING_BET_COMPACT_SMALL_SAMPLE_N', '10')):
        ...
        if not i_all.empty:
            ...
            lines.append(_v439_i_line(i_accel, '중기 1순위 I-MAIN ACCEL'))
            lines.append(_v439_i_line(i_core, '중기 2순위 I-MAIN CORE'))
        ...
        lines += _v439_stock_feature_quick_lines(df, min_n=int(os.environ.get('CLOSING_BET_STOCK_FEATURE_MIN_N', '5')))
        ...
        lines.append("- 단기 L: 거래대금 5000억+, 갭 3~12%, 종가위치 70%+ 우선. 5000억~1조와 갭 6~12%는 가점.")
        ...
        top_n = int(os.environ.get('CLOSING_BET_COMPACT_I_MAIN_SAMPLE_TOP_N', os.environ.get('CLOSING_BET_BACKTEST_DETAIL_TOP_N', '5')))
        ...
        lines.append("- 신규 검색식 LP/SLOCK/IT는 위 성과검증 섹션과 실시간 후보 출력 섹션에서 함께 확인합니다. v4.4.9.21은 ST30-RECLAIM을 하드필터가 아닌 가점/주의 태그로 유지하고, 실시간 S-SAFE 후보를 S-RECLAIM PRIME·S-MOMENTUM PRIME·S-MOMENTUM 3000~5000·S-MOMENTUM WATCH·S-LIQUIDITY RISK로 분리 표시합니다. FAST 실전 출력 후보만 우선 표시하며, A-CONFIRM PRIME/CALM·시장/섹터압력·실전 추적로그를 유지합니다. 넓은 A/C/B/H/SLOCK은 기본 숨김/연구로 유지합니다.")
        compact = "\n".join([str(x) for x in lines if x is not None])
        try:
            out_path = LOG_DIR / 'closing_bet_backtest_summary_v4_4_9_21_final_kick_compact_s_label_summary.txt'
            out_path.write_text(compact, encoding='utf-8')
        except Exception:
            pass
        if _v439_bool_env('CLOSING_BET_SHOW_FULL_BACKTEST_REPORT', '0'):
            compact += "\n\n[원문 상세 리포트]\n" + str(original_report or '')
        return compact
    except Exception as e:
        return (str(original_report or '') + f"\n\n[v4.4.9.13 압축 요약 생성 실패] {type(e).__name__}: {e}")
