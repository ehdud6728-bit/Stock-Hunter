...
CLOSING_BET_SCANNER_VERSION = 'G_MORALES_V4_4_9_15_ST30_RECLAIM_ABTEST_20260527'
...
# v4.4.9.15: ST30-RECLAIM은 매수 후보를 즉시 줄이는 하드필터가 아니라
...
# =============================================================
# v4.4.9.15 ST30-RECLAIM / 조정후 재상승 A/B 검증 유틸
# =============================================================
...
def _calc_st30_reclaim_context(row: pd.Series, hist: pd.DataFrame) -> dict:
    ...
    try:
        ...
        weekly_confirm = bool(reclaim_pass and (weekly.get('st30_weekly_macd_up', 0) == 1))
...
def _format_st30_reclaim_abtest_report(df: pd.DataFrame) -> str:
    ...
    try:
        ...
        fail_m = ~pass_m
        lines = []
        lines.append('[🧪 ST30-RECLAIM 적용/미적용 비교 — 필터 손실 점검 v4.4.9.15]')
...
def _v44915_st30_compact_lines(df: pd.DataFrame) -> list[str]:
    ...
    try:
        ...
        lines.append(_v439_short_trade_line(df[pass_m], '✅ ST30-RECLAIM 통과'))
        lines.append(_v439_short_trade_line(df[weekly_m], '✅ ST30-WEEKLY 확인'))
        ...
        if not practical.empty:
            ...
            lines.append(_v439_short_trade_line(practical[pass_m.loc[practical.index]], '단기 실전군 ST30 통과'))
            lines.append(_v439_short_trade_line(practical[wait_m.loc[practical.index]], '단기 실전군 ST30 대기'))
...
def _build_backtest_summary(
    ...
    try:
        ...
        lines.append(f"[ST30-RECLAIM 비교 리포트 오류] {type(e).__name__}: {e}")
    try:
...
def _v439_build_compact_backtest_report(original_report: str, selected_csv: str = '', raw_csv: str = '', start_date: str = '', end_date: str = '', hold_days: int = 0, top_per_strategy: int = 5, all_candidates: bool = False) -> str:
    ...
    try:
        ...
        sel_text = '전체후보' if all_candidates else f'날짜별 전략별 TOP{top_per_strategy}'
        lines = []
        lines.append("🧪 v4.4.9.15 ST30 RECLAIM A/B TEST")
        lines.append(f"버전: {CLOSING_BET_SCANNER_VERSION}")
        lines.append(f"기간: {start_date} ~ {end_date} | 보유평가: 다음 {hold_days}거래일")
        ...
        lines.append("- 신규 검색식 LP/SLOCK/IT는 위 성과검증 섹션과 실시간 후보 출력 섹션에서 함께 확인합니다. v4.4.9.15는 ST30-RECLAIM을 하드필터가 아닌 A/B 검증 태그로 추가하고, FAST 실전 출력 후보만 우선 표시하며, A-CONFIRM PRIME/CALM·시장/섹터압력·실전 추적로그를 추가합니다. 넓은 A/C/B/H/SLOCK은 기본 숨김/연구로 유지합니다.")
        compact = "\n".join([str(x) for x in lines if x is not None])
        try:
            out_path = LOG_DIR / 'closing_bet_backtest_summary_v4_4_9_15_st30_reclaim_abtest.txt'
