...
CLOSING_BET_SCANNER_VERSION = 'G_MORALES_V3_6_S_CORE_PRIORITY_20260510'
...
def _format_practical_combo_report(df: pd.DataFrame) -> str:
    """v3.6: 백테스트 기반 실전 추천 조합 요약. 핵심은 S-CORE(S1우수응축+S2), A강한돌파는 보조 후보로 분리한다."""
    lines = ["[실전 추천 조합 — v3.6 S-CORE 중심 튜닝 관점]"]
    if df is None or df.empty:
        lines.append("- 데이터 없음")
        return "\n".join(lines)
    try:
...
def _format_tuning_report(df: pd.DataFrame) -> str:
    """v3.6: S/A/B 조건별 튜닝 리포트. 과최적화를 막기 위해 조건별 성과만 비교한다."""
    lines = ["[튜닝 리포트 — 조건별 성과]"]
    if df is None or df.empty:
        lines.append("- 데이터 없음")
        return "\n".join(lines)
...
def _build_backtest_summary(
    ...
    lines.append("- v3.6의 3/5규칙은 보수 모델입니다. 같은 날 손절가와 목표가가 모두 닿으면 손절을 먼저 본 것으로 계산합니다.")
    lines.append("- v3.6의 튜닝 리포트는 최근 백테스트 결과를 조건별 성과로 비교하는 참고용이며, 최소 4주~12주 이상 반복 검증이 필요합니다.")
