...
# =============================================================
# closing_bet_scanner.py — 종가배팅 타점 스캐너 (v4.3.4 I-CORE MAIN MATRIX/REGIME BACKTEST)
# =============================================================
# 전략 구성
# A  : 돌파형 종가배팅
...
CLOSING_BET_SCANNER_VERSION = 'G_MORALES_V4_3_4_I_CORE_MAIN_MATRIX_REGIME_BACKTEST_20260515'

# v4.3.4 I-CORE strict/supply/main-filter + market-regime options
...
# v4.3.4 시장국면/KOSPI 초과수익 검증.
# 상승장 착시를 줄이기 위해 I-CORE 20/40/60일 수익률에서 KOSPI 동기간 수익률을 차감한다.
...
def _send_results(hits: list, mins_left: int):
    ...
    header = (
        ...
        f"⏰ {time_text}\n"
        f"전체 후보 {len(hits)}개 | v4.2.15 S/L/G/H 통합 운영블록 우선 출력\n"
    ...
    def _build_practical_block():
        ...
        block = ['[🎯 실전 운영 후보 — v4.2.15 S/L/G/H + L-TAIL ABSORB FINAL]']
        summary = (
        ...
        block.append('운용: S-CORE SAFE 1순위, 👑L-CORE SAFE/🌊L-TAIL ABSORB 대형주 리더갭 2순위, G-SAFE 3순위입니다. H-TRIANGLE/H-CORE는 신규검증 후보로 함께 표시하되, H-OVERHEAT·H-RISK·일반 H는 기본 제외합니다.\n매수계획: 모든 후보는 1차/2차 진입, 추격금지, 손절/익절 기준을 함께 표시합니다.')
...
def _load_market_index_df_for_i_core() -> pd.DataFrame:
    """v4.3.4 I-CORE 시장국면/KOSPI 초과수익 검증용 지수 데이터.

    - 기본: FinanceDataReader KS11(KOSPI) 사용
    - FDR 실패 시 pykrx KOSPI 지수코드 1001 fallback
    ...
    try:
        ...
        # 1) FDR: KS11
        try:
            raw = fdr.DataReader(I_CORE_MARKET_INDEX, start_ymd, end_ymd)
            df = _prepare_price_df(raw)
            if df is not None and not df.empty and len(df) >= 240:
                df['market_source'] = f'fdr:{I_CORE_MARKET_INDEX}'
                return df
        except Exception as e:
            log_debug(f"I-CORE KOSPI FDR 로드 실패: {type(e).__name__}: {e}")
        # 2) pykrx: KOSPI index code 1001
        try:
            if pykrx_stock is not None:
                start_key = pd.Timestamp(start_ymd).strftime('%Y%m%d')
                end_key = pd.Timestamp(end_ymd).strftime('%Y%m%d')
                raw = pykrx_stock.get_index_ohlcv_by_date(start_key, end_key, '1001')
                df = _prepare_price_df(raw)
                if df is not None and not df.empty and len(df) >= 240:
                    df['market_source'] = 'pykrx:KOSPI(1001)'
                    return df
        except Exception as e:
            log_debug(f"I-CORE KOSPI pykrx 로드 실패: {type(e).__name__}: {e}")
    except Exception as e:
        log_debug(f"I-CORE 시장지수 로드 fatal: {type(e).__name__}: {e}")
    return pd.DataFrame()
...
def _evaluate_backtest_hit(hit: dict, df: pd.DataFrame, signal_idx: int, hold_days: int = 5) -> dict:
    ...
    def _calc_i_core_metrics():
        ...
        try:
            ...
            out.update(_calc_i_core_market_context(sig_dt, out))
        except Exception as e:
...
def _format_i_core_report(df: pd.DataFrame) -> str:
    try:
        ...
        lines = []
        lines.append("[📈 I-CORE 시대중심주 150/200일 시세분출 — v4.3.4 MAIN MATRIX/시장국면]")
        if i.empty:
        ...
        # v4.3.4 MAIN MATRIX: v4.3.3 MAIN_ONLY에서 가장 강했던 조합을 직접 검증한다.
        main_anchor = pd.to_numeric(main.get('i_anchor_days', 0), errors='coerce').fillna(0) if not main.empty else pd.Series(dtype=float)
        main_monthly = pd.to_numeric(main.get('i_monthly_vol_rebuild', 0), errors='coerce').fillna(0) if not main.empty else pd.Series(dtype=float)
        ...
        main_i4 = main[main_phase.eq('I-4')] if not main.empty else main
        lines.append(_format_i_core_stat_block(main_i4, 'MAIN I-4 전체'))
        lines.append(_format_i_core_stat_block(main_i4[(main_anchor >= 120) & (main_anchor <= 180)] if not main_i4.empty else main_i4, 'MAIN I-4 × 기준봉 120~180일'))
        lines.append(_format_i_core_stat_block(main_i4[main_monthly == 1] if not main_i4.empty else main_i4, 'MAIN I-4 × 월봉 거래량 재증가'))
        lines.append(_format_i_core_stat_block(main_i4[(main_anchor >= 120) & (main_anchor <= 180) & (main_monthly == 1)] if not main_i4.empty else main_i4, 'MAIN I-4 × 120~180일 × 월봉거래량'))
        lines.append(_format_i_core_stat_block(main[(main_long_dist > 10) & (main_long_dist <= 18) & (main_anchor >= 120) & (main_anchor <= 180)] if not main.empty else main, 'MAIN 가속(+10~18%) × 기준봉 120~180일'))
        lines.append(_format_i_core_stat_block(main[(main_phase.eq('I-6')) & (main_anchor >= 120) & (main_anchor <= 180)] if not main.empty else main, 'MAIN I-6 × 기준봉 120~180일'))

        # v4.3.4 KOSPI 국면/초과수익: 상승장 착시를 분리한다.
        lines.append("\n[I-CORE 시장국면/초과수익 검증 — KOSPI 보정]")
        ...
        if regime.eq('시장데이터부족').all() or regime.eq('시장검증OFF').all():
            ...
            lines.append(_format_i_core_excess_stat_block(main_i4[(main_anchor >= 120) & (main_anchor <= 180)] if not main_i4.empty else main_i4, 'MAIN I-4 × 120~180일 vs KOSPI'))
        ...
        lines.append("- 뉴스/재료 지속성은 이번 v4.3.0에서는 거래대금·월봉거래량·대표성 프록시로만 측정하며, 후속 버전에서 뉴스 누적 저장소와 연결합니다. v4.3.4는 MAIN 후보에 대해 기준봉 120~180일·월봉거래량·장기선이격 매트릭스와 KOSPI 시장국면/초과수익 검증을 추가합니다.")
        return "\n".join(lines)
    except Exception as e:
