...
CLOSING_BET_SCANNER_VERSION = 'G_MORALES_V3_9_S_CORE_3STAGE_20260510'
...
# v3.9: S-CORE SAFE/NEUTRAL/RISK 3단계 분리 기준. 12주 손절특이점 분석에서 안정적이었던 조건을 별도 표기한다.
...
def _send_results(hits: list, mins_left: int):
    ...
    def _is_s_core_neutral_hit(h: dict) -> bool:
        """v3.9: S-CORE이지만 SAFE/RISK가 아닌 중립 후보.
    ...
    def _build_practical_block():
        ...
        block = ['[🎯 실전 매수 우선 후보 — v3.9 S-CORE 3단계]']
        if not pool:
            block.append('해당 종목 없음 — 오늘은 S-CORE 또는 A강한돌파 기준을 만족한 실전 우선 후보가 없습니다.')
        else:
            block.append('기준: S-CORE를 🟢SAFE / 🟡NEUTRAL / ⚠️RISK 3단계로 분리합니다. SAFE는 최우선, NEUTRAL은 관찰 가능, RISK는 추격주의/강등 후보입니다.')
...
def _bt_mask_s_core_safe(df: pd.DataFrame):
    """v3.9: 12주 손절특이점에서 안정적이었던 S-CORE SAFE 조건.
    - S-CORE 내부
    - RR 1.0~1.5
    - 거래량비 1.5 미만
    - 종가위치 70% 이상
...
def _bt_mask_s_core_risk(df: pd.DataFrame):
    """v3.9: S-CORE 내부 위험 플래그 조건. SAFE와 독립적으로 위험 구간을 잡는다."""
    if df is None or df.empty or 'mode' not in df.columns:
        return pd.Series(False, index=df.index if df is not None else [])
    rr = pd.to_numeric(df.get('rr', 0), errors='coerce').fillna(0)
    volr = pd.to_numeric(df.get('vol_ratio', df.get('today_vol_ratio', 0)), errors='coerce').fillna(0)
...
def _bt_mask_s_core_neutral(df: pd.DataFrame):
    """v3.9: S-CORE 중 SAFE도 RISK도 아닌 중립 후보.
...
def _format_practical_combo_report(df: pd.DataFrame) -> str:
    """v3.9: 백테스트 기반 실전 추천 조합 요약. S-CORE를 SAFE/NEUTRAL/RISK 3단계로 분리한다."""
    lines = ["[실전 추천 조합 — v3.9 S-CORE 3단계 튜닝 관점]"]
    if df is None or df.empty:
        lines.append("- 데이터 없음")
        return "\n".join(lines)
    try:
...
def _format_tuning_report(df: pd.DataFrame) -> str:
    """v3.9: S/A/B 조건별 튜닝 리포트. 과최적화를 막기 위해 조건별 성과만 비교한다."""
    lines = ["[튜닝 리포트 — 조건별 성과]"]
    if df is None or df.empty:
        lines.append("- 데이터 없음")
        return "\n".join(lines)
...
def _format_stoploss_signature_report(df: pd.DataFrame) -> str:
    """v3.9: 손절 종목과 비손절 종목의 차이 및 손절 집중 구간을 요약."""
    lines = ["[🩸 손절 종목 특이점 분석]"]
    if df is None or df.empty:
        lines.append("- 데이터 없음")
        return "\n".join(lines)
...
def _build_backtest_summary(
    ...
    lines.append("- v3.9의 3/5규칙은 보수 모델입니다. 같은 날 손절가와 목표가가 모두 닿으면 손절을 먼저 본 것으로 계산합니다.")
    lines.append("- v3.9의 튜닝/손절특이점 리포트는 최근 백테스트 결과를 조건별 성과로 비교하는 참고용이며, 최소 4주~12주 이상 반복 검증이 필요합니다.")
    ...
    lines.append("- v3.9는 손절발생 종목의 공통점, S-CORE 3단계, 주차별 패턴 변화 감지를 함께 출력합니다.")
    return "\n".join(lines)
