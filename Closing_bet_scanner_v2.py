...
CLOSING_BET_SCANNER_VERSION = 'G_MORALES_V3_7_STOPLOSS_PATTERN_DIAG_20260510'
...
def _send_results(hits: list, mins_left: int):
    ...
    def _build_practical_block():
        ...
        block = ['[🎯 실전 매수 우선 후보 — v3.6 S-CORE 중심]']
        if not pool:
            block.append('해당 종목 없음 — 오늘은 S-CORE(S1우수응축+S2) 또는 A강한돌파 기준을 만족한 실전 우선 후보가 없습니다.')
        else:
            block.append('기준: S-CORE(S1 우수응축형 + S2 실행형)를 최우선으로 보고, A 강한돌파는 보조 후보로 둡니다. B1/B2와 C완화형은 관찰/진단 섹션으로 분리합니다.')
...
def _format_practical_combo_report(df: pd.DataFrame) -> str:
    """v3.7: 백테스트 기반 실전 추천 조합 요약. 핵심은 S-CORE(S1우수응축+S2), A강한돌파는 보조 후보로 분리한다."""
    lines = ["[실전 추천 조합 — v3.7 S-CORE 중심 튜닝 관점]"]
    if df is None or df.empty:
        lines.append("- 데이터 없음")
        return "\n".join(lines)
    try:
        ...
        s_core = df[_bt_mask_s2(df) | _bt_mask_s1_good(df)]
        a_strong = df[_bt_mask_a_strong(df)]
        core_plus_a = df[_bt_mask_s2(df) | _bt_mask_s1_good(df) | _bt_mask_a_strong(df)]
        ...
        lines.append(_format_backtest_trade_rule_block(s_core, 'S-CORE(S1우수응축+S2)'))
        ...
        lines.append("- 해석: 실전 매수 최우선은 S-CORE(S1우수응축+S2)입니다. A강한돌파는 보조 후보로 두고, B1/B2와 C완화형은 손절선행·조건 점검용으로 분리해 해석합니다.")
    except Exception as e:
...
def _format_tuning_report(df: pd.DataFrame) -> str:
    """v3.7: S/A/B 조건별 튜닝 리포트. 과최적화를 막기 위해 조건별 성과만 비교한다."""
    lines = ["[튜닝 리포트 — 조건별 성과]"]
    if df is None or df.empty:
        lines.append("- 데이터 없음")
        return "\n".join(lines)
...
def _format_stoploss_signature_report(df: pd.DataFrame) -> str:
    """v3.7: 손절 종목과 비손절 종목의 차이 및 손절 집중 구간을 요약."""
    lines = ["[🩸 손절 종목 특이점 분석]"]
    if df is None or df.empty:
        lines.append("- 데이터 없음")
        return "\n".join(lines)
...
def _build_backtest_summary(
    ...
    try:
        s_core_mask = _bt_mask_s2(df) | _bt_mask_s1_good(df)
        ...
        lines.append(_format_backtest_trade_rule_block(s_core_df, 'S-CORE(S1우수응축+S2)'))
    ...
    lines.append("- v3.7의 3/5규칙은 보수 모델입니다. 같은 날 손절가와 목표가가 모두 닿으면 손절을 먼저 본 것으로 계산합니다.")
    lines.append("- v3.7의 튜닝/손절특이점 리포트는 최근 백테스트 결과를 조건별 성과로 비교하는 참고용이며, 최소 4주~12주 이상 반복 검증이 필요합니다.")
    ...
    lines.append("- v3.7은 손절발생 종목의 공통점과 주차별 패턴 변화 감지를 함께 출력합니다.")
    return "\n".join(lines)
...
def run_closing_bet_backtest(
    ...
    if core_only and selected_df is not None and not selected_df.empty:
        try:
            core_mask = _bt_mask_s2(selected_df) | _bt_mask_s1_good(selected_df)
