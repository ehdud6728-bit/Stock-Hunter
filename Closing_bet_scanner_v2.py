...
def _send_results(hits: list, mins_left: int):
    ...
    header = (
        ...
        f"⏰ {time_text}\n"
        f"전체 후보 {len(hits)}개 | v4.3.7 S/L/G/H + I-MAIN 통합 운영블록 + 백테스트 발생상세\n"
    ...
    def _build_practical_block():
        ...
        block = ['[🎯 실전 운영 후보 — v4.3.7 S/L/G/H + I-MAIN OUTPUT + DETAIL]']
        summary = (
            f"요약: 🟢S-SAFE {len(safe_pool)}개 | 👑L-SAFE {len(l_safe_pool)}개 | 🌊L-TAIL {len(l_tail_pool)}개 | 💰L-MEGA {len(l_mega_pool)}개 | 🟢G-SAFE {len(g_safe_pool)}개 | "
        ...
        block.append('운용: S-CORE SAFE 1순위, 👑L-CORE SAFE/🌊L-TAIL ABSORB 대형주 리더갭 2순위, G-SAFE 3순위입니다. H-TRIANGLE/H-CORE는 신규검증 후보로 함께 표시합니다. I-MAIN은 단기 종가배팅이 아니라 20/40/60일 중기 누적관찰 후보로 별도 운용합니다.\n매수계획: 단기 후보는 1차/2차 진입·손절/익절, I-MAIN 후보는 분할매집·박스/150/200MA 무효조건을 함께 표시합니다.')
        ...
        if l_core_pool:
            block.append('')
            block.append('[👑 L-CORE SAFE / 🌊 L-TAIL ABSORB — 대형주 리더갭 실전 보조]')
            block.append('v4.2.14 결과 반영: 5000억+ 리더갭은 유지합니다. 윗꼬리25~35%/종가위치65~70%는 제외가 아니라 🌊L-TAIL ABSORB로 표시하고 +3/+5 우선 대응합니다.')
            used_codes = set()
            shown = 0
        ...
        if PRACTICAL_SHOW_I_MAIN and (i_accel_pool or i_core_pool or i_watch_pool or i_add_pool or i_confirm_pool):
            ...
            block.append('v4.3.7 검증 반영: I-MAIN CORE는 20~40일 빠른 반응형, I-MAIN ACCEL은 40~60일 대시세형입니다. 단기 종가배팅이 아니라 분할매집/누적관찰로 봅니다.')
            used_codes = set()
            shown_total = 0
            groups = [
...
def _v439_format_i_sample_rows(sub: pd.DataFrame, title: str) -> list[str]:
    ...
    for _, r in sub.iterrows():
        ...
        excess = _v439_fmt_pct(r.get('i_kosdaq_excess_close_60d', r.get('i_bench_excess_close_60d', np.nan)), signed=True)
        lines.append(f"- {dt} {name}({code}) | {cls} {phase} | 20/40/60 {ret20}/{ret40}/{ret60} | 재료대금 {mat} | 초과 {excess}")
    return lines
...
def _v439_i_samples_section(df: pd.DataFrame, top_n: int = 5) -> list[str]:
    ...
    try:
        ...
        if i.empty:
            return lines
        lines.append("[📋 I-MAIN 발생 샘플 — 종목별 중복제거]")
        best = _v439_dedupe_samples(i, 'i_ret_close_60d', False, top_n)
        ...
        lines += _v439_format_i_sample_rows(best, "성과상위: 종목별 최고 1개")
...
def _v439_build_compact_backtest_report(original_report: str, selected_csv: str = '', raw_csv: str = '', start_date: str = '', end_date: str = '', hold_days: int = 0, top_per_strategy: int = 5, all_candidates: bool = False) -> str:
    ...
    try:
        ...
        sel_text = '전체후보' if all_candidates else f'날짜별 전략별 TOP{top_per_strategy}'
        lines = []
        lines.append("🧪 v4.3.9 COMPACT OPERATION SUMMARY")
        lines.append(f"버전: {CLOSING_BET_SCANNER_VERSION}")
        lines.append(f"기간: {start_date} ~ {end_date} | 보유평가: 다음 {hold_days}거래일")
        ...
        lines.append("2순위 단기: S-CORE NEUTRAL / S2 실행형 — KOSPI200·5조+·거래대금 3000억+ 우대")
        ...
        lines.append(_v439_short_trade_line(l_all[amount.loc[l_all.index].ge(5000) & gap.loc[l_all.index].between(6, 12) & close_loc.loc[l_all.index].ge(70)], 'L 갭6~12×5000억+×종가70+'))

        s_neutral = df[_v439_mask('_bt_mask_s_core_neutral', df)]
        s2 = df[mode.eq('S') & _v439_str_series(df, 's_type').eq('S2')]
        lines.append(_v439_short_trade_line(s_neutral, '단기 2순위 S-CORE NEUTRAL'))
        ...
        lines.append("- 단기 S: S-NEUTRAL/S2 중심. KOSPI200, 5조+, 거래대금 3000억+ 우대. 종가위치 70% 미만은 강등.")
        ...
        try:
            out_path = LOG_DIR / 'closing_bet_backtest_summary_v4_3_9_compact.txt'
        ...
        return (str(original_report or '') + f"\n\n[v4.3.9 압축 요약 생성 실패] {type(e).__name__}: {e}")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--compact-summary', action='store_true', help='v4.3.9 압축 운용 요약을 텔레그램 전송/로그에 사용')
