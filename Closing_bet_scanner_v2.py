...
CLOSING_BET_SCANNER_VERSION = 'G_MORALES_V4_4_9_PATTERN_TUNING_BACKTEST_20260520'
...
def _send_results(hits: list, mins_left: int):
    ...
    def _build_practical_block():
        ...
        block = ['[🎯 실전 운영 후보 — v4.4.9 PATTERN TUNING BACKTEST]']
        summary = (
...
# =============================================================
# v4.4.9 PATTERN TUNING BACKTEST — A/C/H/B 승격검증
# =============================================================
...
def _v439_build_compact_backtest_report(original_report: str, selected_csv: str = '', raw_csv: str = '', start_date: str = '', end_date: str = '', hold_days: int = 0, top_per_strategy: int = 5, all_candidates: bool = False) -> str:
    ...
    try:
        ...
        sel_text = '전체후보' if all_candidates else f'날짜별 전략별 TOP{top_per_strategy}'
        lines = []
        lines.append("🧪 v4.4.9 PATTERN TUNING BACKTEST")
        lines.append(f"버전: {CLOSING_BET_SCANNER_VERSION}")
        lines.append(f"기간: {start_date} ~ {end_date} | 보유평가: 다음 {hold_days}거래일")
        ...
        lines.append("- 신규 검색식 LP/SLOCK/IT는 위 성과검증 섹션과 실시간 후보 출력 섹션에서 함께 확인합니다. v4.4.9는 패턴별 멘탈 요약/후보별 멘탈기준을 유지하고, A-RETEST/C-RECLAIM/H-CORE/B-CONFIRM 튜닝 승격검증을 텔레그램에 추가했습니다. LP-D23 과최적화 검증과 LP 없는 날 대체 시나리오를 같이 봅니다. SLOCK은 실시간 기본 숨김, IT는 ACCEL 중심으로 봅니다.")
        compact = "\n".join([str(x) for x in lines if x is not None])
        try:
            out_path = LOG_DIR / 'closing_bet_backtest_summary_v4_4_9_pattern_tuning_backtest.txt'
