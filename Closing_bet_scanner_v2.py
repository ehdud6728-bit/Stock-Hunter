...
CLOSING_BET_SCANNER_VERSION = 'G_MORALES_V3_2_TUNING_REPORT_PRACTICAL_20260510'
...
def _send_results(hits: list, mins_left: int):
    ...
    def _build_practical_block():
        ...
        block = ['[🎯 실전 우선 후보 — 백테스트 튜닝 반영]']
        if not pool:
            block.append('해당 종목 없음 — 오늘은 S2/S1우수/A강한돌파 기준을 동시에 만족한 실전 우선 후보가 없습니다.')
        else:
            block.append('기준: S2 실행형 > S1 우수관찰 > A 강한돌파 순서. B1/B2는 백테스트상 손절선행이 높아 관찰 섹션으로 분리합니다.')
    ...
    try:
        ...
        try:
            if len(hist) >= 250 and info['amount_b'] >= MIN_AMOUNT / 1e8:
                ...
                # v3.2: 실시간 C전략이 나오는 데 백테스트에서 0건만 나오는 문제를 점검하기 위한 완화형 연결.
                ...
                if is_yma_strict or is_yma_relaxed:
                    ...
                    h.update({
                        ...
                        'kki_comment': '역매공파 타점 포착. 스윙 관점 유효.' if is_yma_strict else '완화형 C 백테스트 후보. 실시간 C와 비교 검증 필요.',
                    })
                    hits.append(h)
        except Exception as e:
...
def _evaluate_backtest_hit(hit: dict, df: pd.DataFrame, signal_idx: int, hold_days: int = 5) -> dict:
    """
    백테스트 평가.
    기존 고가/종가 성과에 더해 v3.1부터 실전형 3/5 익절·손절 시뮬레이션을 추가하고, v3.2부터 튜닝 리포트/실전추천 조합을 함께 계산한다.
...
def _format_practical_combo_report(df: pd.DataFrame) -> str:
    """v3.2: 백테스트 기반 실전 추천 조합 요약."""
    lines = ["[실전 추천 조합 — v3.2 튜닝 관점]"]
    if df is None or df.empty:
        lines.append("- 데이터 없음")
        return "\n".join(lines)
    try:
        ...
        if len(c) > 0:
            lines.append(_format_backtest_trade_rule_block(c, 'C 역매공파'))
        lines.append("- 해석: 핵심군 평균수익/승률이 전체보다 높고 손절선행이 낮으면, 실시간 출력에서 해당 조합을 우선 후보로 올리는 방향이 유효합니다.")
    except Exception as e:
...
def _format_tuning_report(df: pd.DataFrame) -> str:
    """v3.2: S/A/B 조건별 튜닝 리포트. 과최적화를 막기 위해 조건별 성과만 비교한다."""
    lines = ["[튜닝 리포트 — 조건별 성과]"]
    if df is None or df.empty:
        lines.append("- 데이터 없음")
        return "\n".join(lines)
...
def _build_backtest_summary(
    ...
    try:
        core_mask = (df['mode'].astype(str).eq('A')) | ((df['mode'].astype(str).eq('S')) & (df.get('s_type', '').astype(str).eq('S2')))
        core_df = df[core_mask]
        observe_mask = ((df['mode'].astype(str).eq('S')) & (df.get('s_type', '').astype(str).eq('S1'))) | (df['mode'].astype(str).isin(['B1', 'B2']))
        observe_df = df[observe_mask]
        lines.append(_format_backtest_trade_rule_block(core_df, '핵심 실행군(S2+A)'))
        lines.append(_format_backtest_trade_rule_block(observe_df, '관찰/확인군(S1+B1+B2)'))
    except Exception:
        pass
    lines.append("")
    ...
    lines.append("- v3.2의 3/5규칙은 보수 모델입니다. 같은 날 손절가와 목표가가 모두 닿으면 손절을 먼저 본 것으로 계산합니다.")
    lines.append("- v3.2의 튜닝 리포트는 최근 백테스트 결과를 조건별로 비교하는 참고용이며, 최소 4주~12주 이상 반복 검증이 필요합니다.")
    return "\n".join(lines)
