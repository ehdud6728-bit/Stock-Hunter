...
CLOSING_BET_SCANNER_VERSION = 'G_MORALES_V4_4_9_10_A_CONFIRM_VC_CALC_FIX_20260521'
...
def _send_results(hits: list, mins_left: int):
    ...
    header = (
        ...
        f"전체 후보 {len(hits)}개 | v4.4.9.10 기존 검색식 유지 + A-CONFIRM VC 계산 보정 + 태그별 성과검증 + 진입가격 코멘트 + 전용방 강제전송\n"
    ...
    def _build_practical_block():
        ...
        block = ['[🎯 실전 운영 후보 — v4.4.9.10 A-CONFIRM VC CALC FIX]']
        summary = (
        ...
        if a_confirm_pool:
            ...
            block.append('v4.4.9.10 기준: 전일 A-RETEST CORE였고 오늘 전일고가 회복/양봉/거래대금 유지가 확인된 후보입니다. v4.4.9.10부터 거래량 수축·가격지지를 VC-SAFE/VC-WATCH로 함께 표시합니다. 시초 +2% 이상 급등 추격은 피하고, 전일고가 회복가 또는 회복 후 눌림 재지지를 우선합니다.')
...
def _v439_i_samples_section(df: pd.DataFrame, top_n: int = 5) -> list[str]:
    ...
    try:
        ...
        if i.empty:
            return lines
        lines.append("[📋 I-MAIN 발생 샘플 — 종목별 중복제거 v4.4.9.10]")
...
def _v445_pattern_path_profile_lines(df: pd.DataFrame) -> list[str]:
    ...
    try:
        ...
        lines.append('[🧭 패턴별 타점 이후 흐름 프로파일 — v4.4.9.10 보정판]')
        ...
        lines.append('- 해석: v4.4.9.10는 1~3일 첫흔들림, +3전흔들림, 전체최대흔들림을 분리합니다. 전체최대흔들림은 목표 도달 후 흔들림까지 포함될 수 있으므로 실전 판단은 +3전흔들림과 지지선 종가유지를 우선 봅니다.')
    except Exception as e:
...
def _v442_new_pattern_performance_lines(df: pd.DataFrame) -> list[str]:
    ...
    try:
        ...
        lines.append('- 해석: v4.4.9.10부터 LP는 단기 최우선 운용 후보로 보되, D1/D23/D45 타점과 다음날 대응 시나리오를 함께 확인합니다. LP-SAFE는 실행 후보, LP-WATCH는 다음날 양봉·전일고가 회복·거래대금 유지 시 승격, LP-RISK는 제외입니다. SLOCK은 실시간 기본 숨김, IT는 ACCEL 중심으로만 봅니다.')
    except Exception as e:
...
# =============================================================
# v4.4.9.10 A-CONFIRM VC CALC FIX — A/C/H/B 승격검증
# =============================================================
...
# =============================================================
# v4.4.9.10 A-CONFIRM VC CALC FIX — 손절률 25% 이하 후보 찾기
# =============================================================
...
# =============================================================
# v4.4.9.10 A-CONFIRM VC CALC FIX — A/C/B/H 0건 원인 진단
# =============================================================
...
def _v4492_close_loc_fallback_diagnostic_lines(df: pd.DataFrame) -> list[str]:
    ...
    try:
        ...
        amount = _v449_base_liquidity_series(df)
        lines.append('[🧩 종가위치 fallback 보정 — v4.4.9.10]')
...
# =============================================================
# v4.4.9.10 A-CONFIRM VC CALC FIX — 재료/시황 동반 성과검증
# =============================================================
...
# =============================================================
# v4.4.9.10 A-CONFIRM VC CALC FIX — 다음날 진입가별 성과검증
# =============================================================
...
def _v4495_a_confirm_entry_price_test_lines(df: pd.DataFrame, hold_days: int = 60) -> list[str]:
    ...
    try:
        ...
        lines.append('[🎯 A-SAFE 다음날확인 진입가별 백테스트 — v4.4.9.10]')
...
# =============================================================
# v4.4.9.10 A-CONFIRM VC CALC FIX — VC 태그별 성과검증
# =============================================================
def _v4499_vc_performance_lines(df: pd.DataFrame) -> list[str]:
    """A-CONFIRM VC 계산 보정판.

    v4.4.9.9에서 VC-SAFE/VC-WATCH가 전부 0건으로 나왔던 문제를 보정한다.
    저장된 실시간 VC 컬럼이 있으면 우선 사용하고, 없거나 전부 0건이면
    백테스트 행의 OHLCV/지표 프록시(종가위치, 거래량비, 거래대금, RR, 위험태그)로
    VC-SAFE/VC-WATCH를 직접 재계산한다.
    """
    lines: list[str] = []
    try:
        ...
        #    프록시 기준: 가격지지(종가위치), 거래량 냉각(vol_ratio), 거래대금/대표성, RR, 위험태그.
        ...
        # 컬럼이 실제로 채워졌는지 확인. 없으면 보수적 프록시로 대체한다.
        ...
        has_vc_data = bool((('a_vol_contract_prev' in df.columns) or ('a_vol_contract_3d' in df.columns) or ('a_today_vol_ratio' in df.columns)) and (
            int(vcp_col.fillna(0).sum()) + int(vc3_col.fillna(0).sum()) + int(today_vr_col.between(0.01, 10.0).fillna(False).sum()) > 0
        ))
        ...
        # today_vr가 없으면 vol_ratio를 거래량 냉각 프록시로 쓴다.
        ...
        if stored_cnt > 0:
            vc_safe = target & stored_safe
            vc_watch = target & stored_watch
            fallback_used = False
        else:
            # VC-SAFE: 가격을 강하게 지키고, 거래량이 과열되지 않았으며, RR/대금이 정상인 A 확인형.
            vc_safe = target & close_loc.ge(80) & support & amount_hold & no_bear & vol_cool_strong & rr.between(0.8, 1.5) & (~risk_txt)
            # VC-WATCH: 가격 지지는 있으나 거래량 수축 강도 또는 RR/대금 조건이 SAFE보다 약한 확인형.
            vc_watch = target & (~vc_safe) & close_loc.ge(70) & support & no_bear & vol_cool_ok & rr.between(0.5, 1.8) & (~risk_txt)
            fallback_used = True

        need = target & (~vc_safe) & (~vc_watch)
        ...
        lines.append('[🧊 A-CONFIRM VC 태그별 성과검증 — v4.4.9.10 계산보정]')
        ...
        lines.append(
            f"- VC 계산 진단: 대상 {int(target.sum())}건 | 저장VC {stored_cnt}건 | "
        ...
        lines.append(f'- VC 판정 기준: 저장된 a_vc_safe/a_vc_watch가 있으면 우선 사용, 없으면 종가위치·거래량비·거래대금·RR·위험태그로 직접 재계산. 다음날확인 기준: {next_note}')
        lines.append('- 해석: v4.4.9.10은 VC-SAFE 0건 문제를 막기 위해 백테스트에서도 OHLCV 프록시로 직접 계산합니다. VC-SAFE가 전체 대비 손절을 낮추거나 승률을 높일 때만 실전 가점으로 올립니다.')
    except Exception as e:
        lines.append(f'[A-CONFIRM VC 계산보정 오류] {type(e).__name__}: {e}')
    return lines
...
def _v439_build_compact_backtest_report(original_report: str, selected_csv: str = '', raw_csv: str = '', start_date: str = '', end_date: str = '', hold_days: int = 0, top_per_strategy: int = 5, all_candidates: bool = False) -> str:
    ...
    try:
        ...
        sel_text = '전체후보' if all_candidates else f'날짜별 전략별 TOP{top_per_strategy}'
        lines = []
        lines.append("🧪 v4.4.9.10 A-CONFIRM VC CALC FIX")
        lines.append(f"버전: {CLOSING_BET_SCANNER_VERSION}")
        lines.append(f"기간: {start_date} ~ {end_date} | 보유평가: 다음 {hold_days}거래일")
        ...
        lines.append("- 신규 검색식 LP/SLOCK/IT는 위 성과검증 섹션과 실시간 후보 출력 섹션에서 함께 확인합니다. v4.4.9.10는 패턴별 멘탈 요약/후보별 멘탈기준을 유지하고, A-CONFIRM에 거래량 수축/가격지지 VC-SAFE·VC-WATCH 태그를 추가해 A 확인형의 품질을 더 명확히 표시합니다. LP-D23 과최적화 검증과 LP 없는 날 대체 시나리오를 같이 봅니다. SLOCK은 실시간 기본 숨김, IT는 ACCEL 중심으로 봅니다.")
        compact = "\n".join([str(x) for x in lines if x is not None])
        try:
            out_path = LOG_DIR / 'closing_bet_backtest_summary_v4_4_9_9_a_confirm_vc_calc_fix.txt'
        ...
        return (str(original_report or '') + f"\n\n[v4.4.9.10 압축 요약 생성 실패] {type(e).__name__}: {e}")
