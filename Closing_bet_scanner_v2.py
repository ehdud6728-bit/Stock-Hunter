...
CLOSING_BET_SCANNER_VERSION = 'G_MORALES_V4_4_8_CANDIDATE_MENTAL_GUIDE_LP_REGIME_20260519'
...
# ── 종가배팅 전용 텔레그램 설정
# v4.4.8: 후보별 멘탈기준 + 오늘 운용강도 + LP-D23 과최적화 검증.
...
def _send_results(hits: list, mins_left: int):
    ...
    def _pattern_path_text(h: dict, mode: str = '') -> str:
        ...
        if mode == 'LP':
            ...
            if b == 'LP-D1':
                return "예상흐름: 빠른 개미털기후상승형. -1~-3%는 가벼운 흔들림, -3~-6%는 LP식 개미털기 가능. 갭하단/전일종가 종가유지가 핵심."
            if b == 'LP-D23':
                return "예상흐름: 핵심 개미털기후상승형. 2~3일 식힌 뒤 흔들어도 +3/+5 반등이 빠른 편. -7% 이상은 지지선/거래량 확인 필수."
            if b == 'LP-D45':
                return "예상흐름: 충분한 식힘 후 재상승형. 안 깨지면 안정적이나 +5까지 시간이 걸릴 수 있음. 깊은 흔들림은 갭하단 유지 여부로 판단."
    ...
    def _build_practical_block():
        ...
        block = ['[🎯 실전 운영 후보 — v4.4.8 CANDIDATE MENTAL GUIDE + LP REGIME CHECK]']
        summary = (
        ...
        if lp_d23_n > 0:
            ...
            op_line = "🧭 오늘 운용강도: 낮음~보통 — LP/L 핵심 부재. S2/S-NEUTRAL은 소액·기다림 대응, 거래량 증가 음봉은 경계."
        else:
        ...
        if PRACTICAL_SHOW_NEW_PATTERNS and lp_safe_pool:
            ...
            block.append('v4.4.8 운용 반영: LP는 1~5일 내 5000억+ 리더갭 이후 갭하단/전일종가/5·10일선 재지지 후보입니다. D1/D23/D45 타점을 구분하고, 다음날 시초가별 대응 시나리오를 함께 봅니다.')
            shown = 0
            used_codes = set()
        ...
        if l_core_pool:
            ...
            block.append('v4.4.8 기준: L 당일 후보는 단기 2순위입니다. 5000억+·종가70%+는 우대하지만, 가능하면 LP 눌림재지지 타점으로 재진입하는 쪽을 더 우선합니다.')
            used_codes = set()
            shown = 0
        ...
        if PRACTICAL_SHOW_NEW_PATTERNS and it_accel_live_pool:
            ...
            block.append('v4.4.8 기준: IT는 ACCEL만 실시간 촉발 후보로 표시합니다. 단기 종가배팅이 아니라 20/40/60일 중기 1차 분할매집 타이밍입니다. IT-CORE는 보조관찰입니다.')
            used_codes = set()
            shown = 0
...
def _v439_i_samples_section(df: pd.DataFrame, top_n: int = 5) -> list[str]:
    ...
    try:
        ...
        if i.empty:
            return lines
        lines.append("[📋 I-MAIN 발생 샘플 — 종목별 중복제거 v4.4.8]")
...
def _v445_pattern_path_profile_lines(df: pd.DataFrame) -> list[str]:
    ...
    try:
        ...
        lines.append('[🧭 패턴별 타점 이후 흐름 프로파일 — v4.4.8 보정판]')
        ...
        lines.append('- 해석: v4.4.8은 1~3일 첫흔들림, +3전흔들림, 전체최대흔들림을 분리합니다. 전체최대흔들림은 목표 도달 후 흔들림까지 포함될 수 있으므로 실전 판단은 +3전흔들림과 지지선 종가유지를 우선 봅니다.')
    except Exception as e:
...
def _v442_new_pattern_performance_lines(df: pd.DataFrame) -> list[str]:
    ...
    try:
        ...
        lines.append('- 해석: v4.4.8부터 LP는 단기 최우선 운용 후보로 보되, D1/D23/D45 타점과 다음날 대응 시나리오를 함께 확인합니다. LP-SAFE는 실행 후보, LP-WATCH는 다음날 양봉·전일고가 회복·거래대금 유지 시 승격, LP-RISK는 제외입니다. SLOCK은 실시간 기본 숨김, IT는 ACCEL 중심으로만 봅니다.')
    except Exception as e:
...
def _v439_build_compact_backtest_report(original_report: str, selected_csv: str = '', raw_csv: str = '', start_date: str = '', end_date: str = '', hold_days: int = 0, top_per_strategy: int = 5, all_candidates: bool = False) -> str:
    ...
    try:
        ...
        sel_text = '전체후보' if all_candidates else f'날짜별 전략별 TOP{top_per_strategy}'
        lines = []
        lines.append("🧪 v4.4.8 CANDIDATE MENTAL GUIDE + LP REGIME CHECK")
        lines.append(f"버전: {CLOSING_BET_SCANNER_VERSION}")
        lines.append(f"기간: {start_date} ~ {end_date} | 보유평가: 다음 {hold_days}거래일")
        ...
        lines.append("- 신규 검색식 LP/SLOCK/IT는 위 성과검증 섹션과 실시간 후보 출력 섹션에서 함께 확인합니다. v4.4.8은 패턴별 멘탈 요약과 후보별 멘탈기준을 함께 출력합니다. LP-D23 과최적화 검증과 LP 없는 날 대체 시나리오를 같이 봅니다. SLOCK은 실시간 기본 숨김, IT는 ACCEL 중심으로 봅니다.")
        compact = "\n".join([str(x) for x in lines if x is not None])
        try:
            out_path = LOG_DIR / 'closing_bet_backtest_summary_v4_4_8_candidate_mental_guide.txt'
        ...
        return (str(original_report or '') + f"\n\n[v4.4.8 압축 요약 생성 실패] {type(e).__name__}: {e}")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--compact-summary', action='store_true', help='v4.4.8 후보별 멘탈/LP 검증 압축 요약을 텔레그램 전송/로그에 사용')
