...
CLOSING_BET_SCANNER_VERSION = 'G_MORALES_V4_4_9_16_ST30_PATTERN_CROSS_AUDIT_20260527'
...
# =============================================================
# v4.4.9.16 ST30 PATTERN CROSS AUDIT
# =============================================================
...
def _v439_build_compact_backtest_report(original_report: str, selected_csv: str = '', raw_csv: str = '', start_date: str = '', end_date: str = '', hold_days: int = 0, top_per_strategy: int = 5, all_candidates: bool = False) -> str:
    ...
    try:
        ...
        sel_text = '전체후보' if all_candidates else f'날짜별 전략별 TOP{top_per_strategy}'
        lines = []
        lines.append("🧪 v4.4.9.16 ST30 PATTERN CROSS AUDIT")
        lines.append(f"버전: {CLOSING_BET_SCANNER_VERSION}")
        lines.append(f"기간: {start_date} ~ {end_date} | 보유평가: 다음 {hold_days}거래일")
        ...
        lines.append("- 신규 검색식 LP/SLOCK/IT는 위 성과검증 섹션과 실시간 후보 출력 섹션에서 함께 확인합니다. v4.4.9.16은 ST30-RECLAIM을 하드필터가 아닌 패턴별 교차검증/성공누락 감사 태그로 유지하고, FAST 실전 출력 후보만 우선 표시하며, A-CONFIRM PRIME/CALM·시장/섹터압력·실전 추적로그를 추가합니다. 넓은 A/C/B/H/SLOCK은 기본 숨김/연구로 유지합니다.")
