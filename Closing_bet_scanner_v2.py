...
CLOSING_BET_SCANNER_VERSION = 'G_MORALES_V4_4_9_12_LIVE_OPERATION_GUARD_20260521'
...
# v4.4.9.12 LIVE OPERATION GUARD: 실전에서 실제로 볼 후보만 별도 추적한다.
...
def _check_breakout_bet(code: str, name: str) -> dict | None:
    ...
    try:
        ...
        if len(df) >= 2:
            ...
            # v4.4.9.12: v4.4.9.10 검증 결과, 기존 VC-WATCH가 더 좋은 성과를 보였다.
...
def _live_text_pressure_label(raw: str, positive_label: str, neutral_label: str, negative_label: str) -> str:
    """v4.4.9.12: 시장/섹터 압력 텍스트를 간단한 3단계로 정규화."""
    try:
...
def _save_live_operation_guard_rows(rows: list[dict]):
    """v4.4.9.12: 실전 FAST 출력 후보만 따로 누적 저장.
...
def _send_results(hits: list, mins_left: int):
    ...
    header = (
        ...
        f"전체 후보 {len(hits)}개 | v4.4.9.12 LIVE OPERATION GUARD: FAST 실전출력 정리 + PRIME/CALM + 시장/섹터압력 + 실전추적로그\n"
    ...
    def _build_practical_block():
        ...
        # v4.4.9.12: 실제 FAST 실전 출력 후보만 별도 CSV로 추적한다.
        try:
            ...
            try:
                log_error(f"⚠️ v4.4.9.12 실전 추적로그 구성 실패: {_live_log_e}")
            except Exception:
                pass

        block = ['[🎯 실전 운영 후보 — v4.4.9.12 LIVE OPERATION GUARD]']
        summary = (
        ...
        if a_confirm_pool:
            ...
            block.append('v4.4.9.12 기준: 전일 A-RETEST CORE였고 오늘 전일고가 회복/양봉/거래대금 유지가 확인된 후보입니다. PRIME은 힘이 남은 우선 확인형, CALM은 안정형 보조 후보입니다. 시초 +2% 이상 급등 추격은 피하고, 전일고가 회복가 또는 회복 후 눌림 재지지를 우선합니다.')
...
def _v439_i_samples_section(df: pd.DataFrame, top_n: int = 5) -> list[str]:
    ...
    try:
        ...
        if i.empty:
            return lines
        lines.append("[📋 I-MAIN 발생 샘플 — 종목별 중복제거 v4.4.9.12]")
...
def _v445_pattern_path_profile_lines(df: pd.DataFrame) -> list[str]:
    ...
    try:
        ...
        lines.append('[🧭 패턴별 타점 이후 흐름 프로파일 — v4.4.9.12 보정판]')
        ...
        lines.append('- 해석: v4.4.9.12은 1~3일 첫흔들림, +3전흔들림, 전체최대흔들림을 분리합니다. 전체최대흔들림은 목표 도달 후 흔들림까지 포함될 수 있으므로 실전 판단은 +3전흔들림과 지지선 종가유지를 우선 봅니다.')
    except Exception as e:
...
def _v442_new_pattern_performance_lines(df: pd.DataFrame) -> list[str]:
    ...
    try:
        ...
        lines.append('- 해석: v4.4.9.12부터 LP는 단기 최우선 운용 후보로 보되, D1/D23/D45 타점과 다음날 대응 시나리오를 함께 확인합니다. LP-SAFE는 실행 후보, LP-WATCH는 다음날 양봉·전일고가 회복·거래대금 유지 시 승격, LP-RISK는 제외입니다. SLOCK은 실시간 기본 숨김, IT는 ACCEL 중심으로만 봅니다.')
    except Exception as e:
...
# =============================================================
# v4.4.9.12 LIVE OPERATION GUARD — A/C/H/B 승격검증
# =============================================================
...
# =============================================================
# v4.4.9.12 LIVE OPERATION GUARD — 손절률 25% 이하 후보 찾기
# =============================================================
...
# =============================================================
# v4.4.9.12 LIVE OPERATION GUARD — A/C/B/H 0건 원인 진단
# =============================================================
...
def _v4492_close_loc_fallback_diagnostic_lines(df: pd.DataFrame) -> list[str]:
    ...
    try:
        ...
        amount = _v449_base_liquidity_series(df)
        lines.append('[🧩 종가위치 fallback 보정 — v4.4.9.12]')
...
# =============================================================
# v4.4.9.12 LIVE OPERATION GUARD — 재료/시황 동반 성과검증
# =============================================================
...
# =============================================================
# v4.4.9.12 LIVE OPERATION GUARD — 다음날 진입가별 성과검증
# =============================================================
...
def _v4495_a_confirm_entry_price_test_lines(df: pd.DataFrame, hold_days: int = 60) -> list[str]:
    ...
    try:
        ...
        lines.append('[🎯 A-SAFE 다음날확인 진입가별 백테스트 — v4.4.9.12]')
...
# =============================================================
# v4.4.9.12 LIVE OPERATION GUARD — VC 태그별 성과검증
# =============================================================
...
def _v4499_vc_performance_lines(df: pd.DataFrame) -> list[str]:
    ...
    따라서 v4.4.9.12은 내부 계산은 유지하되 실전 라벨을 재정의한다.
    ...
    try:
        ...
        lines.append('[🧊 A-CONFIRM VC 라벨 재보정 — v4.4.9.12]')
        ...
        lines.append('- 해석: v4.4.9.12은 VC를 하드필터로 쓰지 않고 A-CONFIRM 안에서 우선순위를 나누는 태그로 사용합니다. PRIME이 전체 대비 손절을 낮추고 승률을 높이는지 반복 확인합니다.')
    except Exception as e:
...
def _v439_build_compact_backtest_report(original_report: str, selected_csv: str = '', raw_csv: str = '', start_date: str = '', end_date: str = '', hold_days: int = 0, top_per_strategy: int = 5, all_candidates: bool = False) -> str:
    ...
    try:
        ...
        sel_text = '전체후보' if all_candidates else f'날짜별 전략별 TOP{top_per_strategy}'
        lines = []
        lines.append("🧪 v4.4.9.12 LIVE OPERATION GUARD")
        lines.append(f"버전: {CLOSING_BET_SCANNER_VERSION}")
        lines.append(f"기간: {start_date} ~ {end_date} | 보유평가: 다음 {hold_days}거래일")
        ...
        lines.append("- 신규 검색식 LP/SLOCK/IT는 위 성과검증 섹션과 실시간 후보 출력 섹션에서 함께 확인합니다. v4.4.9.12는 FAST 실전 출력 후보만 우선 표시하고, A-CONFIRM PRIME/CALM·시장/섹터압력·실전 추적로그를 추가합니다. 넓은 A/C/B/H/SLOCK은 기본 숨김/연구로 유지합니다.")
        ...
        return (str(original_report or '') + f"\n\n[v4.4.9.12 압축 요약 생성 실패] {type(e).__name__}: {e}")
