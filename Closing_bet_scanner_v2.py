...
CLOSING_BET_SCANNER_VERSION = 'G_MORALES_V4_4_9_48_LIVE_PRIORITY_COMPACT_20260614'
...
def _send_results(hits: list, mins_left: int):
    ...
    header = (
        ...
        f"전체 후보 {len(hits)}개 | v4.4.9.48 LIVE PRIORITY COMPACT: 14:40 후보발굴 + 15:03 실행/포기 압축 최종킥\n"
    ...
    def _sj6_checklist_context(h: dict) -> dict:
        """v4.4.9.48: 신정재 종가베팅 6요소(신·좁·깔·거·조·재) 최종 품질 체크.
        ...
        try:
            ...
            # 좁: v44부터 20일선 이격 100~106을 PRIME, 106~112는 WATCH로 낮춘다.
            narrow_prime = bool(100.0 <= disparity20 <= 106.0)
            narrow_watch = bool((106.0 < disparity20 <= 112.0) or (0.85 <= rr <= 1.45) or (mode == 'LP' and _safe_float(h.get('lp_volume_vs_gap', 0), 0) > 0))
            narrow = bool(narrow_prime or narrow_watch)
            ...
            mat_proxy = _safe_float(h.get('i_material_proxy_score', 0), 0)
            material = bool(('동반강세' in sector_txt) or mat_proxy >= 3 or (material_kw and use_amt >= 1000.0) or use_amt >= 5000.0)
            ...
            # v44: 6점 단순 합산보다 재료·이격·대금·종가위치에 가중치를 둔다.
            material_pts = 25 if (mat_proxy >= 3 or ('동반강세' in sector_txt) or use_amt >= 5000.0) else (18 if (material_kw and use_amt >= 1000.0) else (14 if use_amt >= 1000.0 else (6 if use_amt >= 300.0 else 0)))
            narrow_pts = 20 if narrow_prime else (8 if narrow_watch else (-6 if disparity20 > 112.0 else 0))
            amount_pts = 15 if use_amt >= 5000.0 else (12 if use_amt >= 3000.0 else (9 if use_amt >= 1000.0 else (4 if use_amt >= 300.0 else -8)))
            close_pts = 15 if close_loc >= 85.0 else (9 if close_loc >= 70.0 else -8)
            candle_pts = 10 if (wick <= 5.0 and close_loc >= 85.0) else (7 if (wick <= 10.0 and close_loc >= 75.0) else (-4 if wick >= 15.0 else 2))
            adjust_pts = 10 if adjust else 0
            supply_pts = 4 if supply_ok else 0
            market_pts = 4 if market_ok else -8
            risk_penalty = 0
            if use_amt < 1000.0 and not material:
                risk_penalty -= 12
            if close_loc < 70.0:
                risk_penalty -= 8
            if disparity20 > 112.0 and mode not in ('LP',):
                risk_penalty -= 8
            if mode == 'L' and _is_l_mega_upper_limit_followup(h):
                risk_penalty -= 10
            w_score = max(0, min(100, material_pts + narrow_pts + amount_pts + close_pts + candle_pts + adjust_pts + supply_pts + market_pts + risk_penalty))
            if mode == 'LP':
                w_badge = '🟡SJ-W-LP참고' if w_score < 60 else '✅SJ-W-LP양호'
            else:
                w_badge = '🔥SJ-W-PRIME' if w_score >= 75 else ('✅SJ-W-SAFE' if w_score >= 60 else ('🟡SJ-W-WATCH' if w_score >= 45 else '❌SJ-W-RISK'))
            ...
            short = f"{w_badge} {w_score:.0f}/100 | 6요소 {score}/6 | {bits} | {market_mark} {supply_mark}"
            line = f"📊 SJ가중: {short}"
            return {'score': score, 'w_score': w_score, 'badge': w_badge, 'short': short, 'line': line, 'checks': checks, 'market_ok': market_ok, 'supply_ok': supply_ok, 'supply_unknown': supply_unknown}
        except Exception as e:
    ...
    def _build_practical_block():
        ...
        # v4.4.9.48: 신정재 원본 방식에 가까운 '거래대금 상위 50 → SJ-W 확인' 별도 라이브 풀.
        ...
        def _final_kick_decision(h: dict, pattern_label: str) -> tuple[str, str, list]:
            ...
            # v4.4.9.48: L-MEGA 상한가성 잠금형은 과열 때문에 종가매수는 금지하지만,
        ...
        def _build_final_kick_block() -> str:
            ...
            def _final_kick_display_decision(hh: dict, decision: str, label: str = '') -> str:
                # v4.4.9.48: 추천강도와 진입방식을 분리하고 SJ-TOP50은 3단 표시한다.
                try:
                    if 'SJ-TOP50' in str(label) and _live_good_filter_overlap(hh) and str(decision).startswith('🟡'):
                        return '🔥 강력추천 지정가'
                    if 'SJ-TOP50' in str(label) and str(decision).startswith('🟡'):
                        return '⭐ 우선확인'
            ...
            lines = [
                '🚨 [동시호가 최종킥 — v4.4.9.48 LIVE PRIORITY COMPACT]',
            ...
            for i, (hh, label, decision, why, reasons) in enumerate(picked, 1):
                ...
                if CLOSING_BET_FINAL_KICK_COMPACT:
                    lines += [
                        ...
                        f"SJ-TOP50: {'🔥겹침강력' if ('SJ-TOP50' in str(label) and _live_good_filter_overlap(hh)) else ('⭐6/6우선확인' if hh in sj_top50_sj6_pool else ('🟡TOP50관찰' if hh in sj_top50_watch_pool else '—'))}",
                        f'성격: {exec_style}',
                        f'계획: {entry} / {chase}',
        ...
        block = ['[🎯 실전 운영 후보 — v4.4.9.48 LIVE PRIORITY COMPACT]']
        summary = (
        ...
        if PRACTICAL_SHOW_NEW_PATTERNS and lp_safe_pool:
            ...
            block.append('v4.4.9.48 운용 반영: LP는 1~5일 내 5000억+ 리더갭 이후 갭하단/전일종가/5·10일선 재지지 후보입니다. D1/D23/D45 타점을 구분하고, 다음날 시초가별 대응 시나리오를 함께 봅니다.')
            shown = 0
            used_codes = set()
        ...
        if safe_pool:
            ...
            block.append('v4.4.9.48 기준: S-SAFE는 ST30 통과형을 S-RECLAIM MEGA(5000억+)·3000~5000·PRIME(1000억+)·WATCH(300~1000)·LIQUIDITY RISK(<300)로 나누고, ST30 미통과형은 S-MOMENTUM CALM(1000억+ 거래량냉각)/PRIME/S-LIQUIDITY로 분리합니다. ST30은 하드필터가 아닙니다.')
        ...
        if PRACTICAL_SHOW_NEW_PATTERNS and it_accel_live_pool:
            ...
            block.append('v4.4.9.48 기준: IT는 ACCEL만 실시간 촉발 후보로 표시합니다. 단기 종가배팅이 아니라 20/40/60일 중기 1차 분할매집 타이밍이며, I-LEADER 주봉 사이클 라벨을 보조로 확인합니다.')
            used_codes = set()
            shown = 0
        ...
        if PRACTICAL_SHOW_I_MAIN and (i_accel_pool or i_core_pool or i_watch_pool or i_add_pool or i_confirm_pool):
            ...
            block.append('v4.4.9.48 기준: I-MAIN은 단기 종가배팅이 아니라 20/40/60거래일 중기 누적관찰입니다. ACCEL/CORE에 더해 I-LEADER 라벨로 주봉 사이클 초입·가속·후반·공세종말 위험을 함께 봅니다.')
            used_codes = set()
            shown_total = 0
            groups = [
...
def _v44933_i_outlier_adjustment_lines(i_en: pd.DataFrame, hold_days: int = 0) -> list[str]:
    ...
    try:
        ...
        if not any(len(g) for _, g in groups):
            return lines
        lines.append('[🧯 I-MAIN HORIZON GUARD + 이상치 보정 — v4.4.9.48]')
...
def _v439_i_samples_section(df: pd.DataFrame, top_n: int = 5) -> list[str]:
    ...
    try:
        ...
        if i.empty:
            return lines
        lines.append("[📋 I-MAIN 발생 샘플 — 종목별 중복제거 v4.4.9.48]")
...
def _v44937_lp_flow_line(sub: pd.DataFrame, label: str, min_n: int = 5) -> str:
    ...
    try:
        ...
        # v4.4.9.48: 감사 지표 정합성 보정. 흔들림/최대하락은 양수로 표시되면 오해가 커서 0 이하로 클립한다.
        try:
...
def _v44937_lp_explosion_audit_lines(df: pd.DataFrame) -> list[str]:
    ...
    try:
        ...
        lines.append('[🔥 LP POWER PRIME + STABLE 분리검증 — v4.4.9.48]')
...
def _v44934_s_momentum_core_lines(df: pd.DataFrame) -> list[str]:
    ...
    try:
        ...
        lines.append('[🔥 S-CALM / S-MOMENTUM / S-RECLAIM 핵심압축 — v4.4.9.48]')
    ...
    try:
        ...
        if not i_all.empty:
            try:
                ...
                pass
        lines.append('[🌍 GEO-RISK / 헤드라인 리스크 구간 해석 — v4.4.9.48]')
...
def _v440_l_mega_upper_followup_lines(df: pd.DataFrame) -> list[str]:
    """v4.4.9.48: L-MEGA 상한가성 후속관찰 감사 섹션."""
    lines: list[str] = []
    try:
        if df is None or df.empty:
            return lines
        ...
        if upper.empty and l_mega.empty:
            return lines
        lines.append('[💰 L-MEGA 상한가 후속관찰 감사 — v4.4.9.48]')
...
def _v44942_sj6_score_series(df: pd.DataFrame) -> pd.Series:
    """v4.4.9.48: 신·좁·깔·거·조·재 점수 산출. 후보 생성에는 절대 개입하지 않는 overlay 전용."""
    try:
        if df is None or df.empty:
...
def _v44942_count_parity_lines(df: pd.DataFrame, raw_count=None) -> list[str]:
    """v4.4.9.48: 후보 수 보존 감사. SJ-6가 하드필터처럼 작동하지 않았는지 텔레그램에 명시."""
    lines: list[str] = []
    try:
        ...
        if raw_unknown:
            ...
            rc = int(rc)
            rc_txt = f'{rc}건'
        lines.append('[🧮 COUNT PARITY / SJ OVERLAY 감사 — v4.4.9.48]')
...
def _v44945_sj_integrity_guard_lines(df: pd.DataFrame, raw_df: pd.DataFrame | None = None, raw_count=None, selected_csv: str = '', raw_csv: str = '', all_candidates: bool = False, start_date: str = '', end_date: str = '', hold_days: int = 0) -> list[str]:
    """v4.4.9.48: SJ-W가 후보 생성/삭제에 개입하지 않는지 더 직접적으로 보여주는 무결성 가드.
    ...
    try:
        ...
        lines.append('[🛡️ SJ-W 후보수 무결성 가드 — v4.4.9.48]')
        ...
        lines.append('- 운용해석: v48은 v47의 SJ-TOP50 겹침강력 규칙을 유지하면서, 실전 출력에서 🔥강력추천/⭐우선확인/🟡관찰 3단으로 압축해 한눈에 보이도록 정리한 버전입니다.')
    except Exception as e:
...
def _v44942_sj_overlay_audit_lines(df: pd.DataFrame, raw_count=None) -> list[str]:
    """v4.4.9.48: SJ-6 단독성과보다 기존 패턴과의 교차 성과를 우선 출력."""
    lines: list[str] = []
    try:
        if df is None or df.empty:
            return lines
        ...
        lines.append('[📊 신정재 SJ-6 문맥형 OVERLAY 감사 — v4.4.9.48]')
...
def _v44943_first_num_series(df: pd.DataFrame, cols: list[str], default=0.0) -> pd.Series:
    """v4.4.9.48: 여러 컬럼 중 첫 유효 숫자 Series를 반환한다."""
    try:
...
def _v44943_stop_dist_series(df: pd.DataFrame) -> pd.Series:
    """v4.4.9.48: 진입가 대비 가장 가까운 실전 무효/지지선 거리(%)를 추정한다.
...
def _v44943_material_score_series(df: pd.DataFrame) -> pd.Series:
    """v4.4.9.48: 재료 강도 프록시. 글로벌/섹터/수급 데이터가 없으면 대금·재료대금 점수로 대체한다."""
    try:
        mat = _v439_num_series(df, 'i_material_proxy_score', 0)
        amount = _v439_num_series(df, 'amount_b')
        if 'leader_gap_amount_b' in df.columns:
...
def _v44943_threshold_fail_common_audit_lines(df: pd.DataFrame) -> list[str]:
    """v4.4.9.48: SJ 세부 임계값과 손절선행 후보 공통점을 감사한다."""
    lines: list[str] = []
    try:
        if df is None or df.empty:
            return lines
        lines.append('[🔬 SJ 임계값/손절공통점 감사 — v4.4.9.48]')
...
def _v44944_weighted_sj_score_series(df: pd.DataFrame) -> pd.Series:
    """v4.4.9.48: SJ 단순 6점제를 보조화하고 재료·이격·대금·종가위치에 가중치를 둔다.
...
def _v44944_weighted_context_audit_lines(df: pd.DataFrame) -> list[str]:
    """v4.4.9.48: SJ 가중점수와 손절위험 조합을 감사한다."""
    lines: list[str] = []
    try:
        if df is None or df.empty:
            return lines
        ...
        lines.append('[⚖️ SJ 가중 문맥점수 감사 — v4.4.9.48]')
...
def _v44946_amount_proxy_series(df: pd.DataFrame) -> pd.Series:
    """v4.4.9.48: 거래대금 TOP50 기준에 사용할 대금 프록시."""
    try:
        if df is None or df.empty:
            return pd.Series(dtype=float)
...
def _v44946_top50_amount_mask(df: pd.DataFrame, n: int = 50) -> pd.Series:
    """v4.4.9.48: 신정재 원본 방식에 가까운 '날짜별 거래대금 상위 50' 마스크.
...
def _v44946_s_momentum_mask(df: pd.DataFrame) -> pd.Series:
    """v4.4.9.48: S-MOMENTUM CALM/PRIME에 가까운 백테스트 마스크."""
...
def _v44946_good_filter_overlap_mask(df: pd.DataFrame) -> pd.Series:
    """v4.4.9.48: SJ-TOP50와 겹치면 강력추천 후보로 승격 검토할 기존 고타율 필터."""
    try:
        if df is None or df.empty:
            return pd.Series(dtype=bool)
        mode = _v439_str_series(df, 'mode')
...
def _v44946_sj_top50_audit_lines(df: pd.DataFrame) -> list[str]:
    """v4.4.9.48: 거래대금 상위 50개 안에서만 SJ-W를 다시 확인하는 별도 감사."""
    lines: list[str] = []
    try:
        if df is None or df.empty:
            return lines
        ...
        lines.append('[📊 SJ-TOP50 거래대금 상위 50 감사 — v4.4.9.48]')
        ...
        for label, mask in [
            ...
            ('🔥SJ-TOP50 + SJ 5/6+ + 고타율필터 겹침강력', overlap_sj6_5p),
...
def _v44948_live_priority_compact_audit_lines(df: pd.DataFrame) -> list[str]:
    """v4.4.9.48: 백테스트 리포트용 실전 출력 3단 압축 요약."""
    lines: list[str] = []
    try:
        if df is None or df.empty:
            return lines
        ...
        for label, mask in [
            ...
            if len(sub) > 0:
                lines.append(_v439_short_trade_line(sub, label))
        lines.append('- 운용해석: 강력추천은 기존 고타율 필터와 겹친 경우만, 우선확인은 TOP50+SJ 완성형/품질조합, 관찰은 TOP50+SJ-W60 단독으로 구분합니다. 진입은 여전히 시장5일선과 지정가/다음날 지지확인이 우선입니다.')
    except Exception as e:
...
def _v44946_fetch_index_m5_state(index_code: str) -> dict:
    """v4.4.9.48: 지수 5일선 게이트용 간단 상태. 실패 시 ok=False."""
    try:
...
def _v44946_market5_gate_context() -> dict:
    """v4.4.9.48: KOSPI/KOSDAQ 5일선 기준 종베 시장 게이트."""
    try:
...
def _v44946_market5_gate_lines() -> list[str]:
    ...
    return [
        '[📈 MARKET-5 지수 5일선 종베 게이트 — v4.4.9.48]',
...
def _v44934_build_core_compact_backtest_report(original_report: str, selected_csv: str = '', raw_csv: str = '', start_date: str = '', end_date: str = '', hold_days: int = 0, top_per_strategy: int = 5, all_candidates: bool = False) -> str:
    ...
    try:
        ...
        sel_text = '전체후보' if all_candidates else f'날짜별 전략별 TOP{top_per_strategy}'
        lines: list[str] = []
        lines.append('🧪 v4.4.9.48 LIVE PRIORITY COMPACT')
        lines.append(f'버전: {CLOSING_BET_SCANNER_VERSION}')
        lines.append(f'기간: {start_date} ~ {end_date} | 보유평가: 다음 {hold_days}거래일')
        ...
        lines.append('2순위 단기: L 5000억+ / S-MOMENTUM CALM·PRIME / S2 — SJ-TOP50 단독은 우선확인, 고타율 필터와 겹칠 때만 강력추천 승격')
        ...
        lines.append('단기 실행 우선순위는 LP-STABLE 안정형과 LP-POWER PRIME 강력추천 지정가를 분리하고, LP-SAFE > LP-POWER STABLE > L 5000억+ > S-MOMENTUM CALM > S-MOMENTUM PRIME/S2 > A-CONFIRM입니다. SJ-W는 후보를 삭제하지 않고 LP-SAFE 강등에도 쓰지 않으며, SJ-TOP50 단독은 우선확인으로 두고, LP-SAFE·L5000억+·S-MOMENTUM과 겹칠 때만 강력추천 라벨로 올립니다. 중기 I-MAIN은 평가기간이 충분할 때만 숫자를 확정하고, 최근구간이 약하면 5MA/20MA 재지지 확인형으로 낮춥니다.')
        lines.append('')
        ...
        lines.append('- 신좁깔거조재/SJ-W/SJ-TOP50: SJ-W는 전체 후보 품질계기판, SJ-TOP50은 거래대금 상위50 전용 확인섹션입니다. TOP50+SJ 조건 부합은 우선확인 후보이고, LP-SAFE·L5000억+·S-MOMENTUM과 겹칠 때만 강력추천 후보로 올립니다. A/B/C는 SJ-W RISK·손절위험조합이면 강등합니다.')
        ...
        lines.append('- 과거 패치 버전명은 텔레그램 혼선을 줄이기 위해 v4.4.9.48/누적검증 문구로 정리했습니다.')
        compact = '\n'.join([str(x) for x in lines if x is not None])
        compact = _v44934_cleanup_legacy_labels(compact)
        try:
            out_path = LOG_DIR / 'closing_bet_backtest_summary_v4_4_9_46_market5_sj_top50_gate.txt'
