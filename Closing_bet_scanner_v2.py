...
CLOSING_BET_SCANNER_VERSION = 'G_MORALES_V4_4_9_13_WORKER_MODE_CARD_SPLIT_GUARD_20260521'
...
def _send_results(hits: list, mins_left: int):
    ...
    header = (
        ...
        f"전체 후보 {len(hits)}개 | v4.4.9.13 WORKER MODE + CARD SPLIT GUARD: 직장인 지정가/자동익절 + 카드완료/잘림방지\n"
    ...
    def _build_practical_block():
        ...
        block = ['[🎯 실전 운영 후보 — v4.4.9.13 WORKER MODE + CARD SPLIT GUARD]']
        summary = (
        ...
        if a_confirm_pool:
            ...
            block.append('v4.4.9.13 기준: 전일 A-RETEST CORE였고 오늘 전일고가 회복/양봉/거래대금 유지가 확인된 후보입니다. PRIME은 힘이 남은 우선 확인형, CALM은 안정형 보조 후보입니다. 시초 +2% 이상 급등 추격은 피하고, 전일고가 회복가 또는 회복 후 눌림 재지지를 우선합니다.')
