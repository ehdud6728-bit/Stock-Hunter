...
# =============================================================
# closing_bet_scanner.py — 종가배팅 타점 스캐너 (G Morales v2.5 BACKTEST 완성형)
# =============================================================
# 전략 구성
# A  : 돌파형 종가배팅
...
CLOSING_BET_SCANNER_VERSION = 'G_MORALES_V2_5_BACKTEST_6M_20260509'
...
def _check_closing_bet(code: str, name: str) -> dict | None:
    """
    A / B1 / B2 / G / C 중 우선순위가 가장 높은 전략 1개 반환.
    G는 갭 특화 전략이므로 같은 완전체라면 A보다 우선하도록 mode_rank를 낮게 둔다.
    """
    with DIAG_LOCK:
    ...
    candidates = [x for x in [g, a, b1, b2, c] if x]
    if not candidates:
        return None
    ...
    def _priority(h):
        ...
        mode_rank = {'G': 0, 'A': 1, 'B1': 2, 'B2': 3, 'C': 4}.get(mode, 9)
...
def _send_results(hits: list, mins_left: int):
    ...
    total = (
        min(len(hits_g), 5) + min(len(hits_a), 5) + min(len(hits_b1), 5) +
        min(len(hits_b2), 5) + min(len(hits_c), 5)
    )

    time_text = f"마감까지 {mins_left}분" if mins_left > 0 else "마감직전/마감후"
    header = (
        f"📌 종가배팅 전략별 선별 TOP {total} ({TODAY_STR})\n"
        f"🧩 버전 {CLOSING_BET_SCANNER_VERSION}\n"
        f"⏰ {time_text}\n"
        f"전체 후보 {len(hits)}개 | 출력 {total}개\n"
        f"모랄레스갭(G) {min(len(hits_g), 5)}개 | 돌파형(A) {min(len(hits_a), 5)}개 | "
...
def run_closing_bet_scan(force: bool = False) -> list:
    ...
    # 기존 버그 방지: C전략과 신규 G전략이 최종 합산에서 빠지지 않도록 모두 포함
    hits = hits_g + hits_a + hits_b1 + hits_b2 + hits_c

    log_info(f"\n종가배팅 후보: {len(hits)}개")
    log_info(
        f"모랄레스갭(G): {len(hits_g)}개 | 돌파형(A): {len(hits_a)}개 | "
...
def _prepare_price_df(df: pd.DataFrame) -> pd.DataFrame:
    """FDR 일봉을 스캐너 지표 형식으로 표준화. Date 컬럼은 보존한다."""
    if df is None or df.empty:
        return pd.DataFrame()
    try:
        out = df.copy()
    ...
    try:
        ...
        try:
            if len(hist) >= 250 and info['amount_b'] >= MIN_AMOUNT / 1e8:
                ...
                is_breakout = (info['_close'] >= ma112_now) or (info['_close'] >= ma224_now)
                if is_reverse and is_accumulation and is_concrete and is_breakout:
                    h = _bt_common_payload(code, name, 'C', '역매공파', '완전체', 7, row, hist, idx_label, marcap, ['역배열바닥', '매집흔적', '공구리', '장기선돌파'])
                    h.update({
                        'band_type': 'YMGP',
                        'kki_pattern': '바닥탈출대시세형',
                        'kki_habit': '매집 완료 후 장기 저항 돌파',
...
def _select_backtest_top(df: pd.DataFrame, top_per_strategy: int = 5, all_candidates: bool = False) -> pd.DataFrame:
    ...
    work = work.sort_values(['signal_date', 'mode', '_grade_rank', '_score_sort', '_amount_sort', '_vol_sort'], ascending=[True, True, True, False, False, False])
    ...
    return work.drop(columns=[c for c in ['_grade_rank', '_score_sort', '_amount_sort', '_vol_sort'] if c in work.columns])


def _build_backtest_summary(raw_df: pd.DataFrame, selected_df: pd.DataFrame, start_date: str, end_date: str, hold_days: int, top_per_strategy: int, all_candidates: bool) -> str:
    ...
    def _stat_block(sub: pd.DataFrame, label: str):
        cnt = len(sub)
        if cnt == 0:
            return f"- {label}: 0건"
        return (
    ...
    lines.append("[전체]")
    lines.append(_stat_block(df, '전체'))
    lines.append("")
    lines.append("[전략별]")
    mode_order = ['G', 'A', 'B1', 'B2', 'C']
    for mode in mode_order:
        ...
        lines.append(_stat_block(sub, label))
    lines.append("")
    lines.append("[등급별]")
    for grade, sub in df.groupby('grade', dropna=False):
        lines.append(_stat_block(sub, str(grade)))
    lines.append("")
    lines.append("[주의]")
...
def run_closing_bet_backtest(months: int = 6, start_date: str = '', end_date: str = '', hold_days: int = 5, top_per_strategy: int = 5, all_candidates: bool = False, max_workers: int = None) -> tuple[str, str, str]:
    """과거 n개월 동안 날짜별 종가배팅 신호를 재현하고 성과를 CSV/TXT로 저장한다."""
    log_info(f"✅ BOOTCHECK: {CLOSING_BET_SCANNER_VERSION}")
    _ensure_log_dir()
    now = _now_kst()
    if not end_date:
    ...
    log_info(f"\n{'=' * 60}")
    log_info(f"종가배팅 백테스트 시작: {start_date} ~ {end_date} | hold={hold_days} | top_per_strategy={top_per_strategy}")
    log_info(f"{'=' * 60}")
    ...
    try:
        for future in as_completed(futures, timeout=max(600, SCAN_FUTURES_TIMEOUT * 3)):
            ...
            if done % 100 == 0:
                log_info(f"백테스트 진행: {done}/{len(source_codes)} | 원신호 {len(raw_rows)}건")
    except FuturesTimeoutError as e:
    ...
    if raw_df.empty:
        report = _build_backtest_summary(raw_df, raw_df, start_date, end_date, hold_days, top_per_strategy, all_candidates)
        stamp = now.strftime('%Y%m%d_%H%M%S')
        raw_path = str(LOG_DIR / f"closing_bet_backtest_raw_{stamp}.csv")
        selected_path = str(LOG_DIR / f"closing_bet_backtest_selected_{stamp}.csv")
        BACKTEST_SUMMARY_TXT.write_text(report, encoding='utf-8')
        return report, selected_path, raw_path
    ...
    report = _build_backtest_summary(raw_df, selected_df, start_date, end_date, hold_days, top_per_strategy, all_candidates)
    try:
        BACKTEST_SUMMARY_TXT.write_text(report, encoding='utf-8')
    except Exception as e:
...
if __name__ == '__main__':
    ...
    if args.backtest_months or args.backtest_start or args.backtest_end:
