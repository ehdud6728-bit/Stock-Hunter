...
CLOSING_BET_SCANNER_VERSION = 'G_MORALES_V4_4_9_18_S_SAFE_MISS_SUCCESS_FAIL_AUDIT_20260527'
...
def _v439_build_compact_backtest_report(original_report: str, selected_csv: str = '', raw_csv: str = '', start_date: str = '', end_date: str = '', hold_days: int = 0, top_per_strategy: int = 5, all_candidates: bool = False) -> str:
    ...
    try:
        ...
        lines.append("- 신규 검색식 LP/SLOCK/IT는 위 성과검증 섹션과 실시간 후보 출력 섹션에서 함께 확인합니다. v4.4.9.18은 ST30-RECLAIM을 하드필터가 아닌 패턴별 교차검증/성공누락 감사 태그로 유지하고, S-SAFE 미통과 성공/실패군의 거래대금·종가위치·거래량비·RR·캔들품질 차이를 추가 점검하며, FAST 실전 출력 후보만 우선 표시하며, A-CONFIRM PRIME/CALM·시장/섹터압력·실전 추적로그를 추가합니다. 넓은 A/C/B/H/SLOCK은 기본 숨김/연구로 유지합니다.")
        compact = "\n".join([str(x) for x in lines if x is not None])
        try:
            out_path = LOG_DIR / 'closing_bet_backtest_summary_v4_4_9_18_s_safe_miss_success_fail_audit.txt'
