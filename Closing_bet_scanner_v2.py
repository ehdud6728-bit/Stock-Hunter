...
CLOSING_BET_SCANNER_VERSION = 'G_MORALES_V4_4_8_1_TEXT_POLISH_20260519'
...
def _send_results(hits: list, mins_left: int):
    ...
    header = (
        ...
        f"전체 후보 {len(hits)}개 | v4.4.8.1 기존 검색식 유지 + 신규검증 LP/SLOCK/IT 추가 + 전용방 강제전송\n"
    ...
    def _build_practical_block():
        ...
        block = ['[🎯 실전 운영 후보 — v4.4.8.1 TEXT POLISH]']
        summary = (
        ...
        if l_core_pool:
            ...
            block.append('v4.4.8.1 기준: L 당일 후보는 단기 2순위입니다. 5000억+·종가70%+는 우대하지만, 가능하면 LP 눌림재지지 타점으로 재진입하는 쪽을 더 우선합니다.')
            used_codes = set()
            shown = 0
        ...
        if safe_pool:
            ...
            block.append('v4.4.8.1 기준: S는 S2 실행형/S-NEUTRAL을 우선하고, SAFE는 종가위치·RR·거래대금 조건이 맞을 때만 소액 확인합니다.')
        ...
        if PRACTICAL_SHOW_NEW_PATTERNS and it_accel_live_pool:
            ...
            block.append('v4.4.8.1 기준: IT는 ACCEL만 실시간 촉발 후보로 표시합니다. 단기 종가배팅이 아니라 20/40/60일 중기 1차 분할매집 타이밍입니다. IT-CORE는 보조관찰입니다.')
            used_codes = set()
            shown = 0
        ...
        if PRACTICAL_SHOW_I_MAIN and (i_accel_pool or i_core_pool or i_watch_pool or i_add_pool or i_confirm_pool):
            ...
            block.append('v4.4.8.1 기준: I-MAIN은 단기 종가배팅이 아니라 20/40/60거래일 중기 누적관찰입니다. ACCEL은 고수익 시세분출형, CORE는 안정형 누적관찰로 봅니다.')
            used_codes = set()
            shown_total = 0
            groups = [
        ...
        if neutral_pool:
            ...
            block.append('v4.4.8.1 기준: S2 실행형/S-NEUTRAL은 단기 2순위입니다. 단, 고점권 전략이므로 KOSPI200·5조+·거래대금 3000억+·종가위치70%+를 우대합니다.')
...
def _v439_build_compact_backtest_report(original_report: str, selected_csv: str = '', raw_csv: str = '', start_date: str = '', end_date: str = '', hold_days: int = 0, top_per_strategy: int = 5, all_candidates: bool = False) -> str:
    ...
    try:
        ...
        sel_text = '전체후보' if all_candidates else f'날짜별 전략별 TOP{top_per_strategy}'
        lines = []
        lines.append("🧪 v4.4.8.1 TEXT POLISH")
        lines.append(f"버전: {CLOSING_BET_SCANNER_VERSION}")
        lines.append(f"기간: {start_date} ~ {end_date} | 보유평가: 다음 {hold_days}거래일")
        ...
        lines.append("- 신규 검색식 LP/SLOCK/IT는 위 성과검증 섹션과 실시간 후보 출력 섹션에서 함께 확인합니다. v4.4.8.1은 패턴별 멘탈 요약과 후보별 멘탈기준을 함께 출력하고, 중복 문구/버전 문구를 정리했습니다. LP-D23 과최적화 검증과 LP 없는 날 대체 시나리오를 같이 봅니다. SLOCK은 실시간 기본 숨김, IT는 ACCEL 중심으로 봅니다.")
        compact = "\n".join([str(x) for x in lines if x is not None])
        try:
            out_path = LOG_DIR / 'closing_bet_backtest_summary_v4_4_8_1_text_polish.txt'
