...
# =============================================================
# closing_bet_scanner.py — 종가배팅 타점 스캐너 (G Morales v4.2.13 OPERATION LEADER GAP CORE)
# =============================================================
# 전략 구성
# A  : 돌파형 종가배팅
...
CLOSING_BET_SCANNER_VERSION = 'G_MORALES_V4_2_13_OPERATION_LEADER_GAP_CORE_20260514'
...
def _send_results(hits: list, mins_left: int):
    ...
    header = (
        ...
        f"⏰ {time_text}\n"
        f"전체 후보 {len(hits)}개 | v4.2.13 S/L/G/H 통합 운영블록 우선 출력\n"
    ...
    def _build_practical_block():
        ...
        block = ['[🎯 실전 운영 후보 — v4.2.13 S/L/G/H 통합 + 매수계획 FINAL]']
        summary = (
...
def _format_leader_gap_watch_report(df: pd.DataFrame) -> str:
    ...
    lines = ["[👑 대형주 리더갭 WATCH — SK하이닉스형 갭상승 관전 v4.2.13]"]
    try:
        if df is None or df.empty or 'mode' not in df.columns:
            lines.append("- 데이터 없음")
            return "\n".join(lines)
...
def _build_backtest_summary(
    ...
    lines.append("- v4.2.13부터 실시간 스캔에서도 H-TRIANGLE SAFE와 거래대금×Vol60 핵심셀(500~1000억×2~3배, 500~1000억 세부구간, 300~500억×3~5배)을 S/G/H 통합 운영 후보로 표시하고, 돌파봉 Vol60 배율 운용분류에 더해, 거래대금 구간을 <30억/30~50억/50~100억/100~200억/200~300억/300~500억/500~1000억/1000~2000억/2000~3000억/3000~5000억/5000억~1조/1조+로 세분화해 성과를 분해합니다. H-VOL SAFE는 2~3배이며 v4.2.12에서는 500~1000억 내부를 500~600/600~700/700~850/850~1000억 및 Vol60 1.5~5배로 추가 세분화합니다. H-VOL SAFE는 2~3배, H-VOL SWING은 3~5배, H-OVERHEAT는 5~8배, H-AGGRESSIVE는 8배 이상입니다. 수급 데이터는 이번 버전에서 필수 조건이 아니라 후속 가점 후보입니다. v4.2.13은 대형주 리더갭 5000억+를 L-CORE 실전 보조 후보로 승격하고 3000~5000억은 L-WATCH 관찰로 분리합니다.")
