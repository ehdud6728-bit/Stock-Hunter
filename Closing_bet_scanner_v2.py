...
CLOSING_BET_SCANNER_VERSION = 'G_MORALES_V2_8_S1S2_EXEC_OBSERVE_20260509'
...
def _send_results(hits: list, mins_left: int):
    ...
    header = (
        ...
        f"모랄레스갭(G) {min(len(hits_g), 5)}개 | 고점재응축(S) {min(len(hits_s), 5)}개(S2 {len(hits_s2)}/S1 {len(hits_s1)}) | 돌파형(A) {min(len(hits_a), 5)}개 | "
