...
CLOSING_BET_SCANNER_VERSION = 'G_MORALES_V4_4_9_37_LP_EXPLOSION_BACKTEST_AUDIT_20260612'
...
def _lp_explosion_watch_context(h: dict) -> dict:
    """v4.4.9.37: 안정형 LP와 다음날 폭발형 LP를 분리한다.

    취지:
    - 최종킥은 종가 진입 안정성을 우선하므로, 막판에 살짝 밀린 LP-D23 고베타 후보를 보조 지정가로 낮출 수 있다.
    - 그러나 갭봉 5000억+, D23/D45, 갭하단 유지, 종가 65~85%, 거래량 재점화가 붙은 후보는
      실패가 아니라 '개미털기 후 다음날 폭발' 가능성이 있으므로 별도 WATCH 라벨로 살린다.
    - 이 라벨은 종가 추격 매수 신호가 아니라 다음날 갭하단/전일고가 재확인 1순위 표시다.
    """
    try:
        if _lp_class(h) != 'LP-SAFE':
            return {'is_explosion': False, 'score': 0, 'reason': ''}
        ...
        # 고베타/재료 확산 힌트는 하드필터가 아니라 이유 문구에만 사용한다.
        ...
        # 핵심: 안정형 LP와 다르게 막판 거래량 재점화/흔들림이 붙은 케이스를 폭발 WATCH로 분리한다.
        shake_reignite = bool(1.15 <= volr <= 2.30 and close_loc >= 65.0)
        if not (base and shake_reignite):
            return {'is_explosion': False, 'score': 0, 'reason': ''}
...
def _send_results(hits: list, mins_left: int):
    ...
    header = (
        ...
        f"전체 후보 {len(hits)}개 | v4.4.9.37 LP-EXPLOSION AUDIT: 14:40 후보발굴 + 15:03 실행/포기 압축 최종킥\n"
    ...
    def _build_practical_block():
        ...
        try:
            if CLOSING_BET_LIVE_OPERATION_GUARD and CLOSING_BET_LIVE_TRACKING_LOG:
                ...
                _add_live_pool(lp_explosion_pool, 'LP-EXPLOSION WATCH', 2, PRACTICAL_L_PULLBACK_TOP_N)
        ...
        def _final_kick_decision(h: dict, pattern_label: str) -> tuple[str, str, list]:
            ...
            if mode == 'LP':
                ...
                if exp_ctx.get('is_explosion'):
                    reasons.append('🔥LP-EXPLOSION WATCH')
            if mode == 'A' and _is_a_confirm_live(h):
            ...
            if mode == 'LP':
                ...
                if exp_ctx.get('is_explosion'):
                    ...
                    return '🟡 지정가만 가능', f"LP-EXPLOSION WATCH: 막판 흔들림을 실패로만 보지 않고 다음날 폭발 관찰 1순위로 분리({exp_ctx.get('reason','')})", reasons
        ...
        def _final_kick_execution_style(h: dict, decision: str) -> str:
            ...
            if mode == 'LP':
                ...
                if _is_lp_explosion_watch(h):
                    return '🔥 폭발형 관찰: 종가 추격보다 다음날 갭하단 지지·전일고가 회복 확인'
        ...
        def _build_final_kick_block() -> str:
            ...
            _push(lp_explosion_pool, 'LP-EXPLOSION WATCH', PRACTICAL_L_PULLBACK_TOP_N)
            ...
            lp_explosion_picked = [(hh, label, decision, why, reasons) for hh, label, decision, why, reasons in picked_short if _pick_strategy(hh) == 'LP' and _is_lp_explosion_watch(hh)]
            lp_stable_picked = [(hh, label, decision, why, reasons) for hh, label, decision, why, reasons in picked_short if not (_pick_strategy(hh) == 'LP' and _is_lp_explosion_watch(hh))]
            main_short_line = _final_kick_top_line(lp_stable_picked[:1] if lp_stable_picked else picked_short[:1], '없음')
            aux_short_line = _final_kick_top_line(picked_short[1:], '없음')
            explosion_short_line = _final_kick_top_line(lp_explosion_picked, '없음')
            ...
            core_line = (
                f"핵심후보: LP {len(lp_safe_pool)} / 폭발WATCH {len(lp_explosion_pool)} | L {len(l_safe_pool) + len(l_mega_pool) + len(l_tail_pool)} | "
            ...
            for i, (hh, label, decision, why, reasons) in enumerate(picked, 1):
                ...
                if mode == 'LP' and _is_lp_explosion_watch(hh) and decision.startswith('🟡'):
                    card_decision = '🟡 폭발형 지정가/다음날 관찰'
                    mental = '종가 추격은 금지하되, 다음날 갭하단 지지·전일고가 회복·첫 눌림 재돌파가 나오면 폭발형으로 재평가.'
        ...
        block = ['[🎯 실전 운영 후보 — v4.4.9.37 LP-EXPLOSION AUDIT]']
        summary = (
            f"요약: 🥇LP-SAFE {len(lp_safe_pool)}개 | 🔥LP-EXPLOSION {len(lp_explosion_pool)}개 | 🔁LP-WATCH {len(lp_watch_pool)}개 | ⚠️LP-RISK {len(lp_risk_pool)}개 | 👑L-SAFE {len(l_safe_pool)}개 | 🔒SLOCK검증/숨김 {len(slock_pool)}개 | ⚡IT-ACCEL {len(it_accel_live_pool)}개 | 🌊L-TAIL {len(l_tail_pool)}개 | 💰L-MEGA {len(l_mega_pool)}개 | 💎S-RE-MEGA {len(s_reclaim_mega_pool)}개 | 🟢S-RE3000 {len(s_reclaim_3000_pool)}개 | 🟢S-RE-PRIME {len(s_reclaim_prime_pool)}개 | 🟡S-RE-WATCH {len(s_reclaim_watch_pool)}개 | 🟢S-MOMENTUM CALM {len(s_momentum_calm_pool)}개 | 🔥S-MOMENTUM PRIME {len(s_momentum_prime_pool)}개 | 🟡S-3000~5000 {len(s_momentum_3000_pool)}개 | 🟡S-WATCH {len(s_momentum_watch_pool)}개 | ⚠️S-유동성주의 {len(s_liquidity_risk_pool)}개 | 🟡S관찰 {len(neutral_pool)}개 | 🟢G-SAFE {len(g_safe_pool)}개 | "
        ...
        if PRACTICAL_SHOW_NEW_PATTERNS and lp_safe_pool:
            ...
            block.append('v4.4.9.37 운용 반영: LP는 1~5일 내 5000억+ 리더갭 이후 갭하단/전일종가/5·10일선 재지지 후보입니다. D1/D23/D45 타점을 구분하고, 다음날 시초가별 대응 시나리오를 함께 봅니다.')
            shown = 0
            used_codes = set()
        ...
        if PRACTICAL_SHOW_NEW_PATTERNS and lp_explosion_pool:
            block.append('')
            block.append('[🔥 LP-EXPLOSION WATCH — 다음날 폭발 관찰 1순위]')
            block.append('LP-SAFE 중 막판 흔들림/거래량 재점화가 붙은 고베타형입니다. 종가 추격 신호가 아니라 다음날 갭하단 지지·전일고가 회복을 보는 폭발 관찰 라벨입니다.')
            for j, h in enumerate(lp_explosion_pool[:PRACTICAL_L_PULLBACK_TOP_N], 1):
                ...
                close = _safe_float(h.get('close', h.get('_close', 0)), 0.0)
                block.append(f"{j}) 🔥LP-EXPLOSION WATCH | {name}({code}) | {int(close):,}원 | {ctx.get('reason','')}")
                block.append('   ↳ 대응: 종가 추격보다 다음날 갭하단 지지·전일고가 회복·첫 눌림 재돌파 확인. 안 잡히면 포기.')
                block.append('[카드완료]')
        ...
        if safe_pool:
            ...
            block.append('v4.4.9.37 기준: S-SAFE는 ST30 통과형을 S-RECLAIM MEGA(5000억+)·3000~5000·PRIME(1000억+)·WATCH(300~1000)·LIQUIDITY RISK(<300)로 나누고, ST30 미통과형은 S-MOMENTUM CALM(1000억+ 거래량냉각)/PRIME/S-LIQUIDITY로 분리합니다. ST30은 하드필터가 아닙니다.')
        ...
        if PRACTICAL_SHOW_NEW_PATTERNS and it_accel_live_pool:
            ...
            block.append('v4.4.9.37 기준: IT는 ACCEL만 실시간 촉발 후보로 표시합니다. 단기 종가배팅이 아니라 20/40/60일 중기 1차 분할매집 타이밍이며, I-LEADER 주봉 사이클 라벨을 보조로 확인합니다.')
            used_codes = set()
            shown = 0
        ...
        if PRACTICAL_SHOW_I_MAIN and (i_accel_pool or i_core_pool or i_watch_pool or i_add_pool or i_confirm_pool):
            ...
            block.append('v4.4.9.37 기준: I-MAIN은 단기 종가배팅이 아니라 20/40/60거래일 중기 누적관찰입니다. ACCEL/CORE에 더해 I-LEADER 라벨로 주봉 사이클 초입·가속·후반·공세종말 위험을 함께 봅니다.')
            used_codes = set()
            shown_total = 0
            groups = [
...
def _v44933_i_outlier_adjustment_lines(i_en: pd.DataFrame, hold_days: int = 0) -> list[str]:
    ...
    try:
        ...
        if not any(len(g) for _, g in groups):
            return lines
        lines.append('[🧯 I-MAIN HORIZON GUARD + 이상치 보정 — v4.4.9.37]')
...
def _v44937_first_num_series(df: pd.DataFrame, cols: list[str], default: float = 0.0) -> pd.Series:
    """v4.4.9.37: 백테스트 CSV 컬럼명이 버전별로 달라도 첫 유효 숫자열을 안전하게 가져온다."""
    try:
        for c in cols:
            if c in df.columns:
                s = pd.to_numeric(df[c], errors='coerce')
...
def _v44937_lp_safe_df(df: pd.DataFrame) -> pd.DataFrame:
    """v4.4.9.37: LP-SAFE 모수를 백테스트 CSV에서 최대한 재구성한다."""
    if df is None or df.empty:
...
def _v44937_lp_masks(lp: pd.DataFrame) -> dict[str, pd.Series]:
    """v4.4.9.37: 안정형/폭발형 LP 분리 마스크."""
    idx = lp.index
    if lp is None or lp.empty:
        z = pd.Series(False, index=idx)
        return {'d1': z, 'd23': z, 'd45': z, 'explosion': z, 'stable': z}
    ...
    explosion = (
        ...
        & hold_gap_s.fillna(1).ge(1)
        & ma_hold_s.fillna(1).ge(1)
    )
    stable = ~explosion
    return {'d1': d1.fillna(False), 'd23': d23.fillna(False), 'd45': d45.fillna(False), 'explosion': explosion.fillna(False), 'stable': stable.fillna(False)}


def _v44937_lp_flow_line(sub: pd.DataFrame, label: str, min_n: int = 5) -> str:
    """v4.4.9.37: LP 분리검증용 속도/흔들림/다음날 지표."""
    try:
        base = _v439_short_trade_line(sub, label)
        if sub is None or sub.empty:
            return base
...
def _v44937_lp_explosion_audit_lines(df: pd.DataFrame) -> list[str]:
    """v4.4.9.37: v36 핵심인 안정형/폭발형 LP 분리검증 리포트."""
    lines: list[str] = []
    try:
        ...
        d23 = masks['d23']
        d45 = masks['d45']
        lines.append('[🔥 LP-EXPLOSION WATCH 분리검증 — v4.4.9.37]')
        lines.append('- 목적: LP-SAFE 안에서 “종가진입 안정형”과 “막판 흔들림 후 다음날 폭발 관찰형”이 실제로 분리되는지 확인합니다.')
        lines.append(_v44937_lp_flow_line(lp, 'LP-SAFE 전체'))
        lines.append(_v44937_lp_flow_line(stable, 'LP-STABLE ONEPICK 후보'))
        lines.append(_v44937_lp_flow_line(explosion, '🔥 LP-EXPLOSION WATCH'))
        if len(lp[d23]) > 0:
        ...
        if len(explosion[d23.reindex(explosion.index).fillna(False)]) > 0:
            lines.append(_v44937_lp_flow_line(explosion[d23.reindex(explosion.index).fillna(False)], '🔥 LP-D23 EXPLOSION'))
        if len(lp[d45]) > 0:
        ...
        if len(explosion[d45.reindex(explosion.index).fillna(False)]) > 0:
            lines.append(_v44937_lp_flow_line(explosion[d45.reindex(explosion.index).fillna(False)], '🔥 LP-D45 EXPLOSION'))
        if len(explosion) == 0:
            lines.append('- 판정: 이번 구간에서는 v36 폭발형 조건을 만족한 LP가 없습니다. 조건이 너무 좁을 수 있으므로 최근 HPSP류 구간에서 별도 재검증이 필요합니다.')
        else:
            lines.append('- 판정기준: STABLE은 종가진입 메인, EXPLOSION은 종가추격 금지가 기본입니다. EXPLOSION은 +3 1~2일 도달률·다음날고가·손절선행을 보고 WATCH/승격을 결정합니다.')
            lines += _v44937_lp_sample_lines(explosion, 'LP-EXPLOSION 성공/강세 샘플', top=True, n=3)
            ...
            if not fail_sub.empty:
                lines += _v44937_lp_sample_lines(fail_sub, 'LP-EXPLOSION 실패/손절 샘플', top=False, n=3)
        lines.append('- 운용해석: 안정형이 손절을 낮추면 종가진입 메인으로, 폭발형이 +3/+5 속도를 높이면 다음날 폭발 관찰 1순위로 분리합니다. 단, 폭발형 손절선행이 20~25%를 넘으면 계속 WATCH 유지입니다.')
    except Exception as e:
        lines.append(f'[LP-EXPLOSION 분리검증 오류] {type(e).__name__}: {e}')
    return lines
...
def _v44934_s_momentum_core_lines(df: pd.DataFrame) -> list[str]:
    ...
    try:
        ...
        lines.append('[🔥 S-CALM / S-MOMENTUM / S-RECLAIM 핵심압축 — v4.4.9.37]')
    ...
    try:
        ...
        if not i_all.empty:
            try:
                ...
                pass
        lines.append('[🌍 GEO-RISK / 헤드라인 리스크 구간 해석 — v4.4.9.37]')
...
def _v44934_build_core_compact_backtest_report(original_report: str, selected_csv: str = '', raw_csv: str = '', start_date: str = '', end_date: str = '', hold_days: int = 0, top_per_strategy: int = 5, all_candidates: bool = False) -> str:
    ...
    try:
        ...
        sel_text = '전체후보' if all_candidates else f'날짜별 전략별 TOP{top_per_strategy}'
        lines: list[str] = []
        lines.append('🧪 v4.4.9.37 LP-EXPLOSION BACKTEST AUDIT')
        lines.append(f'버전: {CLOSING_BET_SCANNER_VERSION}')
        lines.append(f'기간: {start_date} ~ {end_date} | 보유평가: 다음 {hold_days}거래일')
        ...
        lines.append('2순위 단기: L 5000억+ / S-MOMENTUM CALM·PRIME / S2 — 추격보다 지정가·지지확인')
        ...
        lines.append('단기 실행 우선순위는 LP-SAFE 안정형과 LP-EXPLOSION WATCH 폭발형을 분리하고, LP-SAFE > L 5000억+ > S-MOMENTUM CALM > S-MOMENTUM PRIME/S2 > A-CONFIRM입니다. 중기 I-MAIN은 평가기간이 충분할 때만 숫자를 확정하고, 최근구간이 약하면 5MA/20MA 재지지 확인형으로 낮춥니다.')
        lines.append('')
        ...
        lines.append('- LP-SAFE: 여전히 단기 최우선. D1은 소액, D23/D45는 갭하단·전일종가 지지 확인 후 +3/+5 자동익절. 막판 흔들림+거래량 재점화가 붙은 후보는 LP-EXPLOSION WATCH로 분리해 다음날 폭발 관찰 1순위로 둡니다.')
        ...
        lines.append('- 과거 패치 버전명은 텔레그램 혼선을 줄이기 위해 v4.4.9.37/누적검증 문구로 정리했습니다.')
        compact = '\n'.join([str(x) for x in lines if x is not None])
        compact = _v44934_cleanup_legacy_labels(compact)
        try:
            out_path = LOG_DIR / 'closing_bet_backtest_summary_v4_4_9_37_lp_explosion_backtest_audit.txt'
