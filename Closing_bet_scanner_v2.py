...
# =============================================================
# closing_bet_scanner.py — 종가배팅 타점 스캐너 (G Morales v4.0 SAFE OUTPUT FINAL 완성형)
# =============================================================
# 전략 구성
# A  : 돌파형 종가배팅
...
CLOSING_BET_SCANNER_VERSION = 'G_MORALES_V4_0_SAFE_OUTPUT_FINAL_20260511'
...
# v4.0: S-CORE SAFE/NEUTRAL/RISK 3단계 분리 기준. 12주 손절특이점 분석에서 안정적이었던 조건을 별도 표기한다.
...
# v4.0: 실시간 텔레그램 출력은 SAFE 중심으로 정리한다.
...
def _send_results(hits: list, mins_left: int):
    ...
    def _is_s_core_neutral_hit(h: dict) -> bool:
        """v4.0: S-CORE이지만 SAFE/RISK가 아닌 중립 후보.
    ...
    def _build_practical_block():
        ...
        a_pool = sorted([h for h in hits if _is_a_strong(h)], key=_priority_practical)

        block = ['[🎯 실전 최우선 후보 — v4.0 SAFE 중심]']
        block.append(
            f"요약: 🟢SAFE {len(safe_pool)}개 | 🟡NEUTRAL {len(neutral_pool)}개 | ⚠️RISK {len(risk_pool)}개 | A보조 {len(a_pool)}개"
        )
        block.append('운용: SAFE 우선 → 부족하면 NEUTRAL 상위만 관찰 → RISK는 기본 제외/추격주의입니다.')
        ...
        if a_pool:
            ...
            block.append('S-CORE가 우선입니다. A는 +3/+5 익절형 보조 후보로만 봅니다.')
...
def _bt_mask_s_core_safe(df: pd.DataFrame):
    """v4.0: 12주 손절특이점에서 안정적이었던 S-CORE SAFE 조건.
    - S-CORE 내부
    - RR 1.0~1.5
    - 거래량비 1.5 미만
    - 종가위치 70% 이상
...
def _bt_mask_s_core_risk(df: pd.DataFrame):
    """v4.0: S-CORE 내부 위험 플래그 조건. SAFE와 독립적으로 위험 구간을 잡는다."""
    if df is None or df.empty or 'mode' not in df.columns:
        return pd.Series(False, index=df.index if df is not None else [])
    rr = pd.to_numeric(df.get('rr', 0), errors='coerce').fillna(0)
    volr = pd.to_numeric(df.get('vol_ratio', df.get('today_vol_ratio', 0)), errors='coerce').fillna(0)
...
def _bt_mask_s_core_neutral(df: pd.DataFrame):
    """v4.0: S-CORE 중 SAFE도 RISK도 아닌 중립 후보.
...
def _format_practical_combo_report(df: pd.DataFrame) -> str:
    """v4.0: 백테스트 기반 실전 추천 조합 요약. S-CORE를 SAFE/NEUTRAL/RISK 3단계로 분리한다."""
    lines = ["[실전 추천 조합 — v4.0 S-CORE 3단계 튜닝 관점]"]
    if df is None or df.empty:
        lines.append("- 데이터 없음")
        return "\n".join(lines)
    try:
...
def _format_tuning_report(df: pd.DataFrame) -> str:
    """v4.0: S/A/B 조건별 튜닝 리포트. 과최적화를 막기 위해 조건별 성과만 비교한다."""
    lines = ["[튜닝 리포트 — 조건별 성과]"]
    if df is None or df.empty:
        lines.append("- 데이터 없음")
        return "\n".join(lines)
...
def _format_stoploss_signature_report(df: pd.DataFrame) -> str:
    """v4.0: 손절 종목과 비손절 종목의 차이 및 손절 집중 구간을 요약."""
    lines = ["[🩸 손절 종목 특이점 분석]"]
    if df is None or df.empty:
        lines.append("- 데이터 없음")
        return "\n".join(lines)
...
def _build_backtest_summary(
    ...
    lines.append("- v4.0의 3/5규칙은 보수 모델입니다. 같은 날 손절가와 목표가가 모두 닿으면 손절을 먼저 본 것으로 계산합니다.")
    lines.append("- v4.0의 튜닝/손절특이점 리포트는 최근 백테스트 결과를 조건별 성과로 비교하는 참고용이며, 최소 4주~12주 이상 반복 검증이 필요합니다.")
    ...
    lines.append("- v4.0는 손절발생 종목의 공통점, S-CORE 3단계, 주차별 패턴 변화 감지를 함께 출력합니다.")
    return "\n".join(lines)
...
def run_closing_bet_backtest(
    ...
    if (safe_only or neutral_only or risk_only or core_only) and selected_df is not None and not selected_df.empty:
        try:
            if safe_only:
