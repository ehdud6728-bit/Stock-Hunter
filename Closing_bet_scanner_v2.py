...
CLOSING_BET_SCANNER_VERSION = 'G_MORALES_V4_4_9_46_MARKET5_SJ_TOP50_GATE_20260614'
...
def _send_results(hits: list, mins_left: int):
    ...
    header = (
        ...
        f"전체 후보 {len(hits)}개 | v4.4.9.46 MARKET5 + SJ TOP50 GATE: 14:40 후보발굴 + 15:03 실행/포기 압축 최종킥\n"
    ...
    def _sj6_checklist_context(h: dict) -> dict:
        """v4.4.9.46: 신정재 종가베팅 6요소(신·좁·깔·거·조·재) 최종 품질 체크.
    ...
    def _build_practical_block():
        ...
        # v4.4.9.46: 신정재 원본 방식에 가까운 '거래대금 상위 50 → SJ-W 확인' 별도 라이브 풀.
        ...
        def _final_kick_decision(h: dict, pattern_label: str) -> tuple[str, str, list]:
            ...
            # v4.4.9.46: L-MEGA 상한가성 잠금형은 과열 때문에 종가매수는 금지하지만,
        ...
        def _build_final_kick_block() -> str:
            ...
            sj_top50_line = f"SJ-TOP50: TOP50 {len(sj_top50_base_pool)} | SJ-W60+ {len(sj_top50_watch_pool)} | SJ-W75+ {len(sj_top50_prime_pool)} | 겹침강력 {len(sj_top50_overlap_pool)}"
            lines = [
                '🚨 [동시호가 최종킥 — v4.4.9.46 MARKET5 + SJ TOP50 GATE]',
            ...
            for i, (hh, label, decision, why, reasons) in enumerate(picked, 1):
                ...
                if 'SJ-TOP50' in str(label) and _live_good_filter_overlap(hh) and decision.startswith('🟡'):
                    ...
                    mental = '거래대금 TOP50 + SJ-W 60+ + 기존 고타율 필터가 겹친 후보입니다. 다만 종가 시장가 추격은 금지하고 지정가·다음날 지지확인 원칙을 유지합니다.'
        ...
        block = ['[🎯 실전 운영 후보 — v4.4.9.46 MARKET5 + SJ TOP50 GATE]']
        summary = (
        ...
        if PRACTICAL_SHOW_NEW_PATTERNS and lp_safe_pool:
            ...
            block.append('v4.4.9.46 운용 반영: LP는 1~5일 내 5000억+ 리더갭 이후 갭하단/전일종가/5·10일선 재지지 후보입니다. D1/D23/D45 타점을 구분하고, 다음날 시초가별 대응 시나리오를 함께 봅니다.')
            shown = 0
            used_codes = set()
        ...
        if safe_pool:
            ...
            block.append('v4.4.9.46 기준: S-SAFE는 ST30 통과형을 S-RECLAIM MEGA(5000억+)·3000~5000·PRIME(1000억+)·WATCH(300~1000)·LIQUIDITY RISK(<300)로 나누고, ST30 미통과형은 S-MOMENTUM CALM(1000억+ 거래량냉각)/PRIME/S-LIQUIDITY로 분리합니다. ST30은 하드필터가 아닙니다.')
        ...
        if PRACTICAL_SHOW_NEW_PATTERNS and it_accel_live_pool:
            ...
            block.append('v4.4.9.46 기준: IT는 ACCEL만 실시간 촉발 후보로 표시합니다. 단기 종가배팅이 아니라 20/40/60일 중기 1차 분할매집 타이밍이며, I-LEADER 주봉 사이클 라벨을 보조로 확인합니다.')
            used_codes = set()
            shown = 0
        ...
        if PRACTICAL_SHOW_I_MAIN and (i_accel_pool or i_core_pool or i_watch_pool or i_add_pool or i_confirm_pool):
            ...
            block.append('v4.4.9.46 기준: I-MAIN은 단기 종가배팅이 아니라 20/40/60거래일 중기 누적관찰입니다. ACCEL/CORE에 더해 I-LEADER 라벨로 주봉 사이클 초입·가속·후반·공세종말 위험을 함께 봅니다.')
            used_codes = set()
            shown_total = 0
            groups = [
...
def _v44933_i_outlier_adjustment_lines(i_en: pd.DataFrame, hold_days: int = 0) -> list[str]:
    ...
    try:
        ...
        if not any(len(g) for _, g in groups):
            return lines
        lines.append('[🧯 I-MAIN HORIZON GUARD + 이상치 보정 — v4.4.9.46]')
...
def _v439_i_samples_section(df: pd.DataFrame, top_n: int = 5) -> list[str]:
    ...
    try:
        ...
        if i.empty:
            return lines
        lines.append("[📋 I-MAIN 발생 샘플 — 종목별 중복제거 v4.4.9.46]")
...
def _v44937_lp_flow_line(sub: pd.DataFrame, label: str, min_n: int = 5) -> str:
    ...
    try:
        ...
        # v4.4.9.46: 감사 지표 정합성 보정. 흔들림/최대하락은 양수로 표시되면 오해가 커서 0 이하로 클립한다.
        try:
...
def _v44937_lp_explosion_audit_lines(df: pd.DataFrame) -> list[str]:
    ...
    try:
        ...
        lines.append('[🔥 LP POWER PRIME + STABLE 분리검증 — v4.4.9.46]')
...
def _v44934_s_momentum_core_lines(df: pd.DataFrame) -> list[str]:
    ...
    try:
        ...
        lines.append('[🔥 S-CALM / S-MOMENTUM / S-RECLAIM 핵심압축 — v4.4.9.46]')
    ...
    try:
        ...
        if not i_all.empty:
            try:
                ...
                pass
        lines.append('[🌍 GEO-RISK / 헤드라인 리스크 구간 해석 — v4.4.9.46]')
...
def _v440_l_mega_upper_followup_lines(df: pd.DataFrame) -> list[str]:
    """v4.4.9.46: L-MEGA 상한가성 후속관찰 감사 섹션."""
    lines: list[str] = []
    try:
        if df is None or df.empty:
            return lines
        ...
        if upper.empty and l_mega.empty:
            return lines
        lines.append('[💰 L-MEGA 상한가 후속관찰 감사 — v4.4.9.46]')
...
def _v44942_sj6_score_series(df: pd.DataFrame) -> pd.Series:
    """v4.4.9.46: 신·좁·깔·거·조·재 점수 산출. 후보 생성에는 절대 개입하지 않는 overlay 전용."""
    try:
        if df is None or df.empty:
...
def _v44942_count_parity_lines(df: pd.DataFrame, raw_count=None) -> list[str]:
    """v4.4.9.46: 후보 수 보존 감사. SJ-6가 하드필터처럼 작동하지 않았는지 텔레그램에 명시."""
    lines: list[str] = []
    try:
        ...
        if raw_unknown:
            ...
            rc = int(rc)
            rc_txt = f'{rc}건'
        lines.append('[🧮 COUNT PARITY / SJ OVERLAY 감사 — v4.4.9.46]')
...
def _v44945_sj_integrity_guard_lines(df: pd.DataFrame, raw_df: pd.DataFrame | None = None, raw_count=None, selected_csv: str = '', raw_csv: str = '', all_candidates: bool = False, start_date: str = '', end_date: str = '', hold_days: int = 0) -> list[str]:
    """v4.4.9.46: SJ-W가 후보 생성/삭제에 개입하지 않는지 더 직접적으로 보여주는 무결성 가드.
    ...
    try:
        ...
        lines.append('[🛡️ SJ-W 후보수 무결성 가드 — v4.4.9.46]')
        ...
        lines.append('- 운용해석: v45는 v44를 실전 기준으로 유지하되, SJ-W가 필터처럼 후보 수를 줄였는지 매번 눈으로 확인할 수 있게 만든 안전장치입니다.')
    except Exception as e:
...
def _v44942_sj_overlay_audit_lines(df: pd.DataFrame, raw_count=None) -> list[str]:
    """v4.4.9.46: SJ-6 단독성과보다 기존 패턴과의 교차 성과를 우선 출력."""
    lines: list[str] = []
    try:
        if df is None or df.empty:
            return lines
        ...
        lines.append('[📊 신정재 SJ-6 문맥형 OVERLAY 감사 — v4.4.9.46]')
...
def _v44943_first_num_series(df: pd.DataFrame, cols: list[str], default=0.0) -> pd.Series:
    """v4.4.9.46: 여러 컬럼 중 첫 유효 숫자 Series를 반환한다."""
    try:
...
def _v44943_stop_dist_series(df: pd.DataFrame) -> pd.Series:
    """v4.4.9.46: 진입가 대비 가장 가까운 실전 무효/지지선 거리(%)를 추정한다.
...
def _v44943_material_score_series(df: pd.DataFrame) -> pd.Series:
    """v4.4.9.46: 재료 강도 프록시. 글로벌/섹터/수급 데이터가 없으면 대금·재료대금 점수로 대체한다."""
    try:
        mat = _v439_num_series(df, 'i_material_proxy_score', 0)
        amount = _v439_num_series(df, 'amount_b')
        if 'leader_gap_amount_b' in df.columns:
...
def _v44943_threshold_fail_common_audit_lines(df: pd.DataFrame) -> list[str]:
    """v4.4.9.46: SJ 세부 임계값과 손절선행 후보 공통점을 감사한다."""
    lines: list[str] = []
    try:
        if df is None or df.empty:
            return lines
        lines.append('[🔬 SJ 임계값/손절공통점 감사 — v4.4.9.46]')
...
def _v44944_weighted_sj_score_series(df: pd.DataFrame) -> pd.Series:
    """v4.4.9.46: SJ 단순 6점제를 보조화하고 재료·이격·대금·종가위치에 가중치를 둔다.
...
def _v44944_weighted_context_audit_lines(df: pd.DataFrame) -> list[str]:
    """v4.4.9.46: SJ 가중점수와 손절위험 조합을 감사한다."""
    lines: list[str] = []
    try:
        if df is None or df.empty:
            return lines
        ...
        lines.append('[⚖️ SJ 가중 문맥점수 감사 — v4.4.9.46]')
...
def _v44946_amount_proxy_series(df: pd.DataFrame) -> pd.Series:
    """v4.4.9.46: 거래대금 TOP50 기준에 사용할 대금 프록시."""
    try:
        if df is None or df.empty:
            return pd.Series(dtype=float)
...
def _v44946_top50_amount_mask(df: pd.DataFrame, n: int = 50) -> pd.Series:
    """v4.4.9.46: 신정재 원본 방식에 가까운 '날짜별 거래대금 상위 50' 마스크.
...
def _v44946_s_momentum_mask(df: pd.DataFrame) -> pd.Series:
    """v4.4.9.46: S-MOMENTUM CALM/PRIME에 가까운 백테스트 마스크."""
...
def _v44946_good_filter_overlap_mask(df: pd.DataFrame) -> pd.Series:
    """v4.4.9.46: SJ-TOP50와 겹치면 강력추천 후보로 승격 검토할 기존 고타율 필터."""
    try:
        if df is None or df.empty:
            return pd.Series(dtype=bool)
        mode = _v439_str_series(df, 'mode')
...
def _v44946_sj_top50_audit_lines(df: pd.DataFrame) -> list[str]:
    """v4.4.9.46: 거래대금 상위 50개 안에서만 SJ-W를 다시 확인하는 별도 감사."""
    lines: list[str] = []
    try:
        if df is None or df.empty:
            return lines
        ...
        lines.append('[📊 SJ-TOP50 거래대금 상위 50 감사 — v4.4.9.46]')
        ...
        for label, mask in [
            ...
            ('SJ-TOP50 + SJ-W 75+', sj_top50_75),
            ('SJ-TOP50 품질조합: 재료2+·1000억+·종가70+·이격100~106', quality),
            ('🔥SJ-TOP50 + 고타율필터 겹침강력', overlap60),
            ('🔥SJ-TOP50 + 고타율필터 + SJ-W75', overlap75),
            ('SJ-TOP50 중 SJ-W RISK', risk_excluded),
        ]:
            sub = df[pd.Series(mask, index=df.index).fillna(False).astype(bool)]
            if len(sub) > 0:
                lines.append(_v439_short_trade_line(sub, label))
        lines.append('- 운용해석: SJ-TOP50 단독은 매수신호가 아니며, LP-SAFE·L5000억+·S-MOMENTUM 같은 기존 고타율 필터와 겹칠 때 “강력추천/우선확인”으로 올립니다. 겹치지 않는 TOP50+SJ는 관찰 후보입니다.')
    except Exception as e:
...
def _v44946_fetch_index_m5_state(index_code: str) -> dict:
    """v4.4.9.46: 지수 5일선 게이트용 간단 상태. 실패 시 ok=False."""
    try:
...
def _v44946_market5_gate_context() -> dict:
    """v4.4.9.46: KOSPI/KOSDAQ 5일선 기준 종베 시장 게이트."""
    try:
...
def _v44946_market5_gate_lines() -> list[str]:
    ...
    return [
        '[📈 MARKET-5 지수 5일선 종베 게이트 — v4.4.9.46]',
...
def _v44934_build_core_compact_backtest_report(original_report: str, selected_csv: str = '', raw_csv: str = '', start_date: str = '', end_date: str = '', hold_days: int = 0, top_per_strategy: int = 5, all_candidates: bool = False) -> str:
    ...
    try:
        ...
        sel_text = '전체후보' if all_candidates else f'날짜별 전략별 TOP{top_per_strategy}'
        lines: list[str] = []
        lines.append('🧪 v4.4.9.46 MARKET5 + SJ TOP50 GATE')
        lines.append(f'버전: {CLOSING_BET_SCANNER_VERSION}')
        lines.append(f'기간: {start_date} ~ {end_date} | 보유평가: 다음 {hold_days}거래일')
        ...
        lines.append('2순위 단기: L 5000억+ / S-MOMENTUM CALM·PRIME / S2 — SJ-TOP50 겹침이면 강력추천 후보로 승격 검토, 추격보다 지정가·지지확인')
        ...
        lines.append('단기 실행 우선순위는 LP-STABLE 안정형과 LP-POWER PRIME 강력추천 지정가를 분리하고, LP-SAFE > LP-POWER STABLE > L 5000억+ > S-MOMENTUM CALM > S-MOMENTUM PRIME/S2 > A-CONFIRM입니다. SJ-W는 후보를 삭제하지 않고 LP-SAFE 강등에도 쓰지 않으며, SJ-TOP50이 LP-SAFE·L5000억+·S-MOMENTUM과 겹치면 강력추천/우선확인 라벨로 올립니다. 중기 I-MAIN은 평가기간이 충분할 때만 숫자를 확정하고, 최근구간이 약하면 5MA/20MA 재지지 확인형으로 낮춥니다.')
        lines.append('')
        ...
        lines.append('- 신좁깔거조재/SJ-W/SJ-TOP50: SJ-W는 전체 후보 품질계기판, SJ-TOP50은 거래대금 상위50 전용 확인섹션입니다. TOP50+SJ-W60+가 LP-SAFE·L5000억+·S-MOMENTUM과 겹치면 강력추천 후보로 올리고, A/B/C는 SJ-W RISK·손절위험조합이면 강등합니다.')
        ...
        lines.append('- 과거 패치 버전명은 텔레그램 혼선을 줄이기 위해 v4.4.9.46/누적검증 문구로 정리했습니다.')
