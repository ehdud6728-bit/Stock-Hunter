...
# =============================================================
# closing_bet_scanner.py — 종가배팅 타점 스캐너 (v4.3.6 I-MAIN OPERATION OUTPUT VALIDATE)
# =============================================================
# 전략 구성
# A  : 돌파형 종가배팅
...
CLOSING_BET_SCANNER_VERSION = 'G_MORALES_V4_3_6_I_MAIN_OPERATION_OUTPUT_VALIDATE_20260515'
...
def _send_results(hits: list, mins_left: int):
    ...
    header = (
        ...
        f"⏰ {time_text}\n"
        f"전체 후보 {len(hits)}개 | v4.3.6 S/L/G/H + I-MAIN 통합 운영블록 우선 출력\n"
    ...
    def _build_practical_block():
        ...
        block = ['[🎯 실전 운영 후보 — v4.3.6 S/L/G/H + I-MAIN OUTPUT VALIDATE]']
        summary = (
        ...
        if PRACTICAL_SHOW_I_MAIN and (i_accel_pool or i_core_pool or i_watch_pool or i_add_pool or i_confirm_pool):
            ...
            block.append('v4.3.5 검증 반영: I-MAIN CORE는 20~40일 빠른 반응형, I-MAIN ACCEL은 40~60일 대시세형입니다. 단기 종가배팅이 아니라 분할매집/누적관찰로 봅니다.')
            used_codes = set()
            shown_total = 0
            groups = [
...
def _format_i_core_report(df: pd.DataFrame) -> str:
    try:
        ...
        lines = []
        lines.append("[📈 I-CORE 시대중심주 150/200일 시세분출 — v4.3.5 I-MAIN CORE/ACCEL·벤치마크]")
        if i.empty:
        ...
        lines.append("- 뉴스/재료 지속성은 이번 v4.3.0에서는 거래대금·월봉거래량·대표성 프록시로만 측정하며, 후속 버전에서 뉴스 누적 저장소와 연결합니다. v4.3.5는 MAIN 후보를 I-MAIN CORE/ACCEL/WATCH로 운용분류하고 KOSPI/KOSDAQ/선택벤치 초과수익을 함께 검증합니다.")
        return "\n".join(lines)
    except Exception as e:
