...
CLOSING_BET_SCANNER_VERSION = 'G_MORALES_V4_4_9_41_SJ6_CHECKLIST_LMEGA_WORDING_FIX_20260612'
...
def _send_results(hits: list, mins_left: int):
    ...
    header = (
        ...
        f"전체 후보 {len(hits)}개 | v4.4.9.41 SJ-6 + L-MEGA WORDING: 14:40 후보발굴 + 15:03 실행/포기 압축 최종킥\n"
    ...
    def _sj6_checklist_context(h: dict) -> dict:
        """v4.4.9.41: 신정재 종가베팅 6요소(신·좁·깔·거·조·재) 최종 품질 체크.
        ...
        try:
            ...
            narrow = bool((98.0 <= disparity20 <= 112.0) or (0.75 <= rr <= 1.60) or (mode == 'LP' and _safe_float(h.get('lp_volume_vs_gap', 0), 0) > 0))
            if mode == 'L' and _is_l_mega_upper_limit_followup(h):
                narrow = False
            # 깔: 종가권 양봉/윗꼬리 억제.
            clean = bool(close_loc >= 80.0 and wick <= 20.0)
            ...
            adjust = bool((mode == 'LP' and 1 <= days_after_gap <= 5) or (90 <= anchor_days <= 260) or (runup20 <= 35.0 and (disparity20 == 0 or disparity20 <= 115.0)))
            ...
            material = bool(material_kw or ('동반강세' in sector_txt) or use_amt >= 5000.0 or _safe_float(h.get('i_material_proxy_score', 0), 0) >= 3)
    ...
    def _build_practical_block():
        ...
        def _final_kick_decision(h: dict, pattern_label: str) -> tuple[str, str, list]:
            ...
            # v4.4.9.41: L-MEGA 상한가성 잠금형은 과열 때문에 종가매수는 금지하지만,
        ...
        def _build_final_kick_block() -> str:
            ...
            lines = [
                '🚨 [동시호가 최종킥 — v4.4.9.41 SJ-6 + L-MEGA WORDING]',
        ...
        block = ['[🎯 실전 운영 후보 — v4.4.9.41 SJ-6 + L-MEGA WORDING]']
        summary = (
        ...
        if PRACTICAL_SHOW_NEW_PATTERNS and lp_safe_pool:
            ...
            block.append('v4.4.9.41 운용 반영: LP는 1~5일 내 5000억+ 리더갭 이후 갭하단/전일종가/5·10일선 재지지 후보입니다. D1/D23/D45 타점을 구분하고, 다음날 시초가별 대응 시나리오를 함께 봅니다.')
            shown = 0
            used_codes = set()
        ...
        if safe_pool:
            ...
            block.append('v4.4.9.41 기준: S-SAFE는 ST30 통과형을 S-RECLAIM MEGA(5000억+)·3000~5000·PRIME(1000억+)·WATCH(300~1000)·LIQUIDITY RISK(<300)로 나누고, ST30 미통과형은 S-MOMENTUM CALM(1000억+ 거래량냉각)/PRIME/S-LIQUIDITY로 분리합니다. ST30은 하드필터가 아닙니다.')
        ...
        if PRACTICAL_SHOW_NEW_PATTERNS and it_accel_live_pool:
            ...
            block.append('v4.4.9.41 기준: IT는 ACCEL만 실시간 촉발 후보로 표시합니다. 단기 종가배팅이 아니라 20/40/60일 중기 1차 분할매집 타이밍이며, I-LEADER 주봉 사이클 라벨을 보조로 확인합니다.')
            used_codes = set()
            shown = 0
        ...
        if PRACTICAL_SHOW_I_MAIN and (i_accel_pool or i_core_pool or i_watch_pool or i_add_pool or i_confirm_pool):
            ...
            block.append('v4.4.9.41 기준: I-MAIN은 단기 종가배팅이 아니라 20/40/60거래일 중기 누적관찰입니다. ACCEL/CORE에 더해 I-LEADER 라벨로 주봉 사이클 초입·가속·후반·공세종말 위험을 함께 봅니다.')
            used_codes = set()
            shown_total = 0
            groups = [
...
def _v44933_i_outlier_adjustment_lines(i_en: pd.DataFrame, hold_days: int = 0) -> list[str]:
    ...
    try:
        ...
        if not any(len(g) for _, g in groups):
            return lines
        lines.append('[🧯 I-MAIN HORIZON GUARD + 이상치 보정 — v4.4.9.41]')
...
def _v439_i_samples_section(df: pd.DataFrame, top_n: int = 5) -> list[str]:
    ...
    try:
        ...
        if i.empty:
            return lines
        lines.append("[📋 I-MAIN 발생 샘플 — 종목별 중복제거 v4.4.9.41]")
...
def _v44937_lp_explosion_audit_lines(df: pd.DataFrame) -> list[str]:
    ...
    try:
        ...
        lines.append('[🔥 LP POWER PRIME + STABLE 분리검증 — v4.4.9.41]')
...
def _v44934_s_momentum_core_lines(df: pd.DataFrame) -> list[str]:
    ...
    try:
        ...
        lines.append('[🔥 S-CALM / S-MOMENTUM / S-RECLAIM 핵심압축 — v4.4.9.41]')
    ...
    try:
        ...
        if not i_all.empty:
            try:
                ...
                pass
        lines.append('[🌍 GEO-RISK / 헤드라인 리스크 구간 해석 — v4.4.9.41]')
...
def _v440_l_mega_upper_followup_lines(df: pd.DataFrame) -> list[str]:
    """v4.4.9.41: L-MEGA 상한가성 후속관찰 감사 섹션."""
    lines: list[str] = []
    try:
        if df is None or df.empty:
            return lines
        ...
        if l_all.empty:
            return lines
        upper = l_all[(amount.loc[l_all.index].ge(5000)) & (close_loc.loc[l_all.index].ge(98)) & (wick.loc[l_all.index].le(3))]
        ...
        if upper.empty and l_mega.empty:
            return lines
        lines.append('[💰 L-MEGA 상한가 후속관찰 감사 — v4.4.9.41]')
...
def _v44941_sj6_backtest_lines(df: pd.DataFrame) -> list[str]:
    """v4.4.9.41: 신·좁·깔·거·조·재 체크리스트 백테스트 요약."""
    lines: list[str] = []
    try:
        if df is None or df.empty:
            return lines
        ...
        narrow = (disp.between(98, 112) | rr.between(0.75, 1.60) | (mode.eq('LP') & lp_days.between(1, 5)))
        clean = close_loc.ge(80) & wick.le(20)
        money = amount.ge(1000) & (vol.ge(1.15) | vol50.ge(2.0) | amount.ge(3000))
        adjust = ((mode.eq('LP') & lp_days.between(1, 5)) | mat.ge(3) | (runup20.le(35) & (disp.eq(0) | disp.le(115))))
        material = (mat.ge(3) | amount.ge(5000))
        sj_score = sin.astype(int) + narrow.astype(int) + clean.astype(int) + money.astype(int) + adjust.astype(int) + material.astype(int)
        lines.append('[📊 신정재 신·좁·깔·거·조·재 체크리스트 — v4.4.9.41]')
        lines.append('- 목적: 신고가·좁은 이격·깔끔한 양봉·거래량/대금·조정기간·재료를 하드필터가 아니라 최종 품질 라벨로 확인합니다.')
        buckets = [
            ('🔥 SJ-PRIME 6/6', sj_score.ge(6)),
            ('✅ SJ-SAFE 5/6', sj_score.eq(5)),
            ('🟡 SJ-WATCH 4/6', sj_score.eq(4)),
            ('❌ SJ-FAIL 0~3/6', sj_score.le(3)),
        ]
        for label, mask in buckets:
            sub = df[mask.fillna(False)]
            if len(sub) > 0:
                lines.append(_v439_short_trade_line(sub, label))
        core = df[sj_score.ge(5).fillna(False)]
        if len(core) > 0:
            lines.append(_v439_short_trade_line(core, '신좁깔거조재 5/6+ 전체'))
        lines.append('- 운용해석: 5/6 이상이면 기존 LP/L/S 후보의 추천강도를 올리되, L-MEGA 상한가후속처럼 이격이 벌어진 종목은 매수방식은 계속 보수적으로 둡니다.')
    except Exception as e:
        lines.append(f'[SJ-6 체크리스트 리포트 오류] {type(e).__name__}: {e}')
    return lines
...
def _v44934_build_core_compact_backtest_report(original_report: str, selected_csv: str = '', raw_csv: str = '', start_date: str = '', end_date: str = '', hold_days: int = 0, top_per_strategy: int = 5, all_candidates: bool = False) -> str:
    ...
    try:
        ...
        sel_text = '전체후보' if all_candidates else f'날짜별 전략별 TOP{top_per_strategy}'
        lines: list[str] = []
        lines.append('🧪 v4.4.9.41 SJ-6 CHECKLIST + L-MEGA WORDING FIX')
        lines.append(f'버전: {CLOSING_BET_SCANNER_VERSION}')
        lines.append(f'기간: {start_date} ~ {end_date} | 보유평가: 다음 {hold_days}거래일')
        ...
        lines.append('2순위 단기: L 5000억+ / S-MOMENTUM CALM·PRIME / S2 — 신좁깔거조재 5/6+면 우대, 추격보다 지정가·지지확인')
        ...
        lines.append('단기 실행 우선순위는 LP-STABLE 안정형과 LP-POWER PRIME 강력추천 지정가를 분리하고, LP-SAFE > LP-POWER STABLE > L 5000억+ > S-MOMENTUM CALM > S-MOMENTUM PRIME/S2 > A-CONFIRM입니다. 중기 I-MAIN은 평가기간이 충분할 때만 숫자를 확정하고, 최근구간이 약하면 5MA/20MA 재지지 확인형으로 낮춥니다.')
        lines.append('')
        ...
        if l_upper_lines:
            lines.append('')
            lines += l_upper_lines
        sj6_lines = _v44941_sj6_backtest_lines(df)
        if sj6_lines:
            lines.append('')
            lines += sj6_lines
        ...
        lines.append('- 단기 LP: 5000억+ 리더갭 이후 1~5일 눌림재지지. D1/D23/D45 타점과 다음날 시초가별 대응을 같이 확인. 신좁깔거조재 5/6+면 추천강도 우대.')
        ...
        lines.append('- 신좁깔거조재: 6/6은 SJ-PRIME, 5/6은 SJ-SAFE, 4/6은 SJ-WATCH로 표시합니다. 단, 시황·수급이 약하면 비중을 낮춥니다.')
        ...
        lines.append('- 과거 패치 버전명은 텔레그램 혼선을 줄이기 위해 v4.4.9.41/누적검증 문구로 정리했습니다.')
        compact = '\n'.join([str(x) for x in lines if x is not None])
        compact = _v44934_cleanup_legacy_labels(compact)
        try:
            out_path = LOG_DIR / 'closing_bet_backtest_summary_v4_4_9_41_sj6_lmega_wording_fix.txt'
