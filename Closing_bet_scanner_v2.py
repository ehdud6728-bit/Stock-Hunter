...
CLOSING_BET_SCANNER_VERSION = 'G_MORALES_V4_4_9_42_SJ_OVERLAY_AUDIT_COUNT_PARITY_FIX_20260612'
...
def _send_results(hits: list, mins_left: int):
    ...
    header = (
        ...
        f"전체 후보 {len(hits)}개 | v4.4.9.42 SJ OVERLAY + COUNT PARITY: 14:40 후보발굴 + 15:03 실행/포기 압축 최종킥\n"
    ...
    def _sj6_checklist_context(h: dict) -> dict:
        """v4.4.9.42: 신정재 종가베팅 6요소(신·좁·깔·거·조·재) 최종 품질 체크.
    ...
    def _build_practical_block():
        ...
        def _final_kick_decision(h: dict, pattern_label: str) -> tuple[str, str, list]:
            ...
            # v4.4.9.42: L-MEGA 상한가성 잠금형은 과열 때문에 종가매수는 금지하지만,
        ...
        def _build_final_kick_block() -> str:
            ...
            lines = [
                '🚨 [동시호가 최종킥 — v4.4.9.42 SJ OVERLAY + COUNT PARITY]',
        ...
        block = ['[🎯 실전 운영 후보 — v4.4.9.42 SJ OVERLAY + COUNT PARITY]']
        summary = (
        ...
        if PRACTICAL_SHOW_NEW_PATTERNS and lp_safe_pool:
            ...
            block.append('v4.4.9.42 운용 반영: LP는 1~5일 내 5000억+ 리더갭 이후 갭하단/전일종가/5·10일선 재지지 후보입니다. D1/D23/D45 타점을 구분하고, 다음날 시초가별 대응 시나리오를 함께 봅니다.')
            shown = 0
            used_codes = set()
        ...
        if safe_pool:
            ...
            block.append('v4.4.9.42 기준: S-SAFE는 ST30 통과형을 S-RECLAIM MEGA(5000억+)·3000~5000·PRIME(1000억+)·WATCH(300~1000)·LIQUIDITY RISK(<300)로 나누고, ST30 미통과형은 S-MOMENTUM CALM(1000억+ 거래량냉각)/PRIME/S-LIQUIDITY로 분리합니다. ST30은 하드필터가 아닙니다.')
        ...
        if PRACTICAL_SHOW_NEW_PATTERNS and it_accel_live_pool:
            ...
            block.append('v4.4.9.42 기준: IT는 ACCEL만 실시간 촉발 후보로 표시합니다. 단기 종가배팅이 아니라 20/40/60일 중기 1차 분할매집 타이밍이며, I-LEADER 주봉 사이클 라벨을 보조로 확인합니다.')
            used_codes = set()
            shown = 0
        ...
        if PRACTICAL_SHOW_I_MAIN and (i_accel_pool or i_core_pool or i_watch_pool or i_add_pool or i_confirm_pool):
            ...
            block.append('v4.4.9.42 기준: I-MAIN은 단기 종가배팅이 아니라 20/40/60거래일 중기 누적관찰입니다. ACCEL/CORE에 더해 I-LEADER 라벨로 주봉 사이클 초입·가속·후반·공세종말 위험을 함께 봅니다.')
            used_codes = set()
            shown_total = 0
            groups = [
...
def _v44933_i_outlier_adjustment_lines(i_en: pd.DataFrame, hold_days: int = 0) -> list[str]:
    ...
    try:
        ...
        if not any(len(g) for _, g in groups):
            return lines
        lines.append('[🧯 I-MAIN HORIZON GUARD + 이상치 보정 — v4.4.9.42]')
...
def _v439_i_samples_section(df: pd.DataFrame, top_n: int = 5) -> list[str]:
    ...
    try:
        ...
        if i.empty:
            return lines
        lines.append("[📋 I-MAIN 발생 샘플 — 종목별 중복제거 v4.4.9.42]")
...
def _v44937_lp_flow_line(sub: pd.DataFrame, label: str, min_n: int = 5) -> str:
    ...
    try:
        ...
        # v4.4.9.42: 감사 지표 정합성 보정. 흔들림/최대하락은 양수로 표시되면 오해가 커서 0 이하로 클립한다.
        try:
...
def _v44937_lp_explosion_audit_lines(df: pd.DataFrame) -> list[str]:
    ...
    try:
        ...
        lines.append('[🔥 LP POWER PRIME + STABLE 분리검증 — v4.4.9.42]')
...
def _v44934_s_momentum_core_lines(df: pd.DataFrame) -> list[str]:
    ...
    try:
        ...
        lines.append('[🔥 S-CALM / S-MOMENTUM / S-RECLAIM 핵심압축 — v4.4.9.42]')
    ...
    try:
        ...
        if not i_all.empty:
            try:
                ...
                pass
        lines.append('[🌍 GEO-RISK / 헤드라인 리스크 구간 해석 — v4.4.9.42]')
...
def _v440_l_mega_upper_followup_lines(df: pd.DataFrame) -> list[str]:
    """v4.4.9.42: L-MEGA 상한가성 후속관찰 감사 섹션."""
    lines: list[str] = []
    try:
        if df is None or df.empty:
            return lines
        ...
        if upper.empty and l_mega.empty:
            return lines
        lines.append('[💰 L-MEGA 상한가 후속관찰 감사 — v4.4.9.42]')
...
def _v44942_sj6_score_series(df: pd.DataFrame) -> pd.Series:
    """v4.4.9.42: 신·좁·깔·거·조·재 점수 산출. 후보 생성에는 절대 개입하지 않는 overlay 전용."""
    try:
        if df is None or df.empty:
...
def _v44942_count_parity_lines(df: pd.DataFrame, raw_count=None) -> list[str]:
    """v4.4.9.42: 후보 수 보존 감사. SJ-6가 하드필터처럼 작동하지 않았는지 텔레그램에 명시."""
    lines: list[str] = []
    try:
        ...
        if raw_unknown:
            ...
            rc = int(rc)
            rc_txt = f'{rc}건'
        lines.append('[🧮 COUNT PARITY / SJ OVERLAY 감사 — v4.4.9.42]')
...
def _v44942_sj_overlay_audit_lines(df: pd.DataFrame, raw_count=None) -> list[str]:
    """v4.4.9.42: SJ-6 단독성과보다 기존 패턴과의 교차 성과를 우선 출력."""
    lines: list[str] = []
    try:
        if df is None or df.empty:
            return lines
        ...
        lines.append('[📊 신정재 SJ-6 OVERLAY 감사 — v4.4.9.42]')
        lines.append('- 목적: 신·좁·깔·거·조·재를 단독 매수신호가 아니라 기존 LP/L/S 후보의 품질 가점·강등 라벨로 검증합니다.')
        for label, mask in [
        ...
        lines.append('- 교차검증: SJ 5/6+가 붙은 기존 강패턴만 추천강도를 올립니다.')
        cross_items = []
        if len(lp_idx) > 0:
        ...
        for label, mask in cross_items:
            ...
            if len(sub) > 0:
                lines.append(_v439_short_trade_line(sub, label))
        lines.append('- 운용해석: SJ 5/6+는 추천강도 가점, SJ 0~3은 강등 신호입니다. 단, 상한가성 L-MEGA는 SJ가 좋아도 신규 종가추격이 아니라 다음날 기준선 확인형으로 둡니다.')
    except Exception as e:
        lines.append(f'[SJ OVERLAY 감사 오류] {type(e).__name__}: {e}')
    return lines


# 하위호환: 기존 호출부가 남아 있어도 v42 overlay 감사로 연결한다.
...
def _v44934_build_core_compact_backtest_report(original_report: str, selected_csv: str = '', raw_csv: str = '', start_date: str = '', end_date: str = '', hold_days: int = 0, top_per_strategy: int = 5, all_candidates: bool = False) -> str:
    ...
    try:
        ...
        sel_text = '전체후보' if all_candidates else f'날짜별 전략별 TOP{top_per_strategy}'
        lines: list[str] = []
        lines.append('🧪 v4.4.9.42 SJ OVERLAY AUDIT + COUNT PARITY FIX')
        lines.append(f'버전: {CLOSING_BET_SCANNER_VERSION}')
        lines.append(f'기간: {start_date} ~ {end_date} | 보유평가: 다음 {hold_days}거래일')
        ...
        lines.append('- 과거 패치 버전명은 텔레그램 혼선을 줄이기 위해 v4.4.9.42/누적검증 문구로 정리했습니다.')
        compact = '\n'.join([str(x) for x in lines if x is not None])
        compact = _v44934_cleanup_legacy_labels(compact)
        try:
            out_path = LOG_DIR / 'closing_bet_backtest_summary_v4_4_9_42_sj_overlay_audit_count_parity_fix.txt'
