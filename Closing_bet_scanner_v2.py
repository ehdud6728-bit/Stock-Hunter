...
CLOSING_BET_SCANNER_VERSION = 'G_MORALES_V4_4_9_44_SJ_WEIGHTED_CONTEXT_SCORE_20260613'
...
def _send_results(hits: list, mins_left: int):
    ...
    header = (
        ...
        f"전체 후보 {len(hits)}개 | v4.4.9.44 SJ WEIGHTED CONTEXT: 14:40 후보발굴 + 15:03 실행/포기 압축 최종킥\n"
    ...
    def _sj6_checklist_context(h: dict) -> dict:
        """v4.4.9.44: 신정재 종가베팅 6요소(신·좁·깔·거·조·재) 최종 품질 체크.
    ...
    def _build_practical_block():
        ...
        def _final_kick_decision(h: dict, pattern_label: str) -> tuple[str, str, list]:
            ...
            # v4.4.9.44: L-MEGA 상한가성 잠금형은 과열 때문에 종가매수는 금지하지만,
        ...
        def _build_final_kick_block() -> str:
            ...
            lines = [
                '🚨 [동시호가 최종킥 — v4.4.9.44 SJ WEIGHTED CONTEXT]',
        ...
        block = ['[🎯 실전 운영 후보 — v4.4.9.44 SJ WEIGHTED CONTEXT]']
        summary = (
        ...
        if PRACTICAL_SHOW_NEW_PATTERNS and lp_safe_pool:
            ...
            block.append('v4.4.9.44 운용 반영: LP는 1~5일 내 5000억+ 리더갭 이후 갭하단/전일종가/5·10일선 재지지 후보입니다. D1/D23/D45 타점을 구분하고, 다음날 시초가별 대응 시나리오를 함께 봅니다.')
            shown = 0
            used_codes = set()
        ...
        if safe_pool:
            ...
            block.append('v4.4.9.44 기준: S-SAFE는 ST30 통과형을 S-RECLAIM MEGA(5000억+)·3000~5000·PRIME(1000억+)·WATCH(300~1000)·LIQUIDITY RISK(<300)로 나누고, ST30 미통과형은 S-MOMENTUM CALM(1000억+ 거래량냉각)/PRIME/S-LIQUIDITY로 분리합니다. ST30은 하드필터가 아닙니다.')
        ...
        if PRACTICAL_SHOW_NEW_PATTERNS and it_accel_live_pool:
            ...
            block.append('v4.4.9.44 기준: IT는 ACCEL만 실시간 촉발 후보로 표시합니다. 단기 종가배팅이 아니라 20/40/60일 중기 1차 분할매집 타이밍이며, I-LEADER 주봉 사이클 라벨을 보조로 확인합니다.')
            used_codes = set()
            shown = 0
        ...
        if PRACTICAL_SHOW_I_MAIN and (i_accel_pool or i_core_pool or i_watch_pool or i_add_pool or i_confirm_pool):
            ...
            block.append('v4.4.9.44 기준: I-MAIN은 단기 종가배팅이 아니라 20/40/60거래일 중기 누적관찰입니다. ACCEL/CORE에 더해 I-LEADER 라벨로 주봉 사이클 초입·가속·후반·공세종말 위험을 함께 봅니다.')
            used_codes = set()
            shown_total = 0
            groups = [
...
def _v44933_i_outlier_adjustment_lines(i_en: pd.DataFrame, hold_days: int = 0) -> list[str]:
    ...
    try:
        ...
        if not any(len(g) for _, g in groups):
            return lines
        lines.append('[🧯 I-MAIN HORIZON GUARD + 이상치 보정 — v4.4.9.44]')
...
def _v439_i_samples_section(df: pd.DataFrame, top_n: int = 5) -> list[str]:
    ...
    try:
        ...
        if i.empty:
            return lines
        lines.append("[📋 I-MAIN 발생 샘플 — 종목별 중복제거 v4.4.9.44]")
...
def _v44937_lp_flow_line(sub: pd.DataFrame, label: str, min_n: int = 5) -> str:
    ...
    try:
        ...
        # v4.4.9.44: 감사 지표 정합성 보정. 흔들림/최대하락은 양수로 표시되면 오해가 커서 0 이하로 클립한다.
        try:
...
def _v44937_lp_explosion_audit_lines(df: pd.DataFrame) -> list[str]:
    ...
    try:
        ...
        lines.append('[🔥 LP POWER PRIME + STABLE 분리검증 — v4.4.9.44]')
...
def _v44934_s_momentum_core_lines(df: pd.DataFrame) -> list[str]:
    ...
    try:
        ...
        lines.append('[🔥 S-CALM / S-MOMENTUM / S-RECLAIM 핵심압축 — v4.4.9.44]')
    ...
    try:
        ...
        if not i_all.empty:
            try:
                ...
                pass
        lines.append('[🌍 GEO-RISK / 헤드라인 리스크 구간 해석 — v4.4.9.44]')
...
def _v440_l_mega_upper_followup_lines(df: pd.DataFrame) -> list[str]:
    """v4.4.9.44: L-MEGA 상한가성 후속관찰 감사 섹션."""
    lines: list[str] = []
    try:
        if df is None or df.empty:
            return lines
        ...
        if upper.empty and l_mega.empty:
            return lines
        lines.append('[💰 L-MEGA 상한가 후속관찰 감사 — v4.4.9.44]')
...
def _v44942_sj6_score_series(df: pd.DataFrame) -> pd.Series:
    """v4.4.9.44: 신·좁·깔·거·조·재 점수 산출. 후보 생성에는 절대 개입하지 않는 overlay 전용."""
    try:
        if df is None or df.empty:
...
def _v44942_count_parity_lines(df: pd.DataFrame, raw_count=None) -> list[str]:
    """v4.4.9.44: 후보 수 보존 감사. SJ-6가 하드필터처럼 작동하지 않았는지 텔레그램에 명시."""
    lines: list[str] = []
    try:
        ...
        if raw_unknown:
            ...
            rc = int(rc)
            rc_txt = f'{rc}건'
        lines.append('[🧮 COUNT PARITY / SJ OVERLAY 감사 — v4.4.9.44]')
...
def _v44942_sj_overlay_audit_lines(df: pd.DataFrame, raw_count=None) -> list[str]:
    """v4.4.9.44: SJ-6 단독성과보다 기존 패턴과의 교차 성과를 우선 출력."""
    lines: list[str] = []
    try:
        if df is None or df.empty:
            return lines
        ...
        lines.append('[📊 신정재 SJ-6 문맥형 OVERLAY 감사 — v4.4.9.44]')
...
def _v44943_first_num_series(df: pd.DataFrame, cols: list[str], default=0.0) -> pd.Series:
    """v4.4.9.44: 여러 컬럼 중 첫 유효 숫자 Series를 반환한다."""
    try:
...
def _v44943_stop_dist_series(df: pd.DataFrame) -> pd.Series:
    """v4.4.9.44: 진입가 대비 가장 가까운 실전 무효/지지선 거리(%)를 추정한다.
...
def _v44943_material_score_series(df: pd.DataFrame) -> pd.Series:
    """v4.4.9.44: 재료 강도 프록시. 글로벌/섹터/수급 데이터가 없으면 대금·재료대금 점수로 대체한다."""
    try:
        mat = _v439_num_series(df, 'i_material_proxy_score', 0)
        amount = _v439_num_series(df, 'amount_b')
        if 'leader_gap_amount_b' in df.columns:
...
def _v44943_threshold_fail_common_audit_lines(df: pd.DataFrame) -> list[str]:
    """v4.4.9.44: SJ 세부 임계값과 손절선행 후보 공통점을 감사한다."""
    lines: list[str] = []
    try:
        if df is None or df.empty:
            return lines
        lines.append('[🔬 SJ 임계값/손절공통점 감사 — v4.4.9.44]')
...
def _v44944_weighted_sj_score_series(df: pd.DataFrame) -> pd.Series:
    """v4.4.9.44: SJ 단순 6점제를 보조화하고 재료·이격·대금·종가위치에 가중치를 둔다.
...
def _v44944_weighted_context_audit_lines(df: pd.DataFrame) -> list[str]:
    """v4.4.9.44: SJ 가중점수와 손절위험 조합을 감사한다."""
    lines: list[str] = []
    try:
        if df is None or df.empty:
            return lines
        ...
        lines.append('[⚖️ SJ 가중 문맥점수 감사 — v4.4.9.44]')
...
def _v44934_build_core_compact_backtest_report(original_report: str, selected_csv: str = '', raw_csv: str = '', start_date: str = '', end_date: str = '', hold_days: int = 0, top_per_strategy: int = 5, all_candidates: bool = False) -> str:
    ...
    try:
        ...
        sel_text = '전체후보' if all_candidates else f'날짜별 전략별 TOP{top_per_strategy}'
        lines: list[str] = []
        lines.append('🧪 v4.4.9.44 SJ WEIGHTED CONTEXT SCORE')
        lines.append(f'버전: {CLOSING_BET_SCANNER_VERSION}')
        lines.append(f'기간: {start_date} ~ {end_date} | 보유평가: 다음 {hold_days}거래일')
        ...
        lines.append('2순위 단기: L 5000억+ / S-MOMENTUM CALM·PRIME / S2 — SJ-W 가중점수는 품질가점·위험강등 전용, 추격보다 지정가·지지확인')
        ...
        lines.append('단기 실행 우선순위는 LP-STABLE 안정형과 LP-POWER PRIME 강력추천 지정가를 분리하고, LP-SAFE > LP-POWER STABLE > L 5000억+ > S-MOMENTUM CALM > S-MOMENTUM PRIME/S2 > A-CONFIRM입니다. SJ-W는 LP-SAFE 강등에는 쓰지 않고, L/S/A/B/C의 품질가점·위험강등에만 문맥 적용합니다. 중기 I-MAIN은 평가기간이 충분할 때만 숫자를 확정하고, 최근구간이 약하면 5MA/20MA 재지지 확인형으로 낮춥니다.')
        lines.append('')
        ...
        lines.append('- 과거 패치 버전명은 텔레그램 혼선을 줄이기 위해 v4.4.9.44/누적검증 문구로 정리했습니다.')
        compact = '\n'.join([str(x) for x in lines if x is not None])
        compact = _v44934_cleanup_legacy_labels(compact)
        try:
            out_path = LOG_DIR / 'closing_bet_backtest_summary_v4_4_9_44_sj_weighted_context_score.txt'
