...
def _load_df(code: str, lookback_days: int = 420) -> pd.DataFrame:
...
def _format_hit(hit: dict, rank: int = 0, mins_left: int = 0) -> str:
    ...
    mode_label = str(_g("mode_label", default=strategy if strategy else "종가배팅")) or (strategy if strategy else "종가배팅")
    ...
    if hit.get("mode") in ("B1", "B2") and hit.get("maejip_chart"):
        chart_str = f"
- 매집차트: {hit.get('maejip_chart')}"

    lines = []
    title = f"{rank}) " if rank > 0 else ""
    header = f"{title}{mode_label} {grade} [{name}({code})] {int(close):,}원{idx_str}"
    lines.append(header)
    lines.append(f"- 점수: {score:.1f} | 거래량비: {vol_ratio:.2f} | 거래대금: {amount_b:.1f}억")
...
"
    ...
    def _safe_grade(hit):
        return str(hit.get("grade") or hit.get("등급") or "B").strip()
    ...
    def _priority(h):
        grade = _safe_grade(h)
        g_rank = 0 if "완전체" in grade else (1 if grade.startswith("A") else 2)
        return (
            g_rank,
            -_safe_score(h),
    ...
    complete_hits = [x for x in hits if str(x.get("complete", "")).strip() in ("1", "True", "true") or bool(x.get("complete", False))]
    a_grade_hits = [x for x in hits if _safe_grade(x).startswith("A")]
    b_grade_hits = [x for x in hits if _safe_grade(x).startswith("B")]

    log_info(f"돌파형 raw 수: {len(hits_a)}")
    log_info(f"ENV엄격형 raw 수: {len(hits_b1)}")
    log_info(f"BB확장형 raw 수: {len(hits_b2)}")
...
"
    ...
    def _append_block(title: str, items: list, tag: str):
        block_lines = [f"
── {title} ──
"]
    ...
    parts.append(_append_block("돌파형(A) TOP5", hits_a, "A"))
    parts.append(_append_block("ENV엄격형(B1) TOP5", hits_b1, "B1"))
    parts.append(_append_block("BB확장형(B2) TOP5", hits_b2, "B2"))
...
" if current else "") + part
    ...
    for i, msg in enumerate(messages, 1):
        log_info(f"텔레그램 전송 {i}/{len(messages)} | 길이={len(msg)}")
