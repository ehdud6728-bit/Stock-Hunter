...
CLOSING_BET_SCANNER_VERSION = 'G_MORALES_V4_4_9_34_REPORT_COMPACT_LEGACY_LABEL_CLEANUP_20260608'
...
def _send_results(hits: list, mins_left: int):
    ...
    header = (
        ...
        f"전체 후보 {len(hits)}개 | v4.4.9.34 I-OUTLIER + S-MOMENTUM POLISH: 14:40 후보발굴 + 15:03 실행/포기 압축 최종킥\n"
    ...
    def _s_safe_live_class(h: dict) -> dict:
        ...
        if amount_b >= 1000 and quality:
            ...
            return {
                ...
                'guide': 'ST30 조정형은 아니지만 1000억+ 유동성과 품질이 살아 있는 추세지속형입니다. LP/L/A-CONFIRM이 없을 때 보조 후보로만 보고, 추격보다 지정가·다음날 지지 확인을 우선합니다.',
            }
    ...
    def _build_practical_block():
        ...
        def _build_final_kick_block() -> str:
            ...
            def _final_kick_top_line(picked_rows, none_text='없음'):
                ...
                return f'{top_name} — {top_decision} | {top_label}'

            # v4.4.9.32: 최종킥 상단은 유지하고 I-LEADER 주도주 라이프사이클 라벨을 중기 후보에 붙인다.
            # 한 종목만 할 때는 단기 메인만 보고, S·L 보조 후보는 소액/지정가로만 읽히게 한다.
            main_short_line = _final_kick_top_line(picked_short[:1], '없음')
            aux_short_line = _final_kick_top_line(picked_short[1:], '없음')
            ...
            s_label_line = (
                ...
                f"🔥MOM1000+ {len(s_momentum_prime_pool)} | 🟡MOM3000~5000 {len(s_momentum_3000_pool)} | "
                f"🟡MOM-WATCH {len(s_momentum_watch_pool)} | 🟡NEUTRAL {len(neutral_pool)} | ⚠️RISK {len(s_liquidity_risk_pool)}"
            )
            s_exec_count = len(s_reclaim_exec_pool) + len(s_momentum_prime_pool) + len(s_momentum_3000_pool) + len(s_momentum_watch_pool)
        ...
        block = ['[🎯 실전 운영 후보 — v4.4.9.34 I-OUTLIER + S-MOMENTUM POLISH]']
        summary = (
            f"요약: 🥇LP-SAFE {len(lp_safe_pool)}개 | 🔁LP-WATCH {len(lp_watch_pool)}개 | ⚠️LP-RISK {len(lp_risk_pool)}개 | 👑L-SAFE {len(l_safe_pool)}개 | 🔒SLOCK검증/숨김 {len(slock_pool)}개 | ⚡IT-ACCEL {len(it_accel_live_pool)}개 | 🌊L-TAIL {len(l_tail_pool)}개 | 💰L-MEGA {len(l_mega_pool)}개 | 💎S-RE-MEGA {len(s_reclaim_mega_pool)}개 | 🟢S-RE3000 {len(s_reclaim_3000_pool)}개 | 🟢S-RE-PRIME {len(s_reclaim_prime_pool)}개 | 🟡S-RE-WATCH {len(s_reclaim_watch_pool)}개 | 🔥S-MOMENTUM PRIME {len(s_momentum_prime_pool)}개 | 🟡S-3000~5000 {len(s_momentum_3000_pool)}개 | 🟡S-WATCH {len(s_momentum_watch_pool)}개 | ⚠️S-유동성주의 {len(s_liquidity_risk_pool)}개 | 🟡S관찰 {len(neutral_pool)}개 | 🟢G-SAFE {len(g_safe_pool)}개 | "
        ...
        block.append('[🚦 v4.4.9.34 실시간 운용판정]\nFAST 실전 출력은 LP-D23/D45, L 5000억+, A-CONFIRM PRIME/CALM, S-RECLAIM/S-MOMENTUM/S-NEUTRAL, A-RETEST CORE②, IT/I-MAIN ACCEL만 우선합니다. S-SAFE는 ST30 통과형도 거래대금별 S-RECLAIM MEGA/3000~5000/PRIME/WATCH/RISK로 나누고, ST30 미통과형은 S-MOMENTUM PRIME(1000억+ 품질군)/RISK로 분리합니다. 넓은 A/C/B/H/SLOCK은 기본 숨김 또는 연구/관찰입니다.\n직장인 모드: 장중 5분봉 확인보다 지정가·추격금지·+3/+5 자동익절·무효가를 먼저 봅니다. 미체결은 포기하는 방식으로 운용합니다.\nA-RETEST CORE②는 당일 보조 후보, A-CONFIRM은 다음날 전일고가 회복·양봉·거래대금 유지 확인형입니다. PRIME은 힘이 남은 우선 확인형, CALM은 안정형 보조 후보입니다.\n시장/섹터 압력이 부담이면 같은 후보라도 소액·확인 후 진입으로 낮추고, 지지선 종가이탈+거래량 증가 음봉은 정상 흔들림이 아니라 위험신호로 봅니다.')
        block.append('')
        ...
        if safe_pool:
            ...
            block.append('v4.4.9.34 기준: S-SAFE는 ST30 통과형을 S-RECLAIM MEGA(5000억+)·3000~5000·PRIME(1000억+)·WATCH(300~1000)·LIQUIDITY RISK(<300)로 나누고, ST30 미통과형은 S-MOMENTUM PRIME(1000억+ 품질군)/S-LIQUIDITY로 분리합니다. ST30은 하드필터가 아닙니다.')
        ...
        if PRACTICAL_SHOW_NEW_PATTERNS and it_accel_live_pool:
            ...
            block.append('v4.4.9.34 기준: IT는 ACCEL만 실시간 촉발 후보로 표시합니다. 단기 종가배팅이 아니라 20/40/60일 중기 1차 분할매집 타이밍이며, I-LEADER 주봉 사이클 라벨을 보조로 확인합니다.')
            used_codes = set()
            shown = 0
        ...
        if PRACTICAL_SHOW_I_MAIN and (i_accel_pool or i_core_pool or i_watch_pool or i_add_pool or i_confirm_pool):
            ...
            block.append('v4.4.9.34 기준: I-MAIN은 단기 종가배팅이 아니라 20/40/60거래일 중기 누적관찰입니다. ACCEL/CORE에 더해 I-LEADER 라벨로 주봉 사이클 초입·가속·후반·공세종말 위험을 함께 봅니다.')
            used_codes = set()
            shown_total = 0
            groups = [
...
def _v44933_i_adjusted_line(sub: pd.DataFrame, label: str, mode: str = 'raw') -> str:
    ...
    try:
        ...
        if mode == 'exclude100' and 'i_ret_close_60d' in work.columns:
            ...
            a20, a40, a60 = _v44933_i_mean_cols(work, ['i_ret_close_20d', 'i_ret_close_40d', 'i_ret_close_60d'])
            return f"- {label} +100%초과 제외: {len(work)}건 | 20d {a20:.2f}% | 40d {a40:.2f}% | 60d {a60:.2f}% | 제외 {out_n}건"
        if mode == 'cap100':
            a20, a40, a60 = _v44933_i_mean_cols(work, ['i_ret_close_20d', 'i_ret_close_40d', 'i_ret_close_60d'], cap_upper=100.0)
            return f"- {label} +100%상한 보정: {len(work)}건 | 20d {a20:.2f}% | 40d {a40:.2f}% | 60d {a60:.2f}%"
        if mode == 'excess':
            ...
            for pref, shown in prefixes:
                ...
                if any(c in work.columns for c in cols):
                    a20, a40, a60 = _v44933_i_mean_cols(work, cols)
                    return f"- {label} 지수대비({shown}): {len(work)}건 | 20d {a20:.2f}% | 40d {a40:.2f}% | 60d {a60:.2f}%"
            return f"- {label} 지수대비: 초과수익 컬럼 없음"
        return _v439_i_line(work, label)
    except Exception as e:
        return f"- {label} 보정 오류: {type(e).__name__}: {e}"


def _v44933_i_outlier_adjustment_lines(i_en: pd.DataFrame) -> list[str]:
    ...
    try:
        ...
        if not any(len(g) for _, g in groups):
            return lines
        lines.append('[🧯 I-MAIN 이상치 보정 성과 — v4.4.9.34]')
        lines.append('- 목적: +100% 이상 이벤트/데이터성 급등이 평균을 과장하는지 원본·제외·상한·지수대비로 나눠 확인합니다.')
        for label, sub in groups:
            if sub is None or sub.empty:
                continue
            lines.append(_v44933_i_adjusted_line(sub, label, 'raw'))
            lines.append(_v44933_i_adjusted_line(sub, label, 'exclude100'))
            lines.append(_v44933_i_adjusted_line(sub, label, 'cap100'))
            ex = _v44933_i_adjusted_line(sub, label, 'excess')
            if '컬럼 없음' not in ex:
                lines.append(ex)
...
def _v439_i_samples_section(df: pd.DataFrame, top_n: int = 5) -> list[str]:
    ...
    try:
        ...
        if i.empty:
            return lines
        lines.append("[📋 I-MAIN 발생 샘플 — 종목별 중복제거 v4.4.9.34]")
...
def _v44934_s_momentum_core_lines(df: pd.DataFrame) -> list[str]:
    ...
    try:
        ...
        lines.append('[🔥 S-MOMENTUM / S-RECLAIM 핵심압축 — v4.4.9.34]')
        ...
        lines.append('- 운용해석: ST30은 하드필터가 아니라 가점/강등 태그입니다. ST30 미통과라도 1000억+·종가75%+·Vol≤1.8·RR정상이면 S-MOMENTUM PRIME으로 살리고, 저거래대금은 RISK/EXCLUDE로 낮춥니다.')
    except Exception as e:
...
def _v44934_build_core_compact_backtest_report(original_report: str, selected_csv: str = '', raw_csv: str = '', start_date: str = '', end_date: str = '', hold_days: int = 0, top_per_strategy: int = 5, all_candidates: bool = False) -> str:
    ...
    try:
        ...
        sel_text = '전체후보' if all_candidates else f'날짜별 전략별 TOP{top_per_strategy}'
        lines: list[str] = []
        lines.append('🧪 v4.4.9.34 REPORT COMPACT + LEGACY LABEL CLEANUP')
        lines.append(f'버전: {CLOSING_BET_SCANNER_VERSION}')
        lines.append(f'기간: {start_date} ~ {end_date} | 보유평가: 다음 {hold_days}거래일')
        ...
        lines.append('2순위 단기: L 5000억+ / S-MOMENTUM PRIME / S2·S-NEUTRAL — 추격보다 지정가·지지확인')
        ...
        lines.append('단기 실행 우선순위는 LP-SAFE > L 5000억+ > S-MOMENTUM PRIME/S2 > A-CONFIRM입니다. 중기 관찰은 I-MAIN ACCEL을 우선하고, I-MAIN CORE는 +100% 이상치 영향을 분리해 5MA/20MA 재지지 확인형으로 낮춥니다.')
        lines.append('')
        ...
        lines.append('- LP-SAFE: 여전히 단기 최우선. D1은 소액, D23/D45는 갭하단·전일종가 지지 확인 후 +3/+5 자동익절.')
        lines.append('- L 5000억+: 강하지만 당일 추격 위험. 다음날 갭하단/전일종가 지지 확인이 더 안전.')
        lines.append('- S-MOMENTUM PRIME: ST30 미통과라도 1000억+·종가75%+·Vol≤1.8·RR정상이면 제외하지 않습니다.')
        lines.append('- I-MAIN ACCEL: 중기 1순위 유지. CORE는 이상치 영향을 확인했으므로 신규 추격보다 재지지 분할관찰.')
        ...
        lines.append('- 단기 LP: 5000억+ 리더갭 이후 1~5일 눌림재지지. D1/D23/D45 타점과 다음날 시초가별 대응을 같이 확인.')
        lines.append('- 단기 L: 거래대금 5000억+, 갭 3~12%, 종가위치 70%+ 우선. 5000억~1조와 갭 6~12%는 가점.')
        lines.append('- 단기 S: S-MOMENTUM PRIME/S2 중심. ST30은 필터가 아니라 가점·비중조절 태그.')
        lines.append('- 중기 I: ACCEL 우선, CORE는 이상치 보정 후 확인형. 추격보다 5MA/20MA 재지지 분할.')
        ...
        lines.append('- 과거 패치 버전명은 텔레그램 혼선을 줄이기 위해 v4.4.9.34/누적검증 문구로 정리했습니다.')
        compact = '\n'.join([str(x) for x in lines if x is not None])
        compact = _v44934_cleanup_legacy_labels(compact)
        try:
            out_path = LOG_DIR / 'closing_bet_backtest_summary_v4_4_9_34_report_compact_legacy_label_cleanup.txt'
