...
CLOSING_BET_SCANNER_VERSION = 'G_MORALES_V4_4_6_PATH_PROFILE_CALIBRATION_20260519'
...
# ── 종가배팅 전용 텔레그램 설정
# v4.4.6: 기본방 오발송 방지 유지 + 출력 문구/파일저장 안내 정리.
...
# v4.4.6: SLOCK은 백테스트 검증에는 남기되 실시간 후보에서는 기본 숨김 처리한다.
...
def _classify_lp_candidate(h: dict) -> dict:
    """v4.4.6: LP 리더갭 눌림재지지 후보를 SAFE/WATCH/RISK로 분류한다.
...
def _lp_timing_bucket(h: dict) -> tuple[str, str]:
    """v4.4.6: LP 타점 시차를 D1 / D23 / D45로 구분한다."""
    try:
...
def _lp_next_day_scenario(h: dict) -> str:
    """v4.4.6: LP-SAFE/LP-WATCH 다음날 대응 시나리오를 텔레그램에 직접 출력한다."""
...
def _send_results(hits: list, mins_left: int):
    ...
    def _gap_support_levels(h: dict, mode: str = '') -> dict:
        """v4.4.6: L/LP/G 갭형 후보의 실전 지지선을 따로 표시한다.
    ...
    def _build_practical_block():
        ...
        # v4.4.6 신규검증 검색식 — 기존 L/S/I 검색식은 그대로 두고 별도 섹션에만 표시한다.
        ...
        block = ['[🎯 실전 운영 후보 — v4.4.6 PATH PROFILE CALIBRATION]']
        summary = (
        ...
        if l_core_pool:
            ...
            block.append('v4.4.6 기준: L 당일 후보는 단기 2순위입니다. 5000억+·종가70%+는 우대하지만, 가능하면 LP 눌림재지지 타점으로 재진입하는 쪽을 더 우선합니다.')
            used_codes = set()
            shown = 0
        ...
        if PRACTICAL_SHOW_NEW_PATTERNS and it_accel_live_pool:
            ...
            block.append('v4.4.6 기준: IT는 ACCEL만 실시간 촉발 후보로 표시합니다. 단기 종가배팅이 아니라 20/40/60일 중기 1차 분할매집 타이밍입니다. IT-CORE는 보조관찰입니다.')
            used_codes = set()
            shown = 0
...
def _v439_i_samples_section(df: pd.DataFrame, top_n: int = 5) -> list[str]:
    ...
    try:
        ...
        if i.empty:
            return lines
        lines.append("[📋 I-MAIN 발생 샘플 — 종목별 중복제거 v4.4.6]")
...
def _v445_pattern_path_profile_lines(df: pd.DataFrame) -> list[str]:
    ...
    try:
        ...
        lines.append('[🧭 패턴별 타점 이후 흐름 프로파일 — v4.4.6 보정판]')
        ...
        lines.append('- 해석: v4.4.6은 1~3일 첫흔들림, +3전흔들림, 전체최대흔들림을 분리합니다. 전체최대흔들림은 목표 도달 후 흔들림까지 포함될 수 있으므로 실전 판단은 +3전흔들림과 지지선 종가유지를 우선 봅니다.')
    except Exception as e:
...
def _v442_new_pattern_performance_lines(df: pd.DataFrame) -> list[str]:
    ...
    try:
        ...
        lines.append('- 해석: v4.4.6부터 LP는 단기 최우선 운용 후보로 보되, D1/D23/D45 타점과 다음날 대응 시나리오를 함께 확인합니다. LP-SAFE는 실행 후보, LP-WATCH는 다음날 양봉·전일고가 회복·거래대금 유지 시 승격, LP-RISK는 제외입니다. SLOCK은 실시간 기본 숨김, IT는 ACCEL 중심으로만 봅니다.')
    except Exception as e:
...
def _v439_build_compact_backtest_report(original_report: str, selected_csv: str = '', raw_csv: str = '', start_date: str = '', end_date: str = '', hold_days: int = 0, top_per_strategy: int = 5, all_candidates: bool = False) -> str:
    ...
    try:
        ...
        sel_text = '전체후보' if all_candidates else f'날짜별 전략별 TOP{top_per_strategy}'
        lines = []
        lines.append("🧪 v4.4.6 PATH PROFILE CALIBRATION SUMMARY")
        lines.append(f"버전: {CLOSING_BET_SCANNER_VERSION}")
        lines.append(f"기간: {start_date} ~ {end_date} | 보유평가: 다음 {hold_days}거래일")
        ...
        lines.append("- 신규 검색식 LP/SLOCK/IT는 위 성과검증 섹션과 실시간 후보 출력 섹션에서 함께 확인합니다. v4.4.6는 LP-D1/D23/D45와 다음날 대응 시나리오를 추가했습니다. SLOCK은 실시간 기본 숨김, IT는 ACCEL 중심으로 봅니다.")
        ...
        return (str(original_report or '') + f"\n\n[v4.4.6 압축 요약 생성 실패] {type(e).__name__}: {e}")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--compact-summary', action='store_true', help='v4.4.6 압축/실시간 운용 요약을 텔레그램 전송/로그에 사용')
