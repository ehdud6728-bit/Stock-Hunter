...
CLOSING_BET_SCANNER_VERSION = 'G_MORALES_V4_4_3_LP_CORE_CLASSIFICATION_20260518'
...
# ── 종가배팅 전용 텔레그램 설정
# v4.4.3: 기본방 오발송 방지 유지 + 출력 문구/파일저장 안내 정리.
...
def _classify_lp_candidate(h: dict) -> dict:
    """v4.4.3: LP 리더갭 눌림재지지 후보를 SAFE/WATCH/RISK로 분류한다.
    ...
    try:
        ...
        if amount_b >= 1000.0:
            reason_bits.append(f'현대금{amount_b:.0f}억')
...
def _send_results(hits: list, mins_left: int):
    ...
    def _entry_plan_text(h: dict, mode: str, label_hint: str = '') -> str:
        ...
        if mode == 'LP':
            return (
                f"매수계획: LP-SAFE는 L 당일 추격이 아니라 1~5일 눌림재지지 확인형입니다. 1차는 갭하단/전일종가/5·10일선 지지 확인 후 20~30%, 2차는 전일고가 회복 또는 양봉 재전환 시만. "
    ...
    def _build_practical_block():
        ...
        # v4.4.3 신규검증 검색식 — 기존 L/S/I 검색식은 그대로 두고 별도 섹션에만 표시한다.
        ...
        block = ['[🎯 실전 운영 후보 — v4.4.3 LP CORE CLASSIFICATION]']
        summary = (
            f"요약: 🥇LP-SAFE {len(lp_safe_pool)}개 | 🔁LP-WATCH {len(lp_watch_pool)}개 | ⚠️LP-RISK {len(lp_risk_pool)}개 | 👑L-SAFE {len(l_safe_pool)}개 | 🔒SLOCK {len(slock_pool)}개 | ⚡IT {len(it_pool)}개 | 🌊L-TAIL {len(l_tail_pool)}개 | 💰L-MEGA {len(l_mega_pool)}개 | 🟢S-SAFE {len(safe_pool)}개 | 🟡S관찰 {len(neutral_pool)}개 | 🟢G-SAFE {len(g_safe_pool)}개 | "
        ...
        if PRACTICAL_SHOW_NEW_PATTERNS and lp_safe_pool:
            ...
            block.append('v4.4.2 검증 반영: LP는 1~5일 내 5000억+ 리더갭 이후 갭하단/전일종가/5·10일선 재지지 후보입니다. 당일 L 추격보다 안정적인 재진입 후보로 +3/+5 우선 대응합니다.')
            shown = 0
            used_codes = set()
...
def _bt_check_leader_gap_pullback_on_hist(code: str, name: str, hist: pd.DataFrame, row: pd.Series, info: dict, idx_label: str, marcap: float) -> dict | None:
    """v4.4.3 백테스트용 LP — L-PULLBACK 리더갭 눌림재지지형.
...
def _check_backtest_strategies_on_df(code: str, name: str, hist: pd.DataFrame) -> list:
    ...
    try:
        ...
        # v4.4.3: 기존 I 후보는 그대로 두고, IT(I-TRIGGER) 신규검색식 후보를 별도 mode로 추가해 텔레그램에서 성과검증한다.
        try:
        ...
        # LP — v4.4.3 신규검색식 백테스트: 기존 L 리더갭 이후 1~5일 눌림재지지
        try:
...
def _v439_i_samples_section(df: pd.DataFrame, top_n: int = 5) -> list[str]:
    ...
    try:
        ...
        if i.empty:
            return lines
        lines.append("[📋 I-MAIN 발생 샘플 — 종목별 중복제거 v4.4.3]")
...
def _v442_new_pattern_performance_lines(df: pd.DataFrame) -> list[str]:
    """v4.4.3: 신규 검색식 LP/SLOCK/IT 성과를 텔레그램 요약 안에서 바로 확인한다.
    ...
    try:
        ...
        lines.append('- 해석: v4.4.3부터 LP는 단기 최우선 신규검증 후보로 승격 검토합니다. LP-SAFE는 실행 후보, LP-WATCH는 다음날 양봉·전일고가 회복·거래대금 유지 시 승격, LP-RISK는 제외입니다. SLOCK은 표본/성과가 약하면 보류하고, IT는 ACCEL 중심으로만 봅니다.')
    except Exception as e:
...
def _v439_build_compact_backtest_report(original_report: str, selected_csv: str = '', raw_csv: str = '', start_date: str = '', end_date: str = '', hold_days: int = 0, top_per_strategy: int = 5, all_candidates: bool = False) -> str:
    ...
    try:
        ...
        sel_text = '전체후보' if all_candidates else f'날짜별 전략별 TOP{top_per_strategy}'
        lines = []
        lines.append("🧪 v4.4.3 LP CORE CLASSIFICATION SUMMARY")
        lines.append(f"버전: {CLOSING_BET_SCANNER_VERSION}")
        lines.append(f"기간: {start_date} ~ {end_date} | 보유평가: 다음 {hold_days}거래일")
        lines.append(f"원신호 {raw_count}건 | 최종검증 {len(df)}건 | 선택방식: {sel_text}")
        lines.append("")
        lines.append("[🎯 운용 결론]")
        lines.append("1순위 단기: LP-SAFE 리더갭 눌림재지지 — L 당일 추격보다 1~5일 재지지 확인형")
        lines.append("2순위 단기: L 리더갭 5000억+ / S2·S-CORE NEUTRAL — L은 5000억+·종가70%+, S는 유동성·RR 우대")
        ...
        lines.append("단기 실행 우선순위: LP-SAFE > L 5000억+ > S2/S-NEUTRAL > A/C 5000억+ 관찰. A/C는 즉시매수 아님, 다음날 전일고가 회복+거래대금 유지 시만 승격. 중기 관찰 우선순위: I-ACCEL > I-CORE. H/B는 검증·관찰만.")
        lines.append("")
        ...
        lines.append("- 단기 LP: 5000억+ L 리더갭 후 1~5일 눌림재지지. LP-SAFE는 +3/+5 우선, LP-WATCH는 다음날 승격 확인.")
        ...
        lines.append("- 신규 검색식 LP/SLOCK/IT는 위 성과검증 섹션과 실시간 후보 출력 섹션에서 함께 확인합니다. LP는 v4.4.3부터 LP-SAFE/LP-WATCH/LP-RISK로 분류합니다. LP-SAFE가 있으면 단기 최우선으로 봅니다.")
        ...
        return (str(original_report or '') + f"\n\n[v4.4.3 압축 요약 생성 실패] {type(e).__name__}: {e}")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--compact-summary', action='store_true', help='v4.4.3 압축/실시간 운용 요약을 텔레그램 전송/로그에 사용')
