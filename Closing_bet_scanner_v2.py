...
CLOSING_BET_SCANNER_VERSION = 'G_MORALES_V4_4_1_NEW_PATTERNS_LIVE_20260518'
...
# ── 종가배팅 전용 텔레그램 설정
# v4.4.1: 기본방 오발송 방지. CLOSING_BET/CLOSE_BET 전용 CHAT_ID가 있으면 반드시 그 방으로 보낸다.
...
_CLOSING_BET_TOKEN_RAW, CLOSING_BET_TOKEN_SOURCE = _env_first('CLOSING_BET_TOKEN', 'CLOSE_BET_TOKEN')
_DEFAULT_TELEGRAM_TOKEN_RAW, DEFAULT_TELEGRAM_TOKEN_SOURCE = _env_first('TELEGRAM_TOKEN')
_CLOSING_BET_CHAT_RAW, CLOSING_BET_CHAT_SOURCE = _env_first('CLOSING_BET_CHAT_ID', 'CLOSE_BET_CHAT_ID')
...
def _telegram_route_ready() -> bool:
    ...
    if not CHAT_ID_LIST:
        try:
            log_error('❌ CLOSING_BET_CHAT_ID/CLOSE_BET_CHAT_ID 없음 — 기본방 오발송 방지를 위해 텔레그램 전송 차단')
            log_error('   GitHub Secrets에 CLOSING_BET_CHAT_ID 또는 CLOSE_BET_CHAT_ID를 설정하세요.')
        except Exception:
            pass
        return False
...
def _v439_i_samples_section(df: pd.DataFrame, top_n: int = 5) -> list[str]:
    ...
    try:
        ...
        if i.empty:
            return lines
        lines.append("[📋 I-MAIN 발생 샘플 — 종목별 중복제거 v4.4.1]")
...
def _v439_build_compact_backtest_report(original_report: str, selected_csv: str = '', raw_csv: str = '', start_date: str = '', end_date: str = '', hold_days: int = 0, top_per_strategy: int = 5, all_candidates: bool = False) -> str:
    ...
    try:
        ...
        sel_text = '전체후보' if all_candidates else f'날짜별 전략별 TOP{top_per_strategy}'
        lines = []
        lines.append("🧪 v4.4.1 OPERATION LIVE DECISION SUMMARY")
        lines.append(f"버전: {CLOSING_BET_SCANNER_VERSION}")
        lines.append(f"기간: {start_date} ~ {end_date} | 보유평가: 다음 {hold_days}거래일")
        ...
        return (str(original_report or '') + f"\n\n[v4.4.1 압축 요약 생성 실패] {type(e).__name__}: {e}")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--compact-summary', action='store_true', help='v4.4.1 압축/실시간 운용 요약을 텔레그램 전송/로그에 사용')
