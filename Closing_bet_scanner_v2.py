...
CLOSING_BET_SCANNER_VERSION = 'G_MORALES_V4_4_1_2_TELEGRAM_CLOSEBET_ROUTE_FIX_20260518'
...
# ── 종가배팅 전용 텔레그램 설정
# v4.4.1.2: 기본방 오발송 방지. TELEGRAM_CLOSEBET_* / CLOSING_BET_* / CLOSE_BET_* 전용 CHAT_ID가 있으면 반드시 그 방으로 보낸다.
...
_CLOSING_BET_CHAT_RAW, CLOSING_BET_CHAT_SOURCE = _env_first('TELEGRAM_CLOSEBET_CHAT_ID', 'TELEGRAM_CLOSE_BET_CHAT_ID', 'TELEGRAM_CLOSEBET_ROOM_ID', 'TELEGRAM_CLOSE_BET_ROOM_ID', 'CLOSING_BET_CHAT_ID', 'CLOSE_BET_CHAT_ID', 'CLOSING_BET_ROOM_ID', 'CLOSE_BET_ROOM_ID', 'CLOSING_BET_TELEGRAM_CHAT_ID', 'CLOSE_BET_TELEGRAM_CHAT_ID')
...
def _make_i_trigger_candidate(i: dict | None) -> dict | None:
    ...
    try:
        ...
        core = _safe_int(i.get('i_core_main_core', i.get('i_main_core', 0)), 0) == 1
...
def _check_backtest_strategies_on_df(code: str, name: str, hist: pd.DataFrame) -> list:
    ...
    try:
        ...
        try:
            hits.extend(_build_icore_hits(code, name, hist, row, info, idx_label, marcap))
        except Exception as e:
            log_debug(f"I-CORE 신호 계산 오류 [{code}/{name}]: {e}")
...
def _v439_i_samples_section(df: pd.DataFrame, top_n: int = 5) -> list[str]:
    ...
    try:
        ...
        if i.empty:
            return lines
        lines.append("[📋 I-MAIN 발생 샘플 — 종목별 중복제거 v4.4.1.2]")
...
def _v439_build_compact_backtest_report(original_report: str, selected_csv: str = '', raw_csv: str = '', start_date: str = '', end_date: str = '', hold_days: int = 0, top_per_strategy: int = 5, all_candidates: bool = False) -> str:
    ...
    try:
        ...
        sel_text = '전체후보' if all_candidates else f'날짜별 전략별 TOP{top_per_strategy}'
        lines = []
        lines.append("🧪 v4.4.1.2 TELEGRAM CLOSEBET ROUTE FIX SUMMARY")
        lines.append(f"버전: {CLOSING_BET_SCANNER_VERSION}")
        lines.append(f"기간: {start_date} ~ {end_date} | 보유평가: 다음 {hold_days}거래일")
        ...
        lines.append("[📁 파일 저장 안내]")
        lines.append(f"- 선택 CSV: {selected_csv}")
        lines.append(f"- 원신호 CSV: {raw_csv}")
        lines.append("- 원문 상세/CSV/HTML은 GitHub Actions artifact의 closing_bet_logs에 저장됩니다. Telegram은 이 압축 요약만 봐도 결론이 나오도록 정리했습니다.")
        compact = "\n".join([str(x) for x in lines if x is not None])
        try:
            out_path = LOG_DIR / 'closing_bet_backtest_summary_v4_4_0_operation_live_decision.txt'
        ...
        return (str(original_report or '') + f"\n\n[v4.4.1.2 압축 요약 생성 실패] {type(e).__name__}: {e}")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--compact-summary', action='store_true', help='v4.4.1.2 압축/실시간 운용 요약을 텔레그램 전송/로그에 사용')
