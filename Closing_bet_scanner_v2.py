...
CLOSING_BET_SCANNER_VERSION = 'G_MORALES_V4_4_2_NEW_PATTERN_PERFORMANCE_TELEGRAM_20260518'
...
# ── 종가배팅 전용 텔레그램 설정
# v4.4.2: 기본방 오발송 방지 유지 + 출력 문구/파일저장 안내 정리.
...
def _send_results(hits: list, mins_left: int):
    ...
    def _entry_plan_text(h: dict, mode: str, label_hint: str = '') -> str:
        ...
        if mode == 'LP':
            return (
                f"매수계획: L 당일 추격이 아니라 1~5일 눌림재지지 검증형입니다. 1차는 갭하단/전일종가/5·10일선 지지 확인 후 20~30%, 2차는 전일고가 회복 또는 양봉 재전환 시만. "
                f"익절: +3% {int(t3):,}원 우선 / +5% {int(t5):,}원 추가. 손절/무효: {stop_txt} 또는 갭하단 재이탈."
            )
        if mode == 'SLOCK':
            return (
    ...
    def _brief_practical_line(h: dict, idx: int) -> str:
        ...
        if mode == 'S' and _is_s1_good(h):
            ...
            tag = 'L-PULLBACK 눌림재지지'
            reason = '최근 1~5일 내 5000억+ L 리더갭 이후 갭하단/전일종가/5·10일선 지지 확인형'
        elif mode == 'SLOCK':
            tag = 'S2-LOCK 상단잠김'
        ...
        if mode == 'LP':
            t3 = close * 1.03 if close > 0 else 0
            t5 = close * 1.05 if close > 0 else 0
            stop_txt = f"손절 {int(stop):,}원 이탈 관리" if stop > 0 else "갭하단/전일종가 재이탈 관리"
            ...
            return (
                ...
                f"   대응: +3% {int(t3):,}원 1차 / +5% {int(t5):,}원 추가 / 갭하단·전일종가·5/10일선 지지 확인 / {stop_txt}\n"
                f"   📌 {plan}"
            )
        if mode == 'L':
            lm = _l_metrics(h)
    ...
    def _build_practical_block():
        ...
        # v4.4.1 신규검증 검색식 — 기존 L/S/I 검색식은 그대로 두고 별도 섹션에만 표시한다.
        ...
        block = ['[🎯 실전 운영 후보 — v4.4.1 NEW PATTERNS LIVE]']
        summary = (
            f"요약: 👑L-SAFE {len(l_safe_pool)}개 | 🔁LP {len(lp_pool)}개 | 🔒SLOCK {len(slock_pool)}개 | ⚡IT {len(it_pool)}개 | 🌊L-TAIL {len(l_tail_pool)}개 | 💰L-MEGA {len(l_mega_pool)}개 | 🟢S-SAFE {len(safe_pool)}개 | 🟡S관찰 {len(neutral_pool)}개 | 🟢G-SAFE {len(g_safe_pool)}개 | "
        ...
        block.append('[🚦 v4.4.1 실시간 운용판정]\n기존 검색식은 훼손하지 않고 신규검증 검색식만 추가했습니다. 단기 실행 우선순위: L 5000억+ 리더갭 > LP 리더갭 눌림재지지 > SLOCK/S2/S-NEUTRAL > G-SAFE. A/C 5000억+는 즉시매수 아님, 다음날 전일고가 회복+거래대금 유지 시만 승격합니다.\n중기 관찰 우선순위: IT I-TRIGGER > I-MAIN ACCEL > I-MAIN CORE > I-WATCH/ADD/CONFIRM. H/B는 검증·관찰만, H-RISK/과열 H는 제외합니다.\n파일저장/CSV 확인 없이 이 텔레그램 블록만으로 신규 검색식 후보와 운용판정을 확인할 수 있게 구성했습니다.')

        if l_core_pool:
            block.append('')
            block.append('[🥇 단기 1순위 L-CORE/MEGA 리더갭 — 5000억+ 우선]')
            block.append('v4.4.1 검증 반영: 단기 종가배팅 1순위는 L 5000억+ 리더갭입니다. 5000억~1조, 갭 6~12%, 종가위치 70%+는 가점이지만 표본이 작으면 과신 금지입니다.')
            used_codes = set()
            shown = 0
        ...
        if PRACTICAL_SHOW_NEW_PATTERNS and lp_pool:
            block.append('')
            block.append('[🔁 신규검증 LP — L-PULLBACK 리더갭 눌림재지지]')
            block.append('기존 L 당일 검색식을 건드리지 않고, 최근 1~5일 내 5000억+ 리더갭 이후 갭하단/전일종가/5·10일선 재지지를 보는 재진입 후보입니다. 당일 추격보다 다음날 지지 확인 우선입니다.')
            shown = 0
            used_codes = set()
            for h in lp_pool:
                code = str(h.get('code',''))
        ...
        if PRACTICAL_SHOW_NEW_PATTERNS and slock_pool:
            block.append('')
            block.append('[🔒 신규검증 SLOCK — S2 상단잠김형]')
            block.append('기존 S2 실행형 중 종가위치80%+·윗꼬리20% 이하·유동성 조건을 만족한 상단잠김 후보입니다. 고점권이므로 +3/+5 우선 회수와 손절선 엄수가 핵심입니다.')
        ...
        if PRACTICAL_SHOW_NEW_PATTERNS and it_pool:
            block.append('')
            block.append('[⚡ 신규검증 IT — I-TRIGGER 중기후보 촉발형]')
            block.append('I-MAIN은 단기 종가배팅이 아니므로, IT는 ACCEL/CORE/WATCH 중 종가위치·거래대금·장기선 이격이 맞은 1차 분할매집 촉발일로만 봅니다. 몰빵/추격 금지입니다.')
            used_codes = set()
            shown = 0
            for h in it_pool:
                code = str(h.get('code',''))
...
def _bt_check_leader_gap_pullback_on_hist(code: str, name: str, hist: pd.DataFrame, row: pd.Series, info: dict, idx_label: str, marcap: float) -> dict | None:
    """v4.4.2 백테스트용 LP — L-PULLBACK 리더갭 눌림재지지형.
...
def _check_backtest_strategies_on_df(code: str, name: str, hist: pd.DataFrame) -> list:
    ...
    try:
        ...
        # v4.4.2: 기존 I 후보는 그대로 두고, IT(I-TRIGGER) 신규검색식 후보를 별도 mode로 추가해 텔레그램에서 성과검증한다.
        try:
        ...
        # LP — v4.4.2 신규검색식 백테스트: 기존 L 리더갭 이후 1~5일 눌림재지지
        try:
...
def _v439_i_samples_section(df: pd.DataFrame, top_n: int = 5) -> list[str]:
    ...
    try:
        ...
        if i.empty:
            return lines
        lines.append("[📋 I-MAIN 발생 샘플 — 종목별 중복제거 v4.4.2]")
...
def _v442_new_pattern_performance_lines(df: pd.DataFrame) -> list[str]:
    """v4.4.2: 신규 검색식 LP/SLOCK/IT 성과를 텔레그램 요약 안에서 바로 확인한다."""
    lines = []
    try:
        if df is None or df.empty or 'mode' not in df.columns:
            return lines
        ...
        slock = df[mode.eq('SLOCK')].copy()
        it = df[mode.eq('IT')].copy()
        lines.append('[🧪 신규 검색식 성과검증 — LP/SLOCK/IT]')
        if lp.empty and slock.empty and it.empty:
            ...
            return lines
        if not lp.empty:
            lines.append(_v439_short_trade_line(lp, 'LP 리더갭 눌림재지지'))
            lp_core = lp[_v439_num_series(lp, 'lp_gap_amount_b', 0).ge(5000) & _v439_num_series(lp, 'lp_gap_pct', 0).between(3, 12) & _v439_num_series(lp, 'close_loc_pct', 0).ge(60)]
            if not lp_core.empty:
                lines.append(_v439_short_trade_line(lp_core, 'LP 핵심셀: 갭봉5000억+·갭3~12·타점종가60+'))
        else:
        ...
        lines.append('- 해석: LP/SLOCK/IT는 기존 L/S/I를 대체하지 않고, 진입 타점 보강용 신규검증 검색식입니다. 표본이 작으면 후보 표시만 참고하고 실전 승격은 반복검증 후 판단합니다.')
    except Exception as e:
...
def _v439_build_compact_backtest_report(original_report: str, selected_csv: str = '', raw_csv: str = '', start_date: str = '', end_date: str = '', hold_days: int = 0, top_per_strategy: int = 5, all_candidates: bool = False) -> str:
    ...
    try:
        ...
        sel_text = '전체후보' if all_candidates else f'날짜별 전략별 TOP{top_per_strategy}'
        lines = []
        lines.append("🧪 v4.4.2 NEW PATTERN PERFORMANCE TELEGRAM SUMMARY")
        lines.append(f"버전: {CLOSING_BET_SCANNER_VERSION}")
        lines.append(f"기간: {start_date} ~ {end_date} | 보유평가: 다음 {hold_days}거래일")
        lines.append(f"원신호 {raw_count}건 | 최종검증 {len(df)}건 | 선택방식: {sel_text}")
        lines.append("")
        lines.append("[🎯 운용 결론]")
        lines.append("1순위 단기: L 리더갭 5000억+ — 특히 5000억~1조, 갭 6~12%, 종가위치 70%+")
        lines.append("2순위 단기: S2 실행형 / S-CORE NEUTRAL — KOSPI200·5조+·거래대금 3000억+ 우대")
        ...
        lines.append("단기 실행 우선순위: L 5000억+ > S2/S-NEUTRAL > A/C 5000억+ 관찰. A/C는 즉시매수 아님, 다음날 전일고가 회복+거래대금 유지 시만 승격. 중기 관찰 우선순위: I-ACCEL > I-CORE. H/B는 검증·관찰만.")
        lines.append("")
        ...
        lines.append("- 신규 검색식 LP/SLOCK/IT는 위 성과검증 섹션과 실시간 후보 출력 섹션에서 함께 확인합니다.")
        compact = "\n".join([str(x) for x in lines if x is not None])
        try:
            out_path = LOG_DIR / 'closing_bet_backtest_summary_v4_4_1_4_new_pattern_performance.txt'
        ...
        return (str(original_report or '') + f"\n\n[v4.4.2 압축 요약 생성 실패] {type(e).__name__}: {e}")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--compact-summary', action='store_true', help='v4.4.2 압축/실시간 운용 요약을 텔레그램 전송/로그에 사용')
