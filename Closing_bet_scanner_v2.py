...
def _load_universe(universe_name: str) -> list:
    ...
    if universe_name == 'kospi200':
        return k200_codes
    if universe_name == 'kospi200+kosdaq150':
        return list(dict.fromkeys(k200_codes + kq150_codes))
    if universe_name == 'hybrid_intersection':
        idx_set = set(k200_codes) | set(kq150_codes)
        return [c for c in top_codes if c in idx_set]

    # hybrid_union 기본: 지수 편입 + 시총 2000억 이상 OR
    listing = _get_krx_listing()
    mcap_codes = []
    ...
    return list(dict.fromkeys(k200_codes + kq150_codes + mcap_codes))
