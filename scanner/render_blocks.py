...
def build_watermelon_summary_block(df: pd.DataFrame) -> str:
    ...
    lines = [
        ...
        f'- 재점화군(빨강): {blue1_n + blue2_n + blue2_preview_n}개 = Blue-1 {blue1_n} / Blue-2 {blue2_n} / 예비 {blue2_preview_n}',
        f'- 후행수박: {late_n}개',
    ]
    return '\n'.join(lines) + '\n'
...
def _render_kki_lines(item: dict) -> str:
    ...
    habit = str(item.get('kki_habit_comment', item.get('kki_habit', item.get('kki_recurrence', ''))) or '').strip()
    current_state = str(item.get('kki_current_state', '') or '').strip()
    explain = str(item.get('kki_reason', item.get('kki_comment', '')) or '').strip()
