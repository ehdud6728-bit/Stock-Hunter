...
# =============================================================
PATTERN_SEQUENCE_BUILD_TAG = "PATTERN_SEQUENCE_GUARD_COMPLETE_V2"
print("✅ PATTERN_SEQUENCE_GUARD_COMPLETE_V2 LOADED")
print("✅ EXPECT_PATTERN_LINES = 🔵파란저항 / 📐삼각수렴 / 🧲하단밴드재현 / 📦세력매집봉 / 🛡️패턴가드")
...
def _pp_detect_blue_resistance(df):
    ...
    try:
        ...
        # 현재가보다 너무 먼 고점은 트리거로 쓰지 않음
        candidates = []
        for label, px in [('동적저항', dyn_res), ('수평저항', box_res)]:
            if px and px > 0 and 0.90 <= close / px <= 1.12:
        ...
        breakout = close >= price * 1.005 and vol_r >= 1.10 and wick <= 45
        wait = -5.0 <= dist < 0.5
        if breakout:
            tag = '🔵파란저항돌파'
    ...
    try:
        ...
        for name, col, tol, fdays, need_ret in bands:
            ...
            if _pp_vol_ratio(df) >= 1.1:
                score += 5
            candidates.append((score, name, lower, dist, st))
        if not candidates:
            return res
        candidates.sort(key=lambda x: x[0], reverse=True)
        score, name, lower, dist, st = candidates[0]
        state = '하단터치대기'
        tag = f'🧲{name}하단재현'
...
def build_pattern_sequence_bundle(temp_df, row=None, signals=None):
    ...
    try:
        ...
        primary_candidates = [d for d in ok_list if cat_of(d) != '보조' and d.get('score', 0) >= 24]
