...
def analyze_final(ticker, name, historical_indices, g_env, l_env, s_map):
    ...
    try:
        ...
        # ✅ PATTERN_VISIBILITY_SCOUT_V3
        # 완성형 신호 전 단계(파란저항 대기/하단밴드 터치/매집봉 후 눌림)는
        # 거래량 3배 조건이 붙기 전 사라지는 경우가 많아서 최소 거래대금 기준으로만
        # 패턴 스카우트 통과문을 하나 더 둔다.
        try:
            _pattern_scout_min_amount = float(os.getenv('PATTERN_SCOUT_MIN_AMOUNT_B', '8'))
        except Exception:
            _pattern_scout_min_amount = 8.0
...
# =============================================================
PATTERN_SEQUENCE_BUILD_TAG = "PATTERN_VISIBILITY_SCOUT_COMPLETE_V3"
print("✅ PATTERN_VISIBILITY_SCOUT_COMPLETE_V3 LOADED")
print("✅ EXPECT_PATTERN_LINES = 🔵파란저항 / 📐삼각수렴 / 🧲하단밴드재현 / 📦세력매집봉 / 👀미완성후보 / 🛡️패턴가드")
...
def _pp_detect_double_bottom(df):
    ...
    try:
        ...
        if true_ok:
            res.update(ok=True, kind='TRUE_DOUBLE_BOTTOM', tag='👣진짜쌍바닥', score=34, price=neck, neckline=neck)
            ...
            res.update(ok=True, kind='SIMILAR_DOUBLE_BOTTOM', tag='👣유사쌍바닥', score=14, price=neck, neckline=neck)
...
def build_pattern_plus_state_block(title, df, category=''):
    try:
        ...
        work = df.copy()
        if category:
            work = work[work.get('패턴카테고리', '').astype(str).str.contains(category, na=False)].copy()
