...
from kki_wave import analyze_kki_and_wave


from scanner.render_telegram import send_telegram_chunks_core
...
# --- telegram global fallback (early) ---
TELEGRAM_TOKEN = globals().get("TELEGRAM_TOKEN", os.getenv("TELEGRAM_TOKEN", ""))
TELEGRAM_CHAT_ID = globals().get("TELEGRAM_CHAT_ID", os.getenv("TELEGRAM_CHAT_ID", ""))
TELEGRAM_REAL_CHAT_ID = globals().get(
    "TELEGRAM_REAL_CHAT_ID",
    os.getenv("TELEGRAM_REAL_CHAT_ID", "") or TELEGRAM_CHAT_ID,
)
TELEGRAM_TEST_CHAT_ID = globals().get(
    "TELEGRAM_TEST_CHAT_ID",
    os.getenv("TELEGRAM_TEST_CHAT_ID", "") or TELEGRAM_CHAT_ID,
)
TELEGRAM_CHAT_ID_EFFECTIVE = globals().get(
    "TELEGRAM_CHAT_ID_EFFECTIVE",
    os.getenv("TELEGRAM_CHAT_ID_EFFECTIVE", "") or TELEGRAM_CHAT_ID,
)
TEST_CHAT_ID_OVERRIDE = globals().get(
    "TEST_CHAT_ID_OVERRIDE",
    os.getenv("TEST_CHAT_ID_OVERRIDE", ""),
)
# --- end telegram global fallback ---


# --- DART fallback compatibility patch ---
DART_API_KEY = globals().get("DART_API_KEY", os.getenv("DART_API_KEY", ""))
DART_ENABLED = globals().get(
    "DART_ENABLED",
    str(os.getenv("DART_ENABLED", "0")).strip().lower() in ("1", "true", "yes", "y", "on")
)
# --- end DART fallback compatibility patch ---



# --- openai api fallback ---
OPENAI_API_KEY = globals().get("OPENAI_API_KEY", os.getenv("OPENAI_API_KEY", ""))
# --- end openai api fallback ---



# --- runtime logging compatibility patch ---
def log_info(msg):
    try:
...
def log_debug(msg):
    try:
        ...
        try:
            ...
            pass
# --- end runtime logging compatibility patch ---


# --- runtime scan logging compatibility patch ---
def log_hit(name, score=None, tags=None):
    try:
        msg = f"[HIT] {name}"
...
def log_progress(done_count, total_count):
    try:
        print(f"진행: {done_count}/{total_count}", flush=True)
    except Exception:
        pass
# --- end runtime scan logging compatibility patch ---


# --- render helper compatibility patch ---
...
def _wrap_stock_card(card_text: str, rank: int = 1) -> str:
    ...
    if not body:
        return ''
    return f"━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n{body}\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n"
# --- end render helper compatibility patch ---
...
def send_telegram_chunks(message: str, title: str = '', max_len: int = 3800):
    ...
    실제 청크 분할 로직은 scanner.render_telegram 으로 분리.
    """
    return send_telegram_chunks_core(
        message=message,
        send_func=lambda chunk: send_telegram_photo(chunk, []),
        title=title,
        max_len=max_len,
    )
...
def _build_wave_brief_line(row) -> str:
    try:
        code = str(row.get('code', row.get('종목코드', '')) or '').strip()
        if not code:
            return ''
        df = fdr_cached(code, days=240)
        if df is None or len(df) < 80:
            return ''
        meta = {
            'name': str(row.get('종목명', row.get('name', '')) or '').strip(),
            'support_band': str(row.get('지원밴드', '') or '').strip(),
        }
        analysis = analyze_kki_and_wave(df, meta=meta, show_threshold=55)
        wave = getattr(analysis, 'wave', None)
        if not wave:
            return ''
        small_zone = str(getattr(wave, 'small_zone', '') or '').strip()
        medium_zone = str(getattr(wave, 'medium_zone', '') or '').strip()
        if not small_zone or small_zone == '데이터부족':
            return ''
        small_angle = float(getattr(wave, 'small_angle', 0.0) or 0.0)
        medium_angle = float(getattr(wave, 'medium_angle', 0.0) or 0.0)
        small_angle_label = str(getattr(wave, 'small_angle_label', '') or '').strip()
        medium_angle_label = str(getattr(wave, 'medium_angle_label', '') or '').strip()
        comment = str(getattr(wave, 'wave_comment', '') or '').strip()
        line = (
            f"🌊 파동: {small_zone} / {medium_zone} | "
            f"상승각도: 소파동 {small_angle_label}({small_angle:+.2f}) / "
            f"중파동 {medium_angle_label}({medium_angle:+.2f})"
        )
        if comment:
            line += f"\n🌀 파동 해설: {comment}"
        return line
...
def append_interpretation_to_block(block: str, row) -> str:
    ...
    wave_brief = _build_wave_brief_line(enriched)
    if wave_brief and '🌊 파동:' not in extra:
        if '🧠 쉬운 해석:' in extra:
            extra = extra.replace('🧠 쉬운 해석:', wave_brief + '\n🧠 쉬운 해석:', 1)
        else:
            extra = wave_brief + ('\n' + extra if extra else '')
    if not extra:
        return block.rstrip()
