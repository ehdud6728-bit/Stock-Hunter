# -*- coding: utf-8 -*-
"""
accumulation_wave_complete_singlefile.py

가장 덜 헷갈리게 쓰는 "완성형 파일"
----------------------------------
이 파일 1개만 프로젝트에 넣고,
legacy_main_patched.py 에서 아래 2줄만 추가해서 쓰면 됩니다.

from accumulation_wave_complete_singlefile import analyze_accum_wave_package
...
주의
- 이 파일은 "붙여넣기 최소형 완성본"입니다.
- 하지만 사용자의 현재 legacy_main_patched.py 전체 원본이 없어서
  그 파일을 100% 대체하는 완전 통합 메인파일까지는 안전하게 만들 수 없습니다.
- 대신, 가장 적은 수정으로 붙일 수 있게 모든 로직을 이 파일 1개에 몰아 넣었습니다.
"""
...
def build_small_wave_from_accum(df: pd.DataFrame, accum_info: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        ...
        "wave_low": wave_low,
        "wave_mid": wave_mid,
        "desc": f"소파동 기준: 저점 {wave_low:.0f} / 전고점 {wave_high:.0f}",
    }
...
def build_mid_wave_from_small_wave(df: pd.DataFrame, small_wave: Dict[str, Any], lookback: int = 60) -> Dict[str, Any]:
    ...
    return {
        ...
        "mid_low": mid_low,
        "mid_mid": mid_mid,
        "desc": f"중파동 기준: 저점 {mid_low:.0f} / 전고점 {mid_high:.0f}",
    }
...
def calc_ma_pullback_score(df: pd.DataFrame) -> Dict[str, Any]:
    ...
    if -3.0 <= d150 <= 8.0:
        score += 18
        reasons.append("MA150근접")
    elif -6.0 <= d150 <= 12.0:
        score += 10
        reasons.append("MA150보통")

    if -3.0 <= d200 <= 10.0:
        score += 18
        reasons.append("MA200근접")
    elif -6.0 <= d200 <= 15.0:
        score += 10
        reasons.append("MA200보통")
    ...
    return {
        ...
        "d150": round(d150, 1),
        "d200": round(d200, 1),
        "desc": " / ".join(reasons) if reasons else "장기이평 거리 애매",
    }
...
def calc_overheat_penalty(df: pd.DataFrame) -> Dict[str, Any]:
    ...
    if disparity >= 130:
        penalty -= 32
        reasons.append("과열-32")
    elif disparity >= 122:
        penalty -= 22
        reasons.append("과열-22")
    elif disparity >= 116:
        penalty -= 12
        reasons.append("과열-12")
    elif disparity >= 112:
        penalty -= 6
        reasons.append("과열-6")
    ...
    return {
        "penalty": penalty,
        "label": "과열주의" if penalty < 0 else "과열양호",
        "disparity": round(disparity, 1),
        "desc": " / ".join(reasons) if reasons else "과열부담 낮음",
    }
...
def calc_blue_zone_hint(df: pd.DataFrame, small_wave: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        ...
        "zone_high": zone_high,
        "label": "파란타점 영역",
        "desc": f"{zone_low:.0f} ~ {zone_high:.0f}",
    }
...
mid_wave_high = pkg["mid_wave"].get("mid_high")
mid_wave_low = pkg["mid_wave"].get("mid_low")
...
if __name__ == "__main__":
    print("accumulation_wave_complete_singlefile.py loaded")
    print("legacy_main_patched.py 에서 import 해서 사용하세요.")
