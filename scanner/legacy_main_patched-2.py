...
# =============================================================
PATTERN_SEQUENCE_BUILD_TAG = "PATTERN_SEQUENCE_COMPLETE_V1"
print("✅ PATTERN_SEQUENCE_COMPLETE_V1 LOADED")
print("✅ EXPECT_PATTERN_LINES = 🔵파란저항 / 📐삼각수렴 / 🧲하단밴드재현 / 📦세력매집봉")
...
def _pp_detect_double_bottom(df):
    ...
    try:
        ...
        if true_ok:
            res.update(ok=True, kind='TRUE_DOUBLE_BOTTOM', tag='👣진짜쌍바닥', score=34)
            ...
            res.update(ok=True, kind='SIMILAR_DOUBLE_BOTTOM', tag='👣유사쌍바닥', score=14)
...
def _pp_detect_triangle(df):
    ...
    try:
        ...
        if breakout and ascending:
            res.update(ok=True, kind='ASC_TRI_BREAKOUT', tag='📈상승삼각형돌파', score=64, state='돌파 확인')
        elif breakout and sym_tri:
            res.update(ok=True, kind='TRI_BREAKOUT', tag='📐삼각수렴돌파', score=58, state='돌파 확인')
        elif ascending:
            res.update(ok=True, kind='ASC_TRI_WAIT', tag='📈상승삼각형대기', score=34, state='매수 대기')
        elif sym_tri:
            res.update(ok=True, kind='TRI_WAIT', tag='📐삼각수렴대기', score=30, state='매수 대기')
        if res['ok']:
