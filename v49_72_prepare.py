...
def main() -> int:
    ap = argparse.ArgumentParser(description='v49.72 immutable universe + search preflight')
    ap.add_argument('--end-date', required=True)
    ap.add_argument('--output-dir', default='v49_72_prepare_output')
    args = ap.parse_args()
    ...
    # v49.72: immutable entry-time classification maps. These are labels only; no future data.
    ...
    try:
        ...
        print(f'v49.72 listing classification map fallback: {type(exc).__name__}: {exc}')
    ...
    # v49.72: component-level authority fingerprints. A code-list hash alone cannot detect
    ...
    upath = out / 'v49_72_universe.json'
    ...
    ppath = out / 'v49_72_preflight.json'
    ppath.write_text(json.dumps(preflight, ensure_ascii=False, indent=2, default=str), encoding='utf-8')
    print(f"PREPARE UNIVERSE VALID · n {len(codes)} · fp {preflight['universe_fingerprint']} · authority {str((preflight.get('input_authority') or {}).get('prepared_authority_sha256',''))[:16]}")
