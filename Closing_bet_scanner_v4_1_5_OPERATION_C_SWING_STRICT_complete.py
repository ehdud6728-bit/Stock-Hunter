...
# =============================================================
# closing_bet_scanner.py — 종가배팅 타점 스캐너 (G Morales v4.1.5 운영형 + C-SWING 엄격형)
# =============================================================
# 전략 구성
# A  : 돌파형 종가배팅
...
CLOSING_BET_SCANNER_VERSION = 'G_MORALES_V4_1_5_OPERATION_C_SWING_STRICT_20260511'
...
# v4.1.5: 역매공파는 종가배팅 후보가 아니라 C-SWING 엄격형 스윙 관심으로 별도 표시한다.
...
def _send_results(hits: list, mins_left: int):
    ...
    # v4.1.5: C는 운영상 전체 C가 아니라 엄격형만 스윙 관심으로 본다.
    ...
    c_swing_main_output_n = min(len(hits_c_swing_strict), PRACTICAL_C_SWING_TOP_N)
    ...
    total = (
        ...
        min(len(hits_b2), 5) + c_swing_main_output_n + c_diag_output_n
    )
    ...
    header = (
        ...
        f"ENV엄격형(B1) {min(len(hits_b1), 5)}개 | BB확장형(B2) {min(len(hits_b2), 5)}개 | 역매공파(C) {min(len(hits_c), 5)}개 | C-SWING엄격 {c_swing_main_output_n}개\n"
    ...
    def _brief_practical_line(h: dict, idx: int) -> str:
        ...
        mode = _pick_strategy(h)
        risk_tags = _s_core_risk_tags(h) if mode == 'S' else (_trade_risk_tags(h) if mode == 'A' else [])
    ...
    def _build_practical_block():
        ...
        c_swing_pool = sorted(hits_c_swing_strict, key=_priority_practical)
        c_swing_output_n = min(len(c_swing_pool), PRACTICAL_C_SWING_TOP_N)

        block = ['[🎯 실전 최우선 후보 — v4.1.5 SAFE + C-SWING 분리]']
        summary = f"요약: 🟢SAFE {len(safe_pool)}개 | 🟡NEUTRAL {len(neutral_pool)}개 | ⚠️S-RISK {len(risk_pool)}개 | A보조 출력 {a_output_n}개/전체 {len(a_pool)}개 | 🏆C-SWING엄격 {c_swing_output_n}개"
        if a_risk_pool:
            summary += f" | ⚠️A주의숨김 {len(a_risk_pool)}개"
        block.append(summary)
        block.append('운용: SAFE는 종가배팅 최우선, NEUTRAL/A는 보조 관찰, C-SWING은 종가배팅이 아닌 10~20거래일 스윙 관심입니다.')
        ...
        if c_swing_pool:
            block.append('')
            block.append('[🏆 C-SWING 역매공파 엄격형 — 스윙 관심]')
            block.append('종가배팅 후보가 아닙니다. 10~20거래일 관점에서 +5/+10과 장기선 종가이탈을 보는 별도 스윙 후보입니다.')
    ...
    sections.append(_build_block("🏆 C-SWING 역매공파 엄격형 TOP3", hits_c_swing_strict[:PRACTICAL_C_SWING_TOP_N], "C"))
...
def _classify_ymgp_stage(hist: pd.DataFrame, row: pd.Series | None = None, info: dict | None = None) -> dict:
    ...
    try:
        ...
        if c3_ready:
            stage = 'C3'
            label = 'C3 눌림완성형'
            desc = '1파 돌파 이후 눌림·거래량 감소·재지지·재상승 확인이 함께 잡힌 2파 준비형'
            bias = '눌림 후 2파 후보'
            rank = 0
        elif c2_progress:
            stage = 'C2'
...
def _build_backtest_summary(
    ...
    try:
        ...
        try:
            ...
            if core_only or safe_only or neutral_only or risk_only or c_only or c_swing_only or c_strict_only:
                if c_swing_only:
    ...
    if not c_df.empty:
        ...
        lines.append("- 해석: C1은 1파 돌파/관심등록, C2는 눌림진행 관찰, C3는 거래량 감소·재지지·재상승 확인까지 갖춘 좁은 2파 준비 후보입니다.")
    ...
    lines.append("- v4.1.5부터 실시간 출력은 S-CORE SAFE 종가배팅과 C-SWING 엄격형 스윙 관심을 분리해 표시합니다. C-SWING 평가는 기존 3/5 단타룰과 별도로 계산됩니다.")
...
def run_closing_bet_backtest(
    ...
    if raw_df.empty:
        report = _build_backtest_summary(
            ...
            all_candidates, weekly_breakdown=weekly_breakdown, diag=diag, core_only=core_only, safe_only=safe_only, neutral_only=neutral_only, risk_only=risk_only, c_only=c_only, c_swing_only=c_swing_only, c_strict_only=c_strict_only,
        )
        try:
    ...
    if (safe_only or neutral_only or risk_only or core_only or c_only or c_swing_only or c_strict_only) and selected_df is not None and not selected_df.empty:
        try:
            if c_swing_only:
    ...
    report = _build_backtest_summary(
        ...
        all_candidates, weekly_breakdown=weekly_breakdown, diag=diag if debug else None, core_only=core_only, safe_only=safe_only, neutral_only=neutral_only, risk_only=risk_only, c_only=c_only, c_swing_only=c_swing_only, c_strict_only=c_strict_only,
    )
    try:
        BACKTEST_SUMMARY_TXT.write_text(report, encoding='utf-8')
        if debug:
