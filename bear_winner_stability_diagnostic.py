...
HEADER = "🧭🐻 [하락장 고유승자 × 0패턴 × 24·12·8·4주 안정성 진단 · RESEARCH_ONLY]"
...
def _prepare_formula_rows(out: Path, eval_df: pd.DataFrame | None = None) -> tuple[pd.DataFrame, str]:
    candidates = [
        ("GEO_FORMULA_EXPANDED", out / "v73_bear_geo_winner_event_master.csv"),
    ...
    for label, path in candidates:
        ...
        if not z.empty:
            ...
            source = label
            break
    if q.empty and isinstance(eval_df, pd.DataFrame):
        q = eval_df.copy()
    if q.empty:
        return q, "NO_INPUT"
    ...
    q["market_regime"] = _series_str(q, ["market_regime", "market_regime_causal", "market_state"], "UNKNOWN").str.upper()
    ...
    for dest, names in {
        "ret1": ["ret1", "day1_ret", "next1_close_ret"],
        "ret3": ["ret3", "day3_ret", "next3_close_ret"],
        "ret5": ["ret5", "day5_ret", "next5_close_ret"],
    ...
    # If the V16 geo output is absent, reconstruct only the technical bear classes.
    ...
    # One formula membership vote per stock event/formula.
...
def _load_replay_dates(out: Path, membership: pd.DataFrame) -> list[pd.Timestamp]:
    ...
    candidates = [
        ...
        "v72_backtest_data_path_audit.csv",
    ]
    for name in candidates:
        ...
        if not dc:
            continue
        for v in pd.to_datetime(q[dc], errors="coerce").dropna():
            dates.add(pd.Timestamp(v).normalize())
    if not dates and not membership.empty:
        for v in pd.to_datetime(membership["signal_date"], errors="coerce").dropna():
            dates.add(pd.Timestamp(v).normalize())
    return sorted(dates)


def _window_list() -> list[int]:
    raw = os.environ.get("V7336617_STABILITY_WINDOWS", "24,12,8,4")
    vals = []
    for x in raw.split(","):
        try:
            v = int(x.strip())
...
def _window_perf(g: pd.DataFrame, formula: str, requested: int, all_dates: list[pd.Timestamp]) -> dict[str, Any]:
    dates = all_dates[-requested:] if len(all_dates) >= requested else all_dates
    z = g[g["signal_date"].isin(dates)].copy()
    r = pd.to_numeric(z["ret3"], errors="coerce")
    ex = pd.to_numeric(z["market_excess3"], errors="coerce")
    date_mean = z.assign(_r=r).groupby("signal_date")["_r"].mean() if len(z) else pd.Series(dtype=float)
    return {
        "formula": formula, "window_requested": requested, "window_actual_signal_days": len(dates), "window_complete": len(dates) >= requested,
        "window_start": min(dates) if dates else pd.NaT, "window_end": max(dates) if dates else pd.NaT,
        "n": len(z), "stocks": z["code"].nunique(), "signal_days": z["signal_date"].nunique(),
...
def _stability(membership: pd.DataFrame, replay_dates: list[pd.Timestamp] | None = None) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    if not dates:
        dates = sorted(pd.to_datetime(membership["signal_date"], errors="coerce").dropna().unique())
    ...
    for formula, g in membership.groupby("formula"):
        ...
        split_dates = dates
        cut = max(1, int(len(split_dates) * 2 / 3))
        train_dates, oos_dates = split_dates[:cut], split_dates[cut:]
        ...
        sample_ok = _num(full.get("n"), 0) >= MIN_ROWS and _num(full.get("signal_days"), 0) >= MIN_SIGNAL_DAYS
        ...
        recent12_ok = all(_num(w12.get(c), -999) > 0 for c in ["d3_median", "d3_trim10", "net50_mean"]) if len(w12) else False
        recent8_ok = all(_num(w8.get(c), -999) > 0 for c in ["d3_median", "d3_trim10", "net50_mean"]) if len(w8) else False
        wf_ok = len(oo) >= 5 and oo["signal_date"].nunique() >= 3 and _num(oo_r.median(), -999) > 0 and _num((oo_r - 0.5).mean(), -999) > 0
        ...
        policy_rows.append({
            "formula": formula, "policy_status": status, "base_window": maxw, "available_signal_days": len(dates),
            "full_n": full.get("n", np.nan), "full_signal_days": full.get("signal_days", np.nan), "full_d3_median": full.get("d3_median", np.nan), "full_d3_trim10": full.get("d3_trim10", np.nan),
            "full_d3_top5_removed": full.get("d3_top5_removed", np.nan), "full_net50_mean": full.get("net50_mean", np.nan), "full_excess3_median": full.get("excess3_median", np.nan),
            "recent12_median": w12.get("d3_median", np.nan), "recent12_trim10": w12.get("d3_trim10", np.nan), "recent12_net50": w12.get("net50_mean", np.nan),
            "recent8_median": w8.get("d3_median", np.nan), "recent8_trim10": w8.get("d3_trim10", np.nan), "recent8_net50": w8.get("net50_mean", np.nan),
            ...
            "sample_ok": sample_ok, "robust_full": robust_full, "excess_required": excess_required, "excess_ok": excess_ok, "recent12_ok": recent12_ok, "recent8_ok": recent8_ok, "walk_forward_ok": wf_ok,
        })
    policy = pd.DataFrame(policy_rows)
    order = {"STABLE_POLICY_CANDIDATE": 0, "REGIME_SENSITIVE": 1, "BROAD_SCANNER_ONLY": 2, "RESEARCH_ONLY": 3, "INSUFFICIENT_SAMPLE": 4}
    if not policy.empty:
...
def _report(master: pd.DataFrame, membership: pd.DataFrame, matched: pd.DataFrame, zero: pd.DataFrame, policy: pd.DataFrame, sector_join: pd.DataFrame, minute: pd.DataFrame, source: str, status: str) -> str:
    ...
    formula_rows = len(membership)
    unique_rows = len(master)
    duplicate_votes_removed = max(0, formula_rows - unique_rows)
    lines = [
        HEADER,
        f"📌 {VERSION} · BEAR_WINNER_STABILITY_GEO_CAUSAL_COMPLETE · RESEARCH_ONLY=True",
        "- 승자·패자 공통점은 신호일+종목 1사건 1표로 계산하고, 검색식은 membership 원장에서만 전개합니다.",
        f"🧾 입력: {source} | 검색식행 {formula_rows} · 고유사건 {unique_rows} · 중복투표 제거 {duplicate_votes_removed} · 하락장 고유사건 {len(bear)} · 상태 {status}",
        "🐻 [하락장 고유사건 분류]",
    ...
    if zshow.empty:
        ...
        for _, r in zshow.sort_values(["dimension", "label"]).head(8).iterrows():
            lines.append(f"- {r['dimension']} · {r['label']}: n{int(r['n'])}/일{int(r['signal_days'])} | D3 중앙 {_num(r['d3_median']):+.2f}%·절사 {_num(r['d3_trim10']):+.2f}%·상5제거 {_num(r['d3_top5_removed']):+.2f}% | 승자 {_num(r['winner_rate']):.1f}%")
    lines.append("📊 [24·12·8·4주 검색식 안정성]")
    pshow = policy.head(8) if not policy.empty else pd.DataFrame()
    if pshow.empty:
        lines.append("- 안정성 표본 없음")
    else:
        for _, r in pshow.iterrows():
            lines.append(f"- {r['formula']}: 24주 중앙 {_num(r['full_d3_median']):+.2f}%·상5 {_num(r['full_d3_top5_removed']):+.2f}% | 12주 {_num(r['recent12_median']):+.2f}% | 8주 {_num(r['recent8_median']):+.2f}% | OOS {_num(r['wf_oos_median']):+.2f}% | {r['policy_status']}")
    ...
    lines += [
        "🔒 [검증 운용]",
        "- 기본 실행창은 24주로 두고, 같은 실행 안에서 최근 12주·8주·4주와 앞 2/3→뒤 1/3 Walk-forward를 동시에 봅니다.",
        "- 24주 평균이 좋아도 최근 8주 또는 Walk-forward가 음수면 REGIME_SENSITIVE이며 LIVE 승격하지 않습니다.",
        "- 공식 과거 지정학 자료는 published_at/official_at과 OFFICIAL_ARCHIVE_CAUSAL이 확인된 행만 성과에 결합합니다.",
        "- 실제 섹터 breadth와 분봉 원장이 없으면 UNKNOWN으로 두며 실패나 중립으로 위장하지 않습니다.",
        "- Actions CSV: " + " · ".join([UNIQUE_MASTER_FILE, FORMULA_MEMBERSHIP_FILE, UNIQUE_MATCHED_FILE, UNIQUE_COMMONALITY_FILE, ZERO_PATTERN_FILE, ZERO_PATTERN_FEATURE_FILE, STABILITY_MATRIX_FILE, STABILITY_POLICY_FILE, OFFICIAL_ARCHIVE_FILE, SECTOR_BREADTH_FILE, SECTOR_BREADTH_JOIN_FILE, MINUTE_SCALE_READINESS_FILE, READINESS_FILE]),
    ]
    return "\n".join(lines)
...
def run_backtest(eval_df: pd.DataFrame | None = None, output_dir: str | Path = "reports", base_report: str = "") -> tuple[str, dict[str, pd.DataFrame]]:
    ...
    replay_dates = _load_replay_dates(out, membership)
    matrix, policy = _stability(membership, replay_dates)
    ...
    tables = [master_with_pattern, membership, matched, commonality, zero_audit, zero_features, matrix, policy, official, sector_history, sector_join, minute]
    for q in tables:
    ...
    readiness = pd.DataFrame([{
        "version": VERSION, "status": status, "source": source, "formula_rows": len(formula_rows), "unique_event_rows": len(master_with_pattern),
        ...
        "matched_control_rows": len(matched), "event_consistency_fail_rows": consistency_fail, "official_archive_rows": len(official),
        "sector_breadth_coverage_pct": sector_cov, "minute_ready_source_count": minute_ready, "stable_policy_candidate_count": stable_count,
        "policy_ready": policy_ready, "recommended_backtest_weeks": 24, "stability_windows": ",".join(map(str, _window_list())),
        "snapshot_id": snapshot, "generated_at": generated_at, "research_only": True, "live_logic_changed": False, "real_order_changed": False,
    }])

    file_map = {
        UNIQUE_MASTER_FILE: master_with_pattern, FORMULA_MEMBERSHIP_FILE: membership, UNIQUE_MATCHED_FILE: matched,
        UNIQUE_COMMONALITY_FILE: commonality, ZERO_PATTERN_FILE: zero_audit, ZERO_PATTERN_FEATURE_FILE: zero_features,
        STABILITY_MATRIX_FILE: matrix, STABILITY_POLICY_FILE: policy, OFFICIAL_ARCHIVE_FILE: official,
        SECTOR_BREADTH_FILE: sector_history, SECTOR_BREADTH_JOIN_FILE: sector_join,
        MINUTE_SCALE_READINESS_FILE: minute, READINESS_FILE: readiness,
    }
    for name, q in file_map.items():
        _write(out / name, q)
    block = _report(master_with_pattern, membership, matched, zero_audit, policy, sector_join, minute, source, status)
    (out / REPORT_FILE).write_text(block, encoding="utf-8")
    return _insert(base_report, block), {
        "unique_master": master_with_pattern, "formula_membership": membership, "matched_control": matched,
        "commonality": commonality, "zero_pattern": zero_audit, "zero_pattern_features": zero_features,
        "stability_matrix": matrix, "stability_policy": policy, "official_archive": official,
        "sector_breadth": sector_history, "sector_join": sector_join, "minute_readiness": minute, "readiness": readiness,
    }
