...
def _deliver_telegram(parts: list[list[str]], out: Path, requested: bool) -> dict:
    ...
    path = out / 'v49_66_telegram_delivery_manifest.json'
    try:
        if requested:
            if delivery['route_validated'] != '1':
                raise RuntimeError('v49.66 Telegram requested but unified route preflight was not validated')
            if not s._telegram_route_ready():
                raise RuntimeError('v49.66 Telegram requested but scanner route is not ready after validated preflight')
            for part_no, part in enumerate(parts, start=1):
                ...
                if int(result.get('success_count', 0) or 0) < 1:
                    raise RuntimeError(f'v49.66 Telegram delivery failed for part {part_no}: {result.get("errors", [])}')
...
def _lp_validation(selected: pd.DataFrame, out: Path, start_date: str, end_date: str) -> pd.DataFrame:
    ...
    if z.empty:
        ...
        r.to_csv(out / 'v49_66_lp_walk_forward.csv', index=False, encoding='utf-8-sig')
        return r
...
def _iit_split(selected: pd.DataFrame, out: Path, start_date: str, end_date: str) -> pd.DataFrame:
    ...
    r = pd.DataFrame(rows)
    r.to_csv(out / 'v49_66_iit_flow_split.csv', index=False, encoding='utf-8-sig')
    return r


# ---- v49.66 winner/loser, market, sector and causal feature audit ----
V4966_ENTRY_FEATURE_META = {
...
V4966_FORBIDDEN_FEATURE_TOKENS = ('path_', 'rule35', 'ret_next', 'ret_close', 'hit3', 'stop_before', 'first_event', 'eval_')
V4966_MARKET_MIN_COVERAGE_PCT = float(os.environ.get('CLOSING_BET_V4966_MARKET_MIN_COVERAGE_PCT', '80') or 80)
V4966_FEATURE_MIN_OOS_N = max(10, int(float(os.environ.get('CLOSING_BET_V4966_FEATURE_MIN_OOS_N', '30') or 30)))
V4966_FEATURE_MIN_LIFT_PCTP = float(os.environ.get('CLOSING_BET_V4966_FEATURE_MIN_LIFT_PCTP', '0.30') or 0.30)


def _v4966_num_series(df: pd.DataFrame, col: str, default=np.nan) -> pd.Series:
    if col in df.columns:
        return pd.to_numeric(df[col], errors='coerce')
    return pd.Series(default, index=df.index, dtype=float)


def _v4966_first_text(df: pd.DataFrame, cols: list[str]) -> pd.Series:
...
def _v4966_index_frame(symbol: str, code: str, label: str, start_date: str, end_date: str) -> pd.DataFrame:
    try:
...
def _v4966_event_bucket_row(row: pd.Series) -> str:
...
def _v4966_attach_context(selected: pd.DataFrame, canonical: pd.DataFrame, universe: dict, start_date: str, end_date: str) -> tuple[pd.DataFrame, dict]:
    x = selected.copy()
    ...
    payload_market = _v4966_first_text(x, ['market','market_name','exchange','index_label','universe_tag'])
    payload_sector = _v4966_first_text(x, ['sector_name','sector','industry','theme_name','theme'])
    ...
    ks = _v4966_index_frame('KS11','1001','KOSPI',request_start,request_end)
    kq = _v4966_index_frame('KQ11','2001','KOSDAQ',request_start,request_end)
    ...
    for n in (1,5,20):
        x[f'stock_excess_{n}d'] = _v4966_num_series(x,f'entry_stock_ret_{n}d') - _v4966_num_series(x,f'market_ret_{n}d')
    x['market_down_context'] = (
        x['market_m5_t1'].eq('M5-비우호') |
        _v4966_num_series(x,'market_ret_1d').lt(0) |
        _v4966_num_series(x,'market_ret_5d').lt(0)
    ).astype(int)
    ...
    psec = _v4966_first_text(base,['sector_name','sector','industry','theme_name','theme'])
    base['sector_label'] = bsec.where(bsec.ne(''),psec)
    base['_ret1'] = _v4966_num_series(base,'entry_stock_ret_1d')
    base['_amount'] = _v4966_num_series(base,'entry_amount_b',0).fillna(0)
    ...
    if not good.empty:
        ...
        r = _v4966_num_series(x,'entry_stock_ret_1d')
        ...
        x['sector_peer_amount_b'] = (pd.to_numeric(x.get('_sector_amount'),errors='coerce')-_v4966_num_series(x,'entry_amount_b',0).fillna(0)).where(n>1)
    ...
    x['event_theme_bucket'] = x.apply(_v4966_event_bucket_row,axis=1)
    diag = {
...
def _v4966_classify_outcomes(x: pd.DataFrame) -> pd.DataFrame:
    z=x.copy()
    z['_gross']=_v4966_num_series(z,'rule35_pnl')
    z['_net20']=z['_gross']-.20; z['_net50']=z['_gross']-.50
    p3=_v4966_num_series(z,'path_first_plus3_day',0).fillna(0)
    p10=_v4966_num_series(z,'path_first_plus10_day',0).fillna(0)
    stopd=_v4966_num_series(z,'path_first_stop_day',0).fillna(0)
    stop=_v4966_num_series(z,'stop_before_3',0).fillna(0).gt(0) | _v4966_num_series(z,'rule35_stop',0).fillna(0).gt(0) | stopd.gt(0)
    hit3=_v4966_num_series(z,'hit3_before_stop',0).fillna(0).gt(0) | _v4966_num_series(z,'rule35_hit3',0).fillna(0).gt(0) | p3.gt(0)
    big=(p10>0)&((stopd<=0)|(p10<stopd))
    pre3=_v4966_num_series(z,'path_pre_plus3_min_low_ret')
    ...
    z['down_market_independent_win']=((z.get('market_down_context',0)==1)&(z['_net50']>0)&(_v4966_num_series(z,'stock_excess_1d')>0)).astype(int)
    return z


def _v4966_anatomy_tables(x: pd.DataFrame, out: Path, start_date: str, end_date: str) -> tuple[pd.DataFrame,pd.DataFrame,pd.DataFrame,pd.DataFrame,pd.DataFrame]:
    split=_split_date(start_date,end_date)
    ...
    for scope in ('ALL','TRAIN','OOS'):
        ...
        for mode in modes:
            ...
            for oc,g in q.groupby('outcome_class'):
                outcome_rows.append({'scope':scope,'strategy':mode,'outcome_class':oc,'n':len(g),'share_pct':len(g)/len(q)*100.0,'net20_mean_pct':g['_net20'].mean(),'net50_mean_pct':g['_net50'].mean(),'max_high_mean_pct':_v4966_num_series(g,'path_max_high_ret').mean(),'mae_mean_pct':_v4966_num_series(g,'path_min_low_ret').mean()})
            win=q[q.net50_group.eq('WIN_NET50')]; loss=q[q.net50_group.eq('LOSS_NET50')]
            for feat,(label,causal) in V4966_ENTRY_FEATURE_META.items():
                a=_v4966_num_series(win,feat).dropna(); b=_v4966_num_series(loss,feat).dropna()
    ...
    for feat in V4966_ENTRY_FEATURE_META:
        low=feat.lower()
        if any(tok in low for tok in V4966_FORBIDDEN_FEATURE_TOKENS):
    ...
    for mode in modes:
        ...
        for feat,(label,causal) in V4966_ENTRY_FEATURE_META.items():
            t=_v4966_num_series(tr,feat); o=_v4966_num_series(oo,feat)
            ...
            if causal!='AUDIT_PROXY' and len(fq)>=V4966_FEATURE_MIN_OOS_N and fmean>0 and lift>=V4966_FEATURE_MIN_LIFT_PCTP and pd.notna(stress3) and stress3>0 and stable_halves:
                status='PAPER_PRIORITY_CANDIDATE'
    ...
    z.to_csv(out/'v49_66_selected_enriched_outcomes.csv',index=False,encoding='utf-8-sig')
    outcomes.to_csv(out/'v49_66_outcome_distribution.csv',index=False,encoding='utf-8-sig')
    features.to_csv(out/'v49_66_winner_loser_feature_anatomy.csv',index=False,encoding='utf-8-sig')
    lifts.to_csv(out/'v49_66_train_oos_feature_lift.csv',index=False,encoding='utf-8-sig')
    market.to_csv(out/'v49_66_market_regime_performance.csv',index=False,encoding='utf-8-sig')
    events.to_csv(out/'v49_66_event_theme_context.csv',index=False,encoding='utf-8-sig')
    down.to_csv(out/'v49_66_down_market_cases.csv',index=False,encoding='utf-8-sig')
    return outcomes,features,lifts,market,events


def main() -> int:
    ap = argparse.ArgumentParser(description='v49.66 global merge + identity/pipeline/attribution audit')
    ap.add_argument('--input-root', default='v49_66_downloads')
    ap.add_argument('--prepare-dir', default='v49_66_prepare_output')
    ...
    if not prep_root.exists():
        prep_root = root
    preflight_path = next(iter(prep_root.rglob('v49_66_preflight.json')), None)
    universe_path = next(iter(prep_root.rglob('v49_66_universe.json')), None)
    ...
    for sid in expected_ids:
        ...
        if str(m.get('control_flow', '')) != 'V49_66_SHARD_EXPORT_RETURN_MAIN_EXIT':
    ...
    raw_path = out / 'v49_66_global_raw.csv'
    canonical_path = out / 'v49_66_global_canonical.csv'
    selected_path = out / 'v49_66_global_selected_top5.csv'
    ...
    populations['PRIMARY'].to_csv(out / 'v49_66_primary_selected_top5.csv', index=False, encoding='utf-8-sig')
    populations['ALL_MATCHED'].to_csv(out / 'v49_66_all_matched_selected_top5.csv', index=False, encoding='utf-8-sig')
    ...
    funnel.to_csv(out / 'v49_66_strategy_funnel.csv', index=False, encoding='utf-8-sig')
    strategy_status.to_csv(out / 'v49_66_strategy_stage_audit.csv', index=False, encoding='utf-8-sig')
    identity_audit.to_csv(out / 'v49_66_identity_audit.csv', index=False, encoding='utf-8-sig')
    zero_df.to_csv(out / 'v49_66_zero_mode_audit.csv', index=False, encoding='utf-8-sig')
    exc_df.to_csv(out / 'v49_66_predicate_exceptions.csv', index=False, encoding='utf-8-sig')
    ...
    if out.resolve() != generated_root.resolve():
        import shutil
        for gp in generated_root.glob('v49_66_common_strategy_*'):
            shutil.copy2(gp, out / gp.name)
    performance_csv = out / 'v49_66_common_strategy_performance.csv'
    portfolio_csv = out / 'v49_66_common_strategy_portfolio.csv'
    ...
    attribution.to_csv(out / 'v49_66_strategy_attribution_performance.csv', index=False, encoding='utf-8-sig')
    lpwf = _lp_validation(selected, out, args.start_date, args.end_date)
    iit = _iit_split(selected, out, args.start_date, args.end_date)

    selected_ctx, anatomy_diag = _v4966_attach_context(selected, canonical, universe, args.start_date, args.end_date)
    selected_ctx = _v4966_classify_outcomes(selected_ctx)
    outcomes, anatomy_features, feature_lifts, market_perf, event_perf = _v4966_anatomy_tables(selected_ctx, out, args.start_date, args.end_date)
    anatomy_status = 'FULL-VALID' if anatomy_diag.get('market_coverage_pct', 0) >= V4966_MARKET_MIN_COVERAGE_PCT and not outcomes.empty else ('PARTIAL-VALID' if not outcomes.empty else 'INVALID')
    feature_candidates = feature_lifts[feature_lifts.status.eq('PAPER_PRIORITY_CANDIDATE')].copy() if not feature_lifts.empty else pd.DataFrame()
    down_cases_path = out / 'v49_66_down_market_cases.csv'
    ...
    lines2 = ['(2/6)', '📊 전략별 OOS · Predicate Mode | v49.66', '──────────']
    if not ptab.empty:
    ...
    lines3 = ['(3/6)', '🔬 전략 Funnel·Pipeline | v49.66', '──────────']
    ...
    lines4 = ['(4/6)', '🧭 전략 귀속·LP·I/IT | v49.66', '──────────']
    for mode in s.CLOSING_BET_V4958_PRIMARY_PRIORITY:
    ...
    lines5 = ['(5/6)', '🧬 상승·하락 경로 해부 | v49.66', '──────────']
    lines5.append(f'- ANATOMY STATUS: {anatomy_status} · 시장커버리지 {anatomy_diag.get("market_coverage_pct",0):.1f}% · 섹터신호프록시 {anatomy_diag.get("sector_coverage_pct",0):.1f}% · 이벤트매핑 {anatomy_diag.get("event_mapped_rows",0)}건')
    ...
    lines6 = ['(6/6)', '🌧️ 하락장 독립상승·개선후보 | v49.66', '──────────']
    ...
    if not feature_candidates.empty:
        lines6.append('[TRAIN 발견 → OOS 고정 통과 · PAPER 우선순위 후보]')
        ...
        lines6.append('- OOS·Top3 제거까지 통과한 단일특징 개선후보 없음')
    ...
    lines6 += ['', '- 섹터동행은 전체 업종 유니버스가 아닌 신호후보 프록시이므로 자동필터 승격 금지.', '- 모든 개선후보는 PAPER 우선순위 연구이며 검색조건·LIVE·자동주문에 자동반영하지 않습니다.']

    report = '\n'.join(lines1 + [''] + lines2 + [''] + lines3 + [''] + lines4 + [''] + lines5 + [''] + lines6)
    (out / 'v49_66_global_summary.txt').write_text(report, encoding='utf-8')
    files = [
        raw_path, canonical_path, selected_path,
        out / 'v49_66_primary_selected_top5.csv', out / 'v49_66_all_matched_selected_top5.csv',
        out / 'v49_66_strategy_funnel.csv', out / 'v49_66_strategy_stage_audit.csv', out / 'v49_66_identity_audit.csv',
        out / 'v49_66_zero_mode_audit.csv', out / 'v49_66_predicate_exceptions.csv',
        out / 'v49_66_strategy_attribution_performance.csv', out / 'v49_66_lp_walk_forward.csv', out / 'v49_66_iit_flow_split.csv',
        out / 'v49_66_selected_enriched_outcomes.csv', out / 'v49_66_outcome_distribution.csv',
        out / 'v49_66_winner_loser_feature_anatomy.csv', out / 'v49_66_train_oos_feature_lift.csv',
        out / 'v49_66_market_regime_performance.csv', out / 'v49_66_event_theme_context.csv',
        out / 'v49_66_down_market_cases.csv',
    ]
    manifest = {
    ...
    (out / 'v49_66_global_manifest.json').write_text(json.dumps(manifest, ensure_ascii=False, indent=2, default=str), encoding='utf-8')
    print(report)
    _deliver_telegram([lines1, lines2, lines3, lines4, lines5, lines6], out, bool(args.send_telegram))
