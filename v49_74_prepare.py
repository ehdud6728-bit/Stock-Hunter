...
def main() -> int:
    ap = argparse.ArgumentParser(description='v49.74 trading-day MARCAP authority + immutable universe/search preflight')
    ap.add_argument('--end-date', required=True)
    ap.add_argument('--output-dir', default='v49_74_prepare_output')
    args = ap.parse_args()
    out = Path(args.output_dir)
    out.mkdir(parents=True, exist_ok=True)

    # v49.74 first resolves an OBSERVED KRX trading date. This removes the v49.73
    ...
    if not trading_asof:
        raise RuntimeError(f'v49.74 trading-date authority missing: {trading_calendar}')
    print(
    ...
    # v49.74 MARCAP SNAPSHOT AUTHORITY LOCK:
    ...
    # v49.74: immutable entry-time classification maps. These are labels only; no future data.
    ...
    try:
        ...
        print(f'v49.74 listing classification map fallback: {type(exc).__name__}: {exc}')
    ...
    if post_audit_marcap_sha != str(marcap_meta.get('marcap_map_sha256','')):
        raise RuntimeError(f'V49.74 MARCAP_PREPARE_MUTATION: post-audit {post_audit_marcap_sha} != authority {marcap_meta.get("marcap_map_sha256","")}')
    ...
    # v49.74: component-level authority fingerprints. A code-list hash alone cannot detect
    ...
    snapshot_path = out / 'v49_74_prepared_authority_snapshot.csv'
    ...
    snapshot_manifest_path = out / 'v49_74_prepared_authority_snapshot.json'
    ...
    upath = out / 'v49_74_universe.json'
    ...
    ppath = out / 'v49_74_preflight.json'
