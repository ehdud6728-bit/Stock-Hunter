...
# =============================================================
# closing_bet_scanner.py — 종가배팅 타점 스캐너 (G Morales v4.1.9 운영 출력 정리)
# =============================================================
# 전략 구성
# A  : 돌파형 종가배팅
...
CLOSING_BET_SCANNER_VERSION = 'G_MORALES_V4_2_7_H_TRIANGLE_AMOUNT_VOL_CORE_20260512'
...
def _send_results(hits: list, mins_left: int):
    ...
    def _build_practical_block():
        ...
        block = ['[🎯 실전 운영 후보 — v4.1.9 S-SAFE + G-SAFE]']
        summary = (
        ...
        block.append('운용: S-CORE SAFE가 1순위, G-SAFE는 SAFE 다음 보조 실전 후보입니다. S/G NEUTRAL은 관찰, A/B/C와 G-AGGRESSIVE는 진단·주의군입니다.')
...
def _evaluate_h_pre_breakout_context(hist: pd.DataFrame, breakout_pos: int) -> dict:
    """v4.2.7: H 신고가거자름 돌파 직전 구조/변동성 진단.
        ...
        try:
            if len(hist) >= 150 and info['amount_b'] >= MIN_AMOUNT / 1e8:
                ...
                for d in range(1, max_days + 1):
                    ...
                    # v4.2.7 핵심: 신고가를 '장대양봉'으로 종가 돌파한 기준봉만 인정한다.
...
def _bt_mask_h_struct_safe(df: pd.DataFrame):
    """v4.2.7: H 신고가 거자름 STRUCT-SAFE 후보.
...
def _bt_mask_h_vol_safe(df: pd.DataFrame):
    """v4.2.7: H-VOL SAFE. 돌파Vol60 2~3배의 정상 강도 신고가 거자름."""
    if df is None or df.empty or 'mode' not in df.columns:
        return pd.Series(False, index=df.index if df is not None else [])
    bvol = pd.to_numeric(df.get('breakout_vol60_ratio', 0), errors='coerce').fillna(0)
...
def _bt_mask_h_vol_swing(df: pd.DataFrame):
    """v4.2.7: H-VOL SWING. 돌파Vol60 3~5배, 3/5보다 MA5 기준 스윙 검증 후보."""
    if df is None or df.empty or 'mode' not in df.columns:
        return pd.Series(False, index=df.index if df is not None else [])
    bvol = pd.to_numeric(df.get('breakout_vol60_ratio', 0), errors='coerce').fillna(0)
...
def _bt_mask_h_vol_overheat(df: pd.DataFrame):
    """v4.2.7: H-VOL OVERHEAT. 돌파Vol60 5~8배, 과열/실패 가능성 후보."""
    if df is None or df.empty or 'mode' not in df.columns:
        return pd.Series(False, index=df.index if df is not None else [])
    bvol = pd.to_numeric(df.get('breakout_vol60_ratio', 0), errors='coerce').fillna(0)
...
def _bt_mask_h_vol_aggressive(df: pd.DataFrame):
    """v4.2.7: H-VOL AGGRESSIVE. 돌파Vol60 8배 이상, 빠른 +3/+5 익절 전용 후보."""
    if df is None or df.empty or 'mode' not in df.columns:
        return pd.Series(False, index=df.index if df is not None else [])
    bvol = pd.to_numeric(df.get('breakout_vol60_ratio', 0), errors='coerce').fillna(0)
...
def _bt_mask_h_safe(df: pd.DataFrame):
    """v4.2.7: 최종 H-SAFE는 돌파Vol60 2~3배 구간으로 제한한다."""
    return _bt_mask_h_vol_safe(df)


def _bt_mask_h_risk(df: pd.DataFrame):
    """v4.2.7: H 신고가 거자름 STRICT 위험 후보.
...
def _bt_mask_h_neutral(df: pd.DataFrame):
    """v4.2.7: H 후보 중 최종 SAFE/SWING/OVERHEAT/AGGRESSIVE/RISK 어디에도 속하지 않는 잔여 관찰 후보."""
    if df is None or df.empty or 'mode' not in df.columns:
        return pd.Series(False, index=df.index if df is not None else [])
    return (
        _bt_mask_h_all(df)
...
def _bt_mask_h_lowvol(df: pd.DataFrame):
    """v4.2.7: H 후보 중 직전 구조 기준 고변동이 아닌 후보."""
    if df is None or df.empty or 'mode' not in df.columns:
        return pd.Series(False, index=df.index if df is not None else [])
...
def _bt_mask_h_prior_structure(df: pd.DataFrame):
    """v4.2.7: H 후보 중 돌파 직전 삼각수렴/횡보/역매공파/눌림 구조가 1개 이상 있는 후보."""
    if df is None or df.empty or 'mode' not in df.columns:
        return pd.Series(False, index=df.index if df is not None else [])
...
def _bt_mask_h_pattern_safe(df: pd.DataFrame):
    """v4.2.7: H-STRUCT + 저변동 + 직전 구조 존재 후보."""
    if df is None or df.empty or 'mode' not in df.columns:
        return pd.Series(False, index=df.index if df is not None else [])
...
def _bt_mask_h_vol_safe_pattern(df: pd.DataFrame):
    """v4.2.7: H-VOL SAFE 2~3배 + 저변동 + 직전 구조 존재 후보."""
    if df is None or df.empty or 'mode' not in df.columns:
        return pd.Series(False, index=df.index if df is not None else [])
...
def _bt_h_breakout_amount(df: pd.DataFrame) -> pd.Series:
    """v4.2.7: 신고가 장대양봉 돌파봉 거래대금(억원) 우선 사용."""
    if df is None:
...
def _bt_mask_h_triangle(df: pd.DataFrame):
    """v4.2.7: 돌파 직전 삼각수렴이 있었던 H 전체 후보."""
    if df is None or df.empty or 'mode' not in df.columns:
        return pd.Series(False, index=df.index if df is not None else [])
...
def _bt_mask_h_triangle_struct(df: pd.DataFrame):
    """v4.2.7: H-STRUCT 조건까지 통과한 직전 삼각수렴 후보."""
    if df is None or df.empty or 'mode' not in df.columns:
        return pd.Series(False, index=df.index if df is not None else [])
...
def _bt_mask_h_core_500_1000_vol23(df: pd.DataFrame):
    """v4.2.7: 돌파봉 거래대금 500~1000억 × Vol60 2~3배 핵심 H 후보."""
    if df is None or df.empty or 'mode' not in df.columns:
        return pd.Series(False, index=df.index if df is not None else [])
...
def _bt_mask_h_core_300_500_vol35(df: pd.DataFrame):
    """v4.2.7: 돌파봉 거래대금 300~500억 × Vol60 3~5배 핵심 H 후보."""
    if df is None or df.empty or 'mode' not in df.columns:
        return pd.Series(False, index=df.index if df is not None else [])
    amt = _bt_h_breakout_amount(df)
    bvol = _bt_h_breakout_vol60(df)
...
def _bt_mask_h_core_1000_2000_vol23(df: pd.DataFrame):
    """v4.2.7: 돌파봉 거래대금 1000~2000억 × Vol60 2~3배 보조 핵심 후보."""
    if df is None or df.empty or 'mode' not in df.columns:
        return pd.Series(False, index=df.index if df is not None else [])
    amt = _bt_h_breakout_amount(df)
    bvol = _bt_h_breakout_vol60(df)
...
def _format_h_v427_core_report(df: pd.DataFrame) -> str:
    """v4.2.7: H-TRIANGLE / 거래대금×Vol 핵심 운용분류 요약."""
    lines = ["[H v4.2.7 핵심 운용분류 — TRIANGLE / AMOUNT×VOL]"]
    try:
        if df is None or df.empty or 'mode' not in df.columns:
            lines.append("- 데이터 없음")
            return "\n".join(lines)
        ...
        lines.append("- 해석: v4.2.7은 일반 H를 실전 후보로 보지 않고, 직전 삼각수렴 또는 돌파봉 거래대금×Vol60 핵심셀만 따로 검증합니다.")
    except Exception as e:
        lines.append(f"- H v4.2.7 핵심분류 리포트 오류: {e}")
    return "\n".join(lines)

def _format_h_pre_structure_report(df: pd.DataFrame) -> str:
    """v4.2.7: H 돌파 직전 구조/고변동 필터 성과."""
    lines = ["[H 직전 구조·고변동 필터 성과 — v4.2.7]"]
    try:
        if df is None or df.empty or 'mode' not in df.columns:
            lines.append("- 데이터 없음")
            return "\n".join(lines)
...
def _format_h_amount_vol_matrix_report(df: pd.DataFrame) -> str:
    """v4.2.7: 돌파봉 거래대금 × 돌파Vol60 교차 성과.

    v4.2.5의 amount_b는 타점봉 거래대금에 가까웠다. v4.2.7에서는 breakout_amount_b를 사용해
    신고가 장대양봉이 실제로 어느 정도 자금으로 터졌는지와 Vol60 배율의 조합을 본다.
    """
    lines = ["[H 돌파봉 거래대금 × Vol60 매트릭스 — v4.2.7]"]
    try:
        if df is None or df.empty or 'mode' not in df.columns:
            lines.append("- 데이터 없음")
            return "\n".join(lines)
...
def _format_h_breakout_volume_report(df: pd.DataFrame) -> str:
    ...
    try:
        ...
        lines.append("- v4.2.7 운용분류: 🟢H-VOL SAFE=2~3배 / 🟣H-VOL SWING=3~5배 / ⚠️H-OVERHEAT=5~8배 / 🔥H-AGGRESSIVE=8배 이상")
...
def _format_h_amount_report(df: pd.DataFrame) -> str:
    """v4.2.7: H 신고가거자름의 거래대금 구간별 성과를 분해한다.
    ...
    """
    lines = ["[H 거래대금 세분화 구간별 성과 — v4.2.7]"]
    try:
        if df is None or df.empty or 'mode' not in df.columns:
            lines.append("- 데이터 없음")
            return "\n".join(lines)
        ...
        def _amount_bucket(frame: pd.DataFrame) -> pd.Series:
            ...
            # v4.2.7: 거래대금 구간 세분화. amount_b 단위는 억원.
...
def _format_h_reason_report(df: pd.DataFrame) -> str:
    ...
    lines = ["[H 신고가 거자름 STRICT VOL 분류/탈락 사유 분석 — v4.2.7]"]
    try:
        if df is None or df.empty or 'mode' not in df.columns:
            lines.append("- 데이터 없음")
            return "\n".join(lines)
        ...
        lines.append("- 해석: H-STRICT는 장대양봉 신고가 돌파봉을 먼저 찾고, 이후 거래량이 마른 짧은 타점봉을 검증합니다. v4.2.7에서는 돌파Vol60 2~3배만 최종 H-SAFE로 분류합니다.")
    except Exception as e:
...
def _build_backtest_summary(
    ...
    lines.append("- v4.2.7부터 H 신고가거자름은 H-TRIANGLE SAFE와 거래대금×Vol60 핵심셀(500~1000억×2~3배, 300~500억×3~5배)을 별도 운용분류로 추가하고, 돌파봉 Vol60 배율 운용분류에 더해, 거래대금 구간을 <30억/30~50억/50~100억/100~200억/200~300억/300~500억/500~1000억/1000~2000억/2000~3000억/3000~5000억/5000억~1조/1조+로 세분화해 성과를 분해합니다. H-VOL SAFE는 2~3배, H-VOL SWING은 3~5배, H-OVERHEAT는 5~8배, H-AGGRESSIVE는 8배 이상입니다. 수급 데이터는 이번 버전에서 필수 조건이 아니라 후속 가점 후보입니다.")
