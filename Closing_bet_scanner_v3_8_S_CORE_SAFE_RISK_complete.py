...
CLOSING_BET_SCANNER_VERSION = 'G_MORALES_V3_8_S_CORE_SAFE_RISK_20260510'
...
# v3.8: S-CORE SAFE/RISK 분리 기준. 12주 손절특이점 분석에서 안정적이었던 조건을 별도 표기한다.
...
def _send_results(hits: list, mins_left: int):
    ...
    def _brief_practical_line(h: dict, idx: int) -> str:
        ...
        safe_label = '🟢SAFE' if _is_s_core_safe_hit(h) else ('⚠️RISK:' + ','.join(risk_tags) if risk_tags else '🟡NORMAL')
    ...
    def _build_practical_block():
        pool = [h for h in hits if (_is_s_core_hit(h) or _is_a_strong(h))]
        pool = sorted(pool, key=_priority_practical)[:7]
        block = ['[🎯 실전 매수 우선 후보 — v3.8 S-CORE SAFE/RISK]']
        if not pool:
            block.append('해당 종목 없음 — 오늘은 S-CORE 또는 A강한돌파 기준을 만족한 실전 우선 후보가 없습니다.')
        else:
            block.append('기준: S-CORE 중 🟢SAFE(RR 1.0~1.5·거래량비<1.5·종가위치70%+)를 최우선으로 보고, ⚠️RISK는 추격주의/감점 후보로 분리합니다.')
...
def _select_backtest_top(df: pd.DataFrame, top_per_strategy: int = 5, all_candidates: bool = False) -> pd.DataFrame:
    ...
    # v3.6: C는 엄격형을 우선하고, 완화형은 진단용 후보로 뒤에 둔다.
...
def _bt_mask_s_core_safe(df: pd.DataFrame):
    """v3.8: 12주 손절특이점에서 안정적이었던 S-CORE SAFE 조건.
    - S-CORE 내부
    - RR 1.0~1.5
    - 거래량비 1.5 미만
    - 종가위치 70% 이상
...
def _bt_mask_s_core_risk(df: pd.DataFrame):
    """v3.8: S-CORE 내부 위험 플래그 조건. SAFE와 독립적으로 위험 구간을 잡는다."""
    if df is None or df.empty or 'mode' not in df.columns:
        return pd.Series(False, index=df.index if df is not None else [])
    rr = pd.to_numeric(df.get('rr', 0), errors='coerce').fillna(0)
    volr = pd.to_numeric(df.get('vol_ratio', df.get('today_vol_ratio', 0)), errors='coerce').fillna(0)
...
def _format_practical_combo_report(df: pd.DataFrame) -> str:
    """v3.8: 백테스트 기반 실전 추천 조합 요약. S-CORE를 SAFE/RISK로 분리한다."""
    lines = ["[실전 추천 조합 — v3.8 S-CORE SAFE/RISK 튜닝 관점]"]
    if df is None or df.empty:
        lines.append("- 데이터 없음")
        return "\n".join(lines)
    try:
        ...
        lines.append("- 해석: 실전 최우선은 S1 우수응축형 중 SAFE 후보입니다. S2는 거래량비 1.5 이상이면 과열 재점화로 보고 감점/추격주의 처리합니다.")
    except Exception as e:
...
def _format_tuning_report(df: pd.DataFrame) -> str:
    """v3.8: S/A/B 조건별 튜닝 리포트. 과최적화를 막기 위해 조건별 성과만 비교한다."""
    lines = ["[튜닝 리포트 — 조건별 성과]"]
    if df is None or df.empty:
        lines.append("- 데이터 없음")
        return "\n".join(lines)
    try:
        ...
        if not s_df.empty:
            ...
            lines.append("- S-CORE SAFE/RISK 분리 성과")
...
def _format_stoploss_signature_report(df: pd.DataFrame) -> str:
    """v3.8: 손절 종목과 비손절 종목의 차이 및 손절 집중 구간을 요약."""
    lines = ["[🩸 손절 종목 특이점 분석]"]
    if df is None or df.empty:
        lines.append("- 데이터 없음")
        return "\n".join(lines)
...
def _format_weekly_pattern_report(df: pd.DataFrame) -> str:
    """v3.8: 주차별로 패턴 강도와 약화/회복 신호를 감지."""
    lines = ["[📡 주차별 패턴 변화 감지]"]
    if df is None or df.empty:
        lines.append("- 데이터 없음")
        return "\n".join(lines)
    try:
        ...
        lines.append(f"- S-CORE 주차 유효성: {valid_weeks}주 중 {pos_weeks}주 플러스 | 주간평균 3/5수익 {avg_pnl:.2f}% | 평균 손절선행 {avg_stop:.1f}%")
        if not prior.empty:
        ...
        if avg_pnl > 0 and avg_stop <= 15:
            lines.append("- 판정: S-CORE 유효. 단, 약한 주에는 +3/+5 익절 우선으로 수익 반납을 줄이는 대응이 적절합니다.")
        elif avg_pnl > 0:
            lines.append("- 판정: S-CORE는 플러스지만 손절선행 관리가 필요합니다. 거래량 과열/종가위치 낮은 후보를 줄이세요.")
        else:
            lines.append("- 판정: S-CORE 약화 구간입니다. 신규 진입 축소 또는 다음날 확인형 전환이 필요합니다.")
    except Exception as e:
...
def _build_backtest_summary(
    ...
    try:
        ...
        lines.append("")
        lines.append(_format_stoploss_signature_report(df))
        lines.append("")
    except Exception as e:
    ...
    if weekly_breakdown:
        ...
        lines.append("")
        lines.append(_format_weekly_pattern_report(df))
    ...
    lines.append("- v3.8의 3/5규칙은 보수 모델입니다. 같은 날 손절가와 목표가가 모두 닿으면 손절을 먼저 본 것으로 계산합니다.")
    lines.append("- v3.8의 튜닝/손절특이점 리포트는 최근 백테스트 결과를 조건별 성과로 비교하는 참고용이며, 최소 4주~12주 이상 반복 검증이 필요합니다.")
    ...
    lines.append("- v3.8은 손절발생 종목의 공통점, S-CORE SAFE/RISK, 주차별 패턴 변화 감지를 함께 출력합니다.")
    return "\n".join(lines)
...
def run_closing_bet_backtest(
    ...
    if raw_df.empty:
        report = _build_backtest_summary(
            ...
            all_candidates, weekly_breakdown=weekly_breakdown, diag=diag, core_only=core_only, safe_only=safe_only,
        )
        try:
    ...
    if (safe_only or core_only) and selected_df is not None and not selected_df.empty:
        try:
            if safe_only:
    ...
    report = _build_backtest_summary(
        ...
        all_candidates, weekly_breakdown=weekly_breakdown, diag=diag if debug else None, core_only=core_only, safe_only=safe_only,
    )
    try:
        BACKTEST_SUMMARY_TXT.write_text(report, encoding='utf-8')
        if debug:
