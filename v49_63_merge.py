...
def _deliver_telegram(parts: list[list[str]], out: Path, requested: bool) -> dict:
    ...
    path = out / 'v49_63_telegram_delivery_manifest.json'
    try:
        if requested:
            if delivery['route_validated'] != '1':
                raise RuntimeError('v49.63 Telegram requested but unified route preflight was not validated')
            if not s._telegram_route_ready():
                raise RuntimeError('v49.63 Telegram requested but scanner route is not ready after validated preflight')
            for part_no, part in enumerate(parts, start=1):
                ...
                if int(result.get('success_count', 0) or 0) < 1:
                    raise RuntimeError(
                        f"v49.63 Telegram delivery failed for part {part_no}: "
                        f"{result.get('errors', [])}"
                    )
            delivery['status'] = 'DELIVERED'
            delivery['success_count'] = sum(int(x.get('success_count', 0) or 0) for x in delivery['parts'])
            print(
                'TELEGRAM DELIVERY ACK ✅ · '
                f"parts {len(delivery['parts'])}/{len(parts)} · "
...
def main() -> int:
    ap = argparse.ArgumentParser(description='v49.63 global shard merge')
    ap.add_argument('--input-root', default='v49_63_downloads')
    ap.add_argument('--prepare-dir', default='v49_63_prepare_output')
    ...
    if not prep_root.exists():
        ...
        prep_root = root
    preflight_path = next(iter(prep_root.rglob('v49_63_preflight.json')), None)
    universe_path = next(iter(prep_root.rglob('v49_63_universe.json')), None)
    ...
    if len(manifests) != args.shard_count or len(raws) != args.shard_count or len(completes) != args.shard_count:
        raise RuntimeError(
            f'shard artifacts incomplete: manifests {len(manifests)}/{args.shard_count}, '
            f'raw {len(raws)}/{args.shard_count}, complete {len(completes)}/{args.shard_count}'
        )
    ...
    manifest_path_by_id = {}
    for path, doc in zip(manifests, manifest_docs):
        manifest_path_by_id[int(doc.get('shard_index', -1))] = path
    ...
    for p in raws:
        try:
            ...
            raise RuntimeError(f'cannot parse shard id from {p}') from e
        raw_by_id[sid] = p
    if sorted(complete_by_id) != expected_ids or sorted(raw_by_id) != expected_ids:
        raise RuntimeError(f'shard completion/raw ids mismatch: complete={sorted(complete_by_id)}, raw={sorted(raw_by_id)}')
    for sid in expected_ids:
        p = raw_by_id[sid]; m = manifest_by_id[sid]; c = complete_by_id[sid]
        if file_sha(p) != str(m.get('raw_sha256', '')) or file_sha(p) != str(c.get('raw_sha256', '')):
            raise RuntimeError(f'raw sha mismatch: shard {sid} {p}')
        if str(m.get('engine_status', '')).upper() != 'VALID':
            raise RuntimeError(f'shard engine invalid: {m}')
        if str(m.get('control_flow', '')) != 'V49_63_SHARD_EXPORT_RETURN_MAIN_EXIT':
            raise RuntimeError(f'shard hard-exit contract invalid: {m}')
        if str(m.get('research_lane', '')) != 'SKIPPED' or str(m.get('telegram', '')) != 'SKIPPED' or str(m.get('post_export_network', '')) != 'FORBIDDEN':
            raise RuntimeError(f'shard isolation contract invalid: {m}')
        if str(c.get('status', '')) != 'SHARD_ONLY_COMPLETE':
            raise RuntimeError(f'shard completion marker invalid: {c}')
        manifest_path = manifest_path_by_id[sid]
        if file_sha(manifest_path) != str(c.get('manifest_sha256', '')):
            raise RuntimeError(f'manifest sha mismatch: shard {sid}')
    ...
    raw_path = out / 'v49_63_global_raw.csv'
    canonical_path = out / 'v49_63_global_canonical.csv'
    selected_path = out / 'v49_63_global_selected_top5.csv'
    ...
    if out.resolve()!=generated_root.resolve():
        import shutil
        for gp in generated_root.glob('v49_63_common_strategy_*'):
            shutil.copy2(gp,out/gp.name)

    performance_csv = out / 'v49_63_common_strategy_performance.csv'
    portfolio_csv = out / 'v49_63_common_strategy_portfolio.csv'
    ...
    report = '\n'.join(lines + [''] + lines2)
    (out / 'v49_63_global_summary.txt').write_text(report, encoding='utf-8')
    manifest = {
    ...
    (out / 'v49_63_global_manifest.json').write_text(json.dumps(manifest, ensure_ascii=False, indent=2, default=str), encoding='utf-8')
    print(report)
    _deliver_telegram([lines, lines2], out, bool(args.send_telegram))
