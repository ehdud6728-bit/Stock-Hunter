...
def stage_boundaries(start_date: str, end_date: str) -> dict[str, Any]:
    ...
    return {
        ...
        'test_purity': 'LOCKED_WITHIN_V49_67_RUN_NOT_HISTORICALLY_UNSEEN_BECAUSE_PRIOR_VERSIONS_EXPOSED_PERIOD',
    }
...
def lp_exit_audit(z: pd.DataFrame, out: Path) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    audit.to_csv(out / 'v49_67_lp_exit_policy_audit.csv', index=False, encoding='utf-8-sig')
    selected_doc.to_csv(out / 'v49_67_lp_exit_selected.csv', index=False, encoding='utf-8-sig')
    stop_cases.to_csv(out / 'v49_67_lp_locked_test_stop_cases.csv', index=False, encoding='utf-8-sig')
...
def run_research(selected_ctx: pd.DataFrame, out: Path, start_date: str, end_date: str, feature_meta: dict[str, tuple[str, str]]) -> dict[str, Any]:
    ...
    if z.sample_stage.eq('INVALID_DATE').any():
        raise RuntimeError('V49_67_TRIPLE_SPLIT_INVALID_DATE')
    ...
    for feat in feature_meta:
        if any(tok in feat.lower() for tok in FORBIDDEN_FEATURE_TOKENS):
            raise RuntimeError(f'V49_67_LEAKAGE_GUARD:{feat}')
    ...
    library.to_csv(out / 'v49_67_lgs_candidate_library.csv', index=False, encoding='utf-8-sig')
    redundancy.to_csv(out / 'v49_67_lgs_redundancy.csv', index=False, encoding='utf-8-sig')
    locked.to_csv(out / 'v49_67_lgs_locked_test.csv', index=False, encoding='utf-8-sig')
    down_library.to_csv(out / 'v49_67_down_market_candidate_library.csv', index=False, encoding='utf-8-sig')
    down_redundancy.to_csv(out / 'v49_67_down_market_redundancy.csv', index=False, encoding='utf-8-sig')
    down_locked.to_csv(out / 'v49_67_down_market_locked_test.csv', index=False, encoding='utf-8-sig')
    ...
    (out / 'v49_67_triple_split_manifest.json').write_text(json.dumps(manifest, ensure_ascii=False, indent=2, default=str), encoding='utf-8')
    return {
