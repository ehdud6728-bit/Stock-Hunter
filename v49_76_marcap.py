...
def resolve_krx_trading_asof(scanner, requested_end_date: str, *, lookback_days: int | None = None) -> dict[str,Any]:
    ...
    ambiguous. v49.76 first resolves a date from an OHLCV/index series whose returned
    ...
    try:
        ...
        raise RuntimeError(f'v49.76 invalid requested end date: {requested_end_date!r}: {exc}')
    ...
    if pykrx_stock is None:
        raise RuntimeError('v49.76 pykrx unavailable for KRX trading-day resolution')
    ...
    raise RuntimeError('v49.76 could not prove KRX trading as-of date from observed OHLCV dates; unverified fallback disabled')
...
def resolve_authoritative_marcap(scanner, codes, requested_end_date: str, *, allow_fallback: bool | None = None, trading_calendar: dict[str,Any] | None = None) -> tuple[dict[str,float], dict[str,Any]]:
    ...
    if not allow_fallback:
        raise RuntimeError(f'v49.76 authoritative trading-day MARCAP unavailable; fallback disabled: {last_err or "no pykrx rows"}')
