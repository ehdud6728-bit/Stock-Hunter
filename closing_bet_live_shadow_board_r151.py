...
VERSION="CLOSEBET_LIVE_SHADOW_BOARD_R151_CONFIDENCE_GUARD_20260919"
RESEARCH_ONLY=True

# Display-confidence guard only. These thresholds DO NOT affect candidate selection,
# score, rank, order, or production logic.
ROUTE_PROX_MIN_N = 3
    ...
    for fkey,medcol in cols.items():
        ...
        for fam,rr in routes.items():
            ...
            if pd.notna(m) and pd.notna(n):
                n_int=int(n)
                # R1.5.1 display-confidence guard:
                # n<3 is too small for feature-route proximity and is excluded from comparison.
                if n_int < ROUTE_PROX_MIN_N:
                    continue
                vals.append((abs(float(x)-float(m)),fam,float(m),n_int))
        if not vals:continue
        vals.sort(key=lambda z:z[0])
        d,fam,m,n=vals[0]
        confidence="LOW_CONFIDENCE" if n < ROUTE_PROX_LOW_CONF_N else "OBSERVED_SAMPLE"
        # No thresholded verdict; only nearest observed route median among eligible samples.
        results.append((FEATURE_LABELS.get(fkey,fkey),fam,m,n,confidence))
    return results
...
def board_for_row(r,evidence,response_map,macro):
    ...
    if rc:
        ...
        if prox:
            ...
            for label,fam,m,n,confidence in prox:
                conf=" · LOW CONFIDENCE" if confidence=="LOW_CONFIDENCE" else ""
                pp.append(f"{label}→{nmap.get(fam,fam)} median(n={n}{conf})")
            lines.append("FEATURE ROUTE PROXIMITY | "+" · ".join(pp[:6]))
            lines.append("※ n<3 경로는 proximity 비교에서 제외 · 3≤n<10은 LOW CONFIDENCE")
...
def main():
    ...
    header=[
        ...
        "※ DISPLAY CONFIDENCE GUARD: n<3 비교 제외 · 3≤n<10 LOW CONFIDENCE",
    ]
    text="\n".join(header)+"\n\n"+"\n\n".join(boards)
