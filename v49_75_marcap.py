...
def resolve_krx_trading_asof(scanner, requested_end_date: str, *, lookback_days: int | None = None) -> dict[str,Any]:
    ...
    ambiguous. v49.75 first resolves a date from an OHLCV/index series whose returned
    ...
    try:
        ...
        raise RuntimeError(f'v49.75 invalid requested end date: {requested_end_date!r}: {exc}')
    ...
    if pykrx_stock is None:
        raise RuntimeError('v49.75 pykrx unavailable for KRX trading-day resolution')
    ...
    raise RuntimeError('v49.75 could not prove KRX trading as-of date from observed OHLCV dates; unverified fallback disabled')
...
def resolve_authoritative_marcap(scanner, codes, requested_end_date: str, *, allow_fallback: bool | None = None, trading_calendar: dict[str,Any] | None = None) -> tuple[dict[str,float], dict[str,Any]]:
    ...
    if not allow_fallback:
        raise RuntimeError(f'v49.75 authoritative trading-day MARCAP unavailable; fallback disabled: {last_err or "no pykrx rows"}')
...
def snapshot_manifest(snapshot: pd.DataFrame, marcap_meta: dict[str,Any], *, mcap_or_min: float) -> dict[str,Any]:
    ...
    if x.empty:
        ...
        sha=_sha_text('\n'.join(lines))
    return {
        'schema':'V49_75_PREPARED_SNAPSHOT_SCHEMA_2_TRADING_DAY_VERIFIED','snapshot_sha256':sha,'rows':int(len(x)),
