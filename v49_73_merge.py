...
import v49_73_research as r70
import v49_73_quality as q70
import v49_73_insight as i72
...
def _deliver_telegram(parts: list[list[str]], out: Path, requested: bool) -> dict:
    ...
    path = out / 'v49_73_telegram_delivery_manifest.json'
    try:
        if requested:
            if delivery['route_validated'] != '1':
                raise RuntimeError('v49.73 Telegram requested but unified route preflight was not validated')
            if not s._telegram_route_ready():
                raise RuntimeError('v49.73 Telegram requested but scanner route is not ready after validated preflight')
            for part_no, part in enumerate(parts, start=1):
                ...
                if int(result.get('success_count', 0) or 0) < 1:
                    raise RuntimeError(f'v49.73 Telegram delivery failed for part {part_no}: {result.get("errors", [])}')
...
def _lp_validation(selected: pd.DataFrame, out: Path, start_date: str, end_date: str) -> pd.DataFrame:
    ...
    if z.empty:
        ...
        r.to_csv(out / 'v49_73_lp_walk_forward.csv', index=False, encoding='utf-8-sig')
        return r
...
def _iit_split(selected: pd.DataFrame, out: Path, start_date: str, end_date: str) -> pd.DataFrame:
    ...
    r = pd.DataFrame(rows)
    r.to_csv(out / 'v49_73_iit_flow_split.csv', index=False, encoding='utf-8-sig')
    return r


# ---- v49.73 winner/loser, market, sector and causal feature audit ----
...
def _v4967_anatomy_tables(x: pd.DataFrame, out: Path, start_date: str, end_date: str) -> tuple[pd.DataFrame,pd.DataFrame,pd.DataFrame,pd.DataFrame,pd.DataFrame]:
    ...
    z.to_csv(out/'v49_73_selected_enriched_outcomes.csv',index=False,encoding='utf-8-sig')
    outcomes.to_csv(out/'v49_73_outcome_distribution.csv',index=False,encoding='utf-8-sig')
    features.to_csv(out/'v49_73_winner_loser_feature_anatomy.csv',index=False,encoding='utf-8-sig')
    lifts.to_csv(out/'v49_73_train_oos_feature_lift.csv',index=False,encoding='utf-8-sig')
    market.to_csv(out/'v49_73_market_regime_performance.csv',index=False,encoding='utf-8-sig')
    events.to_csv(out/'v49_73_event_theme_context.csv',index=False,encoding='utf-8-sig')
    down.to_csv(out/'v49_73_down_market_cases.csv',index=False,encoding='utf-8-sig')
    return outcomes,features,lifts,market,events


def main() -> int:
    ap = argparse.ArgumentParser(description='v49.73 global merge + input reproducibility/completeness authority audit')
    ap.add_argument('--input-root', default='v49_73_downloads')
    ap.add_argument('--prepare-dir', default='v49_73_prepare_output')
    ...
    ap.add_argument('--baseline-input-authority', default='', help='optional prior same-period v49.73 input authority manifest')
    ap.add_argument('--baseline-prepared-snapshot', default='', help='optional prior same-period v49.73 prepared authority snapshot CSV')
    args = ap.parse_args()
    root = Path(args.input_root)
    prep_root = Path(args.prepare_dir)
    if not prep_root.exists():
        prep_root = root
    preflight_path = next(iter(prep_root.rglob('v49_73_preflight.json')), None)
    universe_path = next(iter(prep_root.rglob('v49_73_universe.json')), None)
    prepared_snapshot_path = next(iter(prep_root.rglob('v49_73_prepared_authority_snapshot.csv')), None)
    prepared_snapshot_manifest_path = next(iter(prep_root.rglob('v49_73_prepared_authority_snapshot.json')), None)
    ...
    for sid in expected_ids:
        ...
        if str(m.get('control_flow', '')) != 'V49_73_SHARD_EXPORT_RETURN_MAIN_EXIT':
    ...
    raw_path = out / 'v49_73_global_raw.csv'
    canonical_path = out / 'v49_73_global_canonical.csv'
    selected_path = out / 'v49_73_global_selected_top5.csv'
    ...
    populations['PRIMARY'].to_csv(out / 'v49_73_primary_selected_top5.csv', index=False, encoding='utf-8-sig')
    populations['ALL_MATCHED'].to_csv(out / 'v49_73_all_matched_selected_top5.csv', index=False, encoding='utf-8-sig')
    ...
    # v49.73 INPUT AUTHORITY: reproducibility and data completeness are independent axes.
    ...
    input_authority={
        'version':'v49.73','authority_schema_version':'V49_73_INPUT_AUTHORITY_SCHEMA_3_MARCAP_SNAPSHOT_LOCK',
    ...
    # v49.73 same-period MARCAP parity forensic. This is diagnostic detail for any
    # prepared-authority drift and never changes strategy thresholds.
    _marcap_parity,_marcap_detail=mc73.compare_marcap_snapshots(prepared_snapshot_path,args.baseline_prepared_snapshot,mcap_or_min=float(s.MCAP_OR_MIN))
    input_authority['marcap_parity']=dict(_marcap_parity)
    _marcap_detail.to_csv(out / 'v49_73_marcap_parity_detail.csv',index=False,encoding='utf-8-sig')
    (out / 'v49_73_marcap_parity_summary.json').write_text(json.dumps(_marcap_parity,ensure_ascii=False,indent=2,default=str),encoding='utf-8')
    pd.read_csv(prepared_snapshot_path,dtype={'code':str}).to_csv(out / 'v49_73_prepared_authority_snapshot.csv',index=False,encoding='utf-8-sig')
    (out / 'v49_73_prepared_authority_snapshot.json').write_text(prepared_snapshot_manifest_path.read_text(encoding='utf-8'),encoding='utf-8')

    history_df.to_csv(out / 'v49_73_history_authority_global.csv',index=False,encoding='utf-8-sig')
    ...
    _issue_rows.to_csv(out / 'v49_73_history_completeness_issues.csv',index=False,encoding='utf-8-sig')
    (out / 'v49_73_input_authority_manifest.json').write_text(json.dumps(input_authority,ensure_ascii=False,indent=2,default=str),encoding='utf-8')
    ...
    funnel.to_csv(out / 'v49_73_strategy_funnel.csv', index=False, encoding='utf-8-sig')
    strategy_status.to_csv(out / 'v49_73_strategy_stage_audit.csv', index=False, encoding='utf-8-sig')
    identity_audit.to_csv(out / 'v49_73_identity_audit.csv', index=False, encoding='utf-8-sig')
    zero_df.to_csv(out / 'v49_73_zero_mode_audit.csv', index=False, encoding='utf-8-sig')
    exc_df.to_csv(out / 'v49_73_predicate_exceptions.csv', index=False, encoding='utf-8-sig')
    ...
    if out.resolve() != generated_root.resolve():
        import shutil
        for gp in generated_root.glob('v49_73_common_strategy_*'):
            shutil.copy2(gp, out / gp.name)
    performance_csv = out / 'v49_73_common_strategy_performance.csv'
    portfolio_csv = out / 'v49_73_common_strategy_portfolio.csv'
    ...
    attribution.to_csv(out / 'v49_73_strategy_attribution_performance.csv', index=False, encoding='utf-8-sig')
    ...
    down_cases_path = out / 'v49_73_down_market_cases.csv'
    down_cases = pd.read_csv(down_cases_path, dtype={'code':str}) if down_cases_path.exists() else pd.DataFrame()

    # v49.73: TRAIN -> VALIDATION -> LOCKED_TEST research. Thresholds are discovered on
    ...
    # v49.73 independent SEARCH QUALITY AUDIT: machine intent proxy, winner/loser casebook,
    ...
    # v49.73 SEARCH GAP INSIGHT: audit-only lanes for immediate strategic misses,
    ...
    lines1 = [
        ...
        f'- MARCAP AUTHORITY: CONSENSUS VALID ✅ · {input_authority.get("marcap_snapshot_source","-")} · asof {input_authority.get("marcap_snapshot_asof_date","-")} · sha {str(input_authority.get("marcap_map_sha256",""))[:16]}',
    ...
    lines2 = ['(2/17)', '📊 전략별 OOS · Predicate Mode | v49.73', '──────────']
    if not ptab.empty:
    ...
    lines3 = ['(3/17)', '🔬 전략 Funnel·Pipeline | v49.73', '──────────']
    ...
    lines4 = ['(4/17)', '🧭 전략 귀속·LP·I/IT | v49.73', '──────────']
    for mode in s.CLOSING_BET_V4958_PRIMARY_PRIORITY:
    ...
    lines5 = ['(5/17)', '🧬 상승·하락 경로 해부 | v49.73', '──────────']
    ...
    lines6 = ['(6/17)', '🌧️ 하락장 독립상승·개선후보 | v49.73', '──────────']
    ...
    if not feature_candidates.empty:
        lines6.append('[구형 2단계 탐색 · v49.73 최종 승격근거 아님]')
    ...
    lines6 += ['', '- 섹터동행은 전체 업종 유니버스가 아닌 신호후보 프록시이므로 자동필터 승격 금지.', '- 위 항목은 2단계 탐색 참고자료이며 v49.73 LOCKED TEST 판정과 분리합니다.']

    lines7 = ['(7/17)', '🏁 LP 청산정책 3단계 검증 | v49.73', '──────────']
    b = triple['boundaries']
    ...
    lines8 = ['(8/17)', '🧪 L·G·S·하락장 LOCKED TEST | v49.73', '──────────']
    ...
    lines9 = ['(9/17)', '🎯 검색 의도 적합성 감사 | v49.73', '──────────']
    ...
    lines10 = ['(10/17)', '🕵️ 놓친 상승종목·단계별 누락 | v49.73', '──────────']
    ...
    lines11 = ['(11/17)', '📐 랭킹·분포변화·상시감사 | v49.73', '──────────']
    ...
    lines12 = ['(12/17)', '🚨 기술적 누락 포렌식 | v49.73', '──────────']
    ...
    if tech_fn is None or tech_fn.empty:
        ...
        if len(tech_fn) > 20:
            lines12.append(f'- 나머지 {len(tech_fn)-20}건은 v49_73_technical_false_negative.csv에 전부 저장')
    ...
    lines13 = ['(13/17)', '🔎 즉시상승 미포착 · 정책제외 vs 진짜 패턴누락 | v49.73', '──────────']
    ...
    lines14 = ['(14/17)', '🔀 LP·G 랭킹 역전 · STANDARD_OOS 전용 | v49.73', '──────────']
    ...
    lines15 = ['(15/17)', '⚖️ A BIG_CAPTURABLE vs STOP_FIRST · 상호배타 | v49.73', '──────────']
    ...
    lines16 = ['(16/17)', '🧭 H 이중프로필 · 표본권위 분리 | v49.73', '──────────']
    ...
    lines17 = ['(17/17)', '🔒 입력·MARCAP 권위·표본권위 | v49.73', '──────────']
    ...
    lines17.append(f'- MARCAP SNAPSHOT: CONSENSUS VALID ✅ · source {input_authority.get("marcap_snapshot_source","-")} · requested {universe.get("requested_end_date","-")} · asof {input_authority.get("marcap_snapshot_asof_date","-")} · unit {input_authority.get("marcap_snapshot_unit","-")} · sha {str(input_authority.get("marcap_map_sha256",""))[:16]}')
    ...
    if _mp.get('status')=='CHANGED':
        lines17.append(f'  ⛔ zero→positive {int(_mp.get("zero_to_positive_n",0) or 0)} · positive→zero {int(_mp.get("positive_to_zero_n",0) or 0)} · ≥10x {int(_mp.get("ratio_ge_10x_n",0) or 0)} · ≤0.1x {int(_mp.get("ratio_le_0_1x_n",0) or 0)} · 상세 CSV 확인')
    ...
    lines17 += ['', '- DEGRADED history gap도 fingerprint baseline으로 저장하며 동일 gap 재현 여부를 검증합니다.', '- 같은 기간 input/history/issue/population fingerprint가 달라지면 INVALID fail-closed.', '- MARCAP은 Prepare의 as-of snapshot 단일 권위만 사용하며 shard runtime mutation 1건도 즉시 실패.', '- search/rank/exit/LIVE change NONE · auto_apply=0 · PAPER 유지 · 실제주문 0건']
    ...
    (out / 'v49_73_global_summary.txt').write_text(report, encoding='utf-8')
    files = [
        raw_path, canonical_path, selected_path,
        out / 'v49_73_primary_selected_top5.csv', out / 'v49_73_all_matched_selected_top5.csv',
        out / 'v49_73_strategy_funnel.csv', out / 'v49_73_strategy_stage_audit.csv', out / 'v49_73_identity_audit.csv',
        out / 'v49_73_zero_mode_audit.csv', out / 'v49_73_predicate_exceptions.csv',
        out / 'v49_73_strategy_attribution_performance.csv', out / 'v49_73_lp_walk_forward.csv', out / 'v49_73_iit_flow_split.csv',
        out / 'v49_73_selected_enriched_outcomes.csv', out / 'v49_73_outcome_distribution.csv',
        out / 'v49_73_winner_loser_feature_anatomy.csv', out / 'v49_73_train_oos_feature_lift.csv',
        out / 'v49_73_market_regime_performance.csv', out / 'v49_73_event_theme_context.csv',
        out / 'v49_73_down_market_cases.csv',
        out / 'v49_73_lgs_candidate_library.csv', out / 'v49_73_lgs_redundancy.csv', out / 'v49_73_lgs_locked_test.csv',
        out / 'v49_73_down_market_candidate_library.csv', out / 'v49_73_down_market_redundancy.csv', out / 'v49_73_down_market_locked_test.csv',
        out / 'v49_73_lp_exit_policy_audit.csv', out / 'v49_73_lp_exit_selected.csv', out / 'v49_73_lp_locked_test_stop_cases.csv',
        out / 'v49_73_triple_split_manifest.json',
        out / 'v49_73_search_intent_enriched.csv', out / 'v49_73_search_intent_summary.csv',
        out / 'v49_73_winner_loser_effect_size.csv', out / 'v49_73_manual_casebook.csv',
        out / 'v49_73_ranking_summary.csv', out / 'v49_73_ranking_detail.csv',
        out / 'v49_73_opportunity_miss_summary.csv', out / 'v49_73_opportunity_census_global.csv',
        out / 'v49_73_missed_winner_casebook.csv', out / 'v49_73_technical_false_negative.csv',
        out / 'v49_73_technical_false_negative.json', out / 'v49_73_opportunity_path_summary.csv',
        out / 'v49_73_distribution_drift.csv', out / 'v49_73_search_quality_manifest.json',
        out / 'v49_73_history_authority_global.csv', out / 'v49_73_history_completeness_issues.csv', out / 'v49_73_input_authority_manifest.json', out / 'v49_73_sample_authority_reconciliation.csv',
        out / 'v49_73_prepared_authority_snapshot.csv', out / 'v49_73_prepared_authority_snapshot.json', out / 'v49_73_marcap_parity_summary.json', out / 'v49_73_marcap_parity_detail.csv',
        out / 'v49_73_immediate_miss_policy_summary.csv', out / 'v49_73_immediate_strategic_miss_summary.csv', out / 'v49_73_immediate_strategic_miss_feature_contrast.csv',
        out / 'v49_73_immediate_strategic_miss_casebook.csv', out / 'v49_73_immediate_strategic_miss_detail.csv',
        out / 'v49_73_rank_inversion_summary.csv', out / 'v49_73_rank_inversion_feature_stability.csv', out / 'v49_73_rank_inversion_cases.csv',
        out / 'v49_73_a_big_vs_stop_summary.csv', out / 'v49_73_a_big_vs_stop_feature_stability.csv', out / 'v49_73_a_big_vs_stop_casebook.csv',
        out / 'v49_73_h_intent_v2_summary.csv', out / 'v49_73_h_intent_v1_v2_matrix.csv', out / 'v49_73_h_intent_v2_condition_pass.csv',
        out / 'v49_73_h_intent_v2_detail.csv', out / 'v49_73_hypothesis_registry.csv', out / 'v49_73_insight_manifest.json',
    ]
    manifest = {
    ...
    (out / 'v49_73_global_manifest.json').write_text(json.dumps(manifest, ensure_ascii=False, indent=2, default=str), encoding='utf-8')
    print(report)
