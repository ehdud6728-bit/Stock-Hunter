...
import v49_68_research as r67
import v49_68_quality as q68
...
def _deliver_telegram(parts: list[list[str]], out: Path, requested: bool) -> dict:
    ...
    path = out / 'v49_68_telegram_delivery_manifest.json'
    try:
        if requested:
            if delivery['route_validated'] != '1':
                raise RuntimeError('v49.68 Telegram requested but unified route preflight was not validated')
            if not s._telegram_route_ready():
                raise RuntimeError('v49.68 Telegram requested but scanner route is not ready after validated preflight')
            for part_no, part in enumerate(parts, start=1):
                ...
                if int(result.get('success_count', 0) or 0) < 1:
                    raise RuntimeError(f'v49.68 Telegram delivery failed for part {part_no}: {result.get("errors", [])}')
...
def _lp_validation(selected: pd.DataFrame, out: Path, start_date: str, end_date: str) -> pd.DataFrame:
    ...
    if z.empty:
        ...
        r.to_csv(out / 'v49_68_lp_walk_forward.csv', index=False, encoding='utf-8-sig')
        return r
...
def _iit_split(selected: pd.DataFrame, out: Path, start_date: str, end_date: str) -> pd.DataFrame:
    ...
    r = pd.DataFrame(rows)
    r.to_csv(out / 'v49_68_iit_flow_split.csv', index=False, encoding='utf-8-sig')
    return r


# ---- v49.68 winner/loser, market, sector and causal feature audit ----
...
def _v4967_anatomy_tables(x: pd.DataFrame, out: Path, start_date: str, end_date: str) -> tuple[pd.DataFrame,pd.DataFrame,pd.DataFrame,pd.DataFrame,pd.DataFrame]:
    ...
    z.to_csv(out/'v49_68_selected_enriched_outcomes.csv',index=False,encoding='utf-8-sig')
    outcomes.to_csv(out/'v49_68_outcome_distribution.csv',index=False,encoding='utf-8-sig')
    features.to_csv(out/'v49_68_winner_loser_feature_anatomy.csv',index=False,encoding='utf-8-sig')
    lifts.to_csv(out/'v49_68_train_oos_feature_lift.csv',index=False,encoding='utf-8-sig')
    market.to_csv(out/'v49_68_market_regime_performance.csv',index=False,encoding='utf-8-sig')
    events.to_csv(out/'v49_68_event_theme_context.csv',index=False,encoding='utf-8-sig')
    down.to_csv(out/'v49_68_down_market_cases.csv',index=False,encoding='utf-8-sig')
    return outcomes,features,lifts,market,events


def main() -> int:
    ap = argparse.ArgumentParser(description='v49.68 global merge + identity/pipeline/attribution audit')
    ap.add_argument('--input-root', default='v49_68_downloads')
    ap.add_argument('--prepare-dir', default='v49_68_prepare_output')
    ...
    if not prep_root.exists():
        prep_root = root
    preflight_path = next(iter(prep_root.rglob('v49_68_preflight.json')), None)
    universe_path = next(iter(prep_root.rglob('v49_68_universe.json')), None)
    ...
    for sid in expected_ids:
        ...
        if str(m.get('control_flow', '')) != 'V49_68_SHARD_EXPORT_RETURN_MAIN_EXIT':
    ...
    raw_path = out / 'v49_68_global_raw.csv'
    canonical_path = out / 'v49_68_global_canonical.csv'
    selected_path = out / 'v49_68_global_selected_top5.csv'
    ...
    populations['PRIMARY'].to_csv(out / 'v49_68_primary_selected_top5.csv', index=False, encoding='utf-8-sig')
    populations['ALL_MATCHED'].to_csv(out / 'v49_68_all_matched_selected_top5.csv', index=False, encoding='utf-8-sig')
    ...
    funnel.to_csv(out / 'v49_68_strategy_funnel.csv', index=False, encoding='utf-8-sig')
    strategy_status.to_csv(out / 'v49_68_strategy_stage_audit.csv', index=False, encoding='utf-8-sig')
    identity_audit.to_csv(out / 'v49_68_identity_audit.csv', index=False, encoding='utf-8-sig')
    zero_df.to_csv(out / 'v49_68_zero_mode_audit.csv', index=False, encoding='utf-8-sig')
    exc_df.to_csv(out / 'v49_68_predicate_exceptions.csv', index=False, encoding='utf-8-sig')
    ...
    if out.resolve() != generated_root.resolve():
        import shutil
        for gp in generated_root.glob('v49_68_common_strategy_*'):
            shutil.copy2(gp, out / gp.name)
    performance_csv = out / 'v49_68_common_strategy_performance.csv'
    portfolio_csv = out / 'v49_68_common_strategy_portfolio.csv'
    ...
    attribution.to_csv(out / 'v49_68_strategy_attribution_performance.csv', index=False, encoding='utf-8-sig')
    ...
    down_cases_path = out / 'v49_68_down_market_cases.csv'
    down_cases = pd.read_csv(down_cases_path, dtype={'code':str}) if down_cases_path.exists() else pd.DataFrame()

    # v49.68: TRAIN -> VALIDATION -> LOCKED_TEST research. Thresholds are discovered on
    ...
    triple = r67.run_research(selected_ctx, out, args.start_date, args.end_date, V4967_ENTRY_FEATURE_META)
    ...
    # v49.68 independent SEARCH QUALITY AUDIT: machine intent proxy, winner/loser casebook,
    # material missed-winner census, rank-stage attribution and distribution drift.
    quality = q68.run_quality_audit(raw_df, triple['enriched'], selected, opportunity_df, out, V4967_ENTRY_FEATURE_META)
    ...
    lines2 = ['(2/11)', '📊 전략별 OOS · Predicate Mode | v49.68', '──────────']
    if not ptab.empty:
    ...
    lines3 = ['(3/11)', '🔬 전략 Funnel·Pipeline | v49.68', '──────────']
    ...
    lines4 = ['(4/11)', '🧭 전략 귀속·LP·I/IT | v49.68', '──────────']
    for mode in s.CLOSING_BET_V4958_PRIMARY_PRIORITY:
    ...
    lines5 = ['(5/11)', '🧬 상승·하락 경로 해부 | v49.68', '──────────']
    ...
    lines6 = ['(6/11)', '🌧️ 하락장 독립상승·개선후보 | v49.68', '──────────']
    ...
    if not feature_candidates.empty:
        lines6.append('[구형 2단계 탐색 · v49.68 최종 승격근거 아님]')
    ...
    lines6 += ['', '- 섹터동행은 전체 업종 유니버스가 아닌 신호후보 프록시이므로 자동필터 승격 금지.', '- 위 항목은 2단계 탐색 참고자료이며 v49.68 LOCKED TEST 판정과 분리합니다.']

    lines7 = ['(7/11)', '🏁 LP 청산정책 3단계 검증 | v49.68', '──────────']
    b = triple['boundaries']
    ...
    lines8 = ['(8/11)', '🧪 L·G·S·하락장 LOCKED TEST | v49.68', '──────────']
    ...
    lines9 = ['(9/11)', '🎯 검색 의도 적합성 감사 | v49.68', '──────────']
    ...
    lines10 = ['(10/11)', '🕵️ 놓친 상승종목·단계별 누락 | v49.68', '──────────']
    ...
    lines11 = ['(11/11)', '📐 랭킹·분포변화·상시감사 | v49.68', '──────────']
    ...
    report = '\n'.join(lines1 + [''] + lines2 + [''] + lines3 + [''] + lines4 + [''] + lines5 + [''] + lines6 + [''] + lines7 + [''] + lines8 + [''] + lines9 + [''] + lines10 + [''] + lines11)
    (out / 'v49_68_global_summary.txt').write_text(report, encoding='utf-8')
    files = [
        raw_path, canonical_path, selected_path,
        out / 'v49_68_primary_selected_top5.csv', out / 'v49_68_all_matched_selected_top5.csv',
        out / 'v49_68_strategy_funnel.csv', out / 'v49_68_strategy_stage_audit.csv', out / 'v49_68_identity_audit.csv',
        out / 'v49_68_zero_mode_audit.csv', out / 'v49_68_predicate_exceptions.csv',
        out / 'v49_68_strategy_attribution_performance.csv', out / 'v49_68_lp_walk_forward.csv', out / 'v49_68_iit_flow_split.csv',
        out / 'v49_68_selected_enriched_outcomes.csv', out / 'v49_68_outcome_distribution.csv',
        out / 'v49_68_winner_loser_feature_anatomy.csv', out / 'v49_68_train_oos_feature_lift.csv',
        out / 'v49_68_market_regime_performance.csv', out / 'v49_68_event_theme_context.csv',
        out / 'v49_68_down_market_cases.csv',
        out / 'v49_68_lgs_candidate_library.csv', out / 'v49_68_lgs_redundancy.csv', out / 'v49_68_lgs_locked_test.csv',
        out / 'v49_68_down_market_candidate_library.csv', out / 'v49_68_down_market_redundancy.csv', out / 'v49_68_down_market_locked_test.csv',
        out / 'v49_68_lp_exit_policy_audit.csv', out / 'v49_68_lp_exit_selected.csv', out / 'v49_68_lp_locked_test_stop_cases.csv',
        out / 'v49_68_triple_split_manifest.json',
        out / 'v49_68_search_intent_enriched.csv', out / 'v49_68_search_intent_summary.csv',
        out / 'v49_68_winner_loser_effect_size.csv', out / 'v49_68_manual_casebook.csv',
        out / 'v49_68_ranking_summary.csv', out / 'v49_68_ranking_detail.csv',
        out / 'v49_68_opportunity_miss_summary.csv', out / 'v49_68_opportunity_census_global.csv',
        out / 'v49_68_missed_winner_casebook.csv', out / 'v49_68_distribution_drift.csv',
        out / 'v49_68_search_quality_manifest.json',
    ]
    manifest = {
    ...
    (out / 'v49_68_global_manifest.json').write_text(json.dumps(manifest, ensure_ascii=False, indent=2, default=str), encoding='utf-8')
    print(report)
    _deliver_telegram([lines1, lines2, lines3, lines4, lines5, lines6, lines7, lines8, lines9, lines10, lines11], out, bool(args.send_telegram))
