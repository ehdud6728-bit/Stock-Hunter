...
VERSION="CLOSEBET_LIVE_SHADOW_BOARD_R11_SOURCE_CAPTURE_20260919"
RESEARCH_ONLY=True
...
def find_candidate(root):
    ...
    # R1.1: exact copy-only LIVE sidecar is the first authority.
    # Choose newest capture by filename; execution is preferred over hits.
...
def main():
    ...
    if src is None:
        ...
        if metas:
            try: lane_meta=json.loads(metas[-1].read_text(encoding="utf-8"))
            except Exception: lane_meta={}
        lane=str(lane_meta.get("effective_lane",""))
        ...
        meta={"version":VERSION,"status":status,"research_only":True,
              ...
              "source_run_id":a.source_run_id,"source_lane":lane,
              "next_action":"PATCH_SCANNER_SOURCE_CAPTURE_R11" if status=="NO_LIVE_SIDECAR" else "WAIT_FOR_CANDIDATE_LANE"}
