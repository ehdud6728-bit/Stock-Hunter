...
VERSION = "V1107_1_TOPDOWN_NA_ROLLING_FIX_20260525"
SEP = "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
THIN = "────────────────────────────"
...
def numeric_series(s: pd.Series, default: Optional[float] = None) -> pd.Series:
    """Rolling 계산 전에 object/pd.NA가 섞인 값을 float로 강제 정규화한다.

    GitHub Actions의 pandas 버전에 따라 pd.NA가 포함된 Series는 rolling().mean()에서
    `No numeric types to aggregate`가 발생할 수 있으므로 모든 지표 계산 전에 사용한다.
    """
...
def macd_hist(close: pd.Series) -> Tuple[pd.Series, pd.Series, pd.Series]:
    ...
    sig = ema(macd, 9)
    hist = macd - sig
    return macd.astype("float64"), sig.astype("float64"), hist.astype("float64")


def stochastic_kd(df: pd.DataFrame, n: int = 14, k_smooth: int = 3, d_smooth: int = 3) -> Tuple[pd.Series, pd.Series]:
    low = numeric_series(df["Low"])
    high = numeric_series(df["High"])
    close = numeric_series(df["Close"])
    low_n = low.rolling(n, min_periods=max(1, n // 2)).min()
    high_n = high.rolling(n, min_periods=max(1, n // 2)).max()
    denom = (high_n - low_n).astype("float64")
    denom = denom.mask(denom.abs() < 1e-12)
    raw_k = ((close - low_n) / denom * 100.0).astype("float64")
