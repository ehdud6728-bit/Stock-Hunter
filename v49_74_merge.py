...
import v49_74_research as r70
import v49_74_quality as q70
import v49_74_insight as i72
import v49_74_input as ia72
import v49_74_marcap as mc74
...
def _deliver_telegram(parts: list[list[str]], out: Path, requested: bool) -> dict:
    ...
    path = out / 'v49_74_telegram_delivery_manifest.json'
    try:
        if requested:
            if delivery['route_validated'] != '1':
                raise RuntimeError('v49.74 Telegram requested but unified route preflight was not validated')
            if not s._telegram_route_ready():
                raise RuntimeError('v49.74 Telegram requested but scanner route is not ready after validated preflight')
            for part_no, part in enumerate(parts, start=1):
                ...
                if int(result.get('success_count', 0) or 0) < 1:
                    raise RuntimeError(f'v49.74 Telegram delivery failed for part {part_no}: {result.get("errors", [])}')
...
def _lp_validation(selected: pd.DataFrame, out: Path, start_date: str, end_date: str) -> pd.DataFrame:
    ...
    if z.empty:
        ...
        r.to_csv(out / 'v49_74_lp_walk_forward.csv', index=False, encoding='utf-8-sig')
        return r
...
def _iit_split(selected: pd.DataFrame, out: Path, start_date: str, end_date: str) -> pd.DataFrame:
    ...
    r = pd.DataFrame(rows)
    r.to_csv(out / 'v49_74_iit_flow_split.csv', index=False, encoding='utf-8-sig')
    return r


# ---- v49.74 winner/loser, market, sector and causal feature audit ----
...
def _v4967_anatomy_tables(x: pd.DataFrame, out: Path, start_date: str, end_date: str) -> tuple[pd.DataFrame,pd.DataFrame,pd.DataFrame,pd.DataFrame,pd.DataFrame]:
    ...
    z.to_csv(out/'v49_74_selected_enriched_outcomes.csv',index=False,encoding='utf-8-sig')
    outcomes.to_csv(out/'v49_74_outcome_distribution.csv',index=False,encoding='utf-8-sig')
    features.to_csv(out/'v49_74_winner_loser_feature_anatomy.csv',index=False,encoding='utf-8-sig')
    lifts.to_csv(out/'v49_74_train_oos_feature_lift.csv',index=False,encoding='utf-8-sig')
    market.to_csv(out/'v49_74_market_regime_performance.csv',index=False,encoding='utf-8-sig')
    events.to_csv(out/'v49_74_event_theme_context.csv',index=False,encoding='utf-8-sig')
    down.to_csv(out/'v49_74_down_market_cases.csv',index=False,encoding='utf-8-sig')
    return outcomes,features,lifts,market,events


def main() -> int:
    ap = argparse.ArgumentParser(description='v49.74 global merge + input reproducibility/completeness authority audit')
    ap.add_argument('--input-root', default='v49_74_downloads')
    ap.add_argument('--prepare-dir', default='v49_74_prepare_output')
    ...
    ap.add_argument('--baseline-input-authority', default='', help='optional prior same-period v49.74 input authority manifest')
    ap.add_argument('--baseline-prepared-snapshot', default='', help='optional prior same-period v49.74 prepared authority snapshot CSV')
    ap.add_argument('--legacy-v4973-prepared-snapshot', default='', help='optional prior v49.73 same-request-date snapshot; value parity only, as-of label may differ')
    args = ap.parse_args()
    root = Path(args.input_root)
    prep_root = Path(args.prepare_dir)
    if not prep_root.exists():
        prep_root = root
    preflight_path = next(iter(prep_root.rglob('v49_74_preflight.json')), None)
    universe_path = next(iter(prep_root.rglob('v49_74_universe.json')), None)
    prepared_snapshot_path = next(iter(prep_root.rglob('v49_74_prepared_authority_snapshot.csv')), None)
    prepared_snapshot_manifest_path = next(iter(prep_root.rglob('v49_74_prepared_authority_snapshot.json')), None)
    ...
    for sid in expected_ids:
        ...
        if str(m.get('control_flow', '')) != 'V49_74_SHARD_EXPORT_RETURN_MAIN_EXIT':
    ...
    raw_path = out / 'v49_74_global_raw.csv'
    canonical_path = out / 'v49_74_global_canonical.csv'
    selected_path = out / 'v49_74_global_selected_top5.csv'
    ...
    populations['PRIMARY'].to_csv(out / 'v49_74_primary_selected_top5.csv', index=False, encoding='utf-8-sig')
    populations['ALL_MATCHED'].to_csv(out / 'v49_74_all_matched_selected_top5.csv', index=False, encoding='utf-8-sig')
    ...
    # v49.74 INPUT AUTHORITY: reproducibility and data completeness are independent axes.
    ...
    input_authority={
        'version':'v49.74','authority_schema_version':'V49_74_INPUT_AUTHORITY_SCHEMA_4_TRADING_DAY_MARCAP_LOCK',
    ...
    # v49.74 same-period MARCAP parity forensic. This is diagnostic detail for any
    ...
    _marcap_detail.to_csv(out / 'v49_74_marcap_parity_detail.csv',index=False,encoding='utf-8-sig')
    (out / 'v49_74_marcap_parity_summary.json').write_text(json.dumps(_marcap_parity,ensure_ascii=False,indent=2,default=str),encoding='utf-8')
    _legacy_marcap_parity,_legacy_marcap_detail=mc74.compare_marcap_snapshots(prepared_snapshot_path,args.legacy_v4973_prepared_snapshot,mcap_or_min=float(s.MCAP_OR_MIN),require_same_asof=False)
    input_authority['legacy_v4973_marcap_parity']=dict(_legacy_marcap_parity)
    _legacy_marcap_detail.to_csv(out / 'v49_74_legacy_v4973_marcap_parity_detail.csv',index=False,encoding='utf-8-sig')
    (out / 'v49_74_legacy_v4973_marcap_parity_summary.json').write_text(json.dumps(_legacy_marcap_parity,ensure_ascii=False,indent=2,default=str),encoding='utf-8')
    pd.read_csv(prepared_snapshot_path,dtype={'code':str}).to_csv(out / 'v49_74_prepared_authority_snapshot.csv',index=False,encoding='utf-8-sig')
    (out / 'v49_74_prepared_authority_snapshot.json').write_text(prepared_snapshot_manifest_path.read_text(encoding='utf-8'),encoding='utf-8')

    history_df.to_csv(out / 'v49_74_history_authority_global.csv',index=False,encoding='utf-8-sig')
    ...
    _issue_rows.to_csv(out / 'v49_74_history_completeness_issues.csv',index=False,encoding='utf-8-sig')
    (out / 'v49_74_input_authority_manifest.json').write_text(json.dumps(input_authority,ensure_ascii=False,indent=2,default=str),encoding='utf-8')
    ...
    funnel.to_csv(out / 'v49_74_strategy_funnel.csv', index=False, encoding='utf-8-sig')
    strategy_status.to_csv(out / 'v49_74_strategy_stage_audit.csv', index=False, encoding='utf-8-sig')
    identity_audit.to_csv(out / 'v49_74_identity_audit.csv', index=False, encoding='utf-8-sig')
    zero_df.to_csv(out / 'v49_74_zero_mode_audit.csv', index=False, encoding='utf-8-sig')
    exc_df.to_csv(out / 'v49_74_predicate_exceptions.csv', index=False, encoding='utf-8-sig')
    ...
    if out.resolve() != generated_root.resolve():
        import shutil
        for gp in generated_root.glob('v49_74_common_strategy_*'):
            shutil.copy2(gp, out / gp.name)
    performance_csv = out / 'v49_74_common_strategy_performance.csv'
    portfolio_csv = out / 'v49_74_common_strategy_portfolio.csv'
    ...
    attribution.to_csv(out / 'v49_74_strategy_attribution_performance.csv', index=False, encoding='utf-8-sig')
    ...
    down_cases_path = out / 'v49_74_down_market_cases.csv'
    down_cases = pd.read_csv(down_cases_path, dtype={'code':str}) if down_cases_path.exists() else pd.DataFrame()

    # v49.74: TRAIN -> VALIDATION -> LOCKED_TEST research. Thresholds are discovered on
    ...
    # v49.74 independent SEARCH QUALITY AUDIT: machine intent proxy, winner/loser casebook,
    ...
    # v49.74 SEARCH GAP INSIGHT: audit-only lanes for immediate strategic misses,
    ...
    lines2 = ['(2/17)', '📊 전략별 OOS · Predicate Mode | v49.74', '──────────']
    if not ptab.empty:
    ...
    lines3 = ['(3/17)', '🔬 전략 Funnel·Pipeline | v49.74', '──────────']
    ...
    lines4 = ['(4/17)', '🧭 전략 귀속·LP·I/IT | v49.74', '──────────']
    for mode in s.CLOSING_BET_V4958_PRIMARY_PRIORITY:
    ...
    lines5 = ['(5/17)', '🧬 상승·하락 경로 해부 | v49.74', '──────────']
    ...
    lines6 = ['(6/17)', '🌧️ 하락장 독립상승·개선후보 | v49.74', '──────────']
    ...
    if not feature_candidates.empty:
        lines6.append('[구형 2단계 탐색 · v49.74 최종 승격근거 아님]')
    ...
    lines6 += ['', '- 섹터동행은 전체 업종 유니버스가 아닌 신호후보 프록시이므로 자동필터 승격 금지.', '- 위 항목은 2단계 탐색 참고자료이며 v49.74 LOCKED TEST 판정과 분리합니다.']

    lines7 = ['(7/17)', '🏁 LP 청산정책 3단계 검증 | v49.74', '──────────']
    b = triple['boundaries']
    ...
    lines8 = ['(8/17)', '🧪 L·G·S·하락장 LOCKED TEST | v49.74', '──────────']
    ...
    lines9 = ['(9/17)', '🎯 검색 의도 적합성 감사 | v49.74', '──────────']
    ...
    lines10 = ['(10/17)', '🕵️ 놓친 상승종목·단계별 누락 | v49.74', '──────────']
    ...
    lines11 = ['(11/17)', '📐 랭킹·분포변화·상시감사 | v49.74', '──────────']
    ...
    lines12 = ['(12/17)', '🚨 기술적 누락 포렌식 | v49.74', '──────────']
    ...
    if tech_fn is None or tech_fn.empty:
        ...
        if len(tech_fn) > 20:
            lines12.append(f'- 나머지 {len(tech_fn)-20}건은 v49_74_technical_false_negative.csv에 전부 저장')
    ...
    lines13 = ['(13/17)', '🔎 즉시상승 미포착 · 정책제외 vs 진짜 패턴누락 | v49.74', '──────────']
    ...
    lines14 = ['(14/17)', '🔀 LP·G 랭킹 역전 · STANDARD_OOS 전용 | v49.74', '──────────']
    ...
    lines15 = ['(15/17)', '⚖️ A BIG_CAPTURABLE vs STOP_FIRST · 상호배타 | v49.74', '──────────']
    ...
    lines16 = ['(16/17)', '🧭 H 이중프로필 · 표본권위 분리 | v49.74', '──────────']
    ...
    lines17 = ['(17/17)', '🔒 입력·MARCAP 권위·표본권위 | v49.74', '──────────']
    ...
    _lmp=input_authority.get('legacy_v4973_marcap_parity',{}) or {}
    if _lmp.get('status') not in ('NO_BASELINE','CURRENT_SNAPSHOT_MISSING','BASELINE_READ_ERROR','NOT_COMPARABLE_PERIOD'):
        lines17.append(f'- LEGACY v49.73 MARCAP VALUE PARITY: {_lmp.get("status")} · label {_lmp.get("baseline_asof","-")}→{_lmp.get("current_asof","-")} · changed {int(_lmp.get("changed_codes_n",0) or 0)} · universe cross {int(_lmp.get("universe_pass_to_fail_n",0) or 0)+int(_lmp.get("universe_fail_to_pass_n",0) or 0)}')
    if _mp.get('status')=='CHANGED':
        ...
        lines17.append('  · 전체 종목별 차이는 v49_74_marcap_parity_detail.csv에 저장')
    ...
    (out / 'v49_74_global_summary.txt').write_text(report, encoding='utf-8')
    files = [
        raw_path, canonical_path, selected_path,
        out / 'v49_74_primary_selected_top5.csv', out / 'v49_74_all_matched_selected_top5.csv',
        out / 'v49_74_strategy_funnel.csv', out / 'v49_74_strategy_stage_audit.csv', out / 'v49_74_identity_audit.csv',
        out / 'v49_74_zero_mode_audit.csv', out / 'v49_74_predicate_exceptions.csv',
        out / 'v49_74_strategy_attribution_performance.csv', out / 'v49_74_lp_walk_forward.csv', out / 'v49_74_iit_flow_split.csv',
        out / 'v49_74_selected_enriched_outcomes.csv', out / 'v49_74_outcome_distribution.csv',
        out / 'v49_74_winner_loser_feature_anatomy.csv', out / 'v49_74_train_oos_feature_lift.csv',
        out / 'v49_74_market_regime_performance.csv', out / 'v49_74_event_theme_context.csv',
        out / 'v49_74_down_market_cases.csv',
        out / 'v49_74_lgs_candidate_library.csv', out / 'v49_74_lgs_redundancy.csv', out / 'v49_74_lgs_locked_test.csv',
        out / 'v49_74_down_market_candidate_library.csv', out / 'v49_74_down_market_redundancy.csv', out / 'v49_74_down_market_locked_test.csv',
        out / 'v49_74_lp_exit_policy_audit.csv', out / 'v49_74_lp_exit_selected.csv', out / 'v49_74_lp_locked_test_stop_cases.csv',
        out / 'v49_74_triple_split_manifest.json',
        out / 'v49_74_search_intent_enriched.csv', out / 'v49_74_search_intent_summary.csv',
        out / 'v49_74_winner_loser_effect_size.csv', out / 'v49_74_manual_casebook.csv',
        out / 'v49_74_ranking_summary.csv', out / 'v49_74_ranking_detail.csv',
        out / 'v49_74_opportunity_miss_summary.csv', out / 'v49_74_opportunity_census_global.csv',
        out / 'v49_74_missed_winner_casebook.csv', out / 'v49_74_technical_false_negative.csv',
        out / 'v49_74_technical_false_negative.json', out / 'v49_74_opportunity_path_summary.csv',
        out / 'v49_74_distribution_drift.csv', out / 'v49_74_search_quality_manifest.json',
        out / 'v49_74_history_authority_global.csv', out / 'v49_74_history_completeness_issues.csv', out / 'v49_74_input_authority_manifest.json', out / 'v49_74_sample_authority_reconciliation.csv',
        out / 'v49_74_prepared_authority_snapshot.csv', out / 'v49_74_prepared_authority_snapshot.json', out / 'v49_74_marcap_parity_summary.json', out / 'v49_74_marcap_parity_detail.csv',
        out / 'v49_74_legacy_v4973_marcap_parity_summary.json', out / 'v49_74_legacy_v4973_marcap_parity_detail.csv',
        out / 'v49_74_immediate_miss_policy_summary.csv', out / 'v49_74_immediate_strategic_miss_summary.csv', out / 'v49_74_immediate_strategic_miss_feature_contrast.csv',
        out / 'v49_74_immediate_strategic_miss_casebook.csv', out / 'v49_74_immediate_strategic_miss_detail.csv',
        out / 'v49_74_rank_inversion_summary.csv', out / 'v49_74_rank_inversion_feature_stability.csv', out / 'v49_74_rank_inversion_cases.csv',
        out / 'v49_74_a_big_vs_stop_summary.csv', out / 'v49_74_a_big_vs_stop_feature_stability.csv', out / 'v49_74_a_big_vs_stop_casebook.csv',
        out / 'v49_74_h_intent_v2_summary.csv', out / 'v49_74_h_intent_v1_v2_matrix.csv', out / 'v49_74_h_intent_v2_condition_pass.csv',
        out / 'v49_74_h_intent_v2_detail.csv', out / 'v49_74_hypothesis_registry.csv', out / 'v49_74_insight_manifest.json',
    ]
    manifest = {
    ...
    (out / 'v49_74_global_manifest.json').write_text(json.dumps(manifest, ensure_ascii=False, indent=2, default=str), encoding='utf-8')
    print(report)
    _deliver_telegram([lines1, lines2, lines3, lines4, lines5, lines6, lines7, lines8, lines9, lines10, lines11, lines12, lines13, lines14, lines15, lines16, lines17], out, bool(args.send_telegram))
