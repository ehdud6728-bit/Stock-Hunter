...
def lp_exit_audit(z: pd.DataFrame, out: Path) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    audit.to_csv(out / 'v49_70_lp_exit_policy_audit.csv', index=False, encoding='utf-8-sig')
    selected_doc.to_csv(out / 'v49_70_lp_exit_selected.csv', index=False, encoding='utf-8-sig')
    stop_cases.to_csv(out / 'v49_70_lp_locked_test_stop_cases.csv', index=False, encoding='utf-8-sig')
...
def run_research(selected_ctx: pd.DataFrame, out: Path, start_date: str, end_date: str, feature_meta: dict[str, tuple[str, str]]) -> dict[str, Any]:
    ...
    library.to_csv(out / 'v49_70_lgs_candidate_library.csv', index=False, encoding='utf-8-sig')
    redundancy.to_csv(out / 'v49_70_lgs_redundancy.csv', index=False, encoding='utf-8-sig')
    locked.to_csv(out / 'v49_70_lgs_locked_test.csv', index=False, encoding='utf-8-sig')
    down_library.to_csv(out / 'v49_70_down_market_candidate_library.csv', index=False, encoding='utf-8-sig')
    down_redundancy.to_csv(out / 'v49_70_down_market_redundancy.csv', index=False, encoding='utf-8-sig')
    down_locked.to_csv(out / 'v49_70_down_market_locked_test.csv', index=False, encoding='utf-8-sig')
    ...
    (out / 'v49_70_triple_split_manifest.json').write_text(json.dumps(manifest, ensure_ascii=False, indent=2, default=str), encoding='utf-8')
    return {
