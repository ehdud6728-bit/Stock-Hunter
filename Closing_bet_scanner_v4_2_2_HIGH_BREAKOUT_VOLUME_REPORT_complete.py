...
CLOSING_BET_SCANNER_VERSION = 'G_MORALES_V4_2_2_HIGH_BREAKOUT_VOLUME_REPORT_20260512'
        ...
        try:
            if len(hist) >= 150 and info['amount_b'] >= MIN_AMOUNT / 1e8:
                ...
                for d in range(1, max_days + 1):
                    ...
                    # v4.2.2 핵심: 신고가를 '장대양봉'으로 종가 돌파한 기준봉만 인정한다.
...
def _bt_mask_h_safe(df: pd.DataFrame):
    """v4.2.2: H 신고가 거자름 STRICT SAFE 후보.
    핵심은 '신고가 장대양봉 돌파봉'과 '거래량 마른 짧은 타점봉'을 분리해 모두 통과하는 것이다.
    """
    if df is None or df.empty or 'mode' not in df.columns:
        return pd.Series(False, index=df.index if df is not None else [])
...
def _bt_mask_h_neutral(df: pd.DataFrame):
    """v4.2.2: H 후보 중 SAFE도 RISK도 아닌 중립 후보."""
    if df is None or df.empty or 'mode' not in df.columns:
        return pd.Series(False, index=df.index if df is not None else [])
...
def _format_h_breakout_volume_report(df: pd.DataFrame) -> str:
    ...
    try:
        ...
        lines.append("- 해석 가이드: 2~3배 구간은 '정상 강도', 3~5배는 '강한 수급', 5~8배 이상은 '초강한 수급/과열 가능성'으로 나누어 봅니다. 손절선행과 MA5이탈이 같이 낮은 구간이 실전 후보입니다.")
    except Exception as e:
...
def _format_h_reason_report(df: pd.DataFrame) -> str:
    ...
    lines = ["[H 신고가 거자름 STRICT SAFE/RISK 탈락 사유 분석 — v4.2.2]"]
    try:
        if df is None or df.empty or 'mode' not in df.columns:
            lines.append("- 데이터 없음")
            return "\n".join(lines)
        ...
        lines.append("- 해석: H-STRICT는 장대양봉 신고가 돌파봉을 먼저 찾고, 이후 거래량이 마른 짧은 타점봉만 검증하는 실험 전략입니다.")
    except Exception as e:
...
def _build_backtest_summary(
    ...
    try:
        ...
        if len(h_all_df) > 0:
            ...
            lines.append(_format_backtest_trade_rule_block(df[_bt_mask_h_safe(df)], '🟢 H-SAFE 신고가거자름 STRICT'))
            lines.append(_format_backtest_trade_rule_block(df[_bt_mask_h_neutral(df)], '🟡 H-NEUTRAL 신고가거자름 STRICT'))
            lines.append(_format_backtest_trade_rule_block(df[_bt_mask_h_risk(df)], '⚠️ H-RISK 신고가거자름 STRICT'))
    except Exception:
        pass
    lines.append("")
    ...
    if not h_df.empty:
        ...
        lines.append(_format_backtest_trade_rule_block(h_df[_bt_mask_h_safe(h_df)], '🟢 H-SAFE 신고가거자름 STRICT'))
        lines.append(_format_backtest_trade_rule_block(h_df[_bt_mask_h_neutral(h_df)], '🟡 H-NEUTRAL 신고가거자름 STRICT'))
        lines.append(_format_backtest_trade_rule_block(h_df[_bt_mask_h_risk(h_df)], '⚠️ H-RISK 신고가거자름 STRICT'))
    ...
    lines.append("- --backtest-h-only 옵션을 쓰면 H 신고가거자름 STRICT만 필터링하여 H-SAFE/NEUTRAL/RISK 성과와 5일선 종가이탈 손절 기준을 확인합니다.")
    lines.append("- v4.2.2부터 H 신고가거자름은 '신고가 장대양봉 돌파봉'과 '거래량 마른 짧은 타점봉'을 분리하여 STRICT 조건으로 검증하고, 돌파봉 Vol60 배율별 성과를 추가로 분해합니다. 수급 데이터는 이번 버전에서 필수 조건이 아니라 후속 가점 후보입니다.")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--backtest-h-only', action='store_true', help='H 신고가거자름 STRICT만 필터링하여 H-SAFE/NEUTRAL/RISK 성과를 확인')
