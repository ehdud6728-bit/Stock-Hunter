...
def attach_intent_proxy(df: pd.DataFrame) -> pd.DataFrame:
    z=df.copy()
    z['intent_proxy_version']='v49.75-machine-intent-proxy-v2-sample-authority-safe'
...
def opportunity_audit(opportunities:pd.DataFrame,raw:pd.DataFrame,selected:pd.DataFrame)->tuple[pd.DataFrame,pd.DataFrame,pd.DataFrame,pd.DataFrame,pd.DataFrame]:
    ...
    # v49.75: retain A safe-superset/authority parity columns in both the casebook and
...
def run_quality_audit(raw:pd.DataFrame,selected_ctx:pd.DataFrame,selected:pd.DataFrame,opportunities:pd.DataFrame,out:Path,feature_meta:dict[str,tuple[str,str]])->dict[str,Any]:
    ...
    enriched.to_csv(out/'v49_75_search_intent_enriched.csv',index=False,encoding='utf-8-sig')
    intent.to_csv(out/'v49_75_search_intent_summary.csv',index=False,encoding='utf-8-sig')
    contrast.to_csv(out/'v49_75_winner_loser_effect_size.csv',index=False,encoding='utf-8-sig')
    cases.to_csv(out/'v49_75_manual_casebook.csv',index=False,encoding='utf-8-sig')
    rank_summary.to_csv(out/'v49_75_ranking_summary.csv',index=False,encoding='utf-8-sig')
    rank_detail.to_csv(out/'v49_75_ranking_detail.csv',index=False,encoding='utf-8-sig')
    opp_summary.to_csv(out/'v49_75_opportunity_miss_summary.csv',index=False,encoding='utf-8-sig')
    opp_detail.to_csv(out/'v49_75_opportunity_census_global.csv',index=False,encoding='utf-8-sig')
    opp_cases.to_csv(out/'v49_75_missed_winner_casebook.csv',index=False,encoding='utf-8-sig')
    technical_fn.to_csv(out/'v49_75_technical_false_negative.csv',index=False,encoding='utf-8-sig')
    path_summary.to_csv(out/'v49_75_opportunity_path_summary.csv',index=False,encoding='utf-8-sig')
    drift.to_csv(out/'v49_75_distribution_drift.csv',index=False,encoding='utf-8-sig')
    forensic={'version':'v49.75','count':int(len(technical_fn)),'rows':technical_fn.to_dict(orient='records') if not technical_fn.empty else [],
              'fail_closed':True,'interpretation':'Any authoritative hit absent from the normal path invalidates search quality.'}
    (out/'v49_75_technical_false_negative.json').write_text(json.dumps(forensic,ensure_ascii=False,indent=2,default=str),encoding='utf-8')
    ...
    (out/'v49_75_search_quality_manifest.json').write_text(json.dumps(manifest,ensure_ascii=False,indent=2,default=str),encoding='utf-8')
