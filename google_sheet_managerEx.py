    ...
    try:
                ...
                try:
                    ...
                    rules.append(ConditionalFormatRule(
                        ...
                        booleanRule=BooleanRule(
                            ...
                            format=CellFormat(backgroundColor=Color(1.0, 0.95, 0.8), textFormat=textFormat(bold=True, foregroundColor=Color(0.8, 0.4, 0.0)))
                        )
                    ))
                    rules.save()
                except: pass
                print("✅ [Ex-Sheet] 오늘의 추천종목 탭 업데이트 완료")
            except Exception as e:
                print(f"⚠️ [Ex-Sheet] 오늘의 추천종목 탭 오류: {e}")

        # 💡 [신규 탭 2: AI_추천패턴] - 교정된 들여쓰기 구간
        if ai_recommendation is not None and not ai_recommendation.empty:
            try:
                ...
                print("✅ [Ex-Sheet] AI 추천패턴 탭 업데이트 완료")
            except Exception as e:
        ...
        if not df.empty:
            ...
            if score_col in display_df.columns:
                display_df['종목'] = display_df.apply(
                    lambda x: f"★ {x['종목']}" if int(x[score_col]) >= 130 else x['종목'], 
                    axis=1
                )
