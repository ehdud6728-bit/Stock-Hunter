...
# =============================================================
# closing_bet_scanner.py — 종가배팅 타점 스캐너 (v4.3.1 I-CORE 150/200 BACKTEST)
# =============================================================
# 전략 구성
# A  : 돌파형 종가배팅
...
CLOSING_BET_SCANNER_VERSION = 'G_MORALES_V4_3_1_I_CORE_FLASH_RECOVERY_TAG_BACKTEST_20260515'
...
def _build_icore_hits(code: str, name: str, hist: pd.DataFrame, row: pd.Series, info: dict, idx_label: str, marcap: float) -> list:
    ...
    try:
        ...
        i3 = near_long and dry_volume and close >= long_ma * 0.97 and close >= ma50 * 0.95 and close_loc >= 35.0
        # I-4: 5일선이 150/200일선 위로 올라타는 확인 타점. 과도한 이격은 제외.
        i4 = bool(int(ctx.get('ma5_reclaim_long', 0)) == 1 and -2.0 <= long_dist <= 15.0 and close >= long_ma and close_loc >= 45.0)
        # I-5: 박스/전고점 돌파. 거래량 재증가와 상단마감 필요.
        i5 = bool((int(ctx.get('box_breakout', 0)) == 1 or int(ctx.get('prior_high_breakout', 0)) == 1) and vol_ratio20 >= 1.1 and close_loc >= 60.0 and long_dist <= 22.0)
        ...
        try:
            ...
            if breakout_days > 0 and breakout_ref > 0:
                ...
                i6 = bool(-12.0 <= pull_from_recent_high <= -2.0 and close >= max(ma20, breakout_ref * 0.985) and vol_ratio20 <= 1.2 and close_loc >= 45.0)
        except Exception:
            pass
...
def _format_i_core_report(df: pd.DataFrame) -> str:
    try:
        ...
        lines = []
        lines.append("[📈 I-CORE 시대중심주 150/200일 시세분출 — v4.3.0]")
        if i.empty:
        ...
        lines.append("- 뉴스/재료 지속성은 이번 v4.3.0에서는 거래대금·월봉거래량·대표성 프록시로만 측정하며, 후속 버전에서 뉴스 누적 저장소와 연결합니다.")
        return "\n".join(lines)
    except Exception as e:
