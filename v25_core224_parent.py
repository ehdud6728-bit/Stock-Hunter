#!/usr/bin/env python3
"""V25.4.3 HF5-compatible lightweight ALL-cohort parent.
...
VERSION = "V73.3.6.6.25.4.3-HF5COMPAT"
...
def run(output_dir: str) -> int:
    ...
    parent_block = "\n".join([
        ...
        f"⏱️ lifecycle audit: event-order {audit['daily_event_order_rows']}행 · path-class {audit['daily_path_class_rows']}행 · stop-lens compare {audit['daily_stop_lens_compare_rows']}행 · SINGLE↔30/30/40 {audit['daily_risk_parity_rows']}행 · fill-group {audit['daily_risk_parity_fill_group_rows']}행 · exit-shadow {audit['daily_exit_shadow_rows']}행 · 자동정책선택 0",
