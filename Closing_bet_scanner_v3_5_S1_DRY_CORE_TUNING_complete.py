...
CLOSING_BET_SCANNER_VERSION = 'G_MORALES_V3_5_S1_DRY_CORE_TUNING_20260510'
...
# v3.5: 백테스트상 S전략은 거래량 재점화보다 '고점권 거래량 응축' 성과가 좋아 우수응축 기준을 분리한다.
...
HIGH_REACCUM_S1_GOOD_CLOSE_LOC_MIN = float(os.environ.get('CLOSING_BET_HIGH_REACCUM_S1_GOOD_CLOSE_LOC_MIN', '70.0'))

# v3.5: A 강한돌파는 거래량비 3배 이상 + 거래대금 100억 이상으로 좁힌다.
...
def _evaluate_high_reaccum_signal(df: pd.DataFrame) -> dict:
    ...
    try:
        ...
        # v3.5: 최근 백테스트에서 S 거래량비<1.0, 고점권 응축형의 성과가 가장 좋아서 별도 핵심군으로 승격.
        is_s1_dry_good = bool(
            (not is_s2_exec)
        ...
        # v3.5: S는 거래량 폭발보다 '고점권 응축 후 버팀'이 더 우수하게 나와 응축형을 우대.
        if is_s1_dry_good:
            score_adjust += 12
        ...
        # v3.5 점수 캡: S1 우수응축형은 실전 핵심군으로 승격하되, S2와는 구분한다.
        if is_s1_dry_good:
...
def _send_results(hits: list, mins_left: int):
    ...
    def _is_s1_good(h: dict) -> bool:
        # v3.5: S1 우수응축형은 거래량이 마른 채 고점권에서 버티는 후보를 핵심군으로 본다.
    ...
    def _build_practical_block():
        ...
        block = ['[🎯 실전 매수 우선 후보 — v3.5 S1응축 핵심군]']
        if not pool:
            block.append('해당 종목 없음 — 오늘은 S1우수응축/S2/A강한돌파 기준을 만족한 실전 우선 후보가 없습니다.')
        else:
            block.append('기준: S1 우수응축형 > S2 실행형 > A 강한돌파 순서. B1/B2와 C완화형은 실전 매수 후보가 아니라 관찰/진단 섹션으로 분리합니다.')
    ...
    try:
        ...
        try:
            if len(hist) >= 250 and info['amount_b'] >= MIN_AMOUNT / 1e8:
                ...
                # v3.5: 실시간 C전략이 나오는 데 백테스트에서 0건만 나오는 문제를 점검하기 위한 완화형 연결.
...
def _evaluate_backtest_hit(hit: dict, df: pd.DataFrame, signal_idx: int, hold_days: int = 5) -> dict:
    """
    백테스트 평가.
    기존 고가/종가 성과에 더해 v3.1부터 실전형 3/5 익절·손절 시뮬레이션을 추가하고, v3.5부터 튜닝 리포트/실전추천 조합을 함께 계산한다.
...
def _select_backtest_top(df: pd.DataFrame, top_per_strategy: int = 5, all_candidates: bool = False) -> pd.DataFrame:
    ...
    # v3.5: C는 엄격형을 우선하고, 완화형은 진단용 후보로 뒤에 둔다.
...
def _format_practical_combo_report(df: pd.DataFrame) -> str:
    """v3.5: 백테스트 기반 실전 추천 조합 요약. 핵심군은 S1우수응축/S2/A강한만 포함하고 C 완화형은 진단용으로 분리한다."""
    lines = ["[실전 추천 조합 — v3.5 S1응축 핵심군 튜닝 관점]"]
    if df is None or df.empty:
        lines.append("- 데이터 없음")
        return "\n".join(lines)
    try:
        ...
        lines.append(_format_backtest_trade_rule_block(core, '튜닝 핵심군(S1우수응축+S2+A강한)'))
        ...
        lines.append("- 해석: 실전 매수 우선 후보는 핵심군(S1우수응축+S2+A강한)입니다. B1/B2와 C완화형은 손절선행·조건 점검용으로 분리해 해석합니다.")
    except Exception as e:
...
def _format_bucket_table(df: pd.DataFrame, bucket_col: str, label: str) -> list:
    """v3.5: 조건 구간별 성과를 건수뿐 아니라 3/5규칙 성과까지 출력한다."""
    lines = []
...
def _format_tuning_report(df: pd.DataFrame) -> str:
    """v3.5: S/A/B 조건별 튜닝 리포트. 과최적화를 막기 위해 조건별 성과만 비교한다."""
    lines = ["[튜닝 리포트 — 조건별 성과]"]
    if df is None or df.empty:
        lines.append("- 데이터 없음")
        return "\n".join(lines)
...
def _build_backtest_summary(
    ...
    if core_only:
        sel_text += ' → 핵심군만 필터(S1우수응축+S2+A강한)'
    ...
    try:
        ...
        lines.append(_format_backtest_trade_rule_block(core_df, '튜닝 핵심군(S1우수응축+S2+A강한)'))
    ...
    lines.append("- v3.5의 3/5규칙은 보수 모델입니다. 같은 날 손절가와 목표가가 모두 닿으면 손절을 먼저 본 것으로 계산합니다.")
    lines.append("- v3.5의 튜닝 리포트는 최근 백테스트 결과를 조건별 성과로 비교하는 참고용이며, 최소 4주~12주 이상 반복 검증이 필요합니다.")
    lines.append("- C 완화형은 실전 추천 후보가 아니라 C조건 점검용 진단 후보입니다.")
    lines.append("- --backtest-core-only 옵션을 쓰면 S1우수응축+S2+A강한 핵심군만 따로 검증합니다.")
    return "\n".join(lines)
...
def run_closing_bet_backtest(
    ...
    if raw_df.empty:
        report = _build_backtest_summary(
            ...
            all_candidates, weekly_breakdown=weekly_breakdown, diag=diag, core_only=core_only,
        )
        try:
    ...
    if core_only and selected_df is not None and not selected_df.empty:
        try:
            core_mask = _bt_mask_s2(selected_df) | _bt_mask_s1_good(selected_df) | _bt_mask_a_strong(selected_df)
            selected_df = selected_df[core_mask].copy()
            selected_df['core_filter'] = 'S1우수응축+S2+A강한'
            log_info(f"핵심군 전용 백테스트 필터 적용: {len(selected_df)}건")
        except Exception as e:
    ...
    report = _build_backtest_summary(
        ...
        all_candidates, weekly_breakdown=weekly_breakdown, diag=diag if debug else None, core_only=core_only,
    )
    try:
        BACKTEST_SUMMARY_TXT.write_text(report, encoding='utf-8')
        if debug:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--backtest-core-only', action='store_true', help='백테스트 결과를 S1우수응축+S2+A강한 핵심군만 필터링')
