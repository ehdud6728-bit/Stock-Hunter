...
import v49_72_research as r70
import v49_72_quality as q70
import v49_72_insight as i72
import v49_72_input as ia72
...
def _deliver_telegram(parts: list[list[str]], out: Path, requested: bool) -> dict:
    ...
    path = out / 'v49_72_telegram_delivery_manifest.json'
    try:
        if requested:
            if delivery['route_validated'] != '1':
                raise RuntimeError('v49.72 Telegram requested but unified route preflight was not validated')
            if not s._telegram_route_ready():
                raise RuntimeError('v49.72 Telegram requested but scanner route is not ready after validated preflight')
            for part_no, part in enumerate(parts, start=1):
                ...
                if int(result.get('success_count', 0) or 0) < 1:
                    raise RuntimeError(f'v49.72 Telegram delivery failed for part {part_no}: {result.get("errors", [])}')
...
def _lp_validation(selected: pd.DataFrame, out: Path, start_date: str, end_date: str) -> pd.DataFrame:
    ...
    if z.empty:
        ...
        r.to_csv(out / 'v49_72_lp_walk_forward.csv', index=False, encoding='utf-8-sig')
        return r
...
def _iit_split(selected: pd.DataFrame, out: Path, start_date: str, end_date: str) -> pd.DataFrame:
    ...
    r = pd.DataFrame(rows)
    r.to_csv(out / 'v49_72_iit_flow_split.csv', index=False, encoding='utf-8-sig')
    return r


# ---- v49.72 winner/loser, market, sector and causal feature audit ----
...
def _v4967_anatomy_tables(x: pd.DataFrame, out: Path, start_date: str, end_date: str) -> tuple[pd.DataFrame,pd.DataFrame,pd.DataFrame,pd.DataFrame,pd.DataFrame]:
    ...
    z.to_csv(out/'v49_72_selected_enriched_outcomes.csv',index=False,encoding='utf-8-sig')
    outcomes.to_csv(out/'v49_72_outcome_distribution.csv',index=False,encoding='utf-8-sig')
    features.to_csv(out/'v49_72_winner_loser_feature_anatomy.csv',index=False,encoding='utf-8-sig')
    lifts.to_csv(out/'v49_72_train_oos_feature_lift.csv',index=False,encoding='utf-8-sig')
    market.to_csv(out/'v49_72_market_regime_performance.csv',index=False,encoding='utf-8-sig')
    events.to_csv(out/'v49_72_event_theme_context.csv',index=False,encoding='utf-8-sig')
    down.to_csv(out/'v49_72_down_market_cases.csv',index=False,encoding='utf-8-sig')
...
def main() -> int:
    ...
    ap.add_argument('--baseline-input-authority', default='', help='optional prior same-period v49.72 input authority manifest')
    args = ap.parse_args()
    root = Path(args.input_root)
    prep_root = Path(args.prepare_dir)
    if not prep_root.exists():
        prep_root = root
    preflight_path = next(iter(prep_root.rglob('v49_72_preflight.json')), None)
    universe_path = next(iter(prep_root.rglob('v49_72_universe.json')), None)
    ...
    for sid in expected_ids:
        ...
        if str(m.get('control_flow', '')) != 'V49_72_SHARD_EXPORT_RETURN_MAIN_EXIT':
    ...
    raw_path = out / 'v49_72_global_raw.csv'
    canonical_path = out / 'v49_72_global_canonical.csv'
    selected_path = out / 'v49_72_global_selected_top5.csv'
    ...
    populations['PRIMARY'].to_csv(out / 'v49_72_primary_selected_top5.csv', index=False, encoding='utf-8-sig')
    populations['ALL_MATCHED'].to_csv(out / 'v49_72_all_matched_selected_top5.csv', index=False, encoding='utf-8-sig')
    ...
    funnel.to_csv(out / 'v49_72_strategy_funnel.csv', index=False, encoding='utf-8-sig')
    strategy_status.to_csv(out / 'v49_72_strategy_stage_audit.csv', index=False, encoding='utf-8-sig')
    identity_audit.to_csv(out / 'v49_72_identity_audit.csv', index=False, encoding='utf-8-sig')
    zero_df.to_csv(out / 'v49_72_zero_mode_audit.csv', index=False, encoding='utf-8-sig')
    exc_df.to_csv(out / 'v49_72_predicate_exceptions.csv', index=False, encoding='utf-8-sig')
    ...
    if out.resolve() != generated_root.resolve():
        import shutil
        for gp in generated_root.glob('v49_72_common_strategy_*'):
            shutil.copy2(gp, out / gp.name)
    performance_csv = out / 'v49_72_common_strategy_performance.csv'
    portfolio_csv = out / 'v49_72_common_strategy_portfolio.csv'
    ...
    attribution.to_csv(out / 'v49_72_strategy_attribution_performance.csv', index=False, encoding='utf-8-sig')
    ...
    down_cases_path = out / 'v49_72_down_market_cases.csv'
    down_cases = pd.read_csv(down_cases_path, dtype={'code':str}) if down_cases_path.exists() else pd.DataFrame()

    # v49.72: TRAIN -> VALIDATION -> LOCKED_TEST research. Thresholds are discovered on
    ...
    # v49.72 independent SEARCH QUALITY AUDIT: machine intent proxy, winner/loser casebook,
    ...
    # v49.72 SEARCH GAP INSIGHT: audit-only lanes for immediate strategic misses,
    ...
    if not feature_candidates.empty:
        lines6.append('[구형 2단계 탐색 · v49.72 최종 승격근거 아님]')
    ...
    files = [
        ...
        out / 'v49_72_primary_selected_top5.csv', out / 'v49_72_all_matched_selected_top5.csv',
        out / 'v49_72_strategy_funnel.csv', out / 'v49_72_strategy_stage_audit.csv', out / 'v49_72_identity_audit.csv',
        out / 'v49_72_zero_mode_audit.csv', out / 'v49_72_predicate_exceptions.csv',
        out / 'v49_72_strategy_attribution_performance.csv', out / 'v49_72_lp_walk_forward.csv', out / 'v49_72_iit_flow_split.csv',
        out / 'v49_72_selected_enriched_outcomes.csv', out / 'v49_72_outcome_distribution.csv',
        out / 'v49_72_winner_loser_feature_anatomy.csv', out / 'v49_72_train_oos_feature_lift.csv',
        out / 'v49_72_market_regime_performance.csv', out / 'v49_72_event_theme_context.csv',
        out / 'v49_72_down_market_cases.csv',
        out / 'v49_72_lgs_candidate_library.csv', out / 'v49_72_lgs_redundancy.csv', out / 'v49_72_lgs_locked_test.csv',
        out / 'v49_72_down_market_candidate_library.csv', out / 'v49_72_down_market_redundancy.csv', out / 'v49_72_down_market_locked_test.csv',
        out / 'v49_72_lp_exit_policy_audit.csv', out / 'v49_72_lp_exit_selected.csv', out / 'v49_72_lp_locked_test_stop_cases.csv',
        out / 'v49_72_triple_split_manifest.json',
        out / 'v49_72_search_intent_enriched.csv', out / 'v49_72_search_intent_summary.csv',
        out / 'v49_72_winner_loser_effect_size.csv', out / 'v49_72_manual_casebook.csv',
        out / 'v49_72_ranking_summary.csv', out / 'v49_72_ranking_detail.csv',
        out / 'v49_72_opportunity_miss_summary.csv', out / 'v49_72_opportunity_census_global.csv',
        ...
        out / 'v49_72_a_big_vs_stop_summary.csv', out / 'v49_72_a_big_vs_stop_feature_stability.csv', out / 'v49_72_a_big_vs_stop_casebook.csv',
        out / 'v49_72_h_intent_v2_summary.csv', out / 'v49_72_h_intent_v1_v2_matrix.csv', out / 'v49_72_h_intent_v2_condition_pass.csv',
        out / 'v49_72_h_intent_v2_detail.csv', out / 'v49_72_hypothesis_registry.csv', out / 'v49_72_insight_manifest.json',
    ]
    manifest = {
    ...
    (out / 'v49_72_global_manifest.json').write_text(json.dumps(manifest, ensure_ascii=False, indent=2, default=str), encoding='utf-8')
    print(report)
