...
# =============================================================
# closing_bet_scanner.py — 종가배팅 타점 스캐너 (G Morales v4.1.6 C-SWING 눌림재상승형 검증)
# =============================================================
# 전략 구성
# A  : 돌파형 종가배팅
...
CLOSING_BET_SCANNER_VERSION = 'G_MORALES_V4_1_6_YMGP_PULLBACK_REENTRY_20260511'
...
def _build_backtest_summary(
    ...
    lines.append("- v4.1.6부터 실시간 출력은 S-CORE SAFE 종가배팅과 C-SWING 눌림재상승형 검증 후보를 분리해 표시합니다. C-SWING 평가는 기존 3/5 단타룰과 별도로 계산됩니다.")
...
def run_closing_bet_backtest(
    ...
    if raw_df.empty:
        report = _build_backtest_summary(
            ...
            all_candidates, weekly_breakdown=weekly_breakdown, diag=diag, core_only=core_only, safe_only=safe_only, neutral_only=neutral_only, risk_only=risk_only, c_only=c_only, c_swing_only=c_swing_only, c_strict_only=c_strict_only, c_pullback_only=c_pullback_only,
        )
        try:
    ...
    if (safe_only or neutral_only or risk_only or core_only or c_only or c_swing_only or c_strict_only or c_pullback_only) and selected_df is not None and not selected_df.empty:
        try:
            if c_pullback_only:
    ...
    report = _build_backtest_summary(
        ...
        all_candidates, weekly_breakdown=weekly_breakdown, diag=diag if debug else None, core_only=core_only, safe_only=safe_only, neutral_only=neutral_only, risk_only=risk_only, c_only=c_only, c_swing_only=c_swing_only, c_strict_only=c_strict_only, c_pullback_only=c_pullback_only,
    )
    try:
        BACKTEST_SUMMARY_TXT.write_text(report, encoding='utf-8')
        if debug:
