...
# =============================================================
# closing_bet_scanner.py — 종가배팅 타점 스캐너 (G Morales v4.1.2 YMGP STAGE BACKTEST 완성형)
# =============================================================
# 전략 구성
# A  : 돌파형 종가배팅
...
CLOSING_BET_SCANNER_VERSION = 'G_MORALES_V4_1_2_YMGP_STAGE_BACKTEST_20260511'
...
def _send_results(hits: list, mins_left: int):
    ...
    def _brief_practical_line(h: dict, idx: int) -> str:
        ...
        risk_line = ''
        if risk_tags:
            risk_line = f"\n   위험태그: {' / '.join('⚠️' + t for t in risk_tags)}"
        return (
    ...
    def _build_practical_block():
        ...
        a_pool = [h for h in a_all_pool if not _s_core_risk_tags(h)]
        a_risk_pool = [h for h in a_all_pool if _s_core_risk_tags(h)]
        a_output_n = min(len(a_pool), PRACTICAL_A_TOP_N)

        block = ['[🎯 실전 최우선 후보 — v4.1.1 SAFE 중심]']
        summary = f"요약: 🟢SAFE {len(safe_pool)}개 | 🟡NEUTRAL {len(neutral_pool)}개 | ⚠️S-RISK {len(risk_pool)}개 | A보조 출력 {a_output_n}개/전체 {len(a_pool)}개"
        if a_risk_pool:
            summary += f" | ⚠️A주의숨김 {len(a_risk_pool)}개"
        block.append(summary)
        block.append('운용: SAFE 우선 → 부족하면 NEUTRAL 상위만 관찰 → A는 보조 확인 → RISK는 기본 제외/추격주의입니다.')
...
def _classify_ymgp_stage(hist: pd.DataFrame, row: pd.Series | None = None, info: dict | None = None) -> dict:
    """
    역매공파(C)를 1파 돌파형/눌림진행형/눌림완성형으로 재분류한다.

    C1 1파돌파형     : 장기선 돌파가 막 나온 구간. 관심등록/추격주의.
    C2 눌림진행형   : 1파 이후 눌림은 진행됐지만 재상승 확인은 부족한 구간.
    C3 눌림완성형   : 1파 이후 눌림 + 거래량 감소 + 장기선/20·40선 지지 + 양봉/상단마감 확인 구간.

    이 함수는 실시간 스캔과 백테스트가 같은 기준을 쓰도록 공통화했다.
    """
    try:
        ...
        # 1파 돌파 기준: MA112 또는 MA224를 종가가 아래에서 위로 돌파한 마지막 지점
        ...
        cross112 = (ma112 > 0) & (close_s >= ma112) & (close_s.shift(1) < ma112.shift(1))
        cross224 = (ma224 > 0) & (close_s >= ma224) & (close_s.shift(1) < ma224.shift(1))
        breakout_mask = (cross112 | cross224).fillna(False)
        recent_window = min(35, len(h))
        recent_breaks = [int(i) for i, v in enumerate(breakout_mask.iloc[-recent_window:].tolist(), start=len(h)-recent_window) if bool(v)]

        # 과거 돌파일을 못 찾았지만 현재 장기선 위라면 최근 10일 중 장기선 위로 올라선 첫 구간을 보조로 잡는다.
        if recent_breaks:
            breakout_pos = recent_breaks[-1]
        else:
            above_long = ((ma112 > 0) & (close_s >= ma112)) | ((ma224 > 0) & (close_s >= ma224))
            recent_above = [int(i) for i, v in enumerate(above_long.iloc[-15:].tolist(), start=len(h)-15) if bool(v)] if len(h) >= 15 else []
            breakout_pos = recent_above[0] if recent_above else len(h) - 1

        days_since_breakout = max(0, len(h) - 1 - int(breakout_pos))
        post = h.iloc[int(breakout_pos):].copy()
        if post.empty:
            post = h.tail(1).copy()
        post_high = _safe_float(post['High'].max(), high)
        pullback_pct = round((post_high - close) / post_high * 100.0, 2) if post_high > 0 and close > 0 else 0.0
        try:
            high_pos = int(post['High'].astype(float).idxmax())
            # index가 날짜/라벨일 수 있어 위치로 재계산
            high_pos = int(h.index.get_loc(high_pos)) if high_pos in h.index else int(breakout_pos)
        except Exception:
            try:
                high_pos = int(breakout_pos) + int(post['High'].astype(float).values.argmax())
            except Exception:
                high_pos = int(breakout_pos)
        days_since_high = max(0, len(h) - 1 - int(high_pos))

        # 눌림 때 거래량이 줄었는지: 최근 3일 평균이 직전 10일 평균보다 작거나, 당일 거래량이 VMA20 1.2배 이하
        ...
        volume_dry = bool((prior10_vol > 0 and recent3_vol <= prior10_vol * 0.85) or (vma20_now > 0 and vol <= vma20_now * 1.20))
        vol_ratio_now = round(vol / vma20_now, 2) if vma20_now > 0 else _safe_float(info.get('vol_ratio', 0), 0.0)

        # 재지지 기준: 20/40/112/224/BB40중단 중 현재가 부근의 가장 높은 기준선을 지지선으로 본다.
        bb40 = _calc_bollinger(h, 40, 2.0)
        bb40_mid = _safe_float(bb40['mid'].iloc[-1], 0.0) if len(bb40['mid']) else 0.0
        support_candidates = [x for x in [ma20_now, ma40_now, ma112_now, ma224_now, bb40_mid] if x and x > 0 and x <= close * 1.08]
        support_level = max(support_candidates) if support_candidates else max([x for x in [ma20_now, ma40_now, ma112_now, ma224_now] if x > 0] or [0])
        support_hold = bool(support_level > 0 and low >= support_level * 0.965 and close >= support_level * 0.985)
        ...
        reversal_signal = bool((close >= open_p and close_loc_pct >= 65) or (prev_high > 0 and close >= prev_high) or (prev_close > 0 and close >= prev_close * 1.015))

        # 1파 이후 눌림 판단: 돌파 후 최소 2거래일 이상 지나고, 고점 대비 2.5% 이상 식은 상태
        has_pullback = bool(days_since_breakout >= 2 and pullback_pct >= 2.5 and days_since_high >= 1)

        if has_pullback and support_hold and volume_dry and reversal_signal:
            stage = 'C3'
            label = 'C3 눌림완성형'
            desc = '1파 돌파 이후 눌림·거래량 감소·재지지·양봉/상단마감이 함께 확인된 2파 준비형'
            bias = '눌림 후 2파 후보'
            rank = 0
        elif has_pullback and support_hold:
            stage = 'C2'
            label = 'C2 눌림진행형'
            desc = '1파 이후 눌림과 지지는 보이지만 거래량 감소/재상승 확인이 아직 부족한 관찰형'
            bias = '눌림 관찰'
            rank = 1
        else:
            stage = 'C1'
...
def _select_backtest_top(df: pd.DataFrame, top_per_strategy: int = 5, all_candidates: bool = False) -> pd.DataFrame:
    ...
    # v4.1.2: C는 눌림완성형(C3) → 눌림진행형(C2) → 1파돌파형(C1) 순으로 우선하고, 완화형은 뒤에 둔다.
    ...
    work = work.sort_values(['signal_date', 'mode', '_c_stage_rank', '_c_type_rank', '_grade_rank', '_score_sort', '_amount_sort', '_vol_sort'], ascending=[True, True, True, True, True, False, False, False])
    ...
    return work.drop(columns=[c for c in ['_grade_rank', '_score_sort', '_amount_sort', '_vol_sort', '_c_type_rank', '_c_stage_rank'] if c in work.columns])
...
def _format_practical_combo_report(df: pd.DataFrame) -> str:
    ...
    try:
        ...
        lines.append("- 해석: 실전 최우선은 S1 우수응축형 중 SAFE 후보입니다. NEUTRAL은 관찰 가능, RISK는 추격주의/강등 후보입니다.")
    except Exception as e:
...
def _format_tuning_report(df: pd.DataFrame) -> str:
    """v4.1: S/A/B 조건별 튜닝 리포트. 과최적화를 막기 위해 조건별 성과만 비교한다."""
    lines = ["[튜닝 리포트 — 조건별 성과]"]
    if df is None or df.empty:
        lines.append("- 데이터 없음")
        return "\n".join(lines)
...
def _build_backtest_summary(
    ...
    try:
        ...
        try:
            ...
            if core_only or safe_only or neutral_only or risk_only or c_only:
                if c_only:
                    _scope = 'C 역매공파 선택군'
                elif safe_only:
                    _scope = 'SAFE 선택군'
    ...
    if not c_df.empty:
        ...
        lines.append("- 해석: C1은 1파 돌파 직후라 추격보다 눌림 대기, C2는 눌림 진행 관찰, C3는 눌림완성/2파 준비 후보로 따로 성과를 보는 구간입니다.")
        lines.append("- 해석: C 완화형은 실전 신호가 아니라 백테스트 진단용입니다. C 엄격형과 C3 눌림완성형을 우선 비교하는 편이 안전합니다.")
    ...
    lines.append("- v4.1.2부터 C 역매공파는 C1 1파돌파형 / C2 눌림진행형 / C3 눌림완성형으로 나뉘어 성과가 별도 표시됩니다.")
...
def run_closing_bet_backtest(
    ...
    if raw_df.empty:
        report = _build_backtest_summary(
            ...
            all_candidates, weekly_breakdown=weekly_breakdown, diag=diag, core_only=core_only, safe_only=safe_only, neutral_only=neutral_only, risk_only=risk_only, c_only=c_only,
        )
        try:
    ...
    if (safe_only or neutral_only or risk_only or core_only or c_only) and selected_df is not None and not selected_df.empty:
        try:
            if c_only:
    ...
    report = _build_backtest_summary(
        ...
        all_candidates, weekly_breakdown=weekly_breakdown, diag=diag if debug else None, core_only=core_only, safe_only=safe_only, neutral_only=neutral_only, risk_only=risk_only, c_only=c_only,
    )
    try:
        BACKTEST_SUMMARY_TXT.write_text(report, encoding='utf-8')
        if debug:
