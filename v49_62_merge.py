...
def main() -> int:
    ap = argparse.ArgumentParser(description='v49.62 global shard merge')
    ap.add_argument('--input-root', default='v49_62_downloads')
    ap.add_argument('--prepare-dir', default='v49_62_prepare_output')
    ...
    if not prep_root.exists():
        ...
        prep_root = root
    preflight_path = next(iter(prep_root.rglob('v49_62_preflight.json')), None)
    universe_path = next(iter(prep_root.rglob('v49_62_universe.json')), None)
    ...
    for sid in expected_ids:
        ...
        if str(m.get('control_flow', '')) != 'V49_62_SHARD_EXPORT_RETURN_MAIN_EXIT':
    ...
    raw_path = out / 'v49_62_global_raw.csv'
    canonical_path = out / 'v49_62_global_canonical.csv'
    selected_path = out / 'v49_62_global_selected_top5.csv'
    ...
    if out.resolve()!=generated_root.resolve():
        import shutil
        for gp in generated_root.glob('v49_62_common_strategy_*'):
            shutil.copy2(gp,out/gp.name)

    performance_csv = out / 'v49_62_common_strategy_performance.csv'
    portfolio_csv = out / 'v49_62_common_strategy_portfolio.csv'
    ...
    report = '\n'.join(lines + [''] + lines2)
    (out / 'v49_62_global_summary.txt').write_text(report, encoding='utf-8')
    manifest = {
    ...
    (out / 'v49_62_global_manifest.json').write_text(json.dumps(manifest, ensure_ascii=False, indent=2, default=str), encoding='utf-8')
    print(report)
